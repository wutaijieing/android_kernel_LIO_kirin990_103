...
int32_t venc_smmu_init(bool is_protected, int32_t core_id)
{
	...
	if (is_protected == true) {
		VCODEC_DBG_VENC("config_master smmu reg to secure core_id %d", core_id);
...
void venc_smmu_debug(uint32_t *reg_base, bool first_cfg_flag)
{
	...
	g_vencsmmuregvir.smmu_master_base_viraddr = (uint32_t *)((uint64_t)(uintptr_t)reg_base + VCODEC_VENC_SMMU_MASTER_OFFSET);
	if (g_vencsmmuregvir.smmu_master_base_viraddr == NULL) {
		VCODEC_FATAL_VENC("smmu master base addr is NULL");
		return;
	}

	rd_smmu_master_vreg(0x0044, tmp);
	if (tmp & 0x1) {
		VCODEC_ERR_VENC("enter read interrupt");
		rd_smmu_master_vreg(0x0050, tmp);
		VCODEC_ERR_VENC("read sid 0x%x", tmp);
		rd_smmu_master_vreg(0x0054, tmp);
		VCODEC_ERR_VENC("read addr 0x%x", tmp);
	}

	if (tmp & 0x4) {
		VCODEC_ERR_VENC("enter write interrupt");
		rd_smmu_master_vreg(0x0058, tmp);
		VCODEC_ERR_VENC("write sid 0x%x", tmp);
		rd_smmu_master_vreg(0x005C, tmp);
		VCODEC_ERR_VENC("write addr 0x%x", tmp);
	}
