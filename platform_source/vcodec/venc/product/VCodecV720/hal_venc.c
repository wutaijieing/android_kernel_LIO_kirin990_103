...
int32_t venc_mcore_start_encode(struct encode_info *info)
{
	...
	struct channel_info *msg1 = (struct channel_info *) VRCE_FLAG_MEM_BASE;
	*msg1 = info->channel;
	int32_t ret = venc_ipc_send_msg(MBX1, &msg);
	return ret >= 0 ? 0 : -1;
}
...
static void venc_hal_cfg_ime(struct encode_info *channel_cfg, uint32_t *reg_base)
{
	...
	/* according to <VcodecV720 IT Environment Constrain> 2.3 */
	vedu_hal_cfg(reg_base,
	...
	/* according to <VcodecV720 IT Environment Constrain> 2.4 ~ 2.8 */
	vedu_hal_cfg(reg_base,
	...
	/* according to <VcodecV720 IT Environment Constrain> 2.9 */
	for (i = 0; i < 20; i++)
		vedu_hal_cfg(reg_base,
...
static void venc_hal_cfg_buffer(struct encode_info *channel_cfg, uint32_t *reg_base)
{
	...
	/* according to <VcodecV720 IT Environment Constrain> 1.16 and 1.17 */
	vedu_hal_cfg(reg_base,
	...
	/* according to <VcodecV720 IT Environment Constrain> 4.1 */
	for (i = 0; i < 16; i++) {
		vedu_hal_cfg(reg_base,
...
{
	/* according to <VcodecV720 IT Environment Constrain> 3.1 */
	uint32_t i;
	for (i = 0; i < 80; i++)
		vedu_hal_cfg(reg_base,
	...
	vedu_hal_cfg(reg_base, AXIDFX_OFFSET + 0x700,
				(channel_cfg->channel.rcmode << 8) | channel_cfg->channel.id); // RESERVED_S_NS_0
	vedu_hal_cfg(reg_base, AXIDFX_OFFSET + 0x704,
				(channel_cfg->channel.gop << 16) | channel_cfg->channel.framerate); // RESERVED_S_NS_1
	vedu_hal_cfg(reg_base, AXIDFX_OFFSET + 0x708, channel_cfg->channel.bitrate); // RESERVED_S_NS_2
}

static void venc_hal_cfg_misc(struct encode_info *channel_cfg, uint32_t *reg_base)
{
	/* according to <VcodecV720 IT Environment Constrain> 1.12.1 */
	vedu_hal_cfg(reg_base,
