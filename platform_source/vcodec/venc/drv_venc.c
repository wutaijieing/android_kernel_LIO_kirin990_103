...
static int32_t venc_start_encode(struct encode_info *encode_info,
	struct venc_fifo_buffer *buffer, int32_t core_id)
{
	...
	if(encode_info->all_reg.VEDU_PPFD_ST_CFG.bits.rc_mode == 1)
		ret = venc_mcore_start_encode(encode_info); // start by mcore
	else
...
static int32_t venc_encode(struct encode_info *encode_info, struct venc_fifo_buffer *buffer)
{
	...
	ret = venc_start_encode(encode_info, buffer, core_id);
	if (ret != 0) {
		VCODEC_ERR_VENC("start encode failed, core id: %d", core_id);
		goto exit;
	}
...
{
#ifdef VENC_MCORE_ENABLE
	// when mcore is enabled, eop is taken over by mcore, then mcore uses soft_int_0 to notify CPU.
...
static void venc_encode_done(struct venc_context *ctx)
{
	...
	if (venc->debug_flag & (1LL << PRINT_ENC_COST_TIME)) {
		...
		uint64_t cost_time = osal_get_during_time(ctx->start_time);
