...
{
	atfd_l3share_smc((u64)L3SHARE_FID_VALUE, (u64)CMD_ENABLE_ACP,
			 (u64)id, (u64)size);
}

void l3_cache_disable_acp(unsigned int id)
{
	atfd_l3share_smc((u64)L3SHARE_FID_VALUE, (u64)CMD_DISABLE_ACP,
			 (u64)id, (u64)0);
}

static unsigned int g_l3share_enable_flag;

#ifdef CONFIG_L3CACHE_SHARE_DEBUG
int test_l3_cache_request(u32 moduid, u32 size)
{
	int ret;
	struct l3_cache_request_params test_request_params = {0};

	test_request_params.id = moduid;
	test_request_params.request_size = size;

	ret = l3_cache_request(&test_request_params);
	return ret;
}
EXPORT_SYMBOL(test_l3_cache_request);

int test_l3_cache_release(u32 moduid)
{
	int ret;
	struct l3_cache_release_params test_release_params = {0};

	test_release_params.id = moduid;

	ret = l3_cache_release(&test_release_params);
	return ret;
}
EXPORT_SYMBOL(test_l3_cache_release);

static ssize_t store_l3share_enable(struct device *dev,
...
static ssize_t show_l3share_enable(struct device *dev,
				   struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf_s(buf, PAGE_SIZE, PAGE_SIZE,
			 "g_l3share_enable_flag = %u\n",
			 g_l3share_enable_flag);
	if (ret < 0)
		goto err;
	count += ret;
err:
...
#define L3_SHARE_ATTR_RW(_name) \
	static DEVICE_ATTR(_name, 0660, show_##_name, store_##_name)

L3_SHARE_ATTR_RW(l3share_enable);

static struct attribute *dev_entries[] = {
	&dev_attr_l3share_enable.attr,
	NULL,
};
...
int l3_cache_release(struct l3_cache_release_params *release_params)
{
	...
	if (g_acp_flag == 0) {
		...
		l3share_notifier_call_chain(L3SHARE_ACP_DISABLE,
					    release_params);
#endif
	}
