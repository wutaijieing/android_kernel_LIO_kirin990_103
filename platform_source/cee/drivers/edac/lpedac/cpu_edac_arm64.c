...
static const struct errors_edac g_errors_handlers[] = {
	{"L1 Correctable Error", edac_mntn_hanlde_ce },
	{"L1 Uncorrectable Error", edac_mntn_hanlde_ue },
	{"L2 Correctable Error", edac_mntn_hanlde_ce },
	{"L2 Uncorrectable Error", edac_mntn_hanlde_ue },
	{"L3 Correctable Error", edac_mntn_hanlde_ce },
	{"L3 Uncorrectable Error", edac_mntn_hanlde_ue },
};
				...
				u64 errxstatus, u64 errxmisc)
{
	...
	if (block_nr == L3)
		...
		rdr_syserr_process_for_ap((u32)(MODID_AP_S_CPU0_CE +
					  inst_nr * 2), 0ULL, 0ULL);
#else
	if (block_nr == L3)
				...
				u64 errxstatus, u64 errxmisc)
{
	...
	if (block_nr == L3)
		...
		rdr_syserr_process_for_ap((u32)(MODID_AP_S_CPU0_UE +
					  inst_nr * 2), 0ULL, 0ULL);
#else
	if (block_nr == L3)
...
static int edac_ecc_stopping_cpu(unsigned int cpu __maybe_unused,
...
static int cpu_erp_probe(struct platform_device *pdev)
{
	...
	rc = cpuhp_state_add_instance(g_cpu_edac_cpuhp_state,
				      &drv->node);
	if (rc != 0) {
...
static int __init cpu_erp_init(void)
{
	...
	ret = platform_driver_register(&cpu_erp_driver);
	if (ret != 0)
		cpuhp_remove_multi_state(g_cpu_edac_cpuhp_state);
