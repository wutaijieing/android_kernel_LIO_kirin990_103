...
#define NPU_PM_AVS_TSENSOR_NAME  "npu"
#define SLICE_TO_US		32
#define AVS_INTERVAL		60000UL

static int npu_pm_avs_tsensor;
...
unsigned int update_avs_temperature_info(void)
{
	...
	ret = ipa_get_sensor_value(npu_pm_avs_tsensor, &temp);
...
static int npu_pasensor_mode_cfg(void)
{
	...
	ret = atfd_hisi_service_avs_smc(AVS_NPU_PA_MODE_CFG,
					0ULL, 0ULL, 0ULL);
...
static int npu_pasensor_mode_recovery(void)
{
	...
	ret = atfd_hisi_service_avs_smc(AVS_NPU_PA_MODE_RECOVERY,
					0ULL, 0ULL, 0ULL);

	return ret;
}

static int npu_read_pasensor_code(struct npu_pm_module_profile *module_profile)
{
	int ret;

	ret = atfd_hisi_service_avs_smc(AVS_NPU_GET_PA_VALUE,
			     ...
			     unsigned int open_switch)
{
	...
	g_pa_val = (unsigned int)npu_read_pasensor_code(module_profile);
			  ...
			  int tar_pid)
{
	...
	struct npu_pm_module_profile *main_module = g_dvfs_data.main_module;
...
{
#ifdef CONFIG_THERMAL_TSENSOR
	npu_pm_avs_tsensor = ipa_get_tsensor_id(NPU_PM_AVS_TSENSOR_NAME);
	if (npu_pm_avs_tsensor < 0) {
...
static int npu_pm_avs_memory_cfg_init(struct device *dev,
...
static int npu_pm_pasensor_clk_init(struct device *dev,
				      ...
				      struct npu_pm_module_profile *module_profile)
{
	...
	module_profile->pa_ctrl.pll_clk = npu_pm_find_clock(clk_name);

	return ret;
}

static void npu_pm_avs_info_debug(struct device *dev,
				    struct npu_pm_module_profile *module_profile)
{
	int i;
			       ...
			       const char *main_module_name)
{
	...
	ret = npu_pm_avs_memory_cfg_init(dev, module_profile);
	if (ret != 0) {
		dev_err(dev, "%s npu avs memcfg init failed\n", __func__);
		return ret;
	}

	ret = npu_pm_pasensor_clk_init(dev, module_np, module_profile);
	if (ret != 0) {
	...
	npu_pm_avs_info_debug(dev, module_profile);
