...
#include <uranus/freqdump_kernel.h>
...
/* 0:es 1:cs 2:none */
static unsigned int g_chip_type = 2;
...
void sec_loadmonitor_switch_disable(void)
{
	...
	(void)atfd_service_freqdump_smc(
		(u64)FREQDUMP_LOADMONITOR_SVC_DISABLE, (u64)0, (u64)0, (u64)0);
...
void sec_chip_type_read(void)
{
	...
	memcpy_fromio((void *)(&g_chip_type),
		      (void *)((uintptr_t)g_freqdump_virt_addr +
			       FREQDUMP_BUFF_SIZE - CS_MAX_PERI_INDEX * 4 - 1),
		      sizeof(g_chip_type));
...
{
	int pos = 0;
	int buf_size = sizeof(g_loadmonitor_buf);
	int i, ret;
...
static int loadmonitor_node_dump(struct seq_file *s, void *p)
{
	...
	if (g_chip_type == CS) {
		if (-EINVAL == read_format_cs_loadmonitor_data()) {
			up(&g_loadmonitor_sem);
			return -EINVAL;
		}
	}
					...
					size_t cnt, loff_t *ppos)
{
	...
	if (enable_flags != PERI_MONITOR_EN &&
	    enable_flags != AO_MONITOR_EN && enable_flags != ALL_MONITOR_EN) {
...
static int freqdump_debugfile_init(void)
{
	...
	freqdump_flie =
		debugfs_create_file("freqdump_data", FILE_RIGHT,
	...
	adc_disable_file =
		debugfs_create_file("adc_disable", FILE_RIGHT,
				    g_freqdump_debugfs_root, NULL,
				    &adc_disable_fops);
...
static int __init freqdump_debugfs_init(void)
{
	...
	g_freqdump_phy_addr =
		ATF_SUB_RESERVED_BL31_SHARE_MEM_PHYMEM_BASE + data[0];
