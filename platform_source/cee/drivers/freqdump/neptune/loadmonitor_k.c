...
static void update_ao_data_time_interval(void)
{
	...
	g_ao_data_end_mdelay = div_u64(ktime_to_ns(ktime_get_boottime()),
				       NSEC_PER_MSEC);
}
...
static void set_loadmonitor_enable_flag(void)
{
	...
	pr_err("%s monitor enable flags:%#X.\n",
...
static int enable_monitor(void)
{
	if (g_loadmonitor_status == LOADMONITOR_OFF) {
		...
		pr_err("%s dpm on success, delay time:%lldms, data count:%lld\n",
		       __func__, g_monitor_info.monitor_delay / 1000,
		       g_print_err_data);
		return 0;
...
static int disable_monitor(void)
{
	...
	if (g_loadmonitor_status == LOADMONITOR_ON) {
		...
		pr_err("%s dpm off success, flag:0x%x,%d\n",
		       __func__, g_monitor_delay_value.monitor_enable_flags,
		      monitor_clear);
		return 0;
...
static int ioctrl_get_ip_len(void __user *argp)
{
	...
	g_dpm_data->length = (s32)(u32)(MAX_NUM_LOGIC_IP_DEVICE + 1);
...
static int ioctrl_set_sample_time(const void __user *argp)
{
	...
	if (period > DPM_PERIOD_MAX_SEC || period < DPM_PERIOD_MIN_SEC) {
		...
		g_monitor_delay_value.peri0 =
			period * (LOADMONITOR_PERI_FREQ_CS / 1000);
		g_monitor_delay_value.peri1 =
			period * (LOADMONITOR_PERI_FREQ_CS / 1000);
		g_monitor_delay_value.npu =
			period * (LOADMONITOR_NPU_FREQ / 1000);
		g_monitor_delay_value.ao  =
			period * (LOADMONITOR_AO_FREQ / 1000);
		g_monitor_delay_value.media0 =
			period * (LOADMONITOR_MEDIA0_FREQ / 1000);
		g_monitor_delay_value.media1 =
			period * (LOADMONITOR_MEDIA1_FREQ / 1000);
		g_monitor_delay_value.cpu =
			period * (LOADMONITOR_CPU_FREQ / 1000);

		g_dpm_period = period * LOADMONITOR_PCLK * 1000;
		pr_err("%s flag:0x%x,sample_time:%ums,dpm_period:%u,peri0:%u,peri1:%u,ao:%u,media0:%u,media1:%u,cpu%u\n",
		       __func__, g_monitor_delay_value.monitor_enable_flags,
...
static int ioctrl_get_ddr_freq_info(void __user *argp)
{
	...
	ddr_data_len = (IP_NAME_SIZE + ((u32)ddr_freq_num * (u32)sizeof(u64))) * DDR_INFO_LEN;
...
static bool ip_need_calc(struct ips_info *info, int ip_index)
{
	return ((info->flag &
		g_monitor_delay_value.monitor_enable_flags) ==
		info->flag && (info->mask & freq_bit((u32)ip_index)) != 0);
}
	...
	if (is_ao_data(info)) {
		...
		if (idle_nor_time + busy_nor_time + busy_low_time > 1000) {
			...
			if (g_print_err_data == -1 ||
				...
				pr_err("%s freq:%u, idle:%u, norm:%u, low:%u, dev_index:%d, sensor_index:%d\n",
				       __func__, freq, status->idle,
				       status->busy_norm_volt,
				       status->buys_low_volt, dev_index,
				       sensor_index);
...
{
	int ret, i, j;
	int ips_num = (s32)(u32)(sizeof(struct loadmonitor_ips) / sizeof(struct ips_info));
	int dev_index;
	struct ip_status *status = NULL;
	struct ips_info *info = (struct ips_info *)(&loadmonitor_data_value.ips);
...
{
	unsigned int temp_rd, temp_wr;
	int ret;

	ret = memcpy_s(g_ddr_old, sizeof(g_ddr_old),
		       g_ddr_new, sizeof(g_ddr_new));
	if (ret != EOK)
		pr_err("%s memcpy_s is err. ret:%d\n", __func__, ret);

	/* DMC0 */
	g_ddr_new[0] = loadmonitor_data_value.ddr.dmc_flux_rd[0];
	g_ddr_new[1] = loadmonitor_data_value.ddr.dmc_flux_wr[0];
	if (DDR_TYPE == DMC_CHANNEL_SIZE) {
		/* DMC1 */
		g_ddr_new[0] += loadmonitor_data_value.ddr.dmc_flux_rd[1];
		g_ddr_new[1] += loadmonitor_data_value.ddr.dmc_flux_wr[1];
		/* DMC3 */
		g_ddr_new[0] += loadmonitor_data_value.ddr.dmc_flux_rd[3];
		g_ddr_new[1] += loadmonitor_data_value.ddr.dmc_flux_wr[3];
	}
	/* DMC2 */
	g_ddr_new[0] += loadmonitor_data_value.ddr.dmc_flux_rd[2];
	g_ddr_new[1] += loadmonitor_data_value.ddr.dmc_flux_wr[2];

	temp_rd = (g_ddr_new[0] >= g_ddr_old[0]) ?
		  (g_ddr_new[0] - g_ddr_old[0]) :
		  ((0xffffffff - g_ddr_old[0]) + g_ddr_new[0] + 1);
	temp_wr = (g_ddr_new[1] >= g_ddr_old[1]) ?
		  (g_ddr_new[1] - g_ddr_old[1]) :
	...
	if (g_ddr_suspend_switch == 1) {
		...
		g_ddr_bw_rd += (temp_rd / 1000000) << DDR_BW_RW_BIT_OFFSET;
		g_ddr_bw_wr += (temp_wr / 1000000) << DDR_BW_RW_BIT_OFFSET;
	}
}
...
void update_dpm_data(struct work_struct *work)
{
	...
	(void)work;
	/* get data from bl31 */
	ret = memset_s((void *)(&loadmonitor_data_value),
		       sizeof(loadmonitor_data_value), 0,
	...
	read_format_cs_loadmonitor_data(delay_time_media,
					g_monitor_delay_value.monitor_enable_flags);
	update_ao_data_time_interval();

	calc_monitor_time();
	format_dubai_peri_data(&loadmonitor_data_value.peri);
	...
	g_dpm_data->data[MAX_NUM_LOGIC_IP_DEVICE].off_time = (s64)(u64)g_ddr_bw_rd;
	g_dpm_data->data[MAX_NUM_LOGIC_IP_DEVICE].busy_nor_time = (s64)(u64)g_ddr_bw_wr;
...
static int ioremap_addr(void)
{
	...
	set_loadmonitor_addr(g_freqdump_virt_addr);
	return 0;
fail_to_read_reg:
	of_node_put(np);
	np = NULL;
