 ...
 * Description: thermal ipa framework
 *
...
static unsigned long get_temperature_scale(int temp)
{
	...
	return (unsigned long)(temp_para.t_scale /
			       capacitance[NUM_CAPACITANCES - 1]); /*lint !e571*/
}
				 ...
				 struct platform_device *pdev)
{
	...
	for_each_online_cpu(mask_id.cpuid) { /*lint !e713 !e574*/
	...
	for (i = 0; i < (int)g_cluster_num; i++) {
		...
		thermal_data->cdevs[i] =
			of_cpufreq_power_cooling_register(cdev_np, policy,
							  get_dyn_power_coeff(mask_id.cluster,
									      thermal_data),
