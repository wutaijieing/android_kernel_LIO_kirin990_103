...
void mas_blk_dev_lld_init_unistore(struct blk_dev_lld *blk_lld)
{
	...
	blk_lld->features &= ~BLK_LLD_UFS_CPPLUS_EN;
}

static void mas_blk_bio_set_cpplus_opf(struct bio *bio)
{
	if (blk_queue_query_cpplus_enable(bio->bi_disk->queue) &&
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
		op_is_read(bio_op(bio)) && (bio->mas_bi_opf & REQ_VIP) &&
#else
		op_is_read(bio_op(bio)) && (bio->bi_opf & REQ_VIP) &&
#endif
		task_in_pagefault(current))
		bio->mas_bi_opf |= MAS_REQ_CPPLUS;
}
...
void mas_blk_bio_set_opf_unistore(struct bio *bio)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	if (((bio->mas_bi_opf & REQ_VIP) || (bio->bi_opf & REQ_FG)) &&
#else
	...
	mas_blk_bio_set_cpplus_opf(bio);
}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	ret = __blk_mq_alloc_rq_maps(set);
#else
	...
	struct request_queue *q, unsigned int op, unsigned int flags)
{
	...
	alloc_data.cmd_flags |= (REQ_SYNC | REQ_FG);
...
void mas_blk_queue_cpplus_enable(struct request_queue *q, bool enable)
{
	...
	if (enable)
		lld->features |= BLK_LLD_UFS_CPPLUS_EN;
	else
		lld->features &= ~BLK_LLD_UFS_CPPLUS_EN;
}
...
bool blk_queue_query_cpplus_enable(struct request_queue *q)
{
	...
	return ((lld->features & BLK_LLD_UFS_CPPLUS_EN) ? true : false);
}
