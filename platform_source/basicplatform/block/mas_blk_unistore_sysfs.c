	...
	int data_move_num, unsigned long val, unsigned int mas_sec_size)
{
	...
	data_move_info->done_info.private_data =
		(void *)mas_queue_device_data_move_store_done;
	data_move_info->done_info.mas_sec_size = mas_sec_size;
}
	...
	struct request_queue *q, const char *page, size_t count)
{
	...
	ret = queue_var_store(&val, page, count);
	if (ret < 0) {
		pr_err("%s, get input error\n", __func__);
		goto out;
	}

	if (!lld_fn || !lld_fn->unistore_ops.dev_slc_mode_err_inject) {
		pr_err("%s, lld func error\n", __func__);
		goto out;
	}

	if (val <= 0) {
		pr_err("%s, input val should be 1 or 2\n", __func__);
		goto out;
	}

	if (val == ERR_INJECT)
		ret = lld_fn->unistore_ops.dev_slc_mode_err_inject(q, 1);
	else if (val == ERR_RECOVEY)
		ret = lld_fn->unistore_ops.dev_slc_mode_err_inject(q, 0);

	pr_err("%s, slc_mode_err_inject val: %lu ret:%d!\n", __func__, val, ret);
	...
	struct request_queue *q, char *page)
{
	...
	offset = snprintf(page, PAGE_SIZE,
		"cpplus_en: %d\n", blk_queue_query_cpplus_enable(q));
