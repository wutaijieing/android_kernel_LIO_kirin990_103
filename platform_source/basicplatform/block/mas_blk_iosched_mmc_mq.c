...
#define MAS_MMC_MQ_IO_JAM_MS 2000
...
struct mas_mmc_sched_ds_lld {
	...
	/* Complete time for latest sync & async io */
	ktime_t last_io_compl_tm;
};
...
static inline bool last_io_compl_g_than(
	const struct mas_mmc_sched_ds_lld *ds_lld, const ktime_t base,
	const u64 ms)
{
	return ktime_after(
		base, ktime_add_ms(ds_lld->last_io_compl_tm, ms));
}
	...
	struct mas_mmc_sched_ds_lld *ds_lld, bool update_complete_time)
{
	...
	if (likely(update_complete_time))
		ds_lld->last_io_compl_tm = ktime_get();
}
...
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	if ((req->mas_cmd_flags & REQ_VIP) || (req->cmd_flags & REQ_FG_META))
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
		rq->cmd_flags |= REQ_FG | REQ_SYNC;
		rq->mas_cmd_flags |= REQ_VIP;
#else
...
static inline void mmc_mas_blk_add_list_check(struct request *rq)
{
...
static void mmc_mq_sync_dispatch(const struct request_queue *q)
{
	...
	do {
		...
		if (likely(ret == BLK_STS_OK)) {
			...
			if (last_io_compl_g_than(
				    ds_lld, ktime_get(), MAS_MMC_MQ_IO_JAM_MS) )
				mmc_mq_run_delay_sync_list(q, 1);
			else
				mmc_mq_run_delay_sync_list(q, 0);
			break;
		}
...
static void mmc_mq_ktime_init(struct mas_mmc_sched_ds_lld *ds_lld)
{
	ds_lld->last_io_compl_tm = ktime_get();
}
