...
static int hisi_peri_set_avs(struct peri_volt_poll *pvp, unsigned int avs)
{
	unsigned int val;
...
static int hisi_peri_wait_avs_update(struct peri_volt_poll *pvp)
{
	/* peri avs loop max */
	int loop = 400;
	unsigned int val;
...
static struct peri_volt_ops hisi_peri_volt_ops = {
	...
	.set_avs = hisi_peri_set_avs,
	.wait_avs_update = hisi_peri_wait_avs_update,
