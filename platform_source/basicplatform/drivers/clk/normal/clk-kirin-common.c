...
static int __hi3xxx_pmu_clkgate_pclk_init(struct device_node *np, struct hi3xxx_periclk *pclk)
{
	...
	pclk->pmu_clk_enable = (int)clkgata_val[0];
	...
	u32 *width, u8 *shift)
{
	...
	/* 0:offset 1:bit */
	reg = base + rdata[0];
	*shift = (u32)(ffs(rdata[1]) - 1);
	*width = (u8)(fls(rdata[1]) - ffs(rdata[1]) + 1);
	return reg;
}
	...
	void __iomem *reg_base)
{
	...
	dclk->shift = (u8)(ffs(data[1]) - 1);
	dclk->width = (u8)(fls(data[1]) - ffs(data[1]) + 1);
