...
#ifdef CONFIG_PERI_VOLT_HW_VOTE
...
#ifdef CONFIG_PERI_AVS_IPC_NOTIFY
...
static int plat_peri_set_volt(struct peri_volt_poll *pvp, unsigned int volt)
{
	unsigned int val;
	int ret = 0;
...
static unsigned int plat_peri_get_volt(struct peri_volt_poll *pvp)
{
	unsigned int ret;
...
static int __peri_set_avs(struct peri_volt_poll *pvp, unsigned int avs)
{
	unsigned int val;
...
static int __peri_wait_avs_update(struct peri_volt_poll *pvp)
{
	/* peri avs loop max */
	int loop = 400;
	unsigned int val;
...
static struct peri_volt_ops peri_volt_ops = {
	.set_volt = plat_peri_set_volt,
	.get_volt = plat_peri_get_volt,
	.recalc_volt = plat_peri_get_volt,
	.set_avs = __peri_set_avs,
	.wait_avs_update = __peri_wait_avs_update,
	...
	void __iomem *reg_base)
{
	...
	pvolt->bitsshift = (u32)(ffs(pdata[1]) - 1);
	pvolt->stat = 0;
...
static int peri_volt_poll_probe(struct platform_device *pdev)
{
	...
	for_each_child_of_node(parent, np) {
		...
		perivolt_init_cb = (void *)match->data;
		perivolt_init_cb(np);
	}
