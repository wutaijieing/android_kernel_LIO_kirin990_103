...
static int32_t pcie_get_ldoinfo(struct pcie_kport *pcie)
{
	...
	if (of_property_read_u32_array(np, "avdd12", val, 3)) {
		PCIE_PR_E("Failed to get avdd12 info");
		return -1;
	}
	g_pcie_slt_cfg[pcie->rc_id].avdd12_offset = val[0]; /* Element 0 */
	g_pcie_slt_cfg[pcie->rc_id].avdd12_normal = val[1]; /* Element 1 */
	g_pcie_slt_cfg[pcie->rc_id].avdd12_low = val[2]; /* Element 2 */

	/* bias volt: offset, normal_val, low_val */
	if (of_property_read_u32_array(np, "avdd075", val, 3)) {
		PCIE_PR_E("Failed to get avdd075 info");
		return -1;
	}
	g_pcie_slt_cfg[pcie->rc_id].avdd075_offset = val[0]; /* Element 0 */
	g_pcie_slt_cfg[pcie->rc_id].avdd075_normal = val[1]; /* Element 1 */
	g_pcie_slt_cfg[pcie->rc_id].avdd075_low = val[2]; /* Element 2 */
...
static void pcie_set_vlotage(struct pcie_kport *pcie, enum pcie_voltage vol)
{
	...
	case LOW_VOL:
		...
		pmic_write_reg(g_pcie_slt_cfg[pcie->rc_id].avdd12_offset,
				    g_pcie_slt_cfg[pcie->rc_id].avdd12_low);
		pmic_write_reg(g_pcie_slt_cfg[pcie->rc_id].avdd075_offset,
				    g_pcie_slt_cfg[pcie->rc_id].avdd075_low);
		break;
	case NORMAL_VOL:
		/* fall-through */
	default:
		...
		pmic_write_reg(g_pcie_slt_cfg[pcie->rc_id].avdd12_offset,
				    g_pcie_slt_cfg[pcie->rc_id].avdd12_normal);
		pmic_write_reg(g_pcie_slt_cfg[pcie->rc_id].avdd075_offset,
				    g_pcie_slt_cfg[pcie->rc_id].avdd075_normal);
	}
...
{
	int ret;
	struct pcie_kport *pcie_rc = get_pcie_by_id(APR_RC_ID);
	struct pcie_kport *pcie_ep = get_pcie_by_id(APR_EP_ID);
...
static int loopback_datatransfer(void *ddr, u32 size, u32 dir)
{
	...
	pcie_kport_inbound_atu(pcie_ep->rc_id, ATU_REGION_INDEX0,
...
static long pcie_slt_ioctl(struct file *file, u_int cmd, unsigned long result)
{
	...
	case PCIETESTCMD:
		...
		pcie = get_pcie_by_id(scb.rc_id);
		if (!pcie) {
			ret = -EINVAL;
			goto FAIL;
		}

		scb.test_result = pcie_slt_vary_voltage_test(pcie, scb.loop_times);
