 ...
 * pcie-lexi.c
 *
 * PCIe lexi turn-on/off functions
 *
...
#define HSDT_CRG_FNPLL_CFG6 0x240
#define HSDT_CRG_FNPLL_CFG7 0x244
...
#define PCIE1_CLK_HP_GATE (0x1 << 22)
#define PCIE1_CLK_DEBUNCE_GATE (0x1 << 23)
#define PCIE1_CLK_PHYREF_GATE (0x1 << 27)
#define PCIE1_CLK_IO_GATE (0x1 << 28)
...
#define PCIE0_CTRL_POR_N (0x1 << 7)
#define PCIE1_CTRL_POR_N (0x1 << 23)
#define PCIE_RST_HSDT_TCU (0x1 << 3)
#define PCIE_RST_HSDT_TBU (0x1 << 5)
#define PCIE1_RST_HSDT_TCU (0x1 << 20)
#define PCIE1_RST_HSDT_TBU (0x1 << 22)
...
#define PCIE1_TBU_BASE 0xEC4C0000
 ...
 */
static int pcie_noc_power_lexi(struct pcie_kport *pcie, u32 exit)
{
	u32 mask_bits, val_bits, val;
	u32 timeout = NOC_TIMEOUT_VAL;
	...
	if (exit) {
		...
		while (val & val_bits) {
			...
			udelay(1);
			if (!timeout) {
				PCIE_PR_E("Exit failed : 0x%x", val);
				return -1;
			}
			timeout--;
			val = readl(pcie->pmctrl_base + NOC_POWER_IDLE);
		...
		while ((val & val_bits) != val_bits) {
			...
			udelay(1);
			if (!timeout) {
				PCIE_PR_E("Enter failed : 0x%x", val);
				return -1;
			}
			timeout--;
			val = readl(pcie->pmctrl_base + NOC_POWER_IDLE);
...
static int pcie_fnpll_ctrl(struct pcie_kport *pcie, u32 enable)
{
	...
	/* pll_en = 0 bypass = 1 VCO = 6GHZ POSTDIV = 100MHZ */
	hsdt_crg_reg_write(pcie, 0x32E04E06, HSDT_CRG_FNPLL_CFG6);
	hsdt_crg_reg_write(pcie, 0x00200000, HSDT_CRG_FNPLL_CFG7);

	/* delay 5us then enable FNPLL */
	udelay(5);

	val = hsdt_crg_reg_read(pcie, HSDT_CRG_FNPLL_CFG6);
	val |= FNPLL_EN_BIT;
	hsdt_crg_reg_write(pcie, val, HSDT_CRG_FNPLL_CFG6);
	/* delay 20us then check FNPLL lock status */
	udelay(20);
	...
	while (1) {
		...
		/* Each detection interval is 1 us */
		udelay(1);
		timeout++;
...
static void pcie_ctrl_por_n_rst(struct pcie_kport *pcie, u32 enable)
{
	...
	if (pcie->rc_id == PCIE_RC0)
		val = PCIE0_CTRL_POR_N;
	else
		val = PCIE1_CTRL_POR_N;
...
static int pcie_clk_config(struct pcie_kport *pcie, enum pcie_clk_type clk_type, u32 enable)
{
	...
	case PCIE_EXTERNAL_CLK:
		...
		/* set pcie1 axi clk freq 238M is already useless */
...
static int pcie_turn_on(struct pcie_kport *pcie, enum rc_power_status on_flag)
{
	...
	ret = pcie_noc_power_lexi(pcie, ENABLE);
	if (ret) {
...
SMMU_INIT_FAIL:
	ret = pcie_noc_power_lexi(pcie, 0);
	if (ret)
