	...
	u64 wrmem_addr, int wrmem_val, enum mem_trans_type trans_type)
{
	...
	local_cpu_addr = (__force uintptr_t)ioremap(cpu_addr, TEST_MEM_SIZE);
	if (!local_cpu_addr) {
...
int set_ep_mem_inbound(u32 rc_id, int index)
{
	...
	pcie_kport_inbound_atu(rc_id, index, ATU_TYPE_MEM, temp_addr, busdev, TEST_MEM_SIZE);
...
void pcie_enable_device_test(u32 rc_id)
{
	...
	if (pci_enable_device(pdev))
		PCIE_PR_E("pci_enable_device failed, rcid:%u", rc_id);
...
int pcie_ep_mac_init(u32 pcie_id)
{
	...
	if (pcie_read_dbi(pcie->pci, pcie->pci->dbi_base, ATU_VIEWPORT, REG_DWORD_ALIGN) == 0xffffffff) {
		...
		pcie->pci->atu_base = pcie->pci->dbi_base + DEFAULT_DBI_ATU_OFFSET;
#endif
	} else {
...
{
	int ret = pcie_kport_set_host_speed(rc_id, gen);
...
int set_preset(u32 rc_id, enum pcie_cof_preset_index preset)
{
	u32 cap_pos;
	u32 val = 0;
	struct pcie_port *pp = NULL;
	...
	pcie_rd_own_conf(pp, (int)cap_pos + PCI_EXP_LNKCTL2, REG_DWORD_ALIGN, &val);
	val &= ~(0xf << 12); /* 15:12 bit PCIE_CAP_COMPLIANCE_PRESET */
	val |= (preset << 12); /* 15:12 bit PCIE_CAP_COMPLIANCE_PRESET */
	pcie_wr_own_conf(pp, (int)cap_pos + PCI_EXP_LNKCTL2, REG_DWORD_ALIGN, val);
...
int pcie_set_ep_mode(u32 rc_id)
{
	...
	devm_free_irq(&pdev->dev, (unsigned int)pcie->irq[IRQ_AXI_TIMEOUT].num, (void *)pcie);
#endif
...
int pcie_phy_core_test(u32 rc_id)
{
	struct pcie_kport *pcie = NULL;
...
/*
 * Used to enable/disable a specific preset
 * @p_index: preset index
 */
#define GEN3_EQ_PSET_REQ_VEC_BIT 8
#define GEN3_EQ_PSET_REQ_VEC 0xFFFF
int pcie_phy_enable_fixed_preset(u32 rc_id, enum pcie_cof_preset_index p_index)
{
	struct pcie_kport *pcie = NULL;
	u32 reg_val;
	...
	reg_val = pcie_read_dbi(pcie->pci, pcie->pci->dbi_base,
		PORT_GEN3_EQ_CONTROL_REG, REG_DWORD_ALIGN);
	reg_val &= ~(GEN3_EQ_PSET_REQ_VEC << GEN3_EQ_PSET_REQ_VEC_BIT);
	pcie_write_dbi(pcie->pci, pcie->pci->dbi_base,
		PORT_GEN3_EQ_CONTROL_REG, REG_DWORD_ALIGN, reg_val);

	/* enable one preset */
	reg_val |= (0x1 << (GEN3_EQ_PSET_REQ_VEC_BIT + p_index));
	pcie_write_dbi(pcie->pci, pcie->pci->dbi_base,
		PORT_GEN3_EQ_CONTROL_REG, REG_DWORD_ALIGN, reg_val);
...
/*
 * Used to collect gen3 phy info
 */
int pcie_test_collect_phy3_info(u32 rc_id)
{
	struct pcie_kport *pcie = NULL;
	struct pcie_phy *phy = NULL;
	uint32_t reg_val;
	...
	if (!pcie->phy) {
		PCIE_PR_E("PHY3 is not initialized.\n");
		return -EINVAL;
	}

	phy = pcie->phy;

	reg_val = readl(phy->phy_base + 0x1000);
	PCIE_PR_E("phy3: dcc overflow/underflow intr flag: %d\n",
			(reg_val & (0x1 << 29)) >> 29);

	reg_val = readl(phy->phy_base + 0x404);
	PCIE_PR_E("phy3: tx termk overflow/underflow intr: %d\n",
			(reg_val & (0x1 << 10)) >> 10);
	PCIE_PR_E("phy3: rx termk overflow/underflow intr: %d\n",
			(reg_val & (0x1 << 11)) >> 11);

	reg_val = readl(phy->phy_base + 0xB30);
	PCIE_PR_E("phy3: vco band value in GEN1/2: 0x%x\n", reg_val & 0xFFFFFC00);

	reg_val = readl(phy->phy_base + 0xB38);
	PCIE_PR_E("phy3: vco band value in GEN3: 0x%x\n", reg_val & 0xFFFFFC00);

	reg_val = readl(phy->phy_base + 0x154C);
	PCIE_PR_E("phy3: dcc calibration result: 0x%x\n", reg_val & 0xFFFFF000);

	reg_val = readl(phy->phy_base + 0x0A78);
	PCIE_PR_E("phy3: tx termk result: 0x%x\n", reg_val & 0xFFFFFE00);

	reg_val = readl(phy->phy_base + 0x0A88);
	PCIE_PR_E("phy3: rx termk result: 0x%x\n", reg_val & 0xFFFFFFE0);

	reg_val = readl(phy->phy_base + 0xB70);
	PCIE_PR_E("phy3: digital pll lock result: 0x%x\n", reg_val & 0xFFFFFFFB);

	return 0;
}

#ifdef CONFIG_FMEA_FAULT_INJECTION
int pcie_fault_injection()
{
	if (pmic_read_reg(FAULT_INJECT_REG) == KERNEL_FAULT_PCIE)
		return -EINVAL;
	return 0;
}
#endif
