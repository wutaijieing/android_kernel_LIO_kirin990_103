...
static int sdhci_mshc_i2c_setup(struct platform_device *pdev)
{
	...
	if ((host->quirks2 & SDHCI_QUIRK2_ZODIAC_COMBO_PHY_TC) && !host->i2c_client) {
...
#endif
	if (!(host->quirks2 & SDHCI_QUIRK2_ZODIAC_COMBO_PHY_TC)) {
...
struct sdhci_ops sdhci_mshc_ops = {
	...
	.dump_vendor_regs = sdhci_mshc_dumpregs,
...
#endif
#ifdef CONFIG_MMC_SDHCI_DWC_MSHC
	.adma_write_desc = sdhci_mshc_adma_write_desc,
#endif
};
...
static void sdhci_mshc_populate_dt(struct platform_device *pdev)
{
	...
	if (of_get_property(np, "use-combo-phy-tc", NULL)) {
		host->quirks2 |= SDHCI_QUIRK2_ZODIAC_COMBO_PHY_TC;
