 ...
 *
 *  Copyright (c) 2013 Zodiac Limited.
 *
...
int mmc_get_rw_status(int status)
{
	...
	if (status >= MMC_BLK_CMD_ERR && status <= MMC_BLK_ECC_ERR) {
		...
		if (err_num != 0) {
			printk(KERN_ERR "mmc1 have %d err, but now rw success, clear err!! "
					"status = %d, not report!!\n", err_num, status);
			err_num = 0;
		}
	}
...
unsigned int mmc_calculate_ioworkload_and_rwspeed(unsigned long long time, struct request *rqc, struct gendisk *disk)
{
	...
	if (t_period >= (unsigned long long )PRT_TIME_PERIOD) {
		...
		pr_info("mmcqd/1 rw time:%lld ns, write time:%lld ns,read time:%lld ns\n",
				t_usage, mmcqd_t_usage_wr, mmcqd_t_usage_rd);
		/* worload < 0.01 */
		if (t_period > t_usage*100) {
			pr_info("mmcqd/1 workload < 1%%, duty %lld, period %lld, req_cnt=%d\n",
			...
			pr_info("mmcqd/1 rw_workload = %d%%, w_workload = %d%%, duty %lld, period %lld00, req_cnt=%d\n",
		...
		if (mmcqd_t_usage_wr) {
			...
			if (mmcqd_t_usage_wr) {
				...
				pr_info("mmcqd/1 Write Throughput=%d kB/s, size: %d bytes, time:%lld ms\n",
		...
		if (mmcqd_t_usage_rd) {
			...
			if (mmcqd_t_usage_rd) {
				...
				pr_info("mmcqd/1 Read Throughput=%d kB/s, size: %d bytes, time:%lld ms\n",
