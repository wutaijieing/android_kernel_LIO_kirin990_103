...
static void reset_capacity_fifo(int curr_capacity)
{
	...
	for (i = 0; i < WINDOW_LEN; i++) {
		g_capacity_filter[i] = (unsigned int)curr_capacity;
	...
	struct bci_device_info *di)
{
	...
	if (coul_drv_low_temp_opt() && (bci_temp() < LOW_TEMP_FIVE_DEGREE) &&
		...
		di->monitoring_interval = WORK_INTERVAL_NOARMAL / HALF;
#ifdef CONFIG_DIRECT_CHARGER
	if (direct_charge_in_charging_stage() && di->ac_online &&
		(di->monitoring_interval > (WORK_INTERVAL_NOARMAL / HALF)))
		di->monitoring_interval = WORK_INTERVAL_NOARMAL / HALF;
#endif
#ifdef CONFIG_WIRELESS_CHARGER
...
#ifdef CONFIG_HUAWEI_CHARGER_AP
	info_dump[VBUS] = charge_get_vbus();
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		...
		val->intval = charge_get_vbus() * MILLI_TO_MICRO;
#endif
		break;
...
{
	int i;
	unsigned int array_size = sizeof(di->vth_correct_data);
