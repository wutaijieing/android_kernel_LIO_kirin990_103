...
STATIC int hisi_soh_pmic_acr_calculate_acr(void)
{
	...
	for (i = 0; i < ACR_DATA_FIFO_DEPTH; i++) {
		...
		acr_value += (short)fifo[i] * g_m_r_table[acr_mul_sel][i];
	...
	enum soh_thd_type thd_type, int value, int reg_addr_h, int reg_addr_l)
{
	...
	case VOL_MV:
		...
		reg_value = (unsigned int)coul_drv_convert_mv2regval(value);
		/* write high addr */
		...
		break;
	case TEMP:
		reg_value = (unsigned int)coul_drv_convert_temp_to_adc(value);
		/* write high addr */
...
static void hisi_soh_pmic_remove(struct spmi_device *pdev)
{
