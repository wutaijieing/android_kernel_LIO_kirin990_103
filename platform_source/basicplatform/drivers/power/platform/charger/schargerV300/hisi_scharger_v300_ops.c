...
#include <platform_include/basicplatform/linux/power/platform/charger/hisi_charger_fcp.h>
...
static struct hwfcp_ops hi6523_fcp_protocol_ops = {
...
static struct hwscp_ops hi6523_scp_protocol_ops = {
	.chip_name = "scharger_v300",
...
void hi6523_fcp_scp_ops_register(void)
{
	...
	if (hi6523_is_support_fcp() == 0)
		(void)hwfcp_ops_register(&hi6523_fcp_protocol_ops);

#ifdef CONFIG_DIRECT_CHARGER
	/* if chip support scp, register scp adapter ops */
	if (hi6523_is_support_scp() == 0)
		hwscp_ops_register(&hi6523_scp_protocol_ops);
#endif /* CONFIG_DIRECT_CHARGER */
}
