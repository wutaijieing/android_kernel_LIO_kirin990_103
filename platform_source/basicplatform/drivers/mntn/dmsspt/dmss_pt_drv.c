...
struct dmsspt_cfg {
	...
	u32 filter_mid[MID_GROUP_SIZE]; /* Indicates the MID filtering. The value 0 indicates that the command is not recorded */
};
...
static void dmsspt_enable(int enable)
{
	...
	volatile SOC_DMSS_GLB_INT_CLEAR_STR *int_clear_ptr;
...
#ifdef INTLV_GRAN_START_VALUE0
	intlv_gran_tmp = intlv.reg.intlv_gran + 1;
#else
	intlv_gran_tmp = intlv.reg.intlv_gran;
#endif
	g_pt_cfg.rec_intlv_gran = intlv_gran_tmp;
}
...
static void dmsspt_get_stat(void)
{
	...
	step = g_pt_devp->pt_info.intlv_mode ? (u64)g_pt_devp->pt_info.intlv:((u64)g_pt_devp->pt_info.intlv*2);

	for (i = 0; i < g_pt_devp->pt_info.dmi_num; i++) {
		...
		if (g_pt_devp->pt_stat.trace_roll_cnt[i] &&
		    g_pt_devp->save_cnt == 0) {
...
#ifdef DDRC_FILTER_SRC_PLATFORM
	filter1_ptr->reg.filter_src = cfg->filter_src;
#endif
...
*/
	...
	dmi_num = g_pt_devp->pt_info.dmi_num;
	channel_bw = dmi_num;

	if (dmis >= g_pt_devp->pt_info.dmi_num ||
	    intlv_gran == 0 ||
	...
	*/
	if (g_pt_devp->pt_info.intlv_mode) {
		/* power consumption mode: 0->1->2->3 */
		chsel = dmis + (stat_cur_addr_ch * dmi_num);
	} else {
		/* performance mode: 0->2->1->3 */
		chsel = (dmis * dmi_num) + stat_cur_addr_ch;
	}
	/*
	...
	*/
	...
	phy_addr += g_pt_devp->pt_region.fama_offset;
...
static u16 dmsspt_addr_phy2chans(u64 phy_addr)
{
	...
	channel_bw = g_pt_devp->pt_info.dmi_num;; /* number of bits occupied by the channel ID in the physical address */

	if (phy_addr < g_pt_devp->pt_region.fama_offset) {
...
void dmsspt_stop_store(unsigned int val)
{
	...
	if (val && !dmsspt_is_stop()) {
		...
		g_pt_devp->pt_stat.trace_end_time = sched_clock();
		dmsspt_get_stat();
		(void)dmsspt_save_trace(&g_pt_devp->pt_info, &g_pt_devp->pt_stat, 1);
		spin_unlock_irqrestore(&g_pt_devp->lock, flags);
	}
...
ssize_t dmsspt_comm_show(struct file *filp, char __user *ubuf, size_t size, loff_t *ppos)
{
	...
	if (!access_ok(ubuf, DMSSPT_SHOW_LEN_MAX)) {
...
static irqreturn_t dmsspt_interrupt(int irq, void *dev_id)
{
	...
	volatile SOC_DMSS_GLB_INT_STATUS_STR *int_stat_ptr = DMSS_GLB_INT_STATUS_PTR(g_pt_devp->dmss_base);
	volatile SOC_DMSS_GLB_INT_CLEAR_STR *int_clear_ptr = DMSS_GLB_INT_CLEAR_PTR(g_pt_devp->dmss_base);
	...
	ret = dmsspt_save_trace(&g_pt_devp->pt_info,
				&g_pt_devp->pt_stat,
				(bool)is_stop);
	g_pt_devp->save_cnt++;
	if (ret == 1) {
...
static int dmsspt_platform_adapt(struct device_node *np)
{
	...
	if ((!strncmp(compatible, "hisilicon,dmsspt", cmplen + 1)))
		g_pt_devp->pt_info.dmi_num = 1;
	else
		g_pt_devp->pt_info.dmi_num = DDRC_DMI_NUM;
...
{
	u32 addr_shift_mode = readl(SOC_DMSS_ASI_ADDR_SHIFT_ADDR(g_pt_devp->dmss_base, CPU_ASI_NUM)) &
	...
	if ((addr_shift_mode == ADDR_SHIFT_MODE_1 &&
		...
		g_pt_devp->pt_region.fama_offset = phys - DDR_SIZE_3G512M;
	} else {
		g_pt_devp->pt_region.fama_offset = 0;
	}
	pr_err("%s, fama_offset:[0x%llx]\n", __func__, g_pt_devp->pt_region.fama_offset);
}
					     ...
					     struct platform_device *plat_dev)
{
	...
	g_pt_devp = devm_kzalloc(&plat_dev->dev,
...
static int dmsspt_probe(struct platform_device *pdev)
{
	...
	if (!g_pt_devp->pt_region.base) {
		dev_err(&pdev->dev, "Trace buffer reserved failed\n");
		ret = -ENOMEM;
		goto err;
	}
	...
	g_pt_devp->irq = irq;
	dmsspt_cpu_addr_shift(g_pt_devp->pt_region.base);
	dmsspt_get_intlv();
