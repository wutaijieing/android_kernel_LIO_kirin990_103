...
static char *dmsspt_get_timestamp(void)
{
	...
	tv.tv_sec -= (__kernel_old_time_t)sys_tz.tz_minuteswest * 60;
	rtc_time64_to_tm((unsigned long)tv.tv_sec, &tm);
#else
	struct timeval tv;
	static char databuf[DATE_MAXLEN + 1];
 ...
 */
static int init_params_dmsspt_info(struct pattern_trace_info *pinfo)
{
	u32 i;
	...
	for (i = 0; i < g_pt_info.dmi_num; i++) {
		if (pinfo->pt_rptr[i])
			...
			g_pt_rptr[i] = (u8 *)((u64)g_trace_buffer_vaddr +
					      ((u64)i * (u64)g_pt_info.intlv));
	...
	for (i = g_pt_info.dmi_num; i < DMSS_TRACE_MAX; i++) {
		g_pt_rptr[i] = 0;
		g_pt_info.pt_wptr[i] = 0;
	}

	g_pt_info.intlv_mode = pinfo->intlv_mode;

	/* determine the read step size according to the interleaving mode */
	if (g_pt_info.intlv_mode == TRACE_MODE0) {
		g_step = 2 * g_pt_info.intlv;
	} else if (g_pt_info.intlv_mode == TRACE_MODE1) {
		       ...
		       struct pattern_trace_stat *pstat)
{
	...
	if (init_params_dmsspt_info(pinfo) != 0)
		return -1;
...
int dmsspt_save_trace(struct pattern_trace_info *pinfo,
		      struct pattern_trace_stat *pstat, bool is_tracing_stop)
{
	int i;
	int ret = 0;

	if (dmsspt_check_params(pinfo, pstat) != 0) {
		for (i = 0; i < DMSS_TRACE_MAX; i++) {
					     ...
					     u8 *buffer_end)
{
	...
	pr_err("%s, buffer_end[0x%llx], wptr0[0x%llx] wptr1[0x%llx]\n",
		__func__, (u64)buffer_end,
		info->pt_wptr[TRACE_MODE0], info->pt_wptr[TRACE_MODE1]);
...
static struct file *dmsspt_open(int trace_num, int index)
{
	...
	if (!index) {
		...
		(void)snprintf_s(g_dmsspt_path[trace_num], DMSSPT_PATH_MAXLEN, DMSSPT_PATH_MAXLEN-1, "%s%d_%s", DMSSPT_ROOT"dmsspt", trace_num, dmsspt_get_timestamp());
...
static void dmsspt_finish(int trace_num)
{
	...
	/* wait until the two threads are saved */
	for (i = 0; i < DMSS_TRACE_MAX; i++) {
						      ...
						      int *file_index)
{
	...
	if (down_timeout(g_dump_pt_sem[trace_no], (long)jiffies_priv)) {
	...
	if (g_pt_info.intlv_mode == DMSSPT_NOINIT_MAGIC)
		return SAVE_CONTINUE;
...
static int dmss_pattern_trace_dump_thread(void *data)
{
	...
	if (trace_num < TRACE_MODE0 || trace_num > TRACE_MODE1)
		return -1;
