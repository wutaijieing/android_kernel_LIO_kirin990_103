 ...
 *
 */
static int _rdr_ap_cleartext_print_hk_cpu_onoff(struct file *fp, struct cleartext_param param,
				u8 *addr, struct ap_eh_root *ap_root)
{
	...
	q = (struct dfx_ap_ringbuffer_s *)real_addr;
 ...
 *
 */
static int rdr_ap_cleartext_print_hk_cpu_onoff(const char *dir_path, u64 log_addr, u32 log_len)
{
	...
	if (unlikely(_rdr_ap_cleartext_print_hk_cpu_onoff(fp, param,
 ...
 *
 */
static int rdr_ap_cleartext_print_hk_cpuidle_on_cpu(struct file *fp, struct cleartext_param param,
 ...
 *
 */
static int rdr_ap_cleartext_print_hk_cpuidle(const char *dir_path, u64 log_addr, u32 log_len)
{
	struct file *fp = NULL;
	struct ap_eh_root  *ap_root = NULL;
	int         ret = 0;
	...
	for (i = 0; i < NR_CPUS; i++) {
		...
		if (unlikely(rdr_ap_cleartext_print_hk_cpuidle_on_cpu(fp, param,
 ...
 *
 */
static int rdr_ap_cleartext_print_hk_irq_on_cpu(struct file *fp, struct cleartext_param param,
				u8 *addr, struct ap_eh_root *ap_root, u32 cpu)
{
	struct dfx_ap_ringbuffer_s *q = NULL;
 ...
 *
 */
static int rdr_ap_cleartext_print_hk_irq(const char *dir_path, u64 log_addr, u32 log_len)
{
	struct file *fp = NULL;
	struct ap_eh_root  *ap_root = NULL;
	int         ret = 0;
	...
	for (i = 0; i < NR_CPUS; i++) {
		...
		param.log_addr = log_addr;
		if (unlikely(rdr_ap_cleartext_print_hk_irq_on_cpu(fp, param,
 ...
 *
 */
static int rdr_ap_cleartext_print_ap_root(const char *dir_path, u64 log_addr, u32 log_len)
{
	struct file *fp = NULL;
	struct ap_eh_root *ap_root = NULL;
	bool        error = false;
...
int rdr_ap_cleartext_print(const char *dir_path, u64 log_addr, u32 log_len)
{
	...
	if (unlikely(rdr_ap_cleartext_print_ap_root(dir_path, log_addr, log_len))) {
		BB_PRINT_ERR("rdr_ap_cleartext_print_ap_root() ret error\n");
		return -1;
	}

	if (unlikely(rdr_ap_cleartext_print_hk_irq(dir_path, log_addr, log_len))) {
		BB_PRINT_ERR("rdr_ap_cleartext_print_hk_irq() ret error\n");
		return -1;
	}

	if (unlikely(rdr_ap_cleartext_print_hk_cpuidle(dir_path, log_addr, log_len))) {
		BB_PRINT_ERR("rdr_ap_cleartext_print_hk_cpuidle() ret error\n");
		return -1;
	}

	if (unlikely(rdr_ap_cleartext_print_hk_cpu_onoff(dir_path, log_addr, log_len))) {
		BB_PRINT_ERR("rdr_ap_cleartext_print_hk_cpu_onoff() ret error\n");
		return -1;
	}
