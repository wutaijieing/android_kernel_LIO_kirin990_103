...
#define PR_LOG_TAG VIRT_TO_PHYS_TAG
			       ...
			       size_t count)
{
	...
	while ((len > 0) && (args < 3)) {
		...
		case 0:
			/* cppcheck-suppress * */
			if (sscanf(head, "%u", &pid) != 1) {
				return -1;
			}
			break;
		case 1:
			if (sscanf(head, "%lx", &virt_addr) != 1) {
				return -1;
			}
			break;
		case 2:
			if (sscanf(head, "%u", &virt_len) != 1) {
				return -1;
			}
			break;
		default:
...
#define KVIRT_ATTR_ACCESS 0640
static struct kobj_attribute kvirt_attr =
		__ATTR(virt_to_phys, KVIRT_ATTR_ACCESS, show_phys_addr, store_virt_addr);
