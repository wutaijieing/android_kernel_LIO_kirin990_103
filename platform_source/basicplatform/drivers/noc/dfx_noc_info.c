...
#define PLATFORM_ID_BIT_SHEEP            (18U)
...
struct noc_platform_info g_noc_platform_info[] = {
	...
	{
		.name = "sheep",
		.platform_id = 1 << PLATFORM_ID_BIT_SHEEP,	 /* must be same as the value defined in DTS. */
		.p_noc_info_bus = NULL,
		.p_noc_info_dump = NULL,
		.p_noc_info_filter_initflow = NULL,
		.pfun_get_size = NULL,
...
int noc_get_platform_info_index(unsigned int platform_id)
{
	...
	for (index = 0; index < (int)ARRAY_SIZE_NOC(g_noc_platform_info); index++) {
