...
#endif
#ifdef CONFIG_MNTN_NOC_REGISTER_BBOX
#include "dfx_noc_register_bbox.h"
#endif
...
static noinline u64 dfx_noc_set_npu_nosec(void)
{
	...
	pr_err("%s +\n", __func__);
	arm_smccc_smc(MNTN_NOC_SET_NPU_NOSEC, 0, 0, 0, 0, 0, 0, 0, &res);
	pr_err("%s -\n", __func__);
	return (u64)res.a0;
}
...
NOC_ERROR_CHECK:
	if (is_noc_node_available(node)) {
		...
		pr_err("Finally, noc_node: %s powerdown state change fail!\n",
		       node->name);
		/* while(1);
...
#ifdef CONFIG_DFX_BB
		if (check_mntn_switch(MNTN_NOC_ERROR_REBOOT) && noc_err_smp_stop_acpu(pending)) {
...
static int dfx_noc_part_probe(struct platform_device *pdev, struct device_node *np)
{
	...
	if (of_property_read_u32(np, "reg-dump-nums", &nums)) {
...
static int noc_irq_register(struct dfx_noc_device *noc_dev, struct platform_device *pdev)
{
	...
	if (noc_property_dt.noc_timeout_enable) {
		...
		noc_dev->timeout_irq = (unsigned int)ret;
	...
	if (noc_property_dt.noc_aobus_int_enable) {
		...
		noc_property_dt.noc_aobus_timeout_irq = (unsigned int)ret;
...
#ifdef CONFIG_MNTN_NOC_REGISTER_BBOX
	ret = noc_register_bbox();
	if (ret < 0) {
		pr_err("[%s] noc_register_bbox failed!\n", __func__);
		devm_kfree(&pdev->dev, noc_dev);
		return ret;
	}
#endif
