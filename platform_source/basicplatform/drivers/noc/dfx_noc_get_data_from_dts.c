...
#define ROUTERID_ADDR_TRANSFORM2_SHIFT 8
...
static int dfx_noc_get_routeid_addr_tbl(unsigned int busid, struct device_node *np)
{
	...
	for (i = 0, val = prop->value; i < nums; i++) {
		routeid_addr_tbl[i].init_flow = (int) (be32_to_cpup(val++));
		routeid_addr_tbl[i].targ_flow = (int) (be32_to_cpup(val++));
		routeid_addr_tbl[i].targ_subrange = (int) (be32_to_cpup(val++));
		routeid_addr_tbl[i].init_localaddr = (u64) (be32_to_cpup(val++));
	/*
	 * Read the base address of the targetflow from routeid_list in the DTS. Some targetflow addresses exceed 32 bits.
	 * The value of #address-cells = <1> in the dts file contains a maximum of 32 bits.
	 * Therefore, the lower four bits of the 32 bits are used as flags and are processed in the c program.
	 * The value 1 indicates that four bits (36 bits) are shifted to the left.
	 * The value 2 indicates that eight bits (40 bits) are shifted to the left.
	 * eg:  dts member routeid_list  0x80000001(32bit)
	 *           blow code handle   0x800000000(36bit)
	 */
		...
		if ((routeid_addr_tbl[i].init_localaddr & NOC_ROUTEID_TBL2_FLAG) == NOC_ROUTEID_TBL2_FLAG)
			routeid_addr_tbl[i].init_localaddr = (routeid_addr_tbl[i].init_localaddr -
				NOC_ROUTEID_TBL2_FLAG) << ROUTERID_ADDR_TRANSFORM2_SHIFT;
	}
...
void free_noc_bus_source(void)
{
	unsigned int i, sec_num, mid_num;
	struct noc_sec_info *sec_info = NULL;
	struct noc_mid_info *mid_info = NULL;
