...
static void hi6502_delay_work(struct work_struct *work)
{
	...
	hi6502_readl_u32(&reg_val, hi6502_collect.sctrl_base +
	...
	if (sctrl_val) {
		for_each_set_bit(sctrl_offset, &sctrl_val, HI6502_GPIO_BANK) {
			...
			hi6502_readl_u32(&reg_val, hi6502_collect.base[s] + GPIOMIS);
			pending = reg_val;
			if (pending) {
	...
	struct hi6502_gpio *hi6502)
{
	...
	if (of_get_property(np, "gpio-ranges", NULL)) {
		...
		hi6502->gc.request = gpiochip_usbphy_request;
		hi6502->gc.free = gpiochip_usbphy_free;
	}
	kfree(reg_prop);
