	...
	unsigned int pin, struct pch_function **func)
{
	...
	if(!function) {
		dev_err(pch->dev, "%s function is null\n", __func__);
		return -ENOTSUPP;
	}
	*func = function->data;
	if (!(*func)) {
	...
	unsigned long *configs, unsigned int num_configs)
{
	...
	unsigned int offset = 0, data, i, j;
	u32 arg;
	int ret;
...
{
	int res;
	unsigned int i, nr_pins, mux_bytes;
	...
	struct pch_conf_vals **conf, unsigned long **settings)
{
	...
	cfparam.mask = value[1];
	add_config(conf, cfparam);
	add_setting(settings, pchcnf->param, (unsigned int)ret);
}
...
free_pingroups:
	pinctrl_generic_remove_group(pch->pctl, pch->pctl->num_groups - 1);
	*num_maps = 1;
free_function:
	pinmux_generic_remove_function(pch->pctl, pch->pctl->num_functions - 1);
free_pins:
