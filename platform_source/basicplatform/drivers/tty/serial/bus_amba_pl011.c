/*
 * Amba-pl011.c. Driver for AMBA serial ports
 *
...
static int pl011_uart_tx_work(void *arg)
{
	...
	while (!kthread_should_stop()) {
		...
		if (temp && !atomic_read(&unit->tx_sig))
			pr_debug("%s wait event fail. sig:%d temp:%d\n",
				__func__, atomic_read(&unit->tx_sig), temp);
...
int bus_pl011_probe_get_clk_freq(
...
int bus_pl011_probe_reset_func_enable(
...
static int bus_pl011_check_console_name(const char *amba_console_name,
	struct uart_amba_port *uap)
{
...
static int bus_pl011_check_fifo_status(struct amba_device *dev,
	struct uart_amba_port *uap)
{
...
static int bus_pl011_prepare_fifo_cfg(struct amba_device *dev,
...
void bus_pl011_probe_console_enable(struct amba_device *dev,
	struct uart_amba_port *uap, const char *amba_console_name)
{
	u32 console_fifo_size = 0;
	int console_fifo_cpuon = 0;

	if (bus_pl011_check_console_name(amba_console_name, uap)) {
		dev_info(&dev->dev, "%s:check console name fail\n", __func__);
		return;
	}

	if (bus_pl011_check_fifo_status(dev, uap)) {
		dev_info(&dev->dev, "%s:check fifo status fail\n", __func__);
		return;
	}

	if (bus_pl011_prepare_fifo_cfg(dev, uap, &console_fifo_size,
		&console_fifo_cpuon)) {
...
static void bus_pl011_console_write(
	struct console *co, const char *s, unsigned int count)
{
...
static int bus_pl011_suspend(struct device *dev)
{
...
static int bus_pl011_resume(struct device *dev)
{
	struct uart_amba_port *uap = dev_get_drvdata(dev);
	int ret;
...
#ifdef CONFIG_FMEA_FAULT_INJECTION
static int stub_hs_dw_uart_injection(void)
{
	if (pmic_read_reg(FAULT_INJECT_REG) == KERNEL_FAULT_UART) {
		pr_err("%s start uart init fail fault injection.\n", __func__);
		return -ENODEV;
	}
...
static void bus_pl011_disable_ms(struct uart_port *port)
{
	struct uart_amba_port *uap =
