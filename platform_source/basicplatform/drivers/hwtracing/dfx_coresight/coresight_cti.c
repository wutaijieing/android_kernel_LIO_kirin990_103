 ...
 * coresight-cti.c
 *
 * Coresight Computer Telephony Integration module
 *
...
void coresight_cti_ctx_save(void)
{
	...
	list_for_each_entry(cti, &cti_list, link) {
		...
		for_each_cpu(cpuid, topology_core_cpumask(cpu)) { //lint !e574
...
static void __cti_map_trigin(struct cti_drvdata *drvdata, int trig, int ch)
{
	...
	if (ctien & (0x1u << (unsigned int)ch))
		goto out;
	cti_writel(drvdata, (ctien | (0x1u << (unsigned int)ch)), CTIINEN(trig));
...
static void __cti_map_trigout(struct cti_drvdata *drvdata, int trig, int ch)
{
	...
	if (ctien & (0x1u << (unsigned int)ch))
		goto out;
	cti_writel(drvdata, (ctien | (0x1u << (unsigned int)ch)), CTIOUTEN(trig));
...
static int __cti_enable_gate(struct cti_drvdata *drvdata, int ch)
{
	...
	cti_writel(drvdata, (ctigate & ~(1u << (unsigned int)ch)), CTIGATE);
