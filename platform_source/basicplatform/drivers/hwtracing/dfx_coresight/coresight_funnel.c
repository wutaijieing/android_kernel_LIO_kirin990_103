...
static void funnel_disable_hw(struct funnel_drvdata *drvdata, int inport)
{
	...
	functl = readl_relaxed(drvdata->base + FUNNEL_FUNCTL);
	functl &= ~(1u << (unsigned int)inport);/*lint !e502 */
	writel_relaxed(functl, drvdata->base + FUNNEL_FUNCTL);
	/* backup functl value for restore */
	drvdata->functl = functl;
				...
				struct device_attribute *attr, char *buf)
{
	...
	return snprintf_s(buf, PAGE_SIZE, sizeof(val), "%#x\n", val);
}
...
static int funnel_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	if (!IS_ERR_OR_NULL(drvdata->atclk)) {
		...
		ret = clk_prepare_enable(drvdata->atclk);
		if (ret) {
			dev_err(dev, "atclk prepare failed!\n");
			goto err_next;
		}
	} else {
...
static void funnel_remove(struct amba_device *adev)
{
