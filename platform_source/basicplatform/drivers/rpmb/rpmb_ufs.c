...
static u64 g_cmd_lock_release = 0;
				  ...
				  bool is_write, enum rpmb_req_source source)
{
	...
	g_cmd_lock_done = dfx_getcurtime();
#else
	g_cmd_lock_done = local_clock();
#endif
	/*lint -e501*/
	oldfs = get_fs();
	...
	for (i = 0; i < UFS_IOC_MAX_RPMB_CMD; i++) {
		...
		while (retry_times < RPMB_IOCTL_RETRY_TIMES) {
			ret = (int32_t)blk_scsi_kern_ioctl(
				...
				(!i && is_write), (unsigned int)source);
...
#ifdef CONFIG_DFX_TIME
	g_cmd_lock_release = dfx_getcurtime();
#else
	g_cmd_lock_release = local_clock();
#endif
	temp_cost_time = g_cmd_lock_release - g_cmd_lock_done;
	if (temp_cost_time > RPMB_TIMEOUT_TIME_IN_KERNEL)
		pr_err("rpmb lock time is more than 800ms, cost_time = %llu, source = %d\n",
		       temp_cost_time, source);
...
#ifdef CONFIG_DFX_TIME
	g_resoure_alloc_done = dfx_getcurtime();
#else
	g_resoure_alloc_done = local_clock();
#endif
