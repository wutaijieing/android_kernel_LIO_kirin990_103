...
static void ufshcd_enable_autodiv(struct ufs_hba *hba)
{
	...
	writel(UFS_AUTODIV_VAL, SOC_HSDT_CRG_PERI_AUTODIV0_ADDR(host->hsdt_crg));

	value = readl(SOC_HSDT_CRG_PERI_AUTODIV0_ADDR(host->hsdt_crg));
	value &= ~(BIT(SOC_HSDT_CRG_PERI_AUTODIV0_ufsbus_div_auto_reduce_bypass_lpm3_START));
	writel(value, SOC_HSDT_CRG_PERI_AUTODIV0_ADDR(host->hsdt_crg));

	value = readl(SOC_HSDT_CRG_PERCLKEN4_ADDR(host->hsdt_crg));
	value |= BIT(SOC_HSDT_CRG_PERCLKEN4_gt_clk_autodiv_ufs_subsys_START);
	writel(value, SOC_HSDT_CRG_PERCLKEN4_ADDR(host->hsdt_crg));
}
#else
static void ufshcd_enable_autodiv(struct ufs_hba *hba)
{
	...
	value &= ~(BIT(SOC_SCTRL_SCUFS_AUTODIV_ufsbus_div_auto_reduce_bypass_lpm3_START));
	...
	struct ufs_hba *hba, struct ufs_pa_layer_attr *dev_req_params)
{
	...
	if (dev_req_params->pwr_rx == FAST_MODE ||
		...
		case UFS_HS_G4:
			cfg = hufs_mphy_v200_tc_pre_pmc_fsg4_attr;
			break;
		case UFS_HS_G3:
			cfg = hufs_mphy_v200_tc_pre_pmc_fsg3_attr;
			break;
		case UFS_HS_G2:
			cfg = hufs_mphy_v200_tc_pre_pmc_fsg2_attr;
			break;
		case UFS_HS_G1:
			cfg = hufs_mphy_v200_tc_pre_pmc_fsg1_attr;
			break;
		default:
		...
		cfg = hufs_mphy_v200_tc_pre_pmc_slow_attr;
	}
