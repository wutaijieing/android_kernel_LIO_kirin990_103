...
#include <soc_core_ctrl_interface.h>
#include <soc_smmuv3_tbu_interface.h>
#include <soc_hsdt0_xctrl_interface.h>
#include <soc_hsdt0_sctrl_interface.h>
...
static void ufs_sub_sys_crg_reset_release(struct hufs_host *host)
{
	u32 value;
	int retry = POLLING_RETRY;

	set_bits(BIT(IP_RST_UFS_SUBSYS_CRG), SOC_HSDT_CRG_PERRSTDIS1_ADDR(host->hsdt_crg));
	while (retry--) {
		value = readl(SOC_HSDT_CRG_PERRSTSTAT1_ADDR(host->hsdt_crg));
		if (!(value & BIT(IP_RST_UFS_SUBSYS_CRG)))
			return;
		udelay(1);
	}
	dev_err(host->hba->dev, "UFS Sub-sys CRG reset failed\n");
}

int ufs_wait_for_register_poll(u64 reg, u32 mask, u32 val, u32 timeout_ms)
{
	u32 reg_val;
...
static void ufs_cfg_exit_lowpower_handshake(struct hufs_host *host)
{
	int ret;

	/* 24. config tp Mem SD */
	writel(0x00000050, SOC_UFS_Sysctrl_MEMORY_CTRL_TP_ADDR(host->ufs_sys_ctrl));

	/* 25. config sp Mem SD */
	writel(0x00005858, SOC_UFS_Sysctrl_MEMORY_CTRL_ADDR(host->ufs_sys_ctrl));

	/* 26. check ufs_mem_sp_sdo and ufs_mem_tp_sdo */
	ret = ufs_wait_for_register_poll(SOC_UFS_Sysctrl_MEMORY_CTRL_TP_ADDR(host->ufs_sys_ctrl),
			BIT(UFS_SC_PSW_MEM_SDO), 0, 1);
	if (ret)
		pr_err("tp timeout\n");

	ret = ufs_wait_for_register_poll(SOC_UFS_Sysctrl_MEMORY_CTRL_ADDR(host->ufs_sys_ctrl),
			BIT(UFS_MEM_SP_SDO), 0, 1);
	if (ret)
		pr_err("sp timeout\n");

	/* 27. config UFS powerdomain for low-power handshake */
	writel(PDC_UFS_SYSPDC_PWRD_MASK << 16,
			SOC_HSDT0_XCTRL_QIC_POWERDOMAIN_REQ_ADDR(host->hsdt_ctrl));
	ret = ufs_wait_for_register_poll(SOC_HSDT0_XCTRL_QIC_POWERDOMAIN_ACK_ADDR(host->hsdt_ctrl),
			PDC_UFS_SYSPDC_PWRD_MASK, 0, 2);
	if (ret)
		pr_err("pw domain timeout\n");
}

static void ufs_cfg_enter_lowpower_handshake(struct hufs_host *host)
{
	int ret;

	/* 14. config UFS powerdomain enter low-power handshake */
	writel(((PDC_UFS_SYSPDC_PWRD_MASK << 16) | PDC_UFS_SYSPDC_PWRD_MASK),
			SOC_HSDT0_XCTRL_QIC_POWERDOMAIN_REQ_ADDR(host->hsdt_ctrl));
	ret = ufs_wait_for_register_poll(SOC_HSDT0_XCTRL_QIC_POWERDOMAIN_ACK_ADDR(host->hsdt_ctrl),
			PDC_UFS_SYSPDC_PWRD_MASK, PDC_UFS_SYSPDC_PWRD_MASK, 2);
	if (ret)
		pr_err("%s: pw domain timeout\n", __func__);

	/* 15. config tp Mem SD 1 */
	writel(0x00000054, SOC_UFS_Sysctrl_MEMORY_CTRL_TP_ADDR(host->ufs_sys_ctrl));

	/* 16. config sp Mem SD 1 */
	writel(0x0000585C, SOC_UFS_Sysctrl_MEMORY_CTRL_ADDR(host->ufs_sys_ctrl));
}
...
void ufs_soc_init(struct ufs_hba *hba)
{
	...
	/* 1. Configure IBQE to exit low power consumption */
	writel(0x00080000, SOC_CORE_CTRL_P1_MASK20_ADDR(host->core_ctrl));
	ret = ufs_wait_for_register_poll(SOC_CORE_CTRL_CORE_STAT_P2_1_ADDR(host->core_ctrl),
		BIT(SYSPMC_PWRD_REQ_HSDT0BUS), 0, 1);
	if (ret)
		dev_err(hba->dev, "wait core stat timeout\n");

	/* 3. Set SD and DS to 0 */
	writel(0x00030000, SOC_ACTRL_AO_MEM_CTRL2_ADDR(host->actrl));

	/* 5. release reset of UFS subsystem CRG */
	ufs_sub_sys_crg_reset_release(host);

	/* 8. Enable the asynchronous bridge clock of ufs to qic, set bit1 */
	set_bits(BIT(ACLK_UFS2QIC_ABRG_MASK), SOC_HSDT_CRG_PEREN1_ADDR(host->hsdt_crg));

	/* 14. Release MPHY related ISO */
	writel(0x00210000, SOC_ACTRL_ISO_EN_GROUP0_PERI_ADDR(host->actrl));

	/* 15. Enable clk_ufs_subsys clock */
	writel(GT_CLK_UFS_SUBSYS, SOC_HSDT_CRG_PEREN4_ADDR(host->hsdt_crg));

	/* 16. enable ufs tbu clk, set bit9 */
	set_bits(BIT(CLK_UFS_TBU_MASK), SOC_HSDT_CRG_PEREN1_ADDR(host->hsdt_crg));

	/* 17. release ufs tbu reset, set bit8 */
	set_bits(BIT(IP_RST_UFS_TBU), SOC_HSDT_CRG_PERRSTDIS0_ADDR(host->hsdt_crg));

	/* 18. config smmu tbu ufs bypass */
	set_bits(BIT(SMMU_TBU_BYPASS_BIT),
		 SOC_SMMUv3_TBU_SMMU_TBU_SCR_ADDR(host->ufs_tbu));

	/* 19. enable ip_rst_ufs_subsys, reset ufs sub-system */
	set_bits(BIT(IP_RST_UFS_SUBSYS), SOC_HSDT_CRG_PERRSTEN1_ADDR(host->hsdt_crg));

	/* 24. config UFS powerdomain for low-power handshake */
	writel(PDC_UFS_SYSPDC_PWRD_MASK << 16,
			SOC_HSDT0_XCTRL_QIC_POWERDOMAIN_REQ_ADDR(host->hsdt_ctrl));
	ret = ufs_wait_for_register_poll(SOC_HSDT0_XCTRL_QIC_POWERDOMAIN_ACK_ADDR(host->hsdt_ctrl),
			PDC_UFS_SYSPDC_PWRD_MASK, 0, 2);
	...
	/* 1. release ufs mast-reset */
	set_bits(BIT(IP_RST_UFS_SUBSYS),
		 SOC_HSDT_CRG_PERRSTDIS1_ADDR(host->hsdt_crg));

	/* 2. release internal resets */
	...
	/* 6. check ufs_mem_sp_sdo and ufs_mem_tp_sdo */
	ret = ufs_wait_for_register_poll(SOC_UFS_Sysctrl_MEMORY_CTRL_TP_ADDR(host->ufs_sys_ctrl),
			BIT(UFS_SC_PSW_MEM_SDO), 0, 1);
	if (ret)
		pr_err("tp timeout\n");
...
int hufs_suspend(struct ufs_hba *hba, enum ufs_pm_op pm_op)
{
	...
	/* Step 6. check if the store is complete */
	ret = ufs_access_register_and_check(hba, UFS_CFG_SUSPEND);
	if (ret)
		return ret;

	hufs_disable_all_irq(hba);
	/* Step 12. clear bit0 of ufs_cfg_ram_ctrl */
	value = ufshcd_readl(hba, UFS_CFG_RAM_CTRL);
	value &= (~0x01);
	ufshcd_writel(hba, value, UFS_CFG_RAM_CTRL);

	/* Step 13. enable MPHY ISO */
	mphy_iso_enable(hba);

	/* Step 14 ~ 16. enter lowpower handshake */
	ufs_cfg_enter_lowpower_handshake(host);

	/* Step 18. enable ufs tbu reset, set bit8 */
	set_bits(BIT(IP_RST_UFS_TBU), SOC_HSDT_CRG_PERRSTEN0_ADDR(host->hsdt_crg));

	/* Step 19. close ufs tbu clk, set bit9 */
	set_bits(BIT(CLK_UFS_TBU_MASK), SOC_HSDT_CRG_PERDIS1_ADDR(host->hsdt_crg));

	/* Step 20. enable ufs_tbu sd, set 0x1 */
	writel(0x1, SOC_HSDT0_SCTRL_SC_UFS_TBU_MEM_CTRL_SD_ADDR(host->hsdt_sysctrl));

	/* Step 21. enable ip_rst_ufs_subsys, reset ufs sub-system */
	set_bits(BIT(IP_RST_UFS_SUBSYS), SOC_HSDT_CRG_PERRSTEN1_ADDR(host->hsdt_crg));

	/* Step 22. disable input clk of UFS sub-system */
	set_bits(GT_CLK_UFS_SUBSYS, SOC_HSDT_CRG_PERDIS4_ADDR(host->hsdt_crg));

	/* Step 23. enable reset of UFS sub-system CRG */
	set_bits(BIT(IP_RST_UFS_SUBSYS_CRG), SOC_HSDT_CRG_PERRSTEN1_ADDR(host->hsdt_crg));

	/* Step 28. close the asynchronous bridge clock of ufs to qic, set bit1 */
	set_bits(BIT(ACLK_UFS2QIC_ABRG_MASK), SOC_HSDT_CRG_PERDIS1_ADDR(host->hsdt_crg));

	/* Step 29. enable RET of dual-rail RAM, set DS =1 , SD = 0 */
	writel(0x00030002, SOC_ACTRL_AO_MEM_CTRL2_ADDR(host->actrl));

	/* Step 31. Configure PMU register to disable the reference clock of UFS */
...
int hufs_main_resume(struct ufs_hba *hba, enum ufs_pm_op pm_op)
{
	...
	/* Step 8. Enable the asynchronous bridge clock of ufs to qic, set bit1 */
	set_bits(BIT(ACLK_UFS2QIC_ABRG_MASK), SOC_HSDT_CRG_PEREN1_ADDR(host->hsdt_crg));

	/* Step 14. release the reset of UFS subsys CRG */
	ufs_sub_sys_crg_reset_release(host);

	/* Step 15. Release the reset of UFS_SUBSYS */
	set_bits(BIT(IP_RST_UFS_SUBSYS), SOC_HSDT_CRG_PERRSTDIS1_ADDR(host->hsdt_crg));

	/* Step 16. Enable clk_ufs_subsys clock */
	writel(GT_CLK_UFS_SUBSYS, SOC_HSDT_CRG_PEREN4_ADDR(host->hsdt_crg));

	/* Step 17. enable ufs tbu clk, set bit9 */
	set_bits(BIT(CLK_UFS_TBU_MASK), SOC_HSDT_CRG_PEREN1_ADDR(host->hsdt_crg));

	/* Step 18. release ufs tbu reset, set bit8 */
	set_bits(BIT(IP_RST_UFS_TBU), SOC_HSDT_CRG_PERRSTDIS0_ADDR(host->hsdt_crg));

	/* Step 19. config smmu tbu ufs bypass */
	set_bits(BIT(SMMU_TBU_BYPASS_BIT),
		 SOC_SMMUv3_TBU_SMMU_TBU_SCR_ADDR(host->ufs_tbu));

	/* Step 20. release internal release of UFS Subsys */
	writel(0xFFFFFFFF, host->ufs_sys_ctrl + UFS_SYSCTRL_RESET_CTRL_OFFSET);

	/* 21 ~ 24.config for low-power  */
	ufs_cfg_exit_lowpower_handshake(host);
...
int hufs_spare_resume(struct ufs_hba *hba, enum ufs_pm_op pm_op)
{
	...
	/* Step 6. Enable the asynchronous bridge clock of ufs to qic, set bit1 */
	set_bits(BIT(ACLK_UFS2QIC_ABRG_MASK), SOC_HSDT_CRG_PEREN1_ADDR(host->hsdt_crg));

	/* Step 12. disable RET of dual-rail RAM, DS = 0, SD = 0 */
	writel(0x00030000, SOC_ACTRL_AO_MEM_CTRL2_ADDR(host->actrl));

	/* Step 13. Reset UFS_SUBSYS */
	set_bits(BIT(IP_RST_UFS_SUBSYS), SOC_HSDT_CRG_PERRSTEN1_ADDR(host->hsdt_crg));
	...
	/* Step 21. disable UFS CFG clk */
	writel(0x00080000, host->ufs_sys_ctrl + UFS_SYSCTRL_CRG_CLK_OFFSET);

	/* Step 22. disable MPHY ISO */
	mphy_iso_disable(hba);

	/* Step 23. enable UFS CFG clk */
	writel(0x00080008, host->ufs_sys_ctrl + UFS_SYSCTRL_CRG_CLK_OFFSET);

	/* 24 ~ 27.config for low-power  */
	ufs_cfg_exit_lowpower_handshake(host);

	/* Step 30. wait for UFS MPHY PLL lock */
	udelay(MPHY_PLL_LOCK_WAIT_TIME);
...
int hufs_link_startup_post_change(struct ufs_hba *hba)
{
	...
	if ((host->caps & USE_HUFS_MPHY_TC)) {
		...
		/* avoid FPGA outstanding proble */
		ufshcd_writel(hba, 0x7FFF, 0xF8);
		pr_err("%s: outstanding num is 0x%x\n", __func__, ufshcd_readl(hba, 0xF8));
	}
...
static void ufs_get_smmu_tbu(struct hufs_host *host)
{
	...
	dev = host->hba->dev;
	pdev = to_platform_device(host->hba->dev);

	/* get resource of ufs smmu tbu */
	mem_res = platform_get_resource(pdev, IORESOURCE_MEM, 3);
	host->ufs_tbu = devm_ioremap_resource(dev, mem_res);
}
...
int ufs_get_hufs_hc(struct hufs_host *host)
{
	...
	np = of_find_compatible_node(NULL, NULL, "hisilicon,core_ctrl");
	if (!np) {
		dev_err(host->hba->dev,
			"can't find device node hisilicon,core_ctrl\n");
		return -ENXIO;
	}

	host->core_ctrl = of_iomap(np, 0);
	if (!host->core_ctrl) {
		dev_err(host->hba->dev, "core_ctrl iomap error!\n");
		return -ENOMEM;
	}
