...
static bool ufshcd_dev_data_move_check_need_repeat(u8 stream_id)
{
	unsigned int mas_sec_size = g_data_move_ctl[stream_id].data_move_info.done_info.mas_sec_size;
	unsigned int dest_4k_lba = g_data_move_ctl[stream_id].data_move_info.dest_4k_lba;
	unsigned int dm_stream_addr = g_data_move_ctl[stream_id].stor_info.dm_stream_addr[stream_id];

	if (mas_sec_size) {
		if ((dest_4k_lba / mas_sec_size) == (dm_stream_addr / mas_sec_size)) {
			if (dm_stream_addr <= dest_4k_lba)
				return true;
		} else if (!(dest_4k_lba % mas_sec_size)) {
			return true;
		}
	} else if (dest_4k_lba == dm_stream_addr) {
		return true;
	}
...
static void ufshcd_dev_data_move_pwron_done(int err, u8 pwron_type)
{
	...
	if (!ufshcd_dev_data_move_check_need_repeat(stream_id))
		/* 1:verify Fail, 2: UNC fail, 3:Program Fail 4:IO Fail 5:reset err */
...
int ufshcd_dev_slc_mode_error_inject(
	struct scsi_device *sdev, unsigned char error_status)
{
	int ret;
	struct ufs_hba *hba = shost_priv(sdev->host);
	struct ufs_query_vcmd cmd = { 0 };
	...
	cmd.opcode = UPIU_QUERY_OPCODE_VENDOR_WRITE;
	cmd.idn = QUERY_SLC_MODE_ERROR_INJECT;
	cmd.query_func = UPIU_QUERY_FUNC_STANDARD_WRITE_REQUEST;
	cmd.reserved_osf =
		__cpu_to_be16((u16)error_status << 8);
