...
void ufshcd_unistore_op_register(struct ufs_hba *hba)
{
	...
	hostt->dev_slc_mode_error_inject = ufshcd_dev_slc_mode_error_inject;
#endif
}
...
#define CPPLUS_GROUP_NUM 0x1F
static void ufshcd_set_cpplus_group_num(struct utp_upiu_req *ucd_req_ptr,
	struct request *req)
{
	if (ucd_req_ptr->sc.cdb[0] == READ_10) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
		if ((req->mas_cmd_flags & REQ_CP) && (req->mas_cmd_flags & MAS_REQ_CPPLUS))
#else
		if ((req->cmd_flags & REQ_CP) && (req->mas_cmd_flags & MAS_REQ_CPPLUS))
#endif
			ucd_req_ptr->sc.cdb[6] = CPPLUS_GROUP_NUM;

#if defined(CONFIG_MAS_DEBUG_FS)
		if (mas_blk_unistore_debug_en())
			pr_err("%s, group num 0x%x\n", __func__, ucd_req_ptr->sc.cdb[6]);
#endif
	}
}
	...
	struct scsi_cmnd *scmd, struct ufs_hba *hba)
{
	...
	ufshcd_set_cpplus_group_num(ucd_req_ptr, req);
