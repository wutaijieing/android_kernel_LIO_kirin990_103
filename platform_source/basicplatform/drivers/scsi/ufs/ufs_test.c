...
static int ufs_test_prepare(struct ufs_test *test)
{
	...
	if (!desc_buf)
		return -ENOMEM;
	memset(desc_buf, 0, QUERY_DESC_UNIT_MAX_SIZE); /* unsafe_function_ignore: memset */
	...
	for (i = 0; i < UFS_UPIU_MAX_GENERAL_LUN; i++) {
		err = ufshcd_query_descriptor_retry(test->hba,
						  ...
						  desc_buf, &buf_len);
		if (!err) {
			qlogical_block_count = get_unaligned_be64(&desc_buf[11]);
			test->lun_max_lba[i] = qlogical_block_count;
			if (test->max_addr < qlogical_block_count) {
				test->max_addr = qlogical_block_count;
				test->g_lun = i;
		    }
		} else{
			kfree(desc_buf);
				    ...
				    enum cmdflag cmd_flag, int lun)
{
	...
	memset(buf, 0, (BLOCK_COUNT * block_cnt + UFSHCD_REQ_SENSE_SIZE)); /* unsafe_function_ignore: memset */
...
static int ufs_scsi_send_inqury(struct ufs_test *test)
{
	...
	if (!dma_buf)
		return -ENOMEM;
	memset(dma_buf, 0, BLOCK_COUNT); /* unsafe_function_ignore: memset */
	...
	if (0 !=
		...
		err =
		    ufshcd_query_descriptor_retry(hba,
						  ...
						  desc_buf, &buf_len);
		if (!err) {
			blogical_block_size = desc_buf[10];
			qlogical_block_count = get_unaligned_be64(&desc_buf[11]);
			pr_info("Query_Desc: lba:%d lun%d\n",
				qlogical_block_count, lun);
			if ((sector_size == (1 << blogical_block_size))
			    && (lba == qlogical_block_count - 1))
				err = TEST_STATUS_OK;
		} else {
			kfree(dma_buf);
			kfree(desc_buf);
...
static int ufs_scsi_read_capacity(struct ufs_test *test)
{
	...
	memset(cdb_16, 0, 16); /* unsafe_function_ignore: memset */
...
static int ufs_scsi_mode_sense(struct ufs_test *test)
{
	...
	memset(&cdb[0], 0, MAX_CDB_SIZE); /* unsafe_function_ignore: memset */
...
static int ufs_scsi_request_sense(struct ufs_test *test)
{
	...
	memset(&cdb[0], 0, MAX_COMMAND_SIZE); /* unsafe_function_ignore: memset */
	cdb[0] = REQUEST_SENSE;
...
static int dump_and_check_lun_info(struct ufs_test *test)
{
	...
	int err;
	int blogical_block_size, bmin_addr_block_size, bmax_out_buffersize,
	    bmax_in_bufferize, qlogical_block_count;
	int rpmb_blogical_block_size;
	int i, lb_len;
	...
	for (i = 0; i < 7; i++) {
		err = ufshcd_query_descriptor_retry(hba_addr,
						  ...
						  unit_buf, &unit_len);
		if (!err) {
			blogical_block_size = unit_buf[10];
			pr_info("##Logical_lun##-unit_len:%d lun:%d size:%d\n",
			       unit_len, i, blogical_block_size);
		} else {
			kfree(unit_buf);
			kfree(geometry_buf);
			return err;
	...
	err = ufshcd_query_descriptor_retry(hba_addr,
					...
					unit_buf, &unit_len);
	if (!err) {
		rpmb_blogical_block_size = 1 << unit_buf[10];
		qlogical_block_count = get_unaligned_be64(&unit_buf[11]);
		pr_info("##RPMB_lun##-unit_len:%d size:%d qlogical_block_count:%d\n",
		     unit_len, rpmb_blogical_block_size, qlogical_block_count);
	} else {
		kfree(unit_buf);
		kfree(geometry_buf);
		return err;
	...
	if (!err) {
		bmin_addr_block_size = geometry_buf[18];
		bmax_in_bufferize = geometry_buf[21];
		bmax_out_buffersize = geometry_buf[22];
	} else {
		kfree(geometry_buf);
		return err;
	}
	lb_len = 1 << blogical_block_size;
	if ((lb_len > (bmin_addr_block_size * 512))
	    && (lb_len < (bmax_in_bufferize * 512))
	    && (lb_len < (bmax_out_buffersize * 512)))
		err = TEST_STATUS_OK;
...
static int check_and_test_bkops(struct ufs_test *test)
{
	...
	if (status & MASK_EE_URGENT_BKOPS)
		pr_info("Bkops Urgent Exception Event is opened");
...
static int ufs_test_sequence_read(struct ufs_test *test)
{
	...
	for (i = 0; i < (blk_cnt / PRDT_SG_NUM); i++)
		ret = ufs_test_simple_transfer(test->hba,
					     (test->g_start_addr +
...
static void ufs_test_run(int testcase, struct ufs_test *test)
{
	...
	for (i = start; i < end; i++) {
		...
		if (ufs_scsi_cases[i].cleanup) {
			...
			if (ret)
				pr_info("Warning: Cleanup stage failed! ret:%d\n", ret);
		}
	}
			       ...
			       size_t count, loff_t *ppos)
{
	...
	memcpy(tmp, lbuf, digit_len); /* unsafe_function_ignore: memcpy */
	tmp[digit_len] = 0;
