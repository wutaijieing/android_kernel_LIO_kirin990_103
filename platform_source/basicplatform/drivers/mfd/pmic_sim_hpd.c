...
#if defined(SUB_PMIC_SIM_HPD_EN)
...
static void sub_pmic_ldo_en_clear(u32 ldo_en_reg)
{
	...
	reg_tmp = sub_pmic_reg_read(ldo_en_reg);
	reg_tmp &= ~mask;
	reg_tmp |= value;
	sub_pmic_reg_write(ldo_en_reg, reg_tmp);
}
...
static void sub_pmic_level_bbrp_clear(u32 level_bbrp_reg)
{
	...
	reg_tmp = sub_pmic_reg_read(level_bbrp_reg);
	reg_tmp &= ~mask;
	reg_tmp |= value;
	sub_pmic_reg_write(level_bbrp_reg, reg_tmp);
}
...
void sub_pmic_sim_hpd_proc(void)
{
	...
	pending = sub_pmic_reg_read(SUB_SIM_IRQ_ADDR);
	if (!(pending & SUB_SIM_HPD_PENDING))
		return;

	sub_sim_ctr0 = sub_pmic_reg_read(SUB_SIM_CTRL0_ADDR);
