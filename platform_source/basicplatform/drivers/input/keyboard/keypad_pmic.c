...
#if defined(CONFIG_PMIC_21V900_PMU) || defined(CONFIG_PMIC_55V500_PMU)
...
{
	timer_setup(&(gpio_key->key_up_timer), keypad_pmic_keyup_timer,
		(uintptr_t)gpio_key);
	timer_setup(&(gpio_key->key_down_timer), keypad_pmic_keydown_timer,
		(uintptr_t)gpio_key);

#if defined(CONFIG_HUAWEI_DSM)
	timer_setup(&dsm_gpio_key_timer, dsm_gpio_key_timer_func,
		(uintptr_t)gpio_key);
#endif
}
