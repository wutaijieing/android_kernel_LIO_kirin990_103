...
void hiusbc_intr_handle_evt_ring(unsigned long param)
{
	...
	hiusbc_dbg(HIUSBC_DEBUG_SYS,
		"%ld evt, from:%p (0x%llx dma), to:%p (0x%llx dma)\n",
			event_ring->dequeue - dequeue_backup,
			dequeue_backup,
	...
	dequeue &= MPI_APP_DEVICE_REG_DEV_ER_DEQ_PTR_LO_0_MASK;
...
irqreturn_t hiusbc_interrupt(int irq, void *_hiusbc)
{
	...
	if (!(reg & MPI_APP_DEVICE_REG_DEV_INT_PENDING_0_MASK)) {
		dev_err(hiusbc->dev,
