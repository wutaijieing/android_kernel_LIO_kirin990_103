...
static atomic_t hiusbc_noc_err_flag = ATOMIC_INIT(0);
static u64 hiusbc_noc_err_addr;

static ssize_t hiusbc_noc_err_write(struct file *file,
		const char __user *ubuf, size_t count, loff_t *ppos)
{
	char buf[32] = {0};
	...
	if (sscanf_s(buf, "0x%llx", &hiusbc_noc_err_addr) != 1) {
		pr_err("inject addr illegal !\n");
		return -EFAULT;
	}

	pr_err("%s: 0x%llx\n", __func__, hiusbc_noc_err_addr);
	atomic_set(&hiusbc_noc_err_flag, 1);

	return count;
}

static int hiusbc_noc_err_show(struct seq_file *s, void *reserved)
{
	seq_printf(s, "[noc activity test mode: %d]\n",
			atomic_read(&hiusbc_noc_err_flag));
	return 0;
}

static int hiusbc_noc_err_open(struct inode *inode, struct file *file)
{
	return single_open(file, hiusbc_noc_err_show, inode->i_private);
}

int hiusbc_is_test_noc_err(void)
{
	return atomic_read(&hiusbc_noc_err_flag);
}

u64 hiusbc_get_noc_err_addr(u64 addr)
{
	if (!addr)
		pr_err("[USB.NOC.TEST] get input addr:0x%llx\n", addr);

	return hiusbc_noc_err_addr;
}

static const struct file_operations hiusbc_noc_err_fops = {
	.open			= hiusbc_noc_err_open,
	.write			= hiusbc_noc_err_write,
	.read			= seq_read,
	.llseek			= seq_lseek,
	.release		= single_release,
};
void hiusbc_debugfs_init(struct hiusbc *hiusbc)
{
	...
	debugfs_create_regset32("regdump", 0444, root, hiusbc->regset);
	...
	file = debugfs_create_file("noc_err", 0644, root,
			hiusbc, &hiusbc_noc_err_fops);
	if (!file)
		pr_err("Can't create 'noc_err' debugfs\n");
}
