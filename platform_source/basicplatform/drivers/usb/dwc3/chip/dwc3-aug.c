...
static int aug_asyncbrgclk_init(struct plat_chip_usb_phy *usb_phy)
{
...
static int dwc3_release(struct plat_chip_usb_phy *usb_phy)
{
	...
	ret = aug_asyncbrgclk_init(usb_phy);
	if (ret != 0)
		return ret;
...
static int aug_usb3phy_shutdown(unsigned int support_dp)
{
	usb_dbg("+\n");
	if (!_chip_usb_phy)
		return -ENODEV;
...
static int aug_get_dts_resource(struct device *dev, struct plat_chip_usb_phy *usb_phy)
{
...
static int aug_usb3phy_init(unsigned int support_dp,
		unsigned int eye_diagram_param,
...
static int aug_shared_phy_init(unsigned int support_dp, unsigned int eye_diagram_param,
		unsigned int combophy_flag)
{
	...
	return aug_usb3phy_init(0, eye_diagram_param, 0);
}

static int aug_shared_phy_shutdown(unsigned int support_dp, unsigned int combophy_flag, unsigned int keep_power)
{
	if (!_chip_usb_phy)
		return -ENODEV;
	...
	return aug_usb3phy_shutdown(0);
}

static int dwc3_aug_probe(struct platform_device *pdev)
{
	int ret;
	...
	ret = aug_get_dts_resource(&pdev->dev, _chip_usb_phy);
	if (ret) {
		usb_err("get_dts_resource failed\n");
		goto err_out;
	}

	_chip_usb_phy->phy.init = aug_usb3phy_init;
	_chip_usb_phy->phy.shutdown = aug_usb3phy_shutdown;
	_chip_usb_phy->phy.shared_phy_init = aug_shared_phy_init;
	_chip_usb_phy->phy.shared_phy_shutdown = aug_shared_phy_shutdown;
...
static int dwc3_aug_remove(struct platform_device *pdev)
{
	int ret;
...
#ifdef CONFIG_OF
static const struct of_device_id dwc3_aug_match[] = {
	{ .compatible = "hisilicon,aug-dwc3" },
	{},
};
MODULE_DEVICE_TABLE(of, dwc3_aug_match);
#else
#define dwc3_aug_match NULL
#endif

static struct platform_driver dwc3_aug_driver = {
	.probe		= dwc3_aug_probe,
	.remove		= dwc3_aug_remove,
	.driver		= {
		.name	= "usb3-aug",
		.of_match_table = of_match_ptr(dwc3_aug_match),
	},
};

module_platform_driver(dwc3_aug_driver);
