...
static int pd_get_resource(struct tca_device_s *res, const struct device *dev)
{
	...
	if (of_property_read_string(dev->of_node, "init-mode",
		...
		if (strcmp(init_mode, "usb_dp") == 0)
			...
			res->init_mode = COMBOPHY_INIT_USB_DEVICE_MODE;
		else
	...
	enum tcpc_mux_ctrl_type new_mode, enum typec_plug_orien_e typec_orien)
{
	...
	ktime_get_real_ts64(&tv);
	usb_info("e:tv_sec %ld,tv_nsec: %ld\n", tv.tv_sec, tv.tv_nsec);
	if (ret)
		misc_dump();
	return ret;
}
	...
	enum tca_device_type_e dev_type, enum typec_plug_orien_e typec_orien)
{
	...
	if (tca_dev.wlock && !tca_dev.wlock->active)
...
static int pd_notify_nc(const struct pd_event_member_s *event)
{
	...
	if (((unsigned int)tca_dev.tca_cur_mode & TCPC_DP)
		...
		dpu_dptx_hpd_trigger(event->irq_type, tca_dev.tca_cur_mode, event->typec_orien);
...
static int pd_event_handle_update(const struct pd_event_member_s *event)
{
	...
	ret = dpu_dptx_hpd_trigger(event->irq_type, tca_dev.tca_cur_mode, event->typec_orien);
	if (ret) {
...
static int pd_probe(struct platform_device *pdev)
{
	...
	tca_dev.wlock = wakeup_source_register(dev, "phy");
#else
	tca_dev.wlock = wakeup_source_register("phy");
#endif
	if (!tca_dev.wlock)
		usb_err("tca_dev wakeup source register failed.\n");
	...
	} else if (tca_dev.init_mode == COMBOPHY_INIT_USB_DEVICE_MODE){
...
static int pd_remove(struct platform_device *pdev)
{
	tca_devouces_rel();
	kfifo_free(&tca_dev.kfifo);
	wakeup_source_unregister(tca_dev.wlock);
	tca_dev.wlock = NULL;
