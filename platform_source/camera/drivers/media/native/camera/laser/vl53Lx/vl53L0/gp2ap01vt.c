...
static void gp2ap01vt00f_DebugTimeDuration(struct timespec64 *pstart_tv,
            struct timespec64 *pstop_tv)
{
    if(pstart_tv == NULL || pstop_tv == NULL)
        return;
    long total_sec;
    long total_msec;
    total_sec = pstop_tv->tv_sec - pstart_tv->tv_sec;
    total_msec = (pstop_tv->tv_nsec - pstart_tv->tv_nsec) / NSEC_PER_MSEC;
    total_msec += total_sec * MSEC2USEC;
    laser_dbg("elapsedTime:%ld\n", total_msec);
}
#endif
