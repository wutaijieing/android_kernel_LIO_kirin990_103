...
struct kernel_pmic_ctrl_t ncp6925_ctrl;
...
DEFINE_KERNEL_PMIC_MUTEX(ncp6925);

static int calc_buck_vlotage_ext(struct kernel_pmic_ctrl_t *pmic_ctrl, u32 in, u8 *out);
static int calc_ldo_vlotage(u32 in, u8 *out);
static int ncp6925_seq_config(struct kernel_pmic_ctrl_t *pmic_ctrl, pmic_seq_index_t seq_index, u32 voltage, int state);
static int ncp6925_on(struct kernel_pmic_ctrl_t *pmic_ctrl, void *data);
...
int hw_extern_pmic_query_state(int index, int *state)
{
    ...
    struct kernel_pmic_ctrl_t *pmic_ctrl = kernel_get_pmic_ctrl();
...
static int ncp6925_init(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
    struct kernel_pmic_i2c_client *i2c_client = NULL;
    struct kernel_pmic_i2c_fn_t *i2c_func = NULL;
...
static int ncp6925_exit(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
    struct ncp6925_private_data_t *pdata = NULL;
...
static int ncp6925_off(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
    struct ncp6925_private_data_t *pdata = NULL;
    cam_debug("%s enter.", __func__);
...
static int ncp6925_check_state_exception(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
    struct kernel_pmic_i2c_client *i2c_client = NULL;
    struct kernel_pmic_i2c_fn_t *i2c_func = NULL;
    u8 reg_value = 0;
...
static int twl80125_check_state_exception(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
    struct kernel_pmic_i2c_client *i2c_client = NULL;
    struct kernel_pmic_i2c_fn_t *i2c_func = NULL;
    u8 reg_value = 0;
...
static int ncp6925_match(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
    cam_debug("%s enter.", __func__);
    return 0;
}
...
static void ncp6925_get_extern_cfg(struct device_node* dev_node)
{
    ...
    rc = of_property_read_u32(dev_node, "vendor,pmic_ex_func_disable",
            (u32 *)&pmic_ex_config);
    if (rc < 0) {
...
static int ncp6925_get_dt_data(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
    struct ncp6925_private_data_t *pdata = NULL;
    ...
    struct kernel_pmic_info *pmic_info = NULL;
    ...
    rc = of_property_read_u32_array(dev_node, "vendor,pmic-pin",
            pdata->pin, MAX_PIN);
    if (rc < 0) {
    ...
    rc = of_property_read_u32_array(dev_node, "vendor,pmic-voltage",
    ...
    rc = of_property_read_u32(dev_node, "vendor,pmic_sensor_1v8",
        (u32 *)&pmic_sensor_1v8);
    if (rc < 0)
    ...
    rc = of_property_read_u32(dev_node, "vendor,pmic_sensor_3v3", (u32 *)&pmic_sensor_3v3);
    if (rc < 0)
    ...
    if(of_property_read_u32(dev_node, "vendor,pmic_mutex_flag",(u32 *)&pmic_info->mutex_flag)){
        cam_err("%s read pmic_mutex_flag failed %d\n", __func__, __LINE__);
    }else{
        pmic_lon_flag = 1;
        cam_info("%s vendor,pmic_mutex_flag %d\n", __func__,
...
static int calc_buck_vlotage_ext(struct kernel_pmic_ctrl_t *pmic_ctrl,
                                 u32 in,
                                 u8 *out)
{
    struct ncp6925_private_data_t *pdata = NULL;
...
static int ncp6925_seq_config(struct kernel_pmic_ctrl_t *pmic_ctrl, pmic_seq_index_t seq_index, u32 voltage, int state)
{
    ...
    if(pmic_ctrl->pmic_info.mutex_flag == 1){
        mutex_lock(pmic_ctrl->kernel_pmic_mutex);
    }
    ...
    if(pmic_ctrl->pmic_info.mutex_flag == 1){
        mutex_unlock(pmic_ctrl->kernel_pmic_mutex);
    }
...
static int ncp6925_register_attribute(struct kernel_pmic_ctrl_t *pmic_ctrl, struct device *dev)
{
    return 0;
}
...
static struct kernel_pmic_i2c_client ncp6925_i2c_client;

static struct kernel_pmic_fn_t ncp6925_func_tbl = {
...
struct kernel_pmic_ctrl_t ncp6925_ctrl = {
    ...
    .kernel_pmic_mutex = &pmic_mut_ncp6925,
