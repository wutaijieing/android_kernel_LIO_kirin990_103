...
static int rt5112_boost_seq_config(struct kernel_pmic_ctrl_t *pmic_ctrl,
...
static int rt5112_buck_seq_config(struct kernel_pmic_ctrl_t *pmic_ctrl,
                              pmic_seq_index_t seq_index,
                              u32 voltage,
                              int state)
{
...
// RT5112 function
static int rt5112_ldo_seq_config(struct kernel_pmic_ctrl_t *pmic_ctrl,
                              pmic_seq_index_t seq_index,
                              u32 voltage,
                              int state)
{
...
static int rt5112_seq_config(struct kernel_pmic_ctrl_t *pmic_ctrl,
                              pmic_seq_index_t seq_index,
                              u32 voltage,
                              int state)
{
...
static int rt5112_match(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
    cam_debug("%s enter.", __func__);
    return 0;
}

static int rt5112_get_dt_data(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
    struct rt5112_private_data_t *pdata = NULL;
	...
	rc = of_property_read_u32(dev_node, "vendor,pmic_buck_pwm_mode", &pdata->buck_pwm_mode);
	if (rc < 0) {
		pdata->buck_pwm_mode = 0;
		cam_info("%s, cannot get buck config, set to default buck mode", __func__);
	}
	cam_info("%s vendor,pmic_buck_pwm_mode %d", __func__, pdata->buck_pwm_mode);
	...
	rc = of_property_read_u32(dev_node, "vendor,boost_5v3",
		&pdata->boost_5v3);
	if (rc < 0) {
		pdata->boost_5v3 = 0;
	...
	rc = of_property_read_u32(dev_node, "vendor,boost_5v15",
		&pdata->boost_5v15);
	if (rc < 0) {
		pdata->boost_5v15 = 0;
...
static int rt5112_on(struct kernel_pmic_ctrl_t *pmic_ctrl, void *data)
{
    // check Error registor
    return 0;
}

static int rt5112_off(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
    // Never power off pmic when SOC is running;
    return 0;
}

static int rt5112_init(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
    struct kernel_pmic_i2c_client *i2c_client = NULL;
    struct kernel_pmic_i2c_fn_t *i2c_func = NULL;
    struct rt5112_private_data_t *pdata = NULL;
...
static int rt5112_exit(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
    struct rt5112_private_data_t *pdata = NULL;
...
static int rt5112_register_attribute(struct kernel_pmic_ctrl_t *pmic_ctrl, struct device *dev)
{
    return 0;
}
...
{
	struct kernel_pmic_i2c_client *rt_i2c_client = NULL;
	struct kernel_pmic_ctrl_t *rt_shut_pmic_ctrl = NULL;
	struct kernel_pmic_i2c_fn_t *i2c_func = NULL;
	struct rt5112_private_data_t *pdata = NULL;
	u8 boost_vol_value = 0;

	cam_warn("%s enter", __func__);
	rt_shut_pmic_ctrl = kernel_get_pmic_ctrl();
...
static struct kernel_pmic_i2c_client rt5112_i2c_client;

static struct kernel_pmic_fn_t rt5112_func_tbl = {
...
static struct kernel_pmic_ctrl_t rt5112_ctrl = {
    ...
    .kernel_pmic_mutex = &pmic_mut_rt5112,
