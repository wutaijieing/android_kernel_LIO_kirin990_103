...
struct kernel_pmic_ctrl_t *kernel_pmic_ctrl = NULL;
...
void kernel_set_pmic_ctrl(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
	kernel_pmic_ctrl = pmic_ctrl;
}

struct kernel_pmic_ctrl_t * kernel_get_pmic_ctrl(void)
{
	return kernel_pmic_ctrl;
}

static irqreturn_t kernel_pmic_interrupt_handler(int vec, void *info)
{
	...
	pmic_ctrl = (struct kernel_pmic_ctrl_t *)info;
...
void irq_err_time_reset(struct irq_err_monitor *irq_err)
{
	...
	irq_err->last_time.tv_nsec = irq_err->now.tv_nsec;
}
	...
	const unsigned int sche_work_time)
{
	...
	ktime_get_real_ts64(&irq_err->now);
	irq_err->irq_time = (irq_err->now.tv_sec - irq_err->last_time.tv_sec) * CAM_MICROSECOND_PER_SECOND +
		(irq_err->now.tv_nsec - irq_err->last_time.tv_nsec) / CAM_NANOSECOND_PER_MICROSECOND;
...
static int kernel_pmic_type_ctrl_cfg(struct kernel_pmic_ctrl_t *pmic_ctrl,
...
int kernel_pmic_vote_cfg(struct kernel_pmic_ctrl_t *pmic_ctrl,
	pmic_seq_index_t idx, u32 vol, int state)
{
	int ret = 0;
	...
	if (state == 0) {
		if (pmic_vote_status[idx] > PMIC_ENABLE_CNT) {
			...
			ret = kernel_pmic_type_ctrl_cfg(pmic_ctrl, idx, vol, state);
			if (ret == 0) {
...
int kernel_pmic_setup_intr(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
	...
	cam_info("%s vendor,pmic-intr %u", __func__, pmic_info->intr);
	if (rc < 0) {
		cam_err("%s, failed %d\n", __func__, __LINE__);
	...
	rc = request_threaded_irq(pmic_info->irq, NULL,
			kernel_pmic_interrupt_handler,
			IRQF_TRIGGER_FALLING|IRQF_ONESHOT,
			"kernel_pmic_interrupt",
			(void *)pmic_ctrl);
	if (rc < 0) {
...
EXPORT_SYMBOL(kernel_pmic_setup_intr);
...
EXPORT_SYMBOL(kernel_pmic_release_intr);
...
int kernel_pmic_get_dt_data(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
	...
	rc = of_property_read_string(dev_node, "vendor,pmic_name", &pmic_info->name);
	cam_info("%s vendor,pmic_name %s, rc %d\n", __func__, pmic_info->name, rc);
	if (rc < 0) {
		cam_err("%s failed %d\n", __func__, __LINE__);
		goto fail;
	}

	rc = of_property_read_u32(dev_node, "vendor,pmic_index",
		(u32 *)&pmic_info->index);
	cam_info("%s vendor,pmic_index %d, rc %d\n", __func__,
		pmic_info->index, rc);
	if (rc < 0) {
		cam_err("%s failed %d\n", __func__, __LINE__);
		goto fail;
	...
	rc = of_property_read_u32(dev_node, "vendor,fault_check",
...
EXPORT_SYMBOL(kernel_pmic_config);
	...
	const struct i2c_device_id *id)
{
	...
	pmic_ctrl = (struct kernel_pmic_ctrl_t *)id->driver_data;
	pmic_ctrl->pmic_i2c_client->client = client;/*[false alarm]:pmic_ctrl is not alarm*/
	pmic_ctrl->dev = &client->dev;
	pmic_ctrl->pmic_i2c_client->i2c_func_tbl = &kernel_pmic_i2c_func_tbl;

	rc = kernel_pmic_get_dt_data(pmic_ctrl);
	if (rc < 0) {
		cam_err("%s kernel_pmic_get_dt_data failed.", __func__);
		return -EFAULT;
	}
...
#endif
	kernel_set_pmic_ctrl(pmic_ctrl);
