...
static struct kernel_pmic_ctrl_t fan53880_ctrl;
extern struct dsm_client *client_pmic;
static void pmic_fault_err_work(struct work_struct *work);
static int boost_enabled;

DEFINE_KERNEL_PMIC_MUTEX(fan53880);

static int fan53880_set_boost_load_enable(struct kernel_pmic_ctrl_t *pmic_ctrl, int state)
{
	struct kernel_pmic_i2c_client *i2c_client = NULL;
	struct kernel_pmic_i2c_fn_t *i2c_func = NULL;
...
static int fan53880_force_pwm_mode(struct kernel_pmic_ctrl_t *pmic_ctrl, int state)
{
	struct kernel_pmic_i2c_client *i2c_client = NULL;
	struct kernel_pmic_i2c_fn_t *i2c_func = NULL;
	u8 enable_reg = 0;
...
static int fan53880_boost_seq_config(struct kernel_pmic_ctrl_t *pmic_ctrl,
...
static int fan53880_buck_seq_config(struct kernel_pmic_ctrl_t *pmic_ctrl,
                              pmic_seq_index_t seq_index,
                              u32 voltage,
                              int state)
{
...
// FAN53880 function
static int fan53880_ldo_seq_config(struct kernel_pmic_ctrl_t *pmic_ctrl,
                              pmic_seq_index_t seq_index,
                              u32 voltage,
                              int state)
{
...
static int fan53880_seq_config(struct kernel_pmic_ctrl_t *pmic_ctrl,
                              pmic_seq_index_t seq_index,
                              u32 voltage,
                              int state)
{
	...
	if (seq_index == VOUT_BOOST) {
		...
		ret = kernel_pmic_gpio_boost_enable(pmic_ctrl, state);
	} else if (seq_index < VOUT_LDO_5) {
		if (pdata->ldo_vote == 1) {
			cam_debug("%s, vote set ldo", __func__);
			ret = kernel_pmic_vote_cfg(pmic_ctrl, seq_index, voltage,
				state);
		} else if ((pdata->ldo2_ctrl == FAN53880_LDO2_CTRL) &&
			(seq_index == VOUT_LDO_2)) {
			ret = kernel_pmic_vote_cfg(pmic_ctrl, VOUT_LDO_2, voltage,
				state);
		} else {
...
static int fan53880_match(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
    cam_debug("%s enter.", __func__);
    return 0;
}

static int fan53880_get_dt_data(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
    struct fan53880_private_data_t *pdata = NULL;
    ...
    // PIN Enable gpio
    rc = of_property_read_u32(dev_node, "vendor,pmic-pin", &pdata->pin);
    cam_info("%s vendor,pmic-pin %d", __func__, pdata->pin);
    if (rc  < 0) {
        cam_err("%s, failed %d\n", __func__, __LINE__);
        goto fail;
    }
	...
	// ldo2_ctrl
	rc = of_property_read_u32(dev_node, "vendor,pmic_ldo2_ctrl", &pdata->ldo2_ctrl);
	if (rc < 0) {
		pdata->ldo2_ctrl = 0;
		cam_info("%s, cannot get buck config, set to default ldo2_ctrl", __func__);
	}
	cam_info("%s vendor,pmic_ldo2_ctrl %d", __func__, pdata->ldo2_ctrl);

	rc = of_property_read_u32(dev_node, "vendor,shutdown_4V5",
		&pdata->shutdown_4v5);
	if (rc < 0) {
	...
	rc = of_property_read_u32(dev_node, "vendor,ldo_vote", &pdata->ldo_vote);
	if (rc < 0) {
		pdata->ldo_vote = 0;
		cam_info("%s, not set ldo_vote", __func__);
	}
	cam_info("%s ldo_vote = %u", __func__, pdata->ldo_vote);
	rc = of_property_read_u32(dev_node, "vendor,buck_vote", &pdata->buck_vote);
	if (rc < 0) {
		pdata->buck_vote = 0;
	...
	rc = of_property_read_u32(dev_node, "vendor,pmic_reset", &pdata->reset);
	if (rc < 0) {
		pdata->reset = 1;
		cam_info("%s, cannot get pmic_reset value, set 1", __func__);
	}
	cam_info("%s vendor,pmic_reset %d", __func__, pdata->reset);
	rc = of_property_read_u32(dev_node, "vendor,ctl_otg", &pdata->ctl_otg);
	if (rc < 0) {
		pdata->ctl_otg = 1;
		cam_info("%s, cannot get ctl_otg, set 1", __func__);
	}
	cam_info("%s ctl_otg %u", __func__, pdata->ctl_otg);
	rc = of_property_read_u32(dev_node, "vendor,boost_5v3", &pdata->boost_5v3);
	if (rc < 0) {
		pdata->boost_5v3 = 0;
		cam_info("%s, cannot get ctl_otg, set 1", __func__);
	}
	cam_info("%s boost_5v3 %u", __func__, pdata->boost_5v3);
	rc = of_property_read_u32(dev_node, "vendor,boost_5v15", &pdata->boost_5v15);
	if (rc < 0) {
		pdata->boost_5v15 = 0;
...
static int fan53880_on(struct kernel_pmic_ctrl_t *pmic_ctrl, void *data)
{
    // check Error registor
    return 0;
}

static int fan53880_off(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
    // Never power off pmic when SOC is running;
    return 0;
}

static int fan53880_init(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
    struct kernel_pmic_i2c_client *i2c_client = NULL;
    struct kernel_pmic_i2c_fn_t *i2c_func = NULL;
    struct fan53880_private_data_t *pdata = NULL;
    ...
    ret = kernel_pmic_setup_intr(pmic_ctrl);
    if (ret < 0) {
...
err2:
    gpio_free(pdata->pin);
req_failed:
    kernel_pmic_release_intr(pmic_ctrl);
err:
    return -EIO;
}

static int fan53880_exit(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
    struct fan53880_private_data_t *pdata = NULL;
...
static int fan53880_register_attribute(struct kernel_pmic_ctrl_t *pmic_ctrl, struct device *dev)
{
    return 0;
}

static void pmic_boost_reset(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
...
static void fan53880_pmic_reset(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
	struct kernel_pmic_i2c_client *i2c_client = NULL;
	struct kernel_pmic_i2c_fn_t *i2c_func = NULL;
	u8 pmu_status = 0;
...
static void pmic_fault_err_work(struct work_struct *work)
{
	...
	pmic_ctrl = container_of(work, struct kernel_pmic_ctrl_t,
		pmic_err_work.work);

	fan53880_pmic_reset(pmic_ctrl);
}

static int pmic_check_state_exception(struct kernel_pmic_ctrl_t *pmic_ctrl)
{
    struct kernel_pmic_i2c_client *i2c_client = NULL;
    struct kernel_pmic_i2c_fn_t *i2c_func = NULL;
    struct fan53880_private_data_t *pdata = NULL;
...
{
	struct kernel_pmic_i2c_client *fan_i2c_client = NULL;
	struct kernel_pmic_ctrl_t *fan_shut_pmic_ctrl = NULL;
	struct kernel_pmic_i2c_fn_t *i2c_func = NULL;
	struct fan53880_private_data_t *pdata = NULL;

	cam_warn("%s enter", __func__);
	fan_shut_pmic_ctrl = kernel_get_pmic_ctrl();
...
static struct kernel_pmic_i2c_client fan53880_i2c_client;

static struct kernel_pmic_fn_t fan53880_func_tbl = {
    .pmic_config = kernel_pmic_config,
...
static struct kernel_pmic_ctrl_t fan53880_ctrl = {
    ...
    .kernel_pmic_mutex = &pmic_mut_fan53880,
