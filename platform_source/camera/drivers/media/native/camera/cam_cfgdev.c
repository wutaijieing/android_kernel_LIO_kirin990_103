...
static void cam_cfgdev_dump_meminfo(void)
{
	...
	if (!req) {
		cam_err("%s config req is null", __func__);
		return;
	}
							 ...
							 const char *buf, size_t count)
{
	...
	if (p == buf)
		printk(KERN_INFO ": %s is not in hex or decimal form", buf);
	else
		cam_debug_mask = val;
...
int cam_cfgdev_queue_ack(struct v4l2_event *ev)
{
	...
	if (req && req->one_way != 0) {
		cam_cfg_err("need NOT acknowledge an one way request 0x%pK, \
			0x%08x, %u",
					req->intf, (*(unsigned int *)(req + 1)), req->seq);
		return -EINVAL;
	}
							   ...
							   cam_cfgreq_t *req, int timeout, cam_cfgack_t **pack)
{
	...
	while (true) {
		...
		if (rc == 0) {
			cam_cfg_err("request(0x%pK, 0x%08x, %u) is \
				out of time for ACK",
						req->intf, (*(unsigned int *)(req + 1)),
						req->seq);
			rc = -ETIME;
			break;
		} else if (rc == -ERESTARTSYS) {
			cam_cfg_info("request(0x%pK, 0x%08x, %u) is \
				interrupted! pid: %d, tgid: %d, \
				pending.signal: 0x%lx",
						 req->intf, (*(unsigned int *)(req + 1)),
						 req->seq, current->pid, current->tgid,
							   ...
							   cam_cfgreq_t *req, int timeout, int *ret)
{
	...
	timeout = (int)msecs_to_jiffies(timeout);
						...
						struct v4l2_fh *target, int one_way, int *ret)
{
	...
	req->user = user;
	req->seq = (uint32_t)atomic_add_return(1, &s_sequence);
							   ...
							   struct v4l2_subdev *sd, cam_device_id_constants_t dev_const)
{
	...
	cam_cfg_info("%s[%d] before call v4l2_device_register_subdev", __func__, __LINE__);
...
{
	cam_cfgreq2dev_t *req = (cam_cfgreq2dev_t *)&old->u.data[0];
...
static int cam_cfgdev_get_dts(struct platform_device *pdev)
{
	...
	rc = of_property_read_u32(np, "vendor,binderized",
							  (u32 *)(&g_is_binderized));
	if (rc < 0) {
							   ...
							   struct media_device **pmdev)
{
	...
	if (rc < 0) {
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0))
		cam_cfg_err("media_device_register failed");
		kzfree(mdev);
#else
