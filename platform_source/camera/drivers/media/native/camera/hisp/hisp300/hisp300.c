...
struct rpmsg_service_info {
	struct rpmsg_hisp300_service *kernel_isp_serv;
...
#endif
	cam_debug("%s state=%u system_counter=%llu rate=%u time_second=%ld time_usecond=%ld size=%lu",
			  __func__,
 ...
 *********************************************/
void hisp300_set_timestamp(unsigned int *timestamp_high,
						   unsigned int *timestamp_low)
{
	...
	cam_debug("%s ack_high:0x%x ack_low:0x%x", __func__, *timestamp_high, *timestamp_low);
...
{
	struct rpmsg_hisp300_service *kernel_serv = rpmsg_local.kernel_isp_serv;
...
static int hisp300_init_r8isp_memory_pool(void *cfg)
{
	...
	cam_info("%s: r8_iova_pool_base=0x%x",
			 __func__, s_hisp300.mem_pool[ipool].r8_iova);
	mutex_unlock(&kernel_rpmsg_service_mutex);
	return 0;
}
...
static int hisp300_deinit_r8isp_memory_pool(void *cfg)
{
	...
	if (s_hisp300.mem_pool[ipool].active) {
		...
		cam_buf_put_sgtable(s_hisp300.mem_pool[ipool].sgt);
	}
...
static int hisp300_power_on(hisp_intf_t *i)
{
	...
	struct rpmsg_hisp300_service *kernel_serv = NULL;
...
#pragma GCC visibility push(default)
	kernel_serv->ept =
		rpmsg_create_ept(kernel_serv->rpdev,
						 hisp300_rpmsg_ept_cb, kernel_serv, chinfo);
...
{
	struct rpmsg_hisp300_service *kernel_serv =
