 ...
 * Description:Implement of hisp210
 * Create: 2014-11-11
 *
...
struct rpmsg_service_info {
	struct rpmsg_hisp210_service *kernel_isp_serv;
...
void hisp210_set_timestamp(unsigned int *timestamp_h,
...
void hisp210_handle_msg(hisp_msg_t *msg)
{
	...
	case REQUEST_RESPONSE:
		hisp210_set_timestamp(&(msg->u.ack_request.timestampH),
			&(msg->u.ack_request.timestampL));
		break;
	case MSG_EVENT_SENT:
		hisp210_set_timestamp(&(msg->u.event_sent.timestampH),
...
{
	int rc;
	struct rpmsg_hisp210_service *kernel_serv = rpmsg_local.kernel_isp_serv;
...
static int hisp210_get_a7isp_addr(void *cfg)
{
	...
	ret = hisp_get_sg_table(pcfg->param.sharedFd, &(s_hisp210.pdev->dev),
		&buf,&attach,&table);
	if (ret < 0) {
...
static int find_suitable_mem_pool(struct hisp_cfg_data *pcfg)
{
	...
	if (pcfg->param.type == MAP_TYPE_RAW2YUV) {
		...
		for (ipool = 0; ipool < MEM_POOL_ATTR_MAX; ipool++) {
			if (s_hisp210.mem_pool[ipool].prot ==
				pcfg->param.prot)
				break;
		}
...
static int hisp210_init_r8isp_memory_pool(void *cfg)
{
	...
	s_hisp210.mem_pool[ipool].security_isp_mode =
		pcfg->param.security_isp_mode;
	...
	cam_info("func %s: r8_iova_pool_base=0x%x", __func__,
		s_hisp210.mem_pool[ipool].r8_iova);
	mutex_unlock(&kernel_rpmsg_service_mutex);
	return 0;
}
...
static int hisp210_deinit_r8isp_memory_pool(void *cfg)
{
	...
	if (s_hisp210.mem_pool[ipool].active) {
		...
		cam_buf_put_sgtable(s_hisp210.mem_pool[ipool].sgt);
	}
	rc = memset_s(&(s_hisp210.mem_pool[ipool]),
		sizeof(struct hisp210_mem_pool),
...
static int hisp210_alloc_r8isp_addr(void *cfg)
{
	...
	if (pcfg->param.type == MAP_TYPE_RAW2YUV)
		...
		offset = secure_mode ?
			(r8_iova - s_hisp210.mem_pool[ipool].r8_iova) :
			(r8_iova - s_hisp210.mem_pool[ipool].ion_iova);
	...
	pcfg->param.vaddr =
		(void*)(((unsigned char*)s_hisp210.mem_pool[ipool].ap_va) +
		offset);
...
static int hisp210_config(hisp_intf_t *i, void *cfg)
{
	...
	case HISP_CONFIG_POWER_ON:
		...
		cam_info("%s power on the hisp210", __func__);
	...
	case HISP_CONFIG_POWER_OFF:
		mutex_lock(&hisp_power_lock_mutex);
		if (remote_processor_up) {
			cam_notice("%s power off the hisp210", __func__);
			rc = hisp210_power_off(i);
		}
		mutex_unlock(&hisp_power_lock_mutex);
		break;
...
static int hisp210_open(hisp_intf_t *i)
{
	cam_info("%s hisp210 device open", __func__);
	mutex_lock(&hisp_power_lock_mutex);
	g_hisp_ref++;
	mutex_unlock(&hisp_power_lock_mutex);
	return 0;
...
{
	int rc = 0;
	cam_info("%s hisp210 device close", __func__);
	mutex_lock(&hisp_power_lock_mutex);
...
static int hisp210_power_on(hisp_intf_t *i)
{
	...
	struct rpmsg_hisp210_service *kernel_serv = NULL;
...
#pragma GCC visibility push(default)
	kernel_serv->ept =
		rpmsg_create_ept(kernel_serv->rpdev,
			hisp210_rpmsg_ept_cb, kernel_serv, chinfo);
...
{
	struct rpmsg_hisp210_service *kernel_serv = dev_get_drvdata(&rpdev->dev);
...
static int hisp210_rpmsg_driver_cb(struct rpmsg_device *rpdev,
	void *data, int len, void *priv, u32 src)
{
	cam_info("%s enter ... ", __func__);
...
static int hisp210_send_rpmsg(hisp_intf_t *i, hisp_msg_t *from_user, size_t len)
{
	int rc = 0;
	hisp210_t *hi = NULL;
	struct rpmsg_hisp210_service *kernel_serv = NULL;
...
static int hisp210_recv_rpmsg(hisp_intf_t *i, hisp_msg_t *user_addr, size_t len)
{
	int rc = len;
	hisp210_t *hi = NULL;
	struct rpmsg_hisp210_service *kernel_serv = NULL;
...
static int32_t hisp210_platform_probe(struct platform_device* pdev)
{
	int32_t ret = 0;
...
s_hisp210_driver = {
	...
	.driver = {
		.name ="vendor,chip_isp210",
		.owner = THIS_MODULE,
		.of_match_table = s_hisp210_dt_match,
	},
};

static int __init hisp210_init_module(void)
{
	cam_info("%s enter", __func__);
	return platform_driver_register(&s_hisp210_driver);
}

static void __exit hisp210_exit_module(void)
{
	cam_info("%s enter", __func__);
