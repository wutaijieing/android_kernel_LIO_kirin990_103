...
struct rpmsg_service_info {
	struct rpmsg_hisp310_service *kernel_isp_serv;
...
{
	struct rpmsg_hisp310_service *kernel_serv = rpmsg_local.kernel_isp_serv;
...
static int hisp310_init_r8isp_memory_pool(void *cfg)
{
	...
	cam_info("%s: r8_iova_pool_base=0x%x",
		__func__, s_hisp310.mem_pool[ipool].r8_iova);
	mutex_unlock(&kernel_rpmsg_service_mutex);
	return 0;
}
...
static int hisp310_deinit_r8isp_memory_pool(void *cfg)
{
	...
	if (s_hisp310.mem_pool[ipool].active) {
		...
		cam_buf_put_sgtable(s_hisp310.mem_pool[ipool].sgt);
	}
...
static int hisp310_config_power_on(hisp_intf_t *i, struct hisp_cfg_data *pcfg)
{
	...
	cam_notice("%s power on the hisp310", __func__);
...
static int hisp310_config_power_off(hisp_intf_t *i)
{
	...
	mutex_lock(&hisp_power_lock_mutex);
	if (remote_processor_up) {
		cam_notice("%s power off the hisp310", __func__);
		rc = hisp310_power_off(i);
	}
	mutex_unlock(&hisp_power_lock_mutex);
...
static int hisp310_open(hisp_intf_t *i)
{
	cam_info("%s hisp310 device open", __func__);
...
{
	int rc = 0;
	cam_info("%s hisp310 device close", __func__);
	mutex_lock(&hisp_power_lock_mutex);
...
static int hisp310_power_on(hisp_intf_t *i)
{
	...
	struct rpmsg_hisp310_service *kernel_serv = NULL;
...
#pragma GCC visibility push(default)
	kernel_serv->ept =
		rpmsg_create_ept(kernel_serv->rpdev,
			hisp310_rpmsg_ept_cb, kernel_serv, chinfo);
...
{
	struct rpmsg_hisp310_service *kernel_serv = dev_get_drvdata(&rpdev->dev);
