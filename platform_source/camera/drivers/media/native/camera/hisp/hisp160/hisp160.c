...
struct rpmsg_service_info {
	struct rpmsg_hisp160_service *kernel_isp_serv;
...
{
	int rc = 0;
	struct rpmsg_hisp160_service *kernel_serv = rpmsg_local.kernel_isp_serv;
...
static int hisp160_deinit_r8isp_memory_pool(void *cfg)
{
	...
	if (s_hisp160.mem_pool[ipool].active) {
		...
		cam_buf_put_sgtable(s_hisp160.mem_pool[ipool].sgt);
	}
...
static int hisp160_mem_pool_pre_init(void)
{
	...
	for (ipool = 0; ipool < MEM_POOL_ATTR_MAX; ipool++) {
		...
		if (prot < 0) {
			cam_err("%s unkown ipool %d prot 0x%x", __func__,
				ipool, prot);
			return -EINVAL;
		}
...
static int hisp160_config(hisp_intf_t *i, void *cfg)
{
	...
	case HISP_CONFIG_POWER_ON:
		if (!remote_processor_up) {
			...
			cam_notice("%s power on the hisp160", __func__);
	...
	case HISP_CONFIG_POWER_OFF:
		if (remote_processor_up) {
			cam_notice("%s power off the hisp160", __func__);
			rc = hisp160_power_off(i);
		}
		break;
	case HISP_CONFIG_GET_MAP_ADDR:
		rc = hisp160_get_a7isp_addr(cfg);
		cam_notice("%s get a7 map address 0x%x ", __func__,
...
static int hisp160_power_on(hisp_intf_t *i)
{
	...
	struct rpmsg_hisp160_service *kernel_serv = NULL;
	...
	kernel_serv->ept = rpmsg_create_ept(kernel_serv->rpdev,
		hisp160_rpmsg_ept_cb, kernel_serv, chinfo);
	if (!kernel_serv->ept) {
		rc = -ENOMEM;
		goto FAILED_RET;
	}
...
{
	struct rpmsg_hisp160_service *kernel_serv = dev_get_drvdata(&rpdev->dev);
