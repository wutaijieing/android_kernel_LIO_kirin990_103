...
struct rpmsg_service_info {
	struct rpmsg_hisp250_service *kernel_isp_serv;
...
#else
		((micro_second % CAM_MICROSECOND_PER_SECOND) + (s_timeval.tv_nsec / CAM_NANOSECOND_PER_MICROSECOND));
#endif
...
{
	struct rpmsg_hisp250_service *kernel_serv = rpmsg_local.kernel_isp_serv;
...
static int hisp250_init_r8isp_memory_pool(void *cfg)
{
	...
	cam_info("%s: r8_iova_pool_base=0x%x",
		__func__, s_hisp250.mem_pool[ipool].r8_iova);
	mutex_unlock(&kernel_rpmsg_service_mutex);
	return 0;
}
...
static int hisp250_deinit_r8isp_memory_pool(void *cfg)
{
	...
	if (s_hisp250.mem_pool[ipool].active) {
		...
		cam_buf_put_sgtable(s_hisp250.mem_pool[ipool].sgt);
	}
...
static int hisp250_config(hisp_intf_t *i, void *cfg)
{
	...
	case HISP_CONFIG_POWER_ON:
		...
		if (!remote_processor_up) {
			...
			/* fix the issue after kill CameraDaemon */
			if (pcfg->isSecure == 0)
				hisp_set_boot_mode(NONSEC_CASE);
			else if (pcfg->isSecure == 1)
				hisp_set_boot_mode(SEC_CASE);
	...
	case HISP_CONFIG_LOCK_VOLTAGE:
		...
		break;
	case HISP_CONFIG_SET_CSI_MODE:
		cam_info("%s csi data.%d", __func__, pcfg->cfgdata[0]);
		rc = hisp_csi_mode_sel(pcfg->cfgdata[0]);
		break;
	case HISP_CONFIG_SELECT_AO_I2C:
		cam_info("%s HISP_CONFIG_SELECT_AO_I2C", __func__);
		rc = hisp_i2c_pad_sel();
		break;
	default:
...
static int hisp250_power_on(hisp_intf_t *i)
{
	...
	struct rpmsg_hisp250_service *kernel_serv = NULL;
...
#pragma GCC visibility push(default)
	kernel_serv->ept =
		rpmsg_create_ept(kernel_serv->rpdev, hisp250_rpmsg_ept_cb,
			kernel_serv, chinfo);
...
{
	struct rpmsg_hisp250_service *kernel_serv = dev_get_drvdata(&rpdev->dev);
...
{
	cam_info("%s enter ... ", __func__);
	cam_warn("%s() %d uhm, unexpected message", __func__, __LINE__);
...
static int hisp250_send_rpmsg(hisp_intf_t *i, hisp_msg_t *from_user, size_t len)
{
	...
	kernel_serv = rpmsg_local.kernel_isp_serv;
	if (kernel_serv == NULL) {
		cam_err("%s() %d failed: kernel_serv does not exist", __func__,
			__LINE__);
		rc = -ENODEV;
		goto RET;
	}
...
static int32_t hisp250_platform_suspend(struct platform_device* pdev, pm_message_t state)
{
	int32_t ret = 0;
	mutex_lock(&hisp_power_lock_mutex);
	if (remote_processor_up) {
...
static int32_t hisp250_platform_resume(struct platform_device* pdev)
{
...
static struct platform_driver s_hisp250_driver = {
	...
	.driver = {
		.name = "vendor,chip_isp250",
		.owner = THIS_MODULE,
		.of_match_table = s_hisp250_dt_match,
	},
	.suspend = hisp250_platform_suspend,
	.resume = hisp250_platform_resume,
};
