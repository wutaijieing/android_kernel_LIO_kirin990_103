...
define_kernel_flash_mutex(lm3646_mix_ir);
...
#else
		pdata->lm3646_mix_ir_wakelock = wakeup_source_register(flash_ctrl->dev, "lm3646_mix_ir");
		if (!pdata->lm3646_mix_ir_wakelock) {
			rc = -EINVAL;
...
static const struct of_device_id g_lm3646_mix_ir_dt_match[] = {
	{ .compatible = "vendor,lm3646_mix_ir" },
	{}
};
