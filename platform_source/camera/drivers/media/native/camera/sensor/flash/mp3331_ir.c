...
#include <linux/adc.h> // for ito adc
...
define_kernel_flash_mutex(mp3331_ir);
...
static int hw_mp3331_ir_on(struct hw_flash_ctrl_t *flash_ctrl, void *data)
{
	...
	if (pdata->ito_enable) {
		bool dsm_flag = false;
		int ret = lpm_adc_get_value(12); // ITO Use ADC 12
		if (ret >= 0) {
	...
	struct hw_mp3331_ir_private_data_t *pdata)
{
	...
	rc = of_property_read_u32(of_node, "vendor,ito_protect_enable",
		&pdata->ito_enable);
...
static int hw_mp3331_ir_get_dt_data(struct hw_flash_ctrl_t *flash_ctrl)
{
	...
	pdata->is_power_on = 0;
	rc = of_property_read_u32(of_node, "vendor,need_flash_ldo",
	...
	if (pdata->need_flash_ldo ==  1) {
		rc = of_property_read_u32(of_node, "vendor,flash_ldo_voltage",
			&pdata->flash_ldo_voltage);
	...
	rc = of_property_read_u32(of_node, "vendor,flash_current",
		&pdata->flash_current);
	cam_info("%s chip, flash_current %d, rc %d\n", __func__,
	...
	rc = of_property_read_u32_array(of_node, "vendor,flash-current",
	...
	rc = of_property_read_u32_array(of_node, "vendor,selftest-param",
