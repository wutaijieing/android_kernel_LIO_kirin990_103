...
static struct hw_tps61310_private_data_t g_tps61310_pdata;
static struct hw_flash_ctrl_t g_tps61310_ctrl;
static struct i2c_driver g_tps61310_i2c_driver;

define_kernel_flash_mutex(tps61310);

/* Function define */
static int hw_tps61310_set_strobe0(struct hw_flash_ctrl_t *flash_ctrl,
	unsigned int state)
{
	struct hw_tps61310_private_data_t *pdata = NULL;
	...
	struct hw_flash_cfg_data *cdata)
{
	...
	if (cdata->mode == TORCH_LEFT_MODE)
		val = (val & LED123_DISABLE) | LED13_ENABLE;
	else if (cdata->mode == TORCH_RIGHT_MODE)
	...
	int data)
{
	...
	if (data < pdata->flash_led2_num) {
		cam_debug("%s flash_led2=0x%x", __func__,
			pdata->flash_led2[data]);
	...
	int data)
{
	...
	if ((data < pdata->torch_led13_num) && (data < pdata->torch_led2_num)) {
		...
		cam_warn("%s data %d > torch_led13_num %d or torch_led2_num %d",
			__func__, data, pdata->torch_led13_num,
	...
	i2c_func->i2c_write(i2c_client, REGISTER1,
		(val & MODE_CTRL_SHUTDONW) | TORCH_MODE_CTRL);
...
static int hw_tps61310_get_dt_data(struct hw_flash_ctrl_t *flash_ctrl)
{
	...
	rc = of_property_read_u32(of_node, "vendor,flash_led13_num",
		&pdata->flash_led13_num);
	cam_info("%s vendor,flash_led13_num %d, rc %d\n", __func__,
	...
	rc = of_property_read_u32(of_node, "vendor,flash_led2_num",
		&pdata->flash_led2_num);
	cam_info("%s vendor,flash_led2_num %d, rc %d\n", __func__,
	...
	rc = of_property_read_u32(of_node, "vendor,torch_led13_num",
		&pdata->torch_led13_num);
	cam_info("%s vendor,torch_led13_num %d, rc %d\n", __func__,
	...
	rc = of_property_read_u32(of_node, "vendor,torch_led2_num",
		&pdata->torch_led2_num);
	cam_info("%s vendor,torch_led2_num %d, rc %d\n", __func__,
	...
	rc = of_property_read_u32_array(of_node, "vendor,flash_led2",
	...
	rc = of_property_read_u32_array(of_node, "vendor,flash_led13",
	...
	rc = of_property_read_u32_array(of_node, "vendor,torch_led2",
	...
	rc = of_property_read_u32_array(of_node, "vendor,torch_led13",
	...
	struct device_attribute *attr, char *buf)
{
	...
	rc = snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE,
		...
		"mode=%d, data=%d.\n",
		g_tps61310_ctrl.state.mode, g_tps61310_ctrl.state.mode);
	if (rc <= 0) {
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (cdata.mode == STANDBY_MODE) {
		rc = hw_tps61310_off(&g_tps61310_ctrl);
		if (rc < 0) {
			cam_err("%s tps61310 flash off error", __func__);
			return rc;
		}
	} else {
		rc = hw_tps61310_on(&g_tps61310_ctrl, &cdata);
		if (rc < 0) {
	...
	struct device_attribute *attr, char *buf)
{
	...
	rc = snprintf_s(buf, MAX_ATTRIBUTE_BUFFER_SIZE,
		...
		g_tps61310_ctrl.flash_mask_enable);
	if (rc <= 0) {
		cam_err("%s, %d, ::snprintf_s return error %d",
			__func__, __LINE__, rc);
		return -EINVAL;
...
{
	if (buf[0] == '0')
		g_tps61310_ctrl.flash_mask_enable = false;
	else
		g_tps61310_ctrl.flash_mask_enable = true;
	...
	enum led_brightness brightness)
{
	...
	if (led_bright == STANDBY_MODE) {
		...
		max_level =
			((struct hw_tps61310_private_data_t *)(g_tps61310_ctrl.pdata))->torch_led13_num;
...
static struct device_attribute g_tps61310_lightness =
	__ATTR(lightness, 0664, hw_tps61310_lightness_show,
	hw_tps61310_lightness_store);

static struct device_attribute g_tps61310_flash_mask =
	__ATTR(flash_mask, 0664, hw_tps61310_flash_mask_show,
	hw_tps61310_flash_mask_store);

static int hw_tps61310_register_attribute(struct hw_flash_ctrl_t *flash_ctrl,
	struct device *dev)
{
	int rc;
	...
	flash_ctrl->cdev_torch.brightness_set =
		hw_tps61310_torch_brightness_set;
	...
	rc = device_create_file(dev, &g_tps61310_lightness);
	if (rc < 0) {
		cam_err("%s failed to creat lightness attribute", __func__);
		goto err_create_lightness_file;
	}

	rc = device_create_file(dev, &g_tps61310_flash_mask);
	if (rc < 0) {
...
err_create_flash_mask_file:
	device_remove_file(dev, &g_tps61310_lightness);
...
static int hw_tps61310_remove(struct i2c_client *client)
{
	...
	g_tps61310_ctrl.func_tbl->flash_exit(&g_tps61310_ctrl);

	client->adapter = NULL;
	return 0;
}

static const struct i2c_device_id g_tps61310_id[] = {
	{ "hw_tps61310", (unsigned long)&g_tps61310_ctrl },
	{}
};

static const struct of_device_id g_tps61310_dt_match[] = {
	{ .compatible = "vendor,hw_tps61310" },
	{}
};
MODULE_DEVICE_TABLE(of, g_tps61310_dt_match);

static struct i2c_driver g_tps61310_i2c_driver = {
	.probe = hw_flash_i2c_probe,
	.remove = hw_tps61310_remove,
	.id_table = g_tps61310_id,
	.driver = {
		.name = "hw_tps61310",
		.of_match_table = g_tps61310_dt_match,
	},
};
...
{
	cam_debug("%s erter\n", __func__);
	return i2c_add_driver(&g_tps61310_i2c_driver);
}

static void __exit hw_tps61310_module_exit(void)
{
	cam_debug("%s enter", __func__);
	i2c_del_driver(&g_tps61310_i2c_driver);
}

static struct hw_flash_i2c_client g_tps61310_i2c_client;

static struct hw_flash_fn_t g_tps61310_func_tbl = {
...
static struct hw_flash_ctrl_t g_tps61310_ctrl = {
	.flash_i2c_client = &g_tps61310_i2c_client,
	.func_tbl = &g_tps61310_func_tbl,
	.hw_flash_mutex = &flash_mut_tps61310,
	.pdata = (void *)&g_tps61310_pdata,
