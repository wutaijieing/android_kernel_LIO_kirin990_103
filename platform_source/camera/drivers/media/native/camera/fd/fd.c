...
#include "../../../../include/media/native/fd_drv_cfg.h"
#include "../cam_buf.h"
#include "fd_log.h"
#include "fd.h"
...
{
	if (pdev->smmu_flag == 0) {
		struct sg_table* sgt = cam_buf_get_sgtable(share_fd);
		if (IS_ERR_OR_NULL(sgt)) {
			fd_err("cam_buf_get_sgtable failed!");
			return -1;
		}
		...
		*size = sg_dma_len(sgt->sgl);
		...
		int ret = cam_buf_map_iommu(share_fd, &iommu_format);
		if (ret < 0) {
			fd_err("cam_buf_map_iommu failed!");
			return -1;
		}
		...
		*size = iommu_format.size;
		...
		cam_buf_unmap_iommu(share_fd, &iommu_format);
	}
...
{
	phys_addr_t phys_addr = cam_buf_get_pgd_base();
	if (phys_addr == 0) {
...
static void fd_get_dt_data(struct platform_device *Ddev, struct fd_device *fd_devp)
{
    ...
    ret = of_property_read_u32_array(np, "vendor,top-base", base_array, 2);
    if (ret < 0) {
   ...
   //get smmu flag
    ret = of_property_read_u32(np, "vendor,smmu-flag", &flag);
    if (ret) {
    ...
    ret = of_property_read_u32(np, "vendor,ram-config", &ram_config);
    if (ret) {
