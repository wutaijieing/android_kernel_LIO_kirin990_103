...
{
	int ret = 0;
	bool snclk2 = (id == 2 || id == 3 || id == 4); // 2 , 3, 4 for different sensor
	...
	if (on) {
		...
		}else if((snclk2) && (s_ctrl->isp_snclk2 != NULL)) {
	...
	struct sensor_power_setting *power_setting, int state)
{
	...
	const char *ldo_names[LDO_MAX] = {
		...
		"vcm", "vcm2", "iopw","misp", "avdd0", "avdd1", "miniisp", "iovdd", "oisdrv"};
...
static int kernel_sensor_power_config(sensor_t *s_ctrl, struct sensor_power_setting *power_setting, int on_off)
{
	int rc = 0;
