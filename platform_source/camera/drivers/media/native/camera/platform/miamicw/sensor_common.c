	...
	struct sensor_power_setting *power_setting, int state)
{
	...
	const char *ldo_names[LDO_MAX] = {
		...
		"misp", "avdd0", "avdd1", "miniisp", "iovdd", "oisdrv",\
		"mipiswitch", "afvdd", "drvvdd", "bvdd"
	};
	...
	struct sensor_power_setting *power_setting, int state)
{
	...
	if (ncp6925_ctrl.func_tbl->pmic_seq_config)
		rc = ncp6925_ctrl.func_tbl->pmic_seq_config(&ncp6925_ctrl,
			(pmic_seq_index_t)power_setting->seq_val, power_setting->config_val, state);
...
int hw_sensor_power_up(sensor_t *s_ctrl)
{
	...
	if (s_ctrl->power_setting_array.power_setting == NULL || s_ctrl->power_setting_array.size == 0) {
	...
	sensor_t *sensor)
{
	...
	rc = of_property_read_u32(of_np, "vendor,is_fpga",
		(u32 *)&is_fpga);
	...
	/* get ldo */
	sensor_info->ldo_num = of_property_count_strings(of_np, "vendor,ldo-names");
	if (sensor_info->ldo_num < 0) {
		cam_err("%s failed %d\n", __func__, __LINE__);
	} else {
		cam_debug("ldo num = %d", sensor_info->ldo_num);
		for (i = 0; i < (u32)sensor_info->ldo_num; i++) {
			rc = of_property_read_string_index(of_np, "vendor,ldo-names", i, &sensor_info->ldo[i].supply);
			if (rc < 0) {
				cam_err("%s failed %d\n", __func__, __LINE__);
				goto fail;
			}
