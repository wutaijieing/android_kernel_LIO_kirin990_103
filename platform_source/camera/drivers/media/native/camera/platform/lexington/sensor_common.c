...
static int hw_tof_pmu_back_v1_power_config(struct device *dev, unsigned char on_off)
{
	...
	if (on_off == POWER_ON) {
		...
		if (ret) {
			cam_err("%s: enable tof 6562 failed\n", __func__);
			regulator_put(supply);
			return -1;
		}
		...
		if (ret) {
			cam_err("%s: disable tof 6562 failed\n", __func__);
			regulator_put(supply);
			return -1;
		}
	}
	...
	struct sensor_power_setting *power_setting, int on_off)
{
	...
	case SENSOR_BOOT_5V:
		...
		break;
	case SENSOR_HI6562:
		cam_info("%s, seq_type:%u SENSOR_6562", __func__, power_setting->seq_type);
		if (on_off == POWER_ON) {
...
int hw_sensor_power_up(sensor_t *s_ctrl)
{
	...
	int index;
	int rc;
	int tx_status = WL_TX_STATUS_DEFAULT;
	int lightstrap_query_times = 100;

	if (lightstrap_online_state())
		while (lightstrap_query_times > 0) {
			tx_status = wireless_tx_get_tx_status();
			if (tx_status >= WL_TX_STATUS_PING && tx_status < WL_TX_STATUS_FAULT_BASE) {
				break;
			}
	...
	cam_info("%s lightstrap status %u, lightstrap_query_times %d, tx_status %d",
		__func__, lightstrap_online_state(), lightstrap_query_times, tx_status);
