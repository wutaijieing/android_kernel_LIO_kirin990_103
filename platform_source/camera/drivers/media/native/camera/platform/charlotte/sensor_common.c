...
#define PMIC_SUPPLY_NAME_LEN 20
...
static power_seq_type_tab seq_type_tab[] = {
	...
	{ "sensor_pmic6563_ldo", SENSOR_PMIC6563_LDO },
	{ "sensor_pmic6563_buck", SENSOR_PMIC6563_BUCK },
	{ "sensor_pmic6563_buckboost", SENSOR_PMIC6563_BUCKBOOST },
};
...
static int hw_tof_pmu_back_v1_power_config(struct device *dev, unsigned char on_off)
{
...
static int pmic6563_ldo_power_config(struct device *dev, unsigned char on_off,
		struct sensor_power_setting *power_setting)
{
	struct regulator *supply = NULL;
	int ret;

	char supply_name[PMIC_SUPPLY_NAME_LEN] = {0};
	ret = sprintf_s(supply_name, PMIC_SUPPLY_NAME_LEN, "hi6563_ldo%d", power_setting->seq_val);
	if (ret < 0) {
		pr_err("%s sprintf_s fail!\n", __func__);
		return ret;
	}
	cam_info("%s, seq_type:%u seq_val:%u supply_name:%s", __func__,
		power_setting->seq_type, power_setting->seq_val, supply_name);
	supply = regulator_get(dev, supply_name);
	if (IS_ERR(supply)) {
		cam_err("%s: get %s failed\n", __func__, supply_name);
		regulator_put(supply);
		return -1;
	}

	if (on_off == POWER_ON) {
		ret = regulator_set_voltage(supply, power_setting->config_val, power_setting->config_val);
		if (ret) {
			cam_err("%s: regulator_set_voltage failed\n", __func__);
			regulator_put(supply);
			return -1;
	...
	cam_info("hi_pmic_ldo power succ");
	return 0;
}

static int pmic6563_buck_power_config(struct device *dev, unsigned char on_off,
		struct sensor_power_setting *power_setting)
{
	struct regulator *supply = NULL;
	int ret;
	...
	char supply_name[PMIC_SUPPLY_NAME_LEN] = {0};
	ret = sprintf_s(supply_name, PMIC_SUPPLY_NAME_LEN, "hi6563_buck%d", power_setting->seq_val);
	if (ret < 0) {
		pr_err("%s sprintf_s fail!\n", __func__);
		return ret;
	}
	...
	cam_info("hi_pmic_buck power succ");
	return 0;
}

static int pmic6563_buckboost_power_config(struct device *dev, unsigned char on_off,
		struct sensor_power_setting *power_setting)
{
	struct regulator *supply = NULL;
	int ret;
	...
	char supply_name[PMIC_SUPPLY_NAME_LEN] = {0};
	ret = sprintf_s(supply_name, PMIC_SUPPLY_NAME_LEN, "hi6563_buckboost");
	if (ret < 0) {
		pr_err("%s sprintf_s fail!\n", __func__);
		return ret;
	}
	...
	cam_info("hi_pmic_buckboost power succ");
	return 0;
}
	...
	int state)
{
	...
	if (pin_type == FSIN) {
		...
		config_value = (int)power_setting->config_val;
	...
	struct sensor_power_setting *power_setting, int on_off)
{
	...
	case SENSOR_DVDD1_EN:
		...
		break;
	case SENSOR_AFVDD:
		cam_info("%s, seq_type:%u SENSOR_AFVDD",
			__func__, power_setting->seq_type);
	...
	case SENSOR_HI6562:
		...
		if (on_off == POWER_ON)
			rc = hw_tof_pmu_back_v1_power_config(s_ctrl->dev, POWER_ON);
		else
			rc = hw_tof_pmu_back_v1_power_config(s_ctrl->dev, POWER_OFF);
		break;
	case SENSOR_PMIC6563_LDO:
		cam_info("%s, seq_type:%u SENSOR_PMIC6563_LDO", __func__, power_setting->seq_type);
		rc = pmic6563_ldo_power_config(s_ctrl->dev, on_off, power_setting);
		break;
	case SENSOR_PMIC6563_BUCK:
		cam_info("%s, seq_type:%u SENSOR_PMIC6563_BUCK", __func__, power_setting->seq_type);
		rc = pmic6563_buck_power_config(s_ctrl->dev, on_off, power_setting);
		break;
	case SENSOR_PMIC6563_BUCKBOOST:
		cam_info("%s, seq_type:%u SENSOR_PMIC6563_BOOST", __func__, power_setting->seq_type);
		rc = pmic6563_buckboost_power_config(s_ctrl->dev, on_off, power_setting);
		break;
	default:
...
int hw_sensor_power_up(sensor_t *s_ctrl)
{
	...
	for (index = 0; index < (int)power_setting_array->size; index++) { /*lint !e574 */
...
int hw_sensor_power_down(sensor_t *s_ctrl)
{
	...
	cam_info("%s kernel_sensor_power_config done", __func__);
	...
	hwsensor_board_info_t *sensor_info)
{
	...
	if (sensor_info->ext_type == EXT_INFO_GPIO) {
		...
		count = of_property_count_elems_of_size(of_node,
			"vendor,adc_gpio", sizeof(u32));
		if (count > 0 && count < ADC_GPIO_NUM_MAX) {
			rc = of_property_read_u32_array(of_node, "vendor,adc_gpio",
	...
	sensor_t *sensor)
{
	...
	rc = of_property_read_u32(of_node, "vendor,retry_matchid",
		&sensor_info->matchid_fail_retry_flag); /* retry matchid flag */
	cam_debug("%s vendor,retry_matchid %d, rc %d\n", __func__,
	...
	if (count > 0) {
		ret = of_property_read_u32_array(of_node, "vendor,csi_index",
			(unsigned int *)&sensor_info->csi_id[0], count); /*lint !e64 */
	} else {
	...
	sensor_info->tof_igs_num = TOF_IGS_INVALID;
	rc = of_property_read_u32(of_node,
		"tof-swing-num", &sensor_info->tof_igs_num);
	if (rc < 0)
		cam_info("%s read tof_igs_num failed. %d\n", __func__, __LINE__);

	sensor_info->tof_gpio_num = TOF_IGS_INVALID;
	rc = of_property_read_u32(of_node,
	...
	if (count > 0) {
		ret = of_property_read_u32_array(of_node, "vendor,i2c_index",
			(unsigned int *)&sensor_info->i2c_id[0], count);
	} else {
	...
	if (count > 0) {
		ret = of_property_read_u32_array(of_node, "vendor,ao_i2c_index",
	...
	count = of_property_count_elems_of_size(of_node,
		"vendor,i2c_pad_type", sizeof(u32));
	if (count > 0) {
		ret = of_property_read_u32_array(of_node, "vendor,i2c_pad_type",
	...
	if (sensor_info->ldo_num < 0) {
		...
		for (i = 0; i < (unsigned int)sensor_info->ldo_num; i++) { /*lint !e574 */
	...
	for (i = 0; i < (unsigned int)sensor_info->gpio_num; i++) { /*lint !e574 */
		...
		for (index = 0; index < IO_MAX; index++) {
			if (gpio_ctrl_types[index]) {
				if (!strcmp(gpio_ctrl_types[index], gpio_tag)) /*lint !e421 */
					sensor_info->gpios[index].gpio = (unsigned int)of_get_gpio(of_node, i);
			}
		}
	...
	count = of_property_count_elems_of_size(of_node,
		"vendor,multiplex_gpio_index", sizeof(u32));
	if (count > 0 && count < MULTIPLEX_GPIO_MAX) {
		ret = of_property_read_u32_array(of_node,
			"vendor,multiplex_gpio_index",
			(unsigned int *)&sensor_info->multiplex_gpio_id[0], count);
