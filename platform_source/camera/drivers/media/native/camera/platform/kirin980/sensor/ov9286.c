...
static int ov9286_get_thermal(hwsensor_intf_t *si, void *data);
...
static struct device_attribute g_dev_attr_ov9286thermal =
	__ATTR(irthermal, 0440, hw_ov9286thermal_show, NULL);

static struct sensor_power_setting g_ov9286_power_setting[] = {
	{
...
static hwsensor_vtbl_t g_ov9286_vtbl = {
...
static sensor_t g_ov9286 = {
	.intf = { .vtbl = &g_ov9286_vtbl, },
	.power_setting_array = {
		.size = ARRAY_SIZE(g_ov9286_power_setting),
		.power_setting = g_ov9286_power_setting,
	},
};

static const struct of_device_id g_ov9286_dt_match[] = {
	{
...
MODULE_DEVICE_TABLE(of, g_ov9286_dt_match);

static struct platform_driver g_ov9286_driver = {
	.driver = {
		.name = "huawei,ov9286",
		.owner = THIS_MODULE,
		.of_match_table = g_ov9286_dt_match,
	},
};
...
static int32_t ov9286_platform_probe(struct platform_device *pdev)
{
	int rc = 0;
	...
	id = of_match_node(g_ov9286_dt_match, np);
	if (!id) {
	...
	register_camerafs_attr(&g_dev_attr_ov9286thermal);
...
static int __init ov9286_init_module(void)
{
	cam_info("Enter: %s", __func__);
	return platform_driver_probe(&g_ov9286_driver,
		ov9286_platform_probe);
}

static void __exit ov9286_exit_module(void)
{
	rpmsg_sensor_unregister((void *)&g_ov9286);
	hwsensor_unregister(sensor_to_pdev(g_ov9286));
	platform_driver_unregister(&g_ov9286_driver);
}
