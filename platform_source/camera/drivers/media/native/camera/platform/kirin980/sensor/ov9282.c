...
struct mutex g_ov9282_power_lock;
...
static int ov9282_get_thermal(hwsensor_intf_t *si, void *data);
...
static struct device_attribute g_dev_attr_ov9282thermal =
	__ATTR(irthermal, 0440, hw_ov9282thermal_show, NULL);

static struct sensor_power_setting g_ov9282_power_setting[] = {
	{
...
static hwsensor_vtbl_t g_ov9282_vtbl = {
...
static sensor_t g_ov9282 = {
	.intf = { .vtbl = &g_ov9282_vtbl, },
	.power_setting_array = {
		.size = ARRAY_SIZE(g_ov9282_power_setting),
		.power_setting = g_ov9282_power_setting,
	},
};

static const struct of_device_id g_ov9282_dt_match[] = {
	{
...
MODULE_DEVICE_TABLE(of, g_ov9282_dt_match);

static struct platform_driver g_ov9282_driver = {
	.driver = {
		.name = "huawei,ov9282",
		.owner = THIS_MODULE,
		.of_match_table = g_ov9282_dt_match,
	},
};
...
static int ov9282_config(hwsensor_intf_t *si, void  *argp)
{
	...
	case SEN_CONFIG_POWER_ON:
		mutex_lock(&g_ov9282_power_lock);
		if (!ov9282_power_on) {
			ret = si->vtbl->power_up(si);
			ov9282_power_on = true;
		}
		mutex_unlock(&g_ov9282_power_lock);
		break;
...
static int32_t ov9282_platform_probe(struct platform_device *pdev)
{
	int rc = 0;
	...
	id = of_match_node(g_ov9282_dt_match, np);
	if (!id) {
	...
	sensor->dev = &pdev->dev;
	mutex_init(&g_ov9282_power_lock);
	...
	register_camerafs_attr(&g_dev_attr_ov9282thermal);

ov9282_sensor_probe_fail:
	return rc;
}

static int __init ov9282_init_module(void)
{
	cam_info("Enter: %s", __func__);
	return platform_driver_probe(&g_ov9282_driver,
		ov9282_platform_probe);
}

static void __exit ov9282_exit_module(void)
{
	rpmsg_sensor_unregister((void *)&g_ov9282);
	hwsensor_unregister(sensor_to_pdev(g_ov9282));
	platform_driver_unregister(&g_ov9282_driver);
}
