...
static bool g_ov8856_power_on;

struct mutex g_ov8856_power_lock;

static struct sensor_power_setting g_ov8856_power_setting_hma[] = {
	{
...
static hwsensor_vtbl_t g_ov8856_vtbl = {
...
atomic_t volatile g_ov8856_powered = ATOMIC_INIT(0);
static sensor_t g_ov8856_hma = {
	.intf = { .vtbl = &g_ov8856_vtbl, },
	.power_setting_array = {
		.size = ARRAY_SIZE(g_ov8856_power_setting_hma),
		.power_setting = g_ov8856_power_setting_hma,
	},
	.p_atpowercnt = &g_ov8856_powered,
};

static const struct of_device_id g_ov8856_dt_match[] = {
	{
		.compatible = "huawei,ov8856_hma",
		.data = &g_ov8856_hma.intf,
	},
	{
	},
};
MODULE_DEVICE_TABLE(of, g_ov8856_dt_match);
...
static struct platform_driver g_ov8856_driver = {
	.probe = ov8856_platform_probe,
	.remove = ov8856_platform_remove,
	.driver = {
		.name = "huawei,ov8856",
		.owner = THIS_MODULE,
		.of_match_table = g_ov8856_dt_match,
	},
};
...
static int ov8856_config(hwsensor_intf_t *si, void  *argp)
{
	...
	case SEN_CONFIG_POWER_ON:
		mutex_lock(&g_ov8856_power_lock);
		if (!g_ov8856_power_on) {
			ret = si->vtbl->power_up(si);
			g_ov8856_power_on = true;
		}
		mutex_unlock(&g_ov8856_power_lock);
		break;
	case SEN_CONFIG_POWER_OFF:
		mutex_lock(&g_ov8856_power_lock);
		if (g_ov8856_power_on) {
			ret = si->vtbl->power_down(si);
			g_ov8856_power_on = false;
		}
		mutex_unlock(&g_ov8856_power_lock);
		break;
...
static int32_t ov8856_platform_probe(struct platform_device *pdev)
{
	...
	id = of_match_node(g_ov8856_dt_match, np);
	if (!id) {
	...
	sensor->dev = &pdev->dev;
	mutex_init(&g_ov8856_power_lock);
...
{
	cam_info("Enter: %s", __func__);
	return platform_driver_probe(&g_ov8856_driver,
		ov8856_platform_probe);
}

static void __exit ov8856_exit_module(void)
{
	rpmsg_sensor_unregister((void *)&g_ov8856_hma);
	hwsensor_unregister(sensor_to_pdev(g_ov8856_hma));
	platform_driver_unregister(&g_ov8856_driver);
}
