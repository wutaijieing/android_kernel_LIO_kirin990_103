...
#else
		((micro_second % CAM_MICROSECOND_PER_SECOND) + (u64)s_timeval.tv_nsec / CAM_NANOSECOND_PER_MICROSECOND);
#endif
...
{
	struct miniisp_msg_t *mini_req = (struct miniisp_msg_t *)arg;
	struct msg_req_query_igs_cam *matchReq = NULL;
	...
	matchReq = &(mini_req->u.req_query_igs_cam);
...
static int swing_v4l2_ioctl_cam_extend(void *arg)
{
	...
	swing_cam_log_info("%s extend_cmd=%d", __func__,
		wxt_req->extend_cmd);
