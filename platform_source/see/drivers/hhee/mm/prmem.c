				     ...
				     bool premap)
{
	...
	if (premap && prepare_mappings(chunk, virt_to_pfn((uintptr_t)addr)) != 0) {
		prmem_err(PRMEM_NO_PREP);
		return false;
	}
	chunk->base_pfn = (unsigned long)virt_to_pfn((uintptr_t)addr);
