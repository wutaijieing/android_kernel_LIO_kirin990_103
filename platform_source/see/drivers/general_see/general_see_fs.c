 ...
 * Description: implement general see file system function
 * Create: 2020-02-17
 */
		    ...
		    size_t size)
{
	...
	cnt = (ssize_t)vfs_read(fp, (char __user *)buffer, read_bytes, &fp->f_pos);
	if (cnt < read_bytes) {
...
#ifdef CONFIG_GENERAL_SEE_SUPPORT_ULOADER
		/* cos index only occupy the low 16 byte */
		storage_id = (cos_id >> HISEE_COS_STORAGE_ID_OFF) &
			     HISEE_COS_STORAGE_ID_MASK;
		cos_id = cos_id & HISEE_COS_STORAGE_ID_MASK;
		pr_info("%s: cos id is %x, storage id is %x\n",
			__func__, cos_id, storage_id);
#endif
...
int filesys_hisee_read_image(enum hisee_img_file_type type, char *buffer, size_t bsize)
{
	...
	ret = (int)local_img_header.file[index].size;
	set_hisee_errno(HISEE_OK);
	return ret;
}
...
int hisee_encos_header_init(void)
{
	...
	cnt = (ssize_t)vfs_read(fp, (char __user *)&g_hisee_encos_header,
...
int filesys_rm_cos_flash_file(void)
{
	...
	pr_err("hisee:%s() not delete cos_flash.img in ENG mode.\n", __func__);
	return HISEE_OK;
#else
	int ret;
	mm_segment_t old_fs;
 ...
 * @param[in]   : buf_size, size of data_buf
 * @param[in]   : access_type, indicate access operation
 * @return     : ::int, 0 on success, other value on failure
 */
int access_hisee_image_partition(char *data_buf, unsigned int buf_size,
