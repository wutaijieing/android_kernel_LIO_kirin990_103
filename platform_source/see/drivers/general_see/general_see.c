 ...
 * Description: implement general see common function
 * Create: 2020-02-17
 */
...
static struct hisee_driver_function g_hisee_atf_function_list[] = {
	...
	{ "nvm_format", hisee_nvmformat_func },
#endif
	...
	{ "hisee_chiptest_rt_run", hisee_chiptest_rt_run_func },
	{ "hisee_chiptest_rt_stop", hisee_chiptest_rt_stop_func },
#endif
...
#ifdef CONFIG_GENERAL_SEE_NFC_IRQ_SWITCH
...
{
#if defined CONFIG_DFX_DEBUG_FS && !defined CONFIG_GENERAL_SEE_DISABLE_KEY
	...
	return hisee_channel_test_func(buf, para);
#else
	return HISEE_OK;
#endif
}

static noinline int atfd_hisee_smc(u64 _function_id, u64 _arg0, u64 _arg1, u64 _arg2)
{
...
#if defined(CONFIG_HUAWEI_DSM) && !defined(CONFIG_GENERAL_SEE_DISABLE_KEY)
...
int send_smc_no_ack(enum se_smc_cmd smc_cmd, const char *msg, unsigned int msg_len)
{
	char *buff_virt = NULL;
	phys_addr_t buff_phy = 0;
	u64 buff_size = SIZE_4K;
	struct hisee_module_data *hisee_data_ptr = NULL;
	int ret;
	...
	if (!hisee_data_ptr->cma_device) {
		pr_err("%s: cma device is invalid!\n", __func__);
		ret = HISEE_INVALID_PARAMS;
		return set_errno_then_exit(ret);
	}

	buff_virt = (char *)dma_alloc_coherent(hisee_data_ptr->cma_device, buff_size,
					       &buff_phy, GFP_KERNEL);
	if (!buff_virt) {
	...
	if (smc_cmd != CMD_HISEE_CHANNEL_TEST)
		ret = atfd_hisee_smc((u64)HISEE_FN_MAIN_SERVICE_CMD,
				     (u64)smc_cmd, (u64)buff_phy, buff_size);
	else
		ret = atfd_hisee_smc((u64)HISEE_FN_CHANNEL_TEST_CMD,
				     (u64)smc_cmd, (u64)buff_phy, buff_size);
exit:
	dma_free_coherent(hisee_data_ptr->cma_device, buff_size,
			  buff_virt, buff_phy);
	return ret;
}
		     ...
		     unsigned int timeout, enum se_smc_cmd smc_cmd)
{
	...
	if (smc_cmd != CMD_HISEE_CHANNEL_TEST)
		...
		local_jiffies = (unsigned long)MAX_SCHEDULE_TIMEOUT;

	if (wait_for_completion_timeout(&hisee_data_ptr->smc_completion, local_jiffies) == 0) {
...
#ifdef CONFIG_GENERAL_SEE_APPLET_APDU_TEST_OPTIMIZATION
...
static int hisee_wait_partition_ready(void)
{
	...
	do {
		if (hisee_sys_access(fullpath, 0) == 0)
			break;
		msleep(timeout);
		retry--;
	} while (retry > 0);
...
int hisee_lpmcu_send(rproc_msg_t msg_0, rproc_msg_t msg_1)
{
	...
	rproc_id_t rproc_id = IPC_ACPU_LPM3_MBX_6;
...
#if defined(CONFIG_GENERAL_SEE_MISCIMG_PATCH) && !defined(CONFIG_GENERAL_SEE_DISABLE_KEY)
...
#ifndef CONFIG_GENERAL_SEE_CHIPTEST_RT
	/*
...
static int hisee_check_status(void)
{
	...
	state = send_smc_no_ack(GENERAL_SEE_GET_PINCODE_STATUS, NULL, 0);
...
#ifdef CONFIG_GENERAL_SEE_CHIPTEST_RT
static DEVICE_ATTR(hisee_ioctl, (S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP),
...
static int _probe_init_global_resources(struct device *pdevice)
{
#ifndef CONFIG_GENERAL_SEE_DISABLE_KEY
	...
	hisee_data_ptr = get_hisee_data_ptr();
	hisee_data_ptr->hisee_clk = clk_get(pdevice, "seplat_volt_hold");
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
	hisee_data_ptr->wake_lock = wakeup_source_register(pdevice, "hisi-hisee");
#else
	hisee_data_ptr->wake_lock = wakeup_source_register("hisi-hisee");
#endif
	if (!hisee_data_ptr->wake_lock) {
		pr_err("hisee err create wakeup source failed\n");
...
static int hisee_probe_second_stage(struct platform_device *pdev)
{
	...
	ret = _probe_init_global_resources(pdevice);
	if (ret == HISEE_OK) {
...
static int hisee_is_bypass(struct device *pdevice)
{
	...
	state = send_smc_no_ack(CMD_GET_STATE, NULL, 0);
...
#ifdef CONFIG_GENERAL_SEE_SUPPORT_BYPASS_IMG
