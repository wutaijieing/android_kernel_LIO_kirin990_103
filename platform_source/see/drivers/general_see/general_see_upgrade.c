 ...
 * Description: implement general see image upgrade function
 * Create: 2020-02-17
 */
			     ...
			     enum se_smc_cmd smc_cmd)
{
	...
	struct atf_message_header *p_message_header = NULL;
#if defined CONFIG_GENERAL_SEE_SUPPORT_MULTI_COS && !defined CONFIG_GENERAL_SEE_DISABLE_KEY
	size_t file_size = 0;
#endif
...
int cos_upgrade_image_read(unsigned int cos_id, enum hisee_img_file_type img_type)
{
	...
	ret = hisee_upgrade_image_read(cos_id, ULOADER_IMG_TYPE, smc_cmd);
	check_result_and_goto(ret, out);

	/*
	 * Delay 200ms for uloader boot complete. When uloader boot
	 * success, it willl send cos upgrade request to ATF, then
				      ...
				      unsigned int cos_id)
{
	...
	hisee_data_ptr = get_hisee_data_ptr();
	(void)access_hisee_image_partition((char *)&curr, sizeof(curr), SW_VERSION_READ_TYPE);
	...
	curr.magic = HISEE_SW_VERSION_MAGIC_VALUE;
	(void)access_hisee_image_partition((char *)&curr, sizeof(curr), SW_VERSION_WRITE_TYPE);
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	cos_cnt = (int)(local_img_header.rpmb_cos_cnt + local_img_header.emmc_cos_cnt);
