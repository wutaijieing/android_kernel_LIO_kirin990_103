 ...
 * Description: implement general see power function
 * Create: 2020-02-17
 */
...
{
#ifdef CONFIG_GENERAL_SEE_POWER_MANAGE_COUNT
...
{
	int ret;
	enum se_smc_cmd cmd_to_atf;
	struct hisee_module_data *hisee_data_ptr = NULL;
	unsigned int msg[HISEE_SMC_PARAM_SIZE];
	...
	if (power_cmd == HISEE_POWER_CMD_ON) {
		...
		cmd_to_atf = CMD_HISEE_POWER_OFF;
	}

	/* send power command to atf */
	msg[HISEE_SMC_PARAM_0]  = (unsigned int)op_type;
	msg[HISEE_SMC_PARAM_1]  = op_cosid;
	ret = send_smc_no_ack(cmd_to_atf, (const char *)msg, sizeof(msg));
	if (ret != HISEE_OK) {
		...
		unsigned int op_cosid, int power_cmd)
{
	...
	hisee_data_ptr = get_hisee_data_ptr();
	if (power_cmd == SMX_PROCESS_STEP1_CMD) {
		cmd_to_atf = CMD_SMX_PROCESS_STEP1;
		if (g_power_pre_enable_clk) {
			ret = clk_prepare_enable(hisee_data_ptr->hisee_clk);
			if (ret < 0) {
				pr_err("%s() clk_prepare_enable failed ret=%d.\n",
		...
		cmd_to_atf = CMD_SMX_PROCESS_STEP2;
	}
...
s32 hisee_pincode_vote(u32 op_type)
{
	s32 ret = HISEE_ERROR;
	struct hisee_module_data *hisee_data_ptr = NULL;
	u32 msg[HISEE_SMC_PARAM_SIZE] = {0};
	...
	msg[HISEE_SMC_PARAM_0] = op_type;
	ret = send_smc_no_ack(GENERAL_SEE_PINCODE_VOTE, (const char *)msg, sizeof(msg));
	if (ret != HISEE_OK)
...
static void poweroff_handle(struct timer_list *t)
#else
static void poweroff_handle(unsigned long arg)
#endif
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
	struct timer_entry_list *p_timer_entry = from_timer(p_timer_entry, t, timer);
#else
	struct timer_entry_list *p_timer_entry = (struct timer_entry_list *)(uintptr_t)arg;
#endif
	if (!p_timer_entry) {
		pr_err("%s(): timer entry error\n", __func__);
		return;
	}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0))
	timer_setup(p_timer, poweroff_handle, 0);
#else
	init_timer(p_timer);
...
int wait_hisee_ready(enum hisee_state ready_state, unsigned int timeout_ms)
{
	...
	cnt = timeout_ms / unit;
	do {
		state = (enum hisee_state)send_smc_no_ack(CMD_GET_STATE, NULL, 0);
