...
int read_vrl_dev(void)
{
	...
	g_vrl_buffer = kmalloc(VRL_ALL_BUFLEN, GFP_KERNEL);
	if (!g_vrl_buffer) {
		sec_print_err("vrl kmalloc failed.\n");
		ret = SEC_ERROR;
		goto get_vrl_backup;
	}
...
get_vrl_backup:
	g_vrl_backup_buffer = kmalloc(VRL_ALL_BUFLEN, GFP_KERNEL);
	if (!g_vrl_backup_buffer) {
		sec_print_err("vrl_backup kmalloc failed.\n");
		ret = SEC_ERROR;
		goto err_out;
	}
	ret = bsp_read_bin(PART_VRL_BACKUP, 0, VRL_ALL_BUFLEN, g_vrl_backup_buffer);
	if (ret != SEC_OK) {
		kfree(g_vrl_backup_buffer);
