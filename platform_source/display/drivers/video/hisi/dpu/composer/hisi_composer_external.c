...
static struct hisi_composer_device *g_hisi_external_ov_data;

static void hisi_external_init_default_clk(struct hisi_composer_data *ov_data)
{
...
static int hisi_drv_external_composer_probe(struct platform_device *pdev)
{
	...
	hisi_external_init_default_clk(&ov_dev->ov_data);
...
static void hisi_external_composer_init_data(struct platform_device *pdev, void *dev_data, void *input_data, void *input_ops)
{
	...
	overlay_data->ov_data.ov_id = ONLINE_OVERLAY_ID_EXTERNAL1;
...
static struct platform_driver g_hisi_external_composer_driver = {
	.probe = hisi_drv_external_composer_probe,
	.remove = NULL,
	.suspend = NULL,
	.resume = NULL,
	.shutdown = NULL,
	.driver = {
		.name = HISI_EXTERNAL_OVERLAY_DEV_NAME,
	},
};

int hisi_drv_external_composer_create_device(void *connector_data)
{
	if (!g_hisi_external_ov_data) {
		g_hisi_external_ov_data = hisi_drv_create_device(HISI_EXTERNAL_OVERLAY_DEV_NAME, sizeof(*g_hisi_external_ov_data),
					hisi_external_composer_init_data, connector_data, NULL);
		if (!g_hisi_external_ov_data) {
			disp_pr_err("create %s device fail", HISI_EXTERNAL_OVERLAY_DEV_NAME);
			return -1;
		}
	}
...
void hisi_drv_external_composer_register_connector(struct hisi_connector_device *connector_data)
{
	...
	disp_pr_info("enter +++++++, g_hisi_external_ov_data=%p \n", g_hisi_external_ov_data);

	next_device = devm_kzalloc(&(g_hisi_external_ov_data->pdev->dev), sizeof(*next_device), GFP_KERNEL);
	...
	g_hisi_external_ov_data->next_device[g_hisi_external_ov_data->next_dev_count] = next_device;
	g_hisi_external_ov_data->next_dev_count++;
}

static int __init hisi_drv_external_composer_init(void)
{
	int ret;

	disp_pr_info("enter +++++++ \n");

	ret = platform_driver_register(&g_hisi_external_composer_driver);
	if (ret) {
...
module_init(hisi_drv_external_composer_init);

MODULE_DESCRIPTION("Hisilicon External Composer Driver");
