...
#define CLT_MAX_MEMPOOL_SIZE	(6 * 1024 * 1024)

static struct hisi_composer_device *g_hisi_primay_ov_data;

static void hisi_primary_init_default_clk(struct hisi_composer_data *ov_data)
{
...
static int hisi_drv_primary_composer_probe(struct platform_device *pdev)
{
	...
	/* 1, create resource manager device */

	/* 2, init power manager such regulator and clk */
	hisi_disp_pm_init(hisi_disp_config_get_regulators(), PM_REGULATOR_MAX, hisi_disp_config_get_clks(), PM_CLK_MAX);
	hisi_primary_init_default_clk(&ov_dev->ov_data);

	/* 3, init isr
	 * sence irq now is not used for us, so the third parament is null
	 */

	irq_signals = hisi_disp_config_get_irqs();
	hisi_primary_isr_init(&ov_dev->ov_data.isr_ctrl[COMP_ISR_PRE], irq_signals[DISP_IRQ_SIGNAL_NS_MDP], NULL, 0, NULL, ov_dev);
	...
	/* 5, init timeline */
	hisi_disp_timline_init(ov_dev, &ov_dev->timeline, "primary_timeline");
...
static void hisi_primary_composer_init_data(struct platform_device *pdev, void *dev_data, void *input_data, void *input_ops)
{
	...
	mipi_data = (struct hisi_connector_device *)input_data;

	overlay_data->pdev = pdev;
	overlay_data->next_dev_count = 0;

	overlay_data->ov_data.fix_panel_info = mipi_data->fix_panel_info;
	overlay_data->ov_data.ov_id = ONLINE_OVERLAY_ID_PRIMARY;
...
static struct platform_driver g_hisi_primary_composer_driver = {
	.probe = hisi_drv_primary_composer_probe,
	.remove = NULL,
	.suspend = NULL,
	.resume = NULL,
	.shutdown = NULL,
	.driver = {
		.name = HISI_PRIMARY_OVERLAY_DEV_NAME,
	},
};

int hisi_drv_primary_composer_create_device(void *connector_data)
{
	if (!g_hisi_primay_ov_data) {
		g_hisi_primay_ov_data = hisi_drv_create_device(HISI_PRIMARY_OVERLAY_DEV_NAME, sizeof(*g_hisi_primay_ov_data),
					hisi_primary_composer_init_data, connector_data, NULL);
		if (!g_hisi_primay_ov_data) {
			disp_pr_err("create %s device fail", HISI_PRIMARY_OVERLAY_DEV_NAME);
			return -1;
		}
	}
...
void hisi_drv_primary_composer_register_connector(struct hisi_connector_device *connector_data)
{
	...
	disp_pr_info("enter +++++++, g_hisi_primay_ov_data=%p \n", g_hisi_primay_ov_data);

	next_device = devm_kzalloc(&(g_hisi_primay_ov_data->pdev->dev), sizeof(*next_device), GFP_KERNEL);
	...
	g_hisi_primay_ov_data->next_device[g_hisi_primay_ov_data->next_dev_count] = next_device;
	g_hisi_primay_ov_data->next_dev_count++;
}

static int __init hisi_drv_primary_composer_init(void)
{
	int ret;

	disp_pr_info("enter +++++++ \n");

	ret = platform_driver_register(&g_hisi_primary_composer_driver);
	if (ret) {
...
module_init(hisi_drv_primary_composer_init);

MODULE_DESCRIPTION("Hisilicon Primary Composer Driver");
