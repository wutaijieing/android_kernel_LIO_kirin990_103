...
bool dptx_slice_count_supported(struct dp_ctrl *dptx, uint8_t slice_count)
{
	...
	if (retval)
		disp_pr_err("[DP] %s : DPCD read failed\n", __func__);
...
void dptx_sink_color_depth_capabilities(struct dp_ctrl *dptx)
{
	...
	if (dsc_color_depth & DP_DSC_8_BPC)
		disp_pr_debug("Sink supports 8 bpc\n");

	if (dsc_color_depth & DP_DSC_10_BPC)
		disp_pr_debug("Sink supports 10 bpc\n");

	if (dsc_color_depth & DP_DSC_12_BPC)
		disp_pr_debug("Sink supports 12 bpc\n");
}
...
void dptx_sink_color_format_capabilities(struct dp_ctrl *dptx)
{
	...
	if (dsc_color_format & DP_DSC_RGB)
		disp_pr_debug("Sink supports RGB color format\n");

	if (dsc_color_format & DP_DSC_YCBCR_444)
		disp_pr_debug("Sink supports YCbCr 4:4:4 color format\n");

	if (dsc_color_format & DP_DSC_YCBCR_422_SIMPLE)
		disp_pr_debug("Sink supports YCbCr 4:2:2 SIMPLE color format\n");

	if (dsc_color_format & DP_DSC_YCBCR_422_NATIVE)
		disp_pr_debug("Sink supports YCbCr 4:2:2 NATIVE color format\n");

	if (dsc_color_format & DP_DSC_YCBCR_420_NATIVE)
		disp_pr_debug("Sink supports YCbCr 4:2:0 NATIVE color format\n");
}
...
int dptx_get_rx_line_buffer_depth(struct dp_ctrl *dptx)
{
	...
	disp_pr_debug("[DP] %s PPS buf_bit_depth = %d\n", __func__, buf_bit_depth);
...
int dptx_dsc_initial(struct dp_ctrl *dptx)
{
	...
	if (dptx->dsc && (byte & DP_DSC_DECOMPRESSION_IS_SUPPORTED)) {
		disp_pr_info("[DP] DSC enable! retval=%d.\n", byte);
		dptx->dsc = true;
...
uint32_t g_dsc_decoders = 2;
void set_dsc_decoders(uint32_t dsc_decoders)
{
	if (dsc_decoders > 4)
		dsc_decoders = 2;
	g_dsc_decoders = dsc_decoders;
}
EXPORT_SYMBOL_GPL(set_dsc_decoders);
...
void dptx_dsc_para_init(struct dp_ctrl *dptx)
{
	...
	if (slice_height_div <= 0) {
		disp_pr_err("[DP] slice_height_div calc error\n");
		return;
	}
	...
	if (p_algorithm == NULL) {
		disp_pr_err("[DP] get_dsc_check_instance NULL Pointer\n");
		return;
	}
	...
	dptx->dsc_decoders = g_dsc_decoders; /* for test */
	if (dptx->dsc_decoders == 0) {
		disp_pr_err("[DP] dsc_decoders cannot be zero");
		return;
	}
	input_dsc_info.slice_width = input_dsc_info.pic_width / dptx->dsc_decoders;
	if (slice_height_div == 0) {
		disp_pr_err("[DP] slice_height_div cannot be zero");
		return;
	}
	...
	if (linebuf_depth < 0) {
		disp_pr_err("[DP] linebuf_depth calc error\n");
		return;
	}
