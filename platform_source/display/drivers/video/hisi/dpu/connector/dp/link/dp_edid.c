...
struct dtd g_dtd_test = {
	/* 16 - 1920x1080p @ 59.94/60Hz 16:9 */
	detailed_timing_descriptor(16, 9, 1920, 1080, 280, 45, 88, 4, 44, 5, 0, 0, 0, 148500, 0)
};

void set_dtd_test_code1(uint16_t his, uint16_t vis, uint16_t ha,
	uint16_t va, uint16_t hb, uint16_t vb)
{
	g_dtd_test.h_image_size = his;
	g_dtd_test.v_image_size = vis;
	g_dtd_test.h_active = ha;
	g_dtd_test.v_active = va;
	g_dtd_test.h_blanking = hb;
	g_dtd_test.v_blanking = vb;
}
EXPORT_SYMBOL_GPL(set_dtd_test_code1);

void set_dtd_test_code2(uint16_t hso, uint16_t vso,
	uint16_t hspw, uint16_t vspw, uint8_t hp, uint8_t vp)
{
	g_dtd_test.h_sync_offset = hso;
	g_dtd_test.v_sync_offset = vso;
	g_dtd_test.h_sync_pulse_width = hspw;
	g_dtd_test.v_sync_pulse_width = vspw;
	g_dtd_test.h_sync_polarity = hp;
	g_dtd_test.v_sync_polarity = vp;
}
EXPORT_SYMBOL_GPL(set_dtd_test_code2);

void set_dtd_test_code3(uint8_t il, uint64_t pc, uint16_t pri)
{
	g_dtd_test.interlaced = il;
	g_dtd_test.pixel_clock = pc;
	g_dtd_test.pixel_repetition_input = pri;
}
EXPORT_SYMBOL_GPL(set_dtd_test_code3);
...
int parse_edid(struct dp_ctrl *dptx, uint16_t len)
{
	...
	disp_check_and_return((edid_t == NULL), -EINVAL, err, "[DP] Raw Data is invalid!(NULL error)!\n");

	if (((len / EDID_LENGTH) > 5) || ((len % EDID_LENGTH) != 0) || (len < EDID_LENGTH)) {
		disp_pr_err("[DP] Raw Data length is invalid(not the size of (128 x N , N = [1-5]) uint8_t!");
		return -EINVAL;
	}
	...
	if ((ext_block_num > 0) && (len == ((ext_block_num + 1) * EDID_LENGTH))) {
		...
		disp_pr_err("[DP] Error occurred while parsing, returning with NULL!");
		return -EINVAL;
	}
	disp_pr_debug("[DP] -\n");
...
int check_main_edid(uint8_t *edid)
{
	...
	disp_check_and_return((edid == NULL), -EINVAL, err, "[DP] edid is NULL!\n");
	/* Verify 0 checksum */
	for (i = 0; i < EDID_LENGTH; i++)
		checksum += edid[i];
	if (checksum & 0xFF) {
		disp_pr_err("[DP] EDID checksum failed - data is corrupt. Continuing anyway.\n");
		return -EINVAL;
	}
	...
	for (i = 0; i < EDID_HEADER_END + 1; i++) {
		if (edid[i] != edid_v1_header[i]) {
			disp_pr_info("[DP] first uint8_ts don't match EDID version 1 header\n");
			return -EINVAL;
		}
	}
...
int parse_main(struct dp_ctrl *dptx)
{
	...
	if (check_main_edid(edid_t)) {
		disp_pr_err("[DP] The main edid block is wrong.\n");
		return -EINVAL;
	}
	...
	disp_pr_debug("[DP] EDID version %d revision %d\n", (int)edid_t[EDID_STRUCT_VERSION],
	...
	if (vid_info->dp_monitor_descriptor == NULL) {
		...
		if (vid_info->dp_monitor_descriptor == NULL) {
			disp_pr_err("[DP] dp_monitor_descriptor memory alloc fail\n");
			return -EINVAL;
		}
	}
	...
	for (i = 0; i < DETAILED_TIMING_DESCRIPTION_COUNT; i++, block += DETAILED_TIMING_DESCRIPTION_SIZE) {
		...
		case DETAILED_TIMING_BLOCK:
			...
			if (ret != 0) {
				disp_pr_info("[DP] Timing Description Parsing failed!\n");
				return ret;
			}
			break;
		case MONITOR_LIMITS:
			...
			if (ret != 0) {
				disp_pr_info("[DP] Parsing the monitor limit is failed!\n");
				return ret;
			}
			break;
		case MONITOR_NAME:
			...
			if (ret != 0) {
				disp_pr_err("[DP] The monitor name parsing failed.\n");
				return ret;
			}
			break;
		default:
	...
	disp_pr_debug("[DP] Extensions to follow:\t%d\n", (int)edid_t[EXTENSION_FLAG]);
...
int check_exten_edid(uint8_t *exten)
{
	...
	disp_check_and_return((exten == NULL), -EINVAL, err, "[DP] exten is NULL!\n");

	for (i = 0; i < EDID_LENGTH; i++)
		checksum += exten[i];
	if (checksum & 0xFF) {
		disp_pr_err("[DP] Extension Data checksum failed - data is corrupt. Continuing anyway.\n");
		return -EINVAL;
	}
	/* Check Extension Tag */
	/* ( Header Tag stored in the first uint8_t ) */
	if (exten[0] != EXTENSION_HEADER) {
		disp_pr_err("[DP] Not CEA-EDID Timing Extension, Extension-Parsing will not continue!\n");
		return -EINVAL;
	}
	return 0;
}
...
int parse_extension_timing_description(struct dp_ctrl *dptx, uint8_t *dtd_block, uint32_t dtd_begin, uint16_t dtd_total)
{
	...
	disp_check_and_return((dptx == NULL), -EINVAL, err, "[DP] dptx is NULL!\n");
	disp_check_and_return((dtd_block == NULL), -EINVAL, err, "[DP] dtd_block is NULL!\n");

	if ((dtd_begin + 1 + dtd_total * DETAILED_TIMING_DESCRIPTION_SIZE) > EDID_LENGTH) {
		disp_pr_err("[DP] The dtd total number 0x%x is out of the limit\n", dtd_total);
		return -EINVAL;
	}
	...
	for (i = 0; i < dtd_total; i++, dtd_block += DETAILED_TIMING_DESCRIPTION_SIZE) {
		...
		case DETAILED_TIMING_BLOCK:
			...
			if (ret != 0) {
				disp_pr_err("[DP] Timing Description Parsing failed!");
				return ret;
			}
			break;
		case MONITOR_LIMITS:
...
int parse_extension(struct dp_ctrl *dptx, uint8_t *exten)
{
	...
	if (ret) {
		disp_pr_err("[DP] The check_exten_edid failed.\n");
		return ret;
	}
	/*
	...
	if (exten[2] == 0x00) {
		disp_pr_info("[DP] There are no DTDs present in this extension block and no non-DTD data.\n");
		return -EINVAL;
	...
	if (dtd_total != (EDID_LENGTH - dtd_start_byte - 1) / DETAILED_TIMING_DESCRIPTION_SIZE) {
		disp_pr_info("[DP] The number of native DTDs is not equal the size\n");
	...
	if (ret) {
		disp_pr_err("[DP] Parse the extension block timing information fail.\n");
		return ret;
	}
	...
	if (cea_data_block_collection == 0x04) {
		...
		if (ret != 0) {
			disp_pr_err("[DP] CEA data block Parsing failed!\n");
			return ret;
		}
	}
...
int parse_timing_description(struct dp_ctrl *dptx, uint8_t *dtd)
{
	...
	disp_check_and_return((dptx == NULL), -EINVAL, err, "[DP] dptx is NULL!\n");
	disp_check_and_return((dtd == NULL), -EINVAL, err, "[DP] dtd is NULL!\n");
	...
	if (vid_info->dptx_timing_list == NULL) {
		...
		if (vid_info->dptx_timing_list == NULL) {
			disp_pr_err("[DP] vid_info->dptx_timing_list is NULL");
			return -EINVAL;
		}
	...
	if (node != NULL) {
		...
		disp_pr_err("[DP] kzalloc struct dptx_hdmi_vic fail!\n");
		return -EINVAL;
	}

	disp_pr_info("[DP] The timinginfo %d: hActivePixels is %d, vActivePixels is %d, pixel clock = %llu\n",
...
int parse_timing_description_by_vesaid(struct edid_video *vid_info, uint8_t vesa_id)
{
	...
	disp_check_and_return((vid_info == NULL), -EINVAL, err, "[DP] vid_info is NULL!\n");

	if (!convert_code_to_dtd(&mdtd, vesa_id, 60000, VCEA)) {
		disp_pr_info("[DP] Invalid video mode value %d\n",
						vesa_id);
		return -EINVAL;
	}

	if (mdtd.interlaced) {
		disp_pr_info("[DP] Don't Support interlace mode %d\n",
						vesa_id);
		return -EINVAL;
	}
...
int parse_hdmi_vic_id(uint8_t vic_id)
{
	...
	if (g_hdmi_vic_real_len >= g_hdmi_vic_len) {
		disp_pr_err("[DP] The g_hdmi_vic_real_len is more than g_hdmi_vic_len.\n");
		return -EINVAL;
	}
	...
	if (dptx_hdmi_list == NULL) {
		...
		if (dptx_hdmi_list == NULL) {
			disp_pr_err("[DP] dptx_hdmi_list is NULL");
			return -EINVAL;
		}
...
int parse_audio_spec_info(struct edid_audio *aud_info, struct edid_audio_info *spec_info, uint8_t *cea_data_block)
{
	if ((EXTEN_AUDIO_FORMAT <= 8) && (EXTEN_AUDIO_FORMAT >= 1)) {
		disp_check_and_return((!aud_info || !spec_info), -EINVAL, err, "[DP] aud_info or spec_info is NULL!\n");
		/* Set up SAD fields */
...
int parse_extension_audio_tag(struct edid_audio *aud_info, uint8_t *cea_data_block, uint8_t temp_length)
{
	...
	disp_check_and_return((aud_info == NULL), -EINVAL, err, "[DP] aud_info is NULL!\n");
	disp_check_and_return((cea_data_block == NULL), -EINVAL, err, "[DP] cea_data_block is NULL!\n");
	disp_check_and_return((temp_length < 1), -EINVAL, err, "[DP] The input param temp_length is wrong!\n");
	...
	for (i = 0; i < (temp_length) / 3; i++) {
		...
		if (xa == 0) {
			...
			if (aud_info->spec != NULL) {
				disp_pr_err("[DP] The spec of audio is error.\n");
				return -EINVAL;
			}
			aud_info->spec = kzalloc(sizeof(struct edid_audio_info), GFP_KERNEL);
			if (aud_info->spec == NULL) {
				disp_pr_err("[DP] malloc Audio Spec part failed!\n");
				return -EINVAL;
			}
		} else {
			...
			if (temp_ptr == NULL) {
				disp_pr_err("[DP] Realloc Audio Spec part failed!\n");
				return -EINVAL;
			}
			if (aud_info->spec == NULL) {
				disp_pr_err("[DP] The spec is NULL.\n");
				kfree(temp_ptr);
				return -EINVAL;
			}
		...
		if (parse_audio_spec_info(aud_info, &(aud_info->spec[xa]), cea_data_block)) {
			disp_pr_err("[DP] parse the audio spec info fail.\n");
			return -EINVAL;
		}
...
int parse_extension_video_tag(struct edid_video *vid_info, uint8_t *cea_data_block, uint8_t length)
{
	...
	disp_check_and_return((vid_info == NULL), -EINVAL, err, "[DP] vid_info is NULL!\n");
	disp_check_and_return((cea_data_block == NULL), -EINVAL, err, "[DP] cea_data_block is NULL!\n");
	disp_check_and_return((length < 1), -EINVAL, err, "[DP] The input param length is wrong!\n");

	vid_info->extTiming = kzalloc(length * sizeof(struct ext_timing), GFP_KERNEL);
	if (vid_info->extTiming == NULL) {
		disp_pr_err("[DP] extTiming memory alloc fail\n");
		return -EINVAL;
	}
	...
	uint8_t pos_after_latency)
{
	...
	disp_check_and_no_retval((cea_data_block == NULL), err, "[DP] cea_data_block is NULL!\n");
	...
	for (i = pos_after_latency; i < length; i++) {
		if (i == pos_after_latency) {
			...
			if (g_hdmi_vic_len == 0) {
				disp_pr_info("[DP] This EDID don't include HDMI additional video format (2).\n");
				return;
			}
...
static void parse_hdmi_vsdb_timing_list(struct edid_video *vid_info)
{
	...
	disp_check_and_no_retval((vid_info == NULL), err, "[DP] vid_info is NULL!\n");
	disp_check_and_no_retval((dptx_hdmi_list == NULL), err, "[DP] dptx_hdmi_list is NULL!\n");
	...
	list_for_each_entry_safe(hdmi_vic_node, _node_, dptx_hdmi_list, list_node) {
		...
		case 4:
			...
			break;
		default:
			disp_pr_err("[DP] hdmi_vic_node is illegal!\n");
			break;
		}
...
int parse_extension_vsdb_tag(struct edid_video *vid_info, uint8_t *cea_data_block, uint8_t length)
{
	...
	disp_check_and_return((vid_info == NULL), -EINVAL, err, "[DP] vid_info is NULL!\n");
	disp_check_and_return((cea_data_block == NULL), -EINVAL, err, "[DP] cea_data_block is NULL!\n");
	disp_check_and_return((length < 5), -EINVAL, err, "[DP] VSDB length isn't correct!\n");

	ieee_flag = 0;
	ieee_flag = (cea_data_block[0]) | (cea_data_block[1] << 8) | (cea_data_block[2] << 16);

	if (ieee_flag != 0x000c03) {
		disp_pr_info("[DP] This block isn't belong to HDMI block: %x.\n", ieee_flag);
		return 0;
	}
	...
	for (i = 5; i < MIN(length, 8); i++) {
		...
		case 7:
			...
			if (hdmi_video_present == 0) {
				disp_pr_info("[DP] This EDID don't include HDMI additional video format (1).\n");
				return 0;
			}
			break;
		default:
	...
	if (g_hdmi_vic_len == 0)
		return 0;
	disp_pr_info("[DP] vic_id real length  = %d , vic length = %d !\n", g_hdmi_vic_real_len, g_hdmi_vic_len);
...
int parse_cea_data_block(struct dp_ctrl *dptx, uint8_t *cea_data, uint8_t dtd_start)
{
	...
	disp_check_and_return((dptx == NULL), -EINVAL, err, "[DP] dptx is NULL!\n");
	disp_check_and_return((cea_data == NULL), -EINVAL, err, "[DP] cea_data is NULL!\n");
	disp_check_and_return((dtd_start > (EDID_LENGTH - 1)), -EINVAL, err,
	...
	while (total_length < dtd_start) {
		...
		case EXTENSION_AUDIO_TAG:
			...
			if (parse_extension_audio_tag(aud_info, cea_data_block, block_length)) {
				disp_pr_err("[DP] parse_extension_audio_tag fail.\n");
				return -EINVAL;
			}
		...
		case EXTENSION_VIDEO_TAG:
			...
			if (parse_extension_video_tag(vid_info, cea_data_block, block_length)) {
				disp_pr_err("[DP] parse_extension_video_tag fail.\n");
				return -EINVAL;
			}
			cea_data_block += block_length;
			break;
		case EXTENSION_VENDOR_TAG:
			...
			if (parse_extension_vsdb_tag(vid_info, cea_data_block, block_length)) {
				disp_pr_err("[DP] parse_extension_vsdb_tag fail.\n");
				return -EINVAL;
			}
			cea_data_block += block_length;
			break;
...
{
	disp_check_and_return((block == NULL), -EINVAL, err, "[DP] block is NULL!\n");
...
int parse_monitor_name(struct dp_ctrl *dptx, uint8_t *blockname, uint32_t size)
{
	...
	disp_check_and_return((dptx == NULL), -EINVAL, err, "[DP] dptx is NULL!\n");
	disp_check_and_return((blockname == NULL), -EINVAL, err, "[DP] blockname is NULL!\n");
	disp_check_and_return((size != DETAILED_TIMING_DESCRIPTION_SIZE), -EINVAL, err,
	...
	if (str_end < str_start) {
		disp_pr_err("[DP] The length of monitor name is wrong\n");
		return -EINVAL;
	}
...
bool convert_code_to_dtd(struct dtd *mdtd, uint8_t code, uint32_t refresh_rate, uint8_t video_format)
{
	...
	disp_check_and_return((mdtd == NULL), false, err, "[DP] mdtd is NULL!\n");
	...
	case VCEA:
		if (code == 255) {
			dtd_by_code = g_dtd_test;
			disp_pr_info("[DP] use g_dtd_test.\n");
		} else {
			if (code == 0 || code >= dtd_array_size(cea_modes_dtd)) {
				disp_pr_info("[DP] code error, code is %u\n", code);
				return false;
			}
	...
	default:
		disp_pr_info("[DP] Video Format is ERROR\n");
		return false;
	}
