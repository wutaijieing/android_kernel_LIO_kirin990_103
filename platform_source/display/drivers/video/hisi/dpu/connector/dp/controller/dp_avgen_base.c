...
uint8_t dptx_audio_get_sample_freq_cfg(struct audio_params *aparams)
{
	...
	disp_check_and_return(!aparams, 0, err, "[DP] aparams is NULL\n");
...
void dptx_config_hdr_info_data(struct sdp_full_data *hdr_sdp_data, struct hdr_infoframe *hdr_infoframe)
{
	int i, j;
...
int dptx_update_dss_and_hwc(struct dp_ctrl *dptx)
{
	...
	if (dptx->dptx_vr) {
		if (dptx_check_low_temperature(dptx)) {
			disp_pr_err("[DP] VR device can't work on low temperature!\n");
			return -EINVAL;
		}
		if (dptx->dptx_resolution_switch)
			retval = dptx->dptx_resolution_switch(dptx, HOT_PLUG_IN_VR);
		if (retval)
			disp_pr_err("[DP] HOT_PLUG_IN_VR DSS init fail !!!\n");
	} else {
#if DP_TO_DO
		retval = dptx_change_video_mode_user(dptx);
		if (retval)
			disp_pr_err("[DP] Change mode by user setting error!\n");
#endif
		if (dptx->dptx_resolution_switch)
			retval = dptx->dptx_resolution_switch(dptx, HOT_PLUG_IN);
		if (retval)
			disp_pr_err("[DP] HOT_PLUG_IN DSS init fail !!!\n");
	}
	return 0;
}
...
int dptx_update_panel_info(struct dp_ctrl *dptx)
{
	...
	if (dptx->edid_info.Video.maxHImageSize != 0) {
		...
		disp_pr_err("[DP] The size of display device cannot be got from edid information.\n");
	}

	pinfo->timing_info.pxl_clk_rate = mdtd->pixel_clock * 1000;
	...
	if (((mdtd->h_active + mdtd->h_blanking) * (mdtd->v_active + mdtd->v_blanking)) > 0) {
		...
		if (pixels == 0) {
			disp_pr_err("[DP] pixels cannot be zero");
			return -EINVAL;
		}
		vparams->m_fps = (uint32_t)(pinfo->timing_info.pxl_clk_rate / pixels);
	} else {
		return -1;
	}
...
void dptx_debug_resolution_info(struct dp_ctrl *dptx)
{
	...
	disp_pr_info("[DP] xres=%d\n"
		...
		pinfo->ldi.hsync_plr,
		pinfo->ldi.vsync_plr,
		pinfo->timing_info.pxl_clk_rate_div,
		pinfo->timing_info.pxl_clk_rate,
		vparams->m_fps);
...
int dptx_dss_plugin(struct dp_ctrl *dptx, enum dptx_hot_plug_type etype)
{
	...
	dptx->local_notify_func(dptx, CONNECTED);
	...
	if (hisifd->hpd_open_sub_fnc) {
		...
		if (ret) {
			disp_pr_err("[DP] dss pwr on failed.\n");
...
int dptx_video_mode_change(struct dp_ctrl *dptx, uint8_t vmode, int stream)
{
	...
	if (!convert_code_to_dtd(&mdtd, vparams->mode, vparams->refresh_rate,
		...
		disp_pr_err("[DP] Invalid video mode value %d\n",
						vparams->mode);
		return -EINVAL;
	}
	vparams->mdtd = mdtd;
	...
	disp_pr_info("[DP] The mode is changed as [%d]\n", vparams->mode);
...
int dptx_change_video_mode_user(struct dp_ctrl *dptx)
{
	...
	if (!dptx->same_source) {
		if ((vparams->mdtd.h_active > FHD_TIMING_H_ACTIVE) || (vparams->mdtd.v_active > FHD_TIMING_V_ACTIVE)) {
			...
			needchanged = true;
			disp_pr_info("[DP] Video mode is changed by different source!\n");
		}
	}
	...
	if (dptx->user_mode != 0) {
		...
		needchanged = true;
		disp_pr_info("[DP] Video mode is changed by user setting!\n");
	}

	if (needchanged) {
		retval = dptx_video_mode_change(dptx, vparams->mode, 0);
		if (retval) {
			disp_pr_err("[DP] Change mode error!\n");
			return retval;
		}
	}
	...
	if (dptx_check_low_temperature(dptx)) {
		if ((vparams->mdtd.h_active > FHD_TIMING_H_ACTIVE) || (vparams->mdtd.v_active > FHD_TIMING_V_ACTIVE)) {
			vparams->video_format = VCEA;
			vparams->mode = 16; /* Siwtch to 1080p on PC mode */
			disp_pr_info("[DP] Video mode is changed by low temperature!\n");
