...
static void get_dsi_dphy_ctrl(struct dpu_connector *connector, struct mipi_dsi_phy_ctrl *phy_ctrl)
{
...
static void get_dsi_cphy_ctrl(struct dpu_connector *connector, struct mipi_dsi_phy_ctrl *phy_ctrl)
{
	uint64_t lane_clock;
	struct mipi_panel_info *mipi = &connector->mipi;
...
static void get_mipi_dsi_timing(struct dkmd_connector_info *pinfo, struct mipi_panel_info *mipi)
{
	...
	dpu_pr_info("dsi_bit_clk_upt %llu M, htiming: %d, %d, %d, %d\n",
...
static void mipi_cdphy_init_config(struct dpu_connector *connector,
	...
	struct mipi_dsi_phy_ctrl *phy_ctrl, char __iomem *mipi_dsi_base)
{
	...
	if (is_dual_mipi_panel(&pinfo->base))
		set_reg(DPU_DSI_LDI_CTRL_ADDR(mipi_dsi_base),
				((connector->pipe_sw_post_chn_idx == PIPE_SW_POST_CH_DSI0) ? 0 : 1), 1, 13);

	if (is_mipi_cmd_panel(&pinfo->base)) {
		/* force update is not dependent on external TE signal */
		if ((connector->pipe_sw_post_chn_idx == PIPE_SW_POST_CH_DSI0) && !g_debug_force_update) {
			set_reg(DPU_DSI_TE_CTRL_ADDR(mipi_dsi_base), (0x1 << 17) | (0x1 << 6) | 0x1, 18, 0);
			set_reg(DPU_DSI_TE_CTRL_2_ADDR(mipi_dsi_base), (0x1 << 17) | (0x1 << 6) | 0x1, 18, 0);
		}
		...
		if (mipi->dsi_te_type == DSI1_TE1_TYPE) {
			...
			set_reg(DPU_DSI_TE_VS_WD_2_ADDR(mipi_dsi_base), ((2 * 38 / 1000000) << 12) | 0x3FC, 32, 0);
		} else {
...
void mipi_init(struct dpu_connector *connector)
{
	...
	if (connector->pipe_sw_post_chn_idx == 0) {
		set_reg(DPU_DSI_GLB_DSI0_RESET_EN_ADDR(connector->dpu_base + DPU_MIPI_DSI_GLB_OFFSET),
		1, 1, 0);
	} else if (connector->pipe_sw_post_chn_idx == 1) {
		set_reg(DPU_DSI_GLB_DSI1_RESET_EN_ADDR(connector->dpu_base + DPU_MIPI_DSI_GLB_OFFSET),
		1, 1, 1);
	} else {
		dpu_pr_err("pipe_sw_post_chn_idx is invalid\n");
	}
