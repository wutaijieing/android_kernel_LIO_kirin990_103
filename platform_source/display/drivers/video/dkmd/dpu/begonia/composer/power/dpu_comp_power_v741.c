...
static int32_t composer_regulator_enable(struct composer_manager *comp_mgr)
{
	...
	if (!IS_ERR_OR_NULL(comp_mgr->dss_lite1_regulator)) {
		ret = regulator_enable(comp_mgr->dss_lite1_regulator);
		if (ret)
			dpu_pr_err("dss_lite1_regulator enable failed, error=%d!", ret);
	}
...
static int32_t composer_regulator_disable(struct composer_manager *comp_mgr)
{
	...
	if (!IS_ERR_OR_NULL(comp_mgr->dss_lite1_regulator)) {
		ret = regulator_disable(comp_mgr->dss_lite1_regulator);
		if (ret)
			dpu_pr_err("dss_lite1_regulator disable failed, error=%d!", ret);
	}
