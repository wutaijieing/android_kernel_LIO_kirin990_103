...
static int32_t composer_manager_parse_dt(struct composer_manager *comp_mgr)
{
	...
	comp_mgr->dss_lite1_regulator = devm_regulator_get(&pdev->dev, "regulator_dss_lite1");
	if (IS_ERR(comp_mgr->dss_lite1_regulator)) {
		ret = PTR_ERR(comp_mgr->dss_lite1_regulator);
		dpu_pr_err("dss_lite1_regulator error, ret=%d", ret);
		return ret;
	}
...
static void composer_manager_setup(struct composer_manager *comp_mgr)
{
	...
	(void)snprintf(isr_ctrl->irq_name, sizeof(isr_ctrl->irq_name), "dpu_irq_sdp");
	isr_ctrl->irq_no = comp_mgr->sdp_irq;
	isr_ctrl->isr_fnc = dpu_sdp_isr;
	isr_ctrl->parent = comp_mgr;
	isr_ctrl->unmask = ~(DPU_DPP0_HIACE_NS_INT | DPU_DACC_NS_INT);
...
static const struct of_device_id g_composer_manager_match_table[] = {
	{
		.compatible = "dkmd,composer_manager_v741",
		.data = NULL,
	},
	{},
};
