...
static int mipi_lg_TD4322_panel_on(struct platform_device *pdev)
{
	...
	if (pinfo->lcd_init_step == LCD_INIT_POWER_ON) {
		...
		if (g_lcd_control_tp_power && !g_lcd_fpga_flag) {
			...
			if (error) {
				DPU_FB_ERR("ts resume device err!\n");
			}
		}
	#endif
#endif
		...
		if (g_lcd_control_tp_power && !g_lcd_fpga_flag) {
			...
			if (error) {
				DPU_FB_ERR("ts after resume err!\n");
			}
		}
	#endif
#endif
...
#if 0
	#ifdef CONFIG_HUAWEI_TS
		if (g_lcd_control_tp_power && !g_lcd_fpga_flag && !dpufd->fb_shutdown) {
			error = ts_power_control_notify(TS_BEFORE_SUSPEND, SHORT_SYNC_TIMEOUT);
			if (error) {
				DPU_FB_ERR("ts before suspend err!\n");
			}

			error = ts_power_control_notify(TS_SUSPEND_DEVICE, NO_SYNC_TIMEOUT);
			if (error) {
				DPU_FB_ERR("ts suspend device err!\n");
			}
		}

		if (dpufd->fb_shutdown) {
