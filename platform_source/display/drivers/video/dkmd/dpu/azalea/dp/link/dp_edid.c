...
static bool is_raw_data_length_valid(uint16_t len);
static void update_mdtd(struct dtd *mdtd, struct dtd *dtd_by_code);
static bool is_ext_block_valid(struct dp_ctrl *dptx, uint8_t *edid_t, int16_t ext_block_num, uint16_t len);
static int parse_cea_data_block_collection(struct dp_ctrl *dptx, uint8_t cea_data_block_collection,
	uint8_t *exten, uint8_t dtd_start_byte);

static const uint8_t edid_v1_other_descriptor_flag[2] = { 0x00, 0x00 };
static struct list_head *g_dptx_hdmi_list;
...
int parse_edid(struct dp_ctrl *dptx, uint16_t len)
{
	...
	if (!is_raw_data_length_valid(len)) {
	...
	if (!is_ext_block_valid(dptx, edid_t, ext_block_num, len))
		return -EINVAL;
...
static int parse_main_four_descriptor_block(struct dp_ctrl *dptx, uint8_t *block)
{
	int i;
	int ret;
...
int parse_main(struct dp_ctrl *dptx)
{
	...
	dpu_dp_save_edid(edid_t, DPTX_DEFAULT_EDID_BUFLEN);
#endif
	...
	ret = parse_main_four_descriptor_block(dptx, block);
	if (ret != 0)
		return ret;
...
int parse_extension_timing_description(struct dp_ctrl *dptx, uint8_t *dtd_block, uint32_t dtd_begin,
	uint16_t dtd_total)
{
	uint32_t i;
	int ret;
...
int parse_extension(struct dp_ctrl *dptx, uint8_t *exten)
{
	...
	dpu_dp_imonitor_set_param(DP_PARAM_BASIC_AUDIO, &(aud_info->basicAudio));
#endif
	...
	return parse_cea_data_block_collection(dptx, cea_data_block_collection, exten, dtd_start_byte);
}
...
static void parse_timing_description_fill_timing_info(struct timing_info *node,
	struct edid_video *vid_info, uint8_t *dtd)
{
...
static void parse_timing_description_get_max_value(struct edid_video *vid_info, uint8_t *dtd)
{
...
int parse_timing_description(struct dp_ctrl *dptx, uint8_t *dtd)
{
	...
	parse_timing_description_get_max_value(vid_info, dtd);
	/* node */
	...
	parse_timing_description_fill_timing_info(node, vid_info, dtd);
...
#if defined(CONFIG_HUAWEI_DSM)
	dpu_dp_imonitor_set_param_timing(node->hActivePixels, node->vActivePixels, node->pixelClock, 0);
#endif

	return 0;
}

static struct timing_info *alloc_node(struct dtd *mdtd, struct edid_video *vid_info)
{
	struct timing_info *node = NULL;
...
int parse_timing_description_by_vesaid(struct edid_video *vid_info, uint8_t vesa_id)
{
	...
	DPU_FB_INFO("[DP] The timinginfo %d: hActivePixels is %d, vActivePixels is %d, pixel clock = %llu, fps = %d\n",
		vid_info->mainVCount, node->hActivePixels, node->vActivePixels, node->pixelClock, fps);
#if defined(CONFIG_HUAWEI_DSM)
	dpu_dp_imonitor_set_param_timing(node->hActivePixels, node->vActivePixels, node->pixelClock, vesa_id);
#endif
...
int parse_hdmi_vic_id(uint8_t vic_id)
{
	...
	if (g_dptx_hdmi_list == NULL) {
		g_dptx_hdmi_list = kzalloc(sizeof(struct list_head), GFP_KERNEL);
		if (g_dptx_hdmi_list == NULL) {
			DPU_FB_ERR("[DP] g_dptx_hdmi_list is NULL");
			return -EINVAL;
		}
		INIT_LIST_HEAD(g_dptx_hdmi_list);
	}
	...
	if (node != NULL) {
		...
		list_add_tail(&node->list_node, g_dptx_hdmi_list);
		g_hdmi_vic_real_len++;
	} else {
		DPU_FB_ERR("[DP] kzalloc struct dptx_hdmi_vic fail!\n");
	}
...
static void parse_hdmi_vsdb_timing_list(struct edid_video *vid_info)
{
	...
	dpu_check_and_no_retval((g_dptx_hdmi_list == NULL), ERR, "[DP] g_dptx_hdmi_list is NULL!\n");

#ifdef SUPPORT_HDMI_VSDB_BLOCK
	list_for_each_entry_safe(hdmi_vic_node, _node_, g_dptx_hdmi_list, list_node) {
		...
		case 4:
			...
			break;
		default:
			DPU_FB_ERR("[DP] hdmi_vic_node is illegal!\n");
			break;
		}
	...
	kfree(g_dptx_hdmi_list);
	g_dptx_hdmi_list = NULL;
}
...
int parse_cea_data_block(struct dp_ctrl *dptx, uint8_t *cea_data, uint8_t dtd_start)
{
	...
	while (total_length < dtd_start) {
		...
		case EXTENSION_AUDIO_TAG:
			...
			if (parse_extension_audio_tag(&(dptx->edid_info.Audio), cea_data_block, block_length)) {
		...
		case EXTENSION_VIDEO_TAG:
			...
			if (parse_extension_video_tag(&(dptx->edid_info.Video), cea_data_block, block_length)) {
		...
		case EXTENSION_VENDOR_TAG:
			cea_data_block += 1;
			if (parse_extension_vsdb_tag(&(dptx->edid_info.Video), cea_data_block, block_length)) {
		...
		case EXTENSION_SPEAKER_TAG:
			 ...
			 */
			cea_data_block += 1;
			dptx->edid_info.Audio.extSpeaker = EXTEN_SPEAKER;
...
static inline uint16_t conditional_operator(bool equal, uint16_t val1, uint16_t val2)
{
...
static void vary_vfp(struct dtd *dtd_by_code, uint8_t code, uint32_t refresh_rate)
{
	...
	case 8:
	case 9:
		dtd_by_code->v_blanking = conditional_operator(refresh_rate == 59940, 22, 23);
		dtd_by_code->v_sync_offset = conditional_operator(refresh_rate == 59940, 4, 5);
		break;
	case 12:
	case 13:
		dtd_by_code->v_blanking = conditional_operator(refresh_rate == 60054, 22, 23);
		dtd_by_code->v_sync_offset = conditional_operator(refresh_rate == 60054, 4, 5);
		break;
	case 23:
	case 24:
		dtd_by_code->v_blanking = conditional_operator(refresh_rate == 50080, 24,
			conditional_operator(refresh_rate == 49920, 25, 26));
		dtd_by_code->v_sync_offset = conditional_operator(refresh_rate == 50080, 2,
			conditional_operator(refresh_rate == 49920, 3, 4));
		break;
	case 27:
	case 28:
		dtd_by_code->v_blanking = conditional_operator(refresh_rate == 50080, 24,
