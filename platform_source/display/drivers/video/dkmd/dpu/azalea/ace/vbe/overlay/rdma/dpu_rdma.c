...
void dpu_chn_set_reg_default_value(struct dpu_fb_data_type *dpufd,
	...
	char __iomem *dma_base, dss_rdma_t *s_dma)
{
	...
	if (s_dma->hfbcd_used)
		dpu_hfbcd_set_reg(dpufd, dma_base, s_dma);
#endif

#ifdef SUPPORT_DSS_HEBCD
	if (s_dma->hebcd_used)
		dpu_hebcd_set_reg(dpufd, dma_base, s_dma);
#endif
}
...
static int dpu_rdma_aligned_mask_rect(dss_layer_t *layer, dss_rect_ltrb_t *aligned_mask_rect,
...
static int dpu_rdma_aligned_rect(dss_layer_t *layer, dss_rect_ltrb_t *aligned_rect,
...
static int dpu_rdma_afbc_layer_aligned(dss_layer_t *layer, uint32_t mm_base_0, uint32_t mm_base_1)
{
...
static int dpu_rdma_stretch(dss_layer_t *layer, dss_rect_t *out_aligned_rect,
	const bool *rdma_stretch_enable, struct rdma_config_afbcd_param *afbcd_param)
{
	...
	if (*rdma_stretch_enable) {
		...
		if (layer->transform & DPU_FB_TRANSFORM_FLIP_V)
			afbcd_param->half_block_mode = AFBC_HALF_BLOCK_LOWER_ONLY;
		else
			afbcd_param->half_block_mode = AFBC_HALF_BLOCK_UPPER_ONLY;
	} else {
		rdma_transform = dpu_transform_hal2dma(layer->transform, layer->chn_idx);
...
static int dpu_rdma_offset_pos_set(dss_layer_t *layer, struct dss_rdma_oft_pos *rdma_oft_pos,
	uint32_t *stretched_line_num)
{
...
static int dpu_rdma_set_dpp_and_dma(struct dpu_fb_data_type *dpufd, dss_layer_t *layer,
...
static int dpu_rdma_addr_aligned(dss_layer_t *layer, uint32_t *l2t_interleave_n,
	uint32_t *rdma_addr, bool mmu_enable)
{
	void_unused(mmu_enable);
	*rdma_addr = layer->img.vir_addr;
	if (*rdma_addr & (DMA_ADDR_ALIGN - 1)) {
		DPU_FB_ERR("layer%d rdma_addr is not %d bytes aligned.\n",
			layer->layer_idx, DMA_ADDR_ALIGN);
		return -EINVAL;
	}
	...
	if (layer->need_cap & CAP_TILE) {
		*l2t_interleave_n = dpu_get_rdma_tile_interleave(layer->img.stride);
		...
		if (*rdma_addr & (TILE_DMA_ADDR_ALIGN - 1)) {
			DPU_FB_ERR("layer%d tile rdma_addr is not %d bytes aligned.\n",
				layer->layer_idx, TILE_DMA_ADDR_ALIGN);
			return -EINVAL;
		}
	}
...
static int dpu_rdma_oft_pos_check(struct dss_rdma_oft_pos rdma_oft_pos)
{
...
static int dpu_rdma_mask_pos_set(struct dss_rdma_oft_pos rdma_oft_pos,
...
static int dpu_rdma_payload_set(dss_layer_t *layer, uint32_t *afbc_payload_stride,
...
static int dpu_rdma_head_check(dss_layer_t *layer, dss_rect_ltrb_t aligned_rect,
	dss_rect_ltrb_t afbc_rect, struct rdma_config_afbcd_param *afbcd_param)
{
	if ((afbcd_param->header_addr & (AFBC_HEADER_ADDR_ALIGN - 1)) ||
		...
		DPU_FB_ERR("layer%d is not %d bytes aligned, or afbc_header_stride is not %d bytes aligned!\n",
			layer->layer_idx, AFBC_HEADER_ADDR_ALIGN, AFBC_HEADER_STRIDE_ALIGN);
		return -EINVAL;
	}
...
static uint32_t get_display_addr(dss_layer_t *layer, struct dpu_addr_config add_cfg)
{
...
static void get_addr_config_param(struct dpu_addr_config *add_config, bool mmu_enable,
...
static uint32_t dpu_calculate_display_addr(bool mmu_enable, dss_layer_t *layer,
	dss_rect_ltrb_t *aligned_rect, int add_type, bool is_pixel_10bit)
{
	uint32_t offset;
	struct dpu_addr_config add_config = {0};
	...
	dss_rect_ltrb_t afbc_rect, struct rdma_config_afbcd_param *afbcd_param)
{
	...
	ret = dpu_rdma_head_check(layer, param->aligned_src_rect, afbc_rect, afbcd_param);
	if (ret != 0)
		return -EINVAL;
	...
	dss_rect_ltrb_t afbc_rect, struct rdma_config_param *param)
{
	if ((g_debug_ovl_online_composer > 0) || (g_debug_ovl_offline_composer > 0))
		DPU_FB_INFO("fb%u, mm_base_0=0x%x, mm_base_1=0x%x, mmbuf_size=%d, "
...
static int dpu_afbcd_config(struct dpu_fb_data_type *dpufd, dss_layer_t *layer,
	...
	struct rdma_config_param *param)
{
	...
	ret = dpu_rdma_set_mmbuf_base_and_size(dpufd, layer, &afbc_rect, &param->mm_base_0, &param->mm_base_1);
	if (ret != 0)
		return -EINVAL;

	if (dpu_rdma_afbc_layer_aligned(layer, param->mm_base_0, param->mm_base_1) != 0)
		return -EINVAL;
	...
	/* stretch */
	ret = dpu_rdma_stretch(layer, ov_compose_rect->aligned_rect,
		&ov_compose_flag->rdma_stretch_enable, &afbcd_param);
	if (ret != 0)
		return -EINVAL;

	if (layer->img.afbc_header_addr & (AFBC_SUPER_GRAPH_HEADER_ADDR_ALIGN - 1)) {
		DPU_FB_ERR("layer%d super graph afbc_header_addr is not %d bytes aligned!\n",
			layer->layer_idx, AFBC_SUPER_GRAPH_HEADER_ADDR_ALIGN);
		return -EINVAL;
	}
	...
	/* payload */
	ret = dpu_rdma_payload_set(layer,
	...
	dss_rect_ltrb_t *clip_rect, dss_rect_t *out_aligned_rect, struct rdma_config_param *param)
{
	...
	ret = dpu_rdma_addr_aligned(layer, &(param->l2t_interleave_n), &(param->rdma_addr), param->mmu_enable);
	if (ret != 0) {
	...
	/* aligned_rect */
	ret = dpu_rdma_aligned_rect(layer, &(param->aligned_rect), param->src_rect, param->aligned_pixel);
	if (ret != 0)
		return -EINVAL;

	/* aligned_mask_rect */
	ret = dpu_rdma_aligned_mask_rect(layer,
	...
	if (dpu_rdma_oft_pos_check(param->rdma_oft_pos) != 0)
		return -EINVAL;

	param->rdma_addr = dpu_calculate_display_addr(param->mmu_enable, layer,
	...
	ret = dpu_rdma_mask_pos_set(param->rdma_oft_pos, &(param->rdma_mask_pos),
		param->aligned_mask_rect, &(param->src_rect_mask_enable), param->aligned_pixel);
	if (ret != 0)
		return -EINVAL;
	...
	struct rdma_config_param *param, bool *rdma_stretch_enable)
{
	...
	if (is_yuv_semi_planar || is_yuv_planar) {
		ret = dpu_rdma_offset_pos_set(layer, &param->rdma_oft_pos, &param->stretched_line_num);
		if (ret != 0)
			return -EINVAL;
...
static int rdma_config_param_prepare(struct dpu_fb_data_type *dpufd, dss_layer_t *layer,
	struct rdma_config_param *param, struct dpu_ov_compose_flag *ov_compose_flag)
{
	...
	param->rdma_format = dpu_pixel_format_hal2dma(layer->img.format);
	dpu_check_and_return((param->rdma_format < 0), -EINVAL, ERR, "rdmaformat:%d not support\n",
		layer->img.format);

	param->rdma_transform = dpu_transform_hal2dma(layer->transform, layer->chn_idx);
	...
	struct dpu_ov_compose_flag *ov_compose_flag)
{
	...
	ret = dpu_rdma_set_dpp_and_dma(dpufd, layer, &param.aligned_pixel, &param.src_rect_mask_enable, dma);
	if (ret != 0)
		return -EINVAL;

	if (layer->need_cap & CAP_HFBCD) {
#ifdef SUPPORT_DSS_HFBCD
		ret = dpu_hfbcd_config(dpufd, ovl_idx, layer, ov_compose_rect, ov_compose_flag);
		dpu_check_and_return((ret != 0), -EINVAL, ERR, "dpu_hfbcd_config failed! ret = %d\n", ret);
#endif
		return 0;
	}
...
#ifdef SUPPORT_DSS_HEBCD
		ret = dpu_hebcd_config(dpufd, ovl_idx, layer, ov_compose_rect, ov_compose_flag);
		 dpu_check_and_return((ret != 0), -EINVAL, ERR, "dpu_hebcd_config failed! ret = %d\n", ret);
#endif
		return 0;
	}

	if (layer->need_cap & CAP_AFBCD) {
		ret = dpu_afbcd_config(dpufd, layer, ov_compose_rect, ov_compose_flag, &param);
		return ret;
	}
