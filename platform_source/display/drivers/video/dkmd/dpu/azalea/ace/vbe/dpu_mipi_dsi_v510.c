...
static void get_dsi_dphy_ctrl(struct dpu_fb_data_type *dpufd, struct mipi_dsi_phy_ctrl *phy_ctrl)
{
	...
	DPU_FB_DEBUG("DPHY clock_lane and data_lane config :\n"
	...
	uint64_t lane_byte_clk)
{
	if (is_dual_mipi_panel(dpufd)) {
		if ((mipi_dsi_base == dpufd->mipi_dsi0_base) || dpufd->panel_info.need_set_dsi1_te_ctrl)
...
int mipi_dsi_clk_enable(struct dpu_fb_data_type *dpufd)
{
	...
	if (need_config_dsi0(dpufd)) {
		...
		if (clk_tmp) {
			ret = enable_disp_clk(clk_tmp, dpufd->dss_dphy0_ref_clk_name);
		...
		if (clk_tmp) {
			ret = enable_disp_clk(clk_tmp, dpufd->dss_dphy0_cfg_clk_name);
			dpu_check_and_return(ret, -EINVAL, ERR,
	...
	if (clk_tmp) {
		ret = enable_disp_clk(clk_tmp, dpufd->dss_pclk_pctrl_name);
		dpu_check_and_return(ret, -EINVAL, ERR,
	...
	if (need_config_dsi1(dpufd)) {
		...
		if (clk_tmp) {
			ret = enable_disp_clk(clk_tmp, dpufd->dss_dphy1_ref_clk_name);
			dpu_check_and_return(ret, -EINVAL, ERR,
		...
		if (clk_tmp) {
			ret = enable_disp_clk(clk_tmp, dpufd->dss_dphy1_cfg_clk_name);
			dpu_check_and_return(ret, -EINVAL, ERR,
...
int mipi_dsi_clk_disable(struct dpu_fb_data_type *dpufd)
{
	...
	if (need_config_dsi0(dpufd)) {
		clk_tmp = dpufd->dss_dphy0_ref_clk;
		if (clk_tmp)
			disable_disp_clk(clk_tmp, dpufd->dss_dphy0_ref_clk_name);

		clk_tmp = dpufd->dss_dphy0_cfg_clk;
		if (clk_tmp)
			disable_disp_clk(clk_tmp, dpufd->dss_dphy0_cfg_clk_name);
	}

#ifdef CONFIG_PCLK_PCTRL_USED
	clk_tmp = dpufd->dss_pclk_pctrl_clk;
	if (clk_tmp)
		disable_disp_clk(clk_tmp, dpufd->dss_pclk_pctrl_name);
#endif

	if (need_config_dsi1(dpufd)) {
		clk_tmp = dpufd->dss_dphy1_ref_clk;
		if (clk_tmp)
			disable_disp_clk(clk_tmp, dpufd->dss_dphy1_ref_clk_name);

		clk_tmp = dpufd->dss_dphy1_cfg_clk;
		if (clk_tmp)
			disable_disp_clk(clk_tmp, dpufd->dss_dphy1_cfg_clk_name);
	}
...
int mipi_dsi_ulps_cfg(struct dpu_fb_data_type *dpufd, int enable)
{
	...
	dpu_check_and_return(!dpufd, -EINVAL, ERR, "dpufd is NULL\n");
	mipi_dsi_base = get_mipi_dsi_base(dpufd);
	dpu_check_and_return(!mipi_dsi_base, -1, ERR, "mipi_dsi_base is NULL\n");
