...
static int dss_dump_reg(const char *pathname)
{
	...
	ret = snprintf(path, (size_t)DSS_LOG_PATH_MAXLEN, "%s%s", pathname, DSS_LOG_NAME);
	if (ret)
		DSS_MNTN_ERR("path name snprintf fail\n");
	...
	for (i = 0; i < g_num_peri_reg; i++) {
		...
		ret = snprintf(buff, MNTN_DUMP_BYTES, "peri register 0x%x = 0x%x\n",
			peri_reg[i], temp_register);
		if (ret)
			DSS_MNTN_ERR("peri reg snprintf fail\n");
	...
	for (i = 0; i < g_num_pctrl_reg; i++) {
		...
		ret = snprintf(buff, MNTN_DUMP_BYTES, "pctrl register 0x%x = 0x%x\n",
			pctrl_reg[i], temp_register);
		if (ret)
			DSS_MNTN_ERR("pctrl reg snprintf fail\n");
		ret = dss_mntn_savebuf2fs(path, buff, strlen(buff), true);
		memset(buff, 0, MNTN_DUMP_BYTES);
	}
	...
	for (i = 0; i < g_num_media1_reg; i++) {
		...
		ret = snprintf(buff, MNTN_DUMP_BYTES, "media1 register 0x%x = 0x%x\n",
			media1_reg[i], temp_register);
		if (ret)
			DSS_MNTN_ERR("media1 reg snprintf fail\n");
		ret = dss_mntn_savebuf2fs(path, buff, strlen(buff), true);
		memset(buff, 0, MNTN_DUMP_BYTES);
	}
