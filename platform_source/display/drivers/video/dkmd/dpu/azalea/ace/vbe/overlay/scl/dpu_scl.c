...
static int dpu_scl_coef_set(struct dpu_fb_data_type *dpufd,
	int col, int module_base)
{
	...
	ret = dpu_scl_write_coefs(dpufd, false, addr, coef_lut, lut_tap_addr);

	return ret;
}

static int dpu_v1_scl_coef_set(struct dpu_fb_data_type *dpufd,
	int col, int module_base)
{
	int ret;
	char __iomem *addr = NULL;
...
static int dpu_scl_write_coefs_sub(struct dpu_fb_data_type *dpufd,
	int col, int chn_idx, int module_base)
{
	...
	if (chn_idx == DSS_RCHN_V1) {
		ret = dpu_v1_scl_coef_set(dpufd, col, module_base);
		if (ret < 0) {
			DPU_FB_ERR("fail to set channel v1 scl coefficients\n");
			return -EINVAL;
		}
	} else {
		ret = dpu_scl_coef_set(dpufd, col, module_base);
		if (ret < 0) {
	...
	for (i = 0; i < lut_tap_addr.row; i++) {
		for (j = 0; j < lut_tap_addr.col; j += 2) {
			if ((lut_tap_addr.col % 2) && (j == lut_tap_addr.col - 1))
				groups[j / 2] = ((unsigned int)(*((int *)p + i * lut_tap_addr.col + j)) & 0xFFF) | (0 << 16);
			else
				groups[j / 2] = ((unsigned int)(*((int *)p + i * lut_tap_addr.col + j)) & 0xFFF) |
					((unsigned int)(*((int *)p + i * lut_tap_addr.col + j + 1)) << 16);
		}
...
int dpu_scl_coef_on(struct dpu_fb_data_type *dpufd, bool enable_cmdlist, int coef_lut_idx)
{
	...
	for (i = 0; i < DSS_CHN_MAX_DEFINE; i++) {
		...
		if (module_base != 0) {
			...
			ret = dpu_scl_write_coefs_sub(dpufd, TAP6, i, module_base);
			if (ret < 0)
				DPU_FB_ERR("Error to write H0_Y_COEF coefficients\n");

			if (i == DSS_RCHN_V0) {
				dpu_arsr2p_coef_on(dpufd, enable_cmdlist);
				continue;
			}

			ret = dpu_scl_write_coefs_sub(dpufd, TAP5, i, module_base);
			if (ret < 0)
				DPU_FB_ERR("Error to write Y_COEF coefficients\n");

			ret = dpu_scl_write_coefs_sub(dpufd, TAP4, i, module_base);
			if (ret < 0)
	...
	dss_rect_t *dst_rect, struct zoom_ratio *img_zoom_ratio)
{
	...
	chn_idx = (uint32_t)(layer->chn_idx);
...
int dpu_scl_config(struct dpu_fb_data_type *dpufd,
	dss_layer_t *layer, dss_rect_t *aligned_rect, bool rdma_stretch_enable)
{
	...
	void_unused(rdma_stretch_enable);
