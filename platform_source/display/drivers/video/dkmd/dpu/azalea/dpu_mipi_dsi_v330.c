...
struct mipi_dsi_horizontal_timing {
	uint32_t hline_time;
	uint32_t hsa_time;
	uint32_t hbp_time;
};

static bool is_mipi_dsi_phy_status_enable(const char __iomem *mipi_dsi_base,
	uint32_t phy_status_val, uint32_t *tmp)
{
	bool is_ready = false;
	...
	*tmp = 0;
	...
	do {
		*tmp = inp32(mipi_dsi_base + MIPIDSI_PHY_STATUS_OFFSET);
		if ((*tmp & phy_status_val) == phy_status_val) {
			is_ready = true;
			break;
		}
	...
	uint32_t *m_pll, uint32_t *n_pll)
{
	...
	*n_pll = 2;
	if (((*lane_clock) > 2400) && ((*lane_clock) <= 2500))
		*n_pll = 1;
...
static void mipi_dsi_clk_data_lane_dphy_prepare(const struct dpu_panel_info *pinfo,
	...
	uint32_t unit_tx_byte_clk_hs, uint32_t ui)
{
	 ...
	 * clocked by TXBYTECLKHS
	 */
	clk_data->clk_t_lpx = (uint32_t)(2000 * accuracy + 10 * accuracy +
		pinfo->mipi.clk_t_lpx_adjust * ui - clk_data->clk_t_hs_prepare);
	...
	struct mipi_dsi_phy_ctrl *phy_ctrl, struct dpu_fb_data_type *dpufd, uint64_t lane_clock)
{
	...
	unit_tx_byte_clk_hs = 8 * ui; /* unit of measurement */

	mipi_dsi_clk_data_lane_dphy_prepare(pinfo, &clk_data, dpufd, unit_tx_byte_clk_hs, ui);
	...
	phy_ctrl->clk_lane_lp2hs_time = phy_ctrl->clk_pre_delay + phy_ctrl->clk_t_lpx +
		phy_ctrl->clk_t_hs_prepare + phy_ctrl->clk_t_hs_zero + 5 + 7;
...
static void get_dsi_cphy_ctrl(struct dpu_fb_data_type *dpufd, struct mipi_dsi_phy_ctrl *phy_ctrl)
{
	...
	dpu_check_and_no_retval((!dpufd || !phy_ctrl), ERR, "dpufd or phy_ctrl is NULL.\n");
	pinfo = &(dpufd->panel_info);
...
static void mipi_phy_mask_ctrl(char __iomem *mipi_dsi_base, bool mask)
{
	if (mask) {
		mipi_config_phy_test_code(mipi_dsi_base, 0x00010030, 0x20);
		DPU_FB_INFO("mipi phy mask ctrl mask.\n");
	} else {
		mipi_config_phy_test_code(mipi_dsi_base, 0x00010030, 0x00);
		DPU_FB_INFO("mipi phy mask ctrl unmask.\n");
	}
}
...
static void mipi_dsi_cphy_config(char __iomem *mipi_dsi_base, struct dpu_panel_info *pinfo)
{
	...
	mipi_config_phy_test_code(mipi_dsi_base, 0x00010048,
		pinfo->dsi_phy_ctrl.rg_0p8v + (pinfo->dsi_phy_ctrl.rg_2p5g << 1) + (pinfo->dsi_phy_ctrl.rg_320m << 2) +
	...
	if (pinfo->mipi.dsi_bit_clk > 1500) {
		...
		mipi_config_phy_test_code(mipi_dsi_base, 0x00010059, 0x16);
		mipi_config_phy_test_code(mipi_dsi_base, 0x0001005a, 0x26);
	} else {
		...
		mipi_config_phy_test_code(mipi_dsi_base, 0x00010059, 0x10);
		mipi_config_phy_test_code(mipi_dsi_base, 0x0001005a, 0x26);
	}
...
static void mipi_dsi_dphy_config(char __iomem *mipi_dsi_base, struct dpu_panel_info *pinfo)
{
	...
	if (pinfo->mipi.dsi_bit_clk == 1250) {
		mipi_config_phy_test_code(mipi_dsi_base, 0x00010053, 0xc3);
		mipi_config_phy_test_code(mipi_dsi_base, 0x0001005A, 0xf0);
	} else {
	...
	if (pinfo->mipi.rg_vrefsel_vcm_adjust != 0) {
		mipi_config_phy_test_code(mipi_dsi_base, 0x00010059, pinfo->mipi.rg_vrefsel_vcm_adjust);
		if (pinfo->mipi.support_de_emphasis) {
			for (i = 0; i < DE_EMPHASIS_REG_TIME; i++)
	...
	struct dpu_panel_info *pinfo)
{
	...
	outp32(mipi_dsi_base + MIPIDSI_PHY_TST_CTRL0_OFFSET, 0x00000000);
	if (dpufd->panel_info.ramless_aod)
		mipi_phy_mask_ctrl(mipi_dsi_base, true);
	...
	if (!is_mipi_dsi_phy_status_enable(mipi_dsi_base, 0x00000001, &tmp))
	...
	if (!is_mipi_dsi_phy_status_enable(mipi_dsi_base, cmp_stopstate_val, &tmp))
...
static void mipi_init_dsi_horizontal_timing(struct dpu_fb_data_type *dpufd, dss_rect_t rect,
	char __iomem *mipi_dsi_base, struct mipi_dsi_horizontal_timing *horizontal_timing)
{
	struct dpu_panel_info *pinfo = NULL;
	uint64_t pixel_clk;

	pixel_clk = mipi_pixel_clk(dpufd);
	dpu_check_and_no_retval((pixel_clk == 0), INFO, "pixel_clk is zero!\n");

	pinfo = &(dpufd->panel_info);

	if (pinfo->mipi.phy_mode == DPHY_MODE) {
		if (pinfo->mipi_no_round == MIPI_NO_ROUND) {
			horizontal_timing->hsa_time = pinfo->ldi.h_pulse_width *
				pinfo->dsi_phy_ctrl.lane_byte_clk / pixel_clk;
			horizontal_timing->hbp_time = pinfo->ldi.h_back_porch *
				pinfo->dsi_phy_ctrl.lane_byte_clk / pixel_clk;
			horizontal_timing->hline_time = (pinfo->ldi.h_pulse_width +
				pinfo->ldi.h_back_porch + rect.w + pinfo->ldi.h_front_porch) *
				pinfo->dsi_phy_ctrl.lane_byte_clk / pixel_clk;
		} else {
			horizontal_timing->hsa_time = round1(pinfo->ldi.h_pulse_width *
				pinfo->dsi_phy_ctrl.lane_byte_clk, pixel_clk);
			horizontal_timing->hbp_time = round1(pinfo->ldi.h_back_porch *
				pinfo->dsi_phy_ctrl.lane_byte_clk, pixel_clk);
			horizontal_timing->hline_time =
				round1((pinfo->ldi.h_pulse_width + pinfo->ldi.h_back_porch +
				rect.w + pinfo->ldi.h_front_porch) * pinfo->dsi_phy_ctrl.lane_byte_clk, pixel_clk);
		}
	} else {
		horizontal_timing->hsa_time =
			round1(pinfo->ldi.h_pulse_width * pinfo->dsi_phy_ctrl.lane_word_clk, pixel_clk);
		horizontal_timing->hbp_time =
			round1(pinfo->ldi.h_back_porch * pinfo->dsi_phy_ctrl.lane_word_clk, pixel_clk);
		horizontal_timing->hline_time =
			round1((pinfo->ldi.h_pulse_width + pinfo->ldi.h_back_porch +
			rect.w + pinfo->ldi.h_front_porch) * pinfo->dsi_phy_ctrl.lane_word_clk, pixel_clk);
	}

	set_reg(mipi_dsi_base + MIPIDSI_VID_HSA_TIME_OFFSET, horizontal_timing->hsa_time, 12, 0);
	set_reg(mipi_dsi_base + MIPIDSI_VID_HBP_TIME_OFFSET, horizontal_timing->hbp_time, 12, 0);
	set_reg(mipi_dsi_base + MIPIDSI_VID_HLINE_TIME_OFFSET, horizontal_timing->hline_time, 15, 0);
}
...
{
	struct mipi_dsi_horizontal_timing horizontal_timing = {0};
	struct dpu_panel_info *pinfo = NULL;
	dss_rect_t rect;
	...
	if (is_mipi_cmd_panel(dpufd)) {
		...
		if (pinfo->mipi.hs_wr_to_time == 0)
			...
			set_reg(mipi_dsi_base + MIPIDSI_HS_WR_TO_CNT_OFFSET, (0x1 << 24) |
				(pinfo->mipi.hs_wr_to_time * pinfo->dsi_phy_ctrl.lane_byte_clk / 1000000000UL), 25, 0);
	}
	 ...
	 */
	mipi_init_dsi_horizontal_timing(dpufd, rect, mipi_dsi_base, &horizontal_timing);
	DPU_FB_INFO("hsa_time = %d, hbp_time = %d, hline_time = %d.\n",
		horizontal_timing.hsa_time, horizontal_timing.hbp_time, horizontal_timing.hline_time);
	...
	if (is_mipi_cmd_panel(dpufd))
		mipi_dsi_auto_ulps_config(pinfo, mipi_dsi_base, horizontal_timing.hline_time);
...
void ramless_aod_mipi_on(struct dpu_fb_data_type *dpufd)
{
	if (dpufd->panel_info.ramless_aod) {
...
void ramless_aod_ulps_cfg(struct dpu_fb_data_type *dpufd)
{
	if (dpufd->panel_info.ramless_aod) {
		mipi_dsi_ulps_cfg(dpufd, false);  /* enter ulps */
		mdelay(1);
		mipi_phy_mask_ctrl(dpufd->mipi_dsi0_base, false);  /* unmask */
		mipi_dsi_ulps_cfg(dpufd, true);  /* exit ulps */
	}
}
...
static void pane_enter_ulps_bypass_powerdown(struct platform_device *pdev, struct dpu_fb_data_type *dpufd)
{
	int delay_count = 0;

	if (panel_next_bypass_powerdown_ulps_support(pdev) || dpufd->panel_info.ramless_aod) {
		disable_ldi(dpufd);
		while ((((uint32_t)inp32(dpufd->dss_base + DSS_LDI0_OFFSET + LDI_VSTATE)) & 0x7FF) != 0x1) {
			if (++delay_count > 16) {  /* wait 16ms */
...
int mipi_dsi_off(struct platform_device *pdev)
{
	...
	if (dpufd->index == PRIMARY_PANEL_IDX) {
		...
		pane_enter_ulps_bypass_powerdown(pdev, dpufd);
	} else if (dpufd->index == EXTERNAL_PANEL_IDX) {
		mipi_dsi_off_sub(dpufd, dpufd->mipi_dsi1_base);
	} else {
		DPU_FB_ERR("fb%d, not supported!\n", dpufd->index);
...
static int mipi_dsi_check_ulps_stopstate(const struct dpu_fb_data_type *dpufd,
	const char __iomem *mipi_dsi_base, uint32_t status_val, uint32_t *need_pll_retry, bool enter_ulps)
{
	uint32_t tmp;
	uint32_t try_times;
	...
	if (enter_ulps) { /* ulps enter */
		...
		try_times = 0;
		tmp = inp32(mipi_dsi_base + MIPIDSI_PHY_STATUS_OFFSET);
		while ((tmp & status_val) != status_val) {
			udelay(10);
			if (++try_times > 100) {  /* try 1ms */
...
static void mipi_dsi_get_ulps_stopstate(const struct dpu_fb_data_type *dpufd,
	...
	char __iomem *mipi_dsi_base)
{
	...
	dpu_check_and_return((!dpufd || !mipi_dsi_base), -EINVAL, ERR, "dpufd or mipi_dsi_base is NULL\n");
	DPU_FB_DEBUG("fb%d, +!\n", dpufd->index);
...
#endif
	if (dpufd->panel_info.ramless_aod == 0) {
...
static void mipi_dsi_bit_clk_upt_isr_handler_prepare(struct dpu_fb_data_type *dpufd,
	uint32_t *vfp_time, uint64_t *lane_byte_clk)
{
	...
	uint8_t esd_enable, timeval_compatible *tv0)
{
	...
	ldi_vstate = inp32(dpufd->dss_base + DSS_LDI0_OFFSET + LDI_VSTATE) & 0x7FF;
	...
	if ((!is_ready) || (ldi_vstate != 0x1)) {
		...
		DPU_FB_DEBUG("PERI_STAT0 is not ready or ldi_vstate=0x%x not in idle\n", ldi_vstate);
		return -1;
	}
...
static void mipi_dsi_set_cdphy_bit_clk_upt(struct dpu_fb_data_type *dpufd,
	struct dpu_panel_info *pinfo, struct mipi_dsi_phy_ctrl *phy_ctrl, uint8_t esd_enable)
{
	DPU_FB_DEBUG("start, phy status: 0x%x, vstate: 0x%x\n",
		inp32(dpufd->mipi_dsi0_base + MIPIDSI_PHY_STATUS_OFFSET),
		inp32(dpufd->mipi_dsi0_base + LDI_VSTATE));
	...
	if (is_mipi_cmd_panel(dpufd)) {
		...
		DPU_FB_DEBUG("after, phy status: 0x%x, vstate: 0x%x\n",
			inp32(dpufd->mipi_dsi0_base + MIPIDSI_PHY_STATUS_OFFSET),
			inp32(dpufd->mipi_dsi0_base + LDI_VSTATE));
...
int mipi_dsi_bit_clk_upt_isr_handler(struct dpu_fb_data_type *dpufd)
{
	...
	if (spin_trylock(&dpufd->mipi_resource_lock) == 0) {
		DPU_FB_DEBUG("dsi_bit_clk %u will update in next frame\n", dsi_bit_clk_upt);
		return 0;
	}
	...
	mipi_dsi_bit_clk_upt_isr_handler_prepare(dpufd, &vfp_time, &lane_byte_clk);
	...
	mipi_dsi_set_cdphy_bit_clk_upt(dpufd, pinfo, &phy_ctrl, esd_enable);
