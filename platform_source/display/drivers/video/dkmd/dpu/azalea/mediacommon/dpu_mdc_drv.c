...
#define DTS_COMP_DPU_MDC_PANEL	"hisilicon,mediacommon_panel"

#ifdef CONFIG_DPU_FB_V410
static int dpu_mediacommon_panel_on(struct platform_device *pdev)
{
...
static int dpu_mediacommon_panel_off(struct platform_device *pdev)
{
	struct dpu_fb_data_type *dpufd = NULL;
...
static int dpu_mediacommon_panel_on(struct platform_device *pdev)
{
	...
	dpu_mdc_interrupt_config(dpufd);
	if (!dpufd->dss_module_resource_initialized) {
		dpu_mdc_module_default(dpufd);
		dpufd->dss_module_resource_initialized = true;
	}
	dpu_mdc_mif_on(dpufd);
	dpu_mdc_smmu_on(dpufd);
	dpu_mdc_scl_coef_on(dpufd, SCL_COEF_YUV_IDX);

	dpu_mdc_mctl_on(dpufd);
...
static int dpu_mediacommon_panel_remove(struct platform_device *pdev)
{
	struct dpu_fb_data_type *dpufd = NULL;
...
static struct dpu_panel_info dpu_mdc_panel_info = {0};
static struct dpu_fb_panel_data dpu_mdc_panel_data = {
	.panel_info = &dpu_mdc_panel_info,
	.on = dpu_mediacommon_panel_on,
	.off = dpu_mediacommon_panel_off,
	.remove = dpu_mediacommon_panel_remove,
};

static int dpu_mdc_probe(struct platform_device *pdev)
{
	int ret;
	...
	if (dpu_fb_device_probe_defer(PANEL_MEDIACOMMON, BL_SET_BY_NONE))
	...
	pdev->id = 1; /*lint !e613*/
	pinfo = dpu_mdc_panel_data.panel_info;
	...
	ret = platform_device_add_data(pdev, &dpu_mdc_panel_data,
	...
	this_dev = dpu_fb_add_device(pdev);
	if (this_dev != NULL)
...
static const struct of_device_id dpu_device_match_table[] = {
	{
		.compatible = DTS_COMP_DPU_MDC_PANEL,
		.data = NULL,
	},
	{},
};

MODULE_DEVICE_TABLE(of, dpu_device_match_table);
...
static struct platform_driver this_driver = {
	...
	.shutdown = NULL,
	.driver = {
		.name = "dpu_mediacommon_panel",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(dpu_device_match_table),
	},
};

static int __init dpu_mdc_panel_init(void)
{
	int ret;
...
module_init(dpu_mdc_panel_init);
