 ...
 * DSS ARSR
 */
static int dpu_arsr1p_write_coefs(struct dpu_fb_data_type *dpufd, bool enable_cmdlist,
	char __iomem *addr, const int **p, const struct arsr_scl_para *scl_tap)
{
	...
	for (i = 0; i < scl_tap->row; i++) {
		if (scl_tap->col == ARSR2P_TAP4) {
			coef_value_half = (*((int *)p + i * scl_tap->col) & 0x1FF) |
				(*((int *)p + i * scl_tap->col + 1) << 9) |
				(*((int *)p + i * scl_tap->col + 2) << 18);
			coef_value_end = (*((int *)p + i * scl_tap->col + 3) & 0x1FF);
		} else {
			coef_value_half = (*((int *)p + i * scl_tap->col)) |
				((*((int *)p + i * scl_tap->col + 1) & 0x1FF) << 9) |
				(*((int *)p + i * scl_tap->col + 2) << 18);
			coef_value_end = (*((int *)p + i * scl_tap->col + 3)) |
				((*((int *)p + i * scl_tap->col + 4) & 0x1FF) << 9) |
				(*((int *)p + i * scl_tap->col + 5) << 18);
		}
...
static void arsr_pre_para_init(struct arsr2p_info *arsr_para)
{
	...
	arsr_para->skin_thres_y = (300 | (332 << 10) | (600 << 20));
	arsr_para->skin_thres_u = (20 | (40 << 10) | (452 << 20));
	arsr_para->skin_thres_v = (24 | (48 << 10) | (580 << 20));
	arsr_para->skin_cfg0 = (512 | (12 << 13));  /* (yslop | (clipdata << 13)) */
	arsr_para->skin_cfg1 = (819);  /* (uslop) */
	arsr_para->skin_cfg2 = (682);  /* (vslop) */
	arsr_para->shoot_cfg1 = (372 | (20 << 16));  /* (shootslop1 | (shootgradalpha << 16)) */
	/* (shootgradsubthrl | (shootgradsubthrh << 16)), default shootgradsubthrh is 0 */
	arsr_para->shoot_cfg2 = ((8 << 16) | (-80 & 0x7ff));
	arsr_para->shoot_cfg3 = 15;  /* shootshpratio, needconfirm */
	arsr_para->sharp_cfg3 = (128 | (192 << 16));  /* (shptocuthigh0 ) | (shptocuthigh1 << 16) */
	arsr_para->sharp_cfg4 = (40 | (160 << 16));   /* (sharpshootctrll | (sharpshootctrlh << 16) */
	arsr_para->sharp_cfg6 = (8 | (8 << 16));   /* (blendshshootctrl  | (sharpcoring << 16) */
	/* | (6 << 16) | (12 << 24));
	 * (skinthresh | (skinthresl << 8) | (shctrllowstart << 16) | (shctrlhighend << 24))
	 */
	arsr_para->sharp_cfg7 = (10 | (6 << 8));
	arsr_para->sharp_cfg8 = (16 | (160 << 8));  /* (linearshratiol | (linearshratioh << 8)) */
	arsr_para->texturw_analysts = (64 | (80 << 16));  /* (difflow | (diffhigh << 16)) */
	arsr_para->intplshootctrl = (8);  /* (intplshootctrl) */
	arsr_para->sharp_level = 0x20002;
...
static void arsr2p_basic_set_reg(struct dpu_fb_data_type *dpufd,
	char __iomem *arsr2p_base, const struct dss_arsr2p *s_arsr2p)
{
...
static int dpu_arsr2p_config_check_width(const dss_rect_t *dest_rect, int source_width,
	struct dss_arsr2p_scl_flag scl_flag, int source_height, uint32_t index)
{
	if (!dest_rect) {
	...
	if ((source_width < ARSR2P_MIN_INPUT) || (dest_rect->w < ARSR2P_MIN_INPUT) ||
		...
		if ((!scl_flag.en_hscl) && (!scl_flag.en_vscl)) {
			DPU_FB_DEBUG("fb%d, src_rect.w[%d] or dst_rect.w[%d] is smaller than 16 or "
				"larger than 2560, arsr2p bypass!\n", index, source_width, dest_rect->w);
			return 0;
		}
		DPU_FB_ERR("fb%d, src_rect[%d, %d] or dst_rect[%d, %d] is smaller than 16 or "
			"larger than 2560!\n",
	...
	const dss_rect_t *src_rect, const dss_rect_t *dst_rect)
{
	...
	if (src_rect->h > dst_rect->h) {
		...
		arsr2p_pos_size->iv_inc = (DSS_HEIGHT(src_rect->h) * ARSR2P_INC_FACTOR + ARSR2P_INC_FACTOR / 2 -
			arsr2p_pos_size->iv_top) / DSS_HEIGHT(dst_rect->h);
	} else {
		/* ivinc=(arsr_input_height*65536+65536-ivtop)/(arsr_output_height+1) */
		arsr2p_pos_size->iv_inc = (DSS_HEIGHT(src_rect->h) * ARSR2P_INC_FACTOR + ARSR2P_INC_FACTOR -
			arsr2p_pos_size->iv_top) / dst_rect->h;
	}
...
static void dpu_arsr2p_check_vhscl(struct dss_arsr_mode *arsr_mode,
	struct dss_arsr2p_scl_flag scl_flag)
{
	if ((!scl_flag.en_hscl) && (!scl_flag.en_vscl)) {
...
static void dpu_arsr2p_set_param(struct dss_arsr2p *arsr2p,
	const struct dss_arsr2p_pos_size *arsr2p_pos_size, const struct dss_arsr_mode *arsr_mode,
	const dss_rect_t *src_rect, const dss_rect_t *dst_rect)
{
	arsr2p->arsr_input_width_height = set_bits32(arsr2p->arsr_input_width_height, DSS_HEIGHT(src_rect->h), 13, 0);
	arsr2p->arsr_input_width_height = set_bits32(arsr2p->arsr_input_width_height,
		DSS_WIDTH(arsr2p_pos_size->src_width), 13, 16);
	arsr2p->arsr_output_width_height = set_bits32(arsr2p->arsr_output_width_height,
	...
	dss_rect_t *aligned_rect, bool rdma_stretch_enable)
{
	...
	dpu_arsr2p_check_vhscl(&arsr_mode, scl_flag);
...
{
	int ret;
	struct arsr_scl_para scl_tap4 = { NULL, NULL, ARSR2P_PHASE_NUM, ARSR2P_TAP4 };
	struct arsr_scl_para scl_tap6 = { NULL, NULL, ARSR2P_PHASE_NUM, ARSR2P_TAP6 };
	...
	ret = dpu_arsr1p_write_coefs(dpufd, enable_cmdlist, scl_lut_base + ARSR_POST_COEFF_H_Y0,
		(const int **)g_coef_y_scl_up_tap6, &scl_tap6);
	if (ret < 0)
	...
	ret = dpu_arsr1p_write_coefs(dpufd, enable_cmdlist, scl_lut_base + ARSR_POST_COEFF_H_UV0,
		(const int **)g_coef_auv_scl_up_tap4, &scl_tap4);
	if (ret < 0)
