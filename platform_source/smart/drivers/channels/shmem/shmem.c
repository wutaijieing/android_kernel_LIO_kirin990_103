...
STATIC int shmem_ipc_send(unsigned char cmd, enum obj_tag module_id, unsigned int size, bool is_lock)
{
	...
	winfo.wr_len = (int)sizeof(struct shmem_ipc_data);
	if (is_lock) {
...
int contexthub_shmem_init(void)
{
	...
	g_shmem_lock = wakeup_source_register(NULL, "ch_shmem_lock");
#else
	g_shmem_lock = wakeup_source_register("ch_shmem_lock");
#endif
	if (g_shmem_lock == NULL) {
		pr_err("%s register ch_shmem_lock wakeup source fail\n", __func__);
		return -1;
	}
	return ret;
}
