		...
		struct kbase_fault *fault)
{
	...
	if (kbase_hw_has_issue(kbdev, BASE_HW_ISSUE_8245)) {
		...
		if (dsm_report_enable(kbdev->hisi_dev_data.error_num.page_fault,
			30, kbdev->hisi_dev_data.factory_mode)) {
			gpu_dsm_report(kbdev, GPU_PAGE_FAULT_DSM_NO,
				 "Unhandled page fault. For this GPU version \
				 we now soft-reset the GPU as part of \
				 page fault recovery.");
		}
#endif
		kbdev->hisi_dev_data.error_num.page_fault++;
		kbdev->hisi_dev_data.error_num.ts = dfx_getcurtime();
#ifdef CONFIG_DFX_BB
#if defined(CONFIG_LP_ENABLE_HPM_DATA_COLLECT) || defined(CONFIG_MALI_GPU_BUG_ON)
		/* benchmark data collect or config bug on */
		rdr_syserr_process_for_ap((u32)MODID_AP_S_PANIC_GPU, 0ull, 0ull);
#endif
#endif
		reset_status = kbase_prepare_to_reset_gpu(kbdev);
