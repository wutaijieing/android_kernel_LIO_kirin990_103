...
static int kbase_set_hi_features_mask(struct kbase_device *kbdev)
{
	...
	u32 gpu_vid = kbdev->hisi_dev_data.gpu_vid;
	...
	case GPU_ID2_MAKE(7, 0, 9, 0, 1, 1, 0):
		...
		break;
		// only for trym FPGA,because one core trym GPU ID is 90810011 different from 8 core trym
	...
	case GPU_ID2_MAKE(9, 2, 0, 2, 0, 0, 3):
		hi_features = kbase_hi_feature_tbex_r0p0;
		break;
	case GPU_ID2_MAKE(9, 0, 9, 1, 0, 0, 0):
		hi_features = kbase_hi_feature_tnax_r0p0;
		break;
	default:
		dev_err(kbdev->dev,
...
{
	if (kbdev->hisi_dev_data.regulator) {
		int refcount;

		if (unlikely(regulator_enable(kbdev->hisi_dev_data.regulator))) {
			dev_err(kbdev->dev, "Failed to enable regulator\n");
			BUG_ON(1);
		}

		refcount = atomic_inc_return(&kbdev->hisi_dev_data.regulator_refcount);
		...
		if (kbase_has_hi_feature(kbdev, KBASE_FEATURE_CORE_READY_PERIOD) ||
			(kbdev->hisi_dev_data.gpu_chip_type == 2)) {
...
static inline void kbase_platform_off(struct kbase_device *kbdev)
{
	...
	if (kbdev->hisi_dev_data.regulator && refcount) {
		if (unlikely(regulator_disable(kbdev->hisi_dev_data.regulator)))
