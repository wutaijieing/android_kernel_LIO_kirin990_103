...
/*
 * runtime_pm_delay_show - Show callback for the runtime_pm_delay sysfs file.
 *
 ...
 * Return: The number of bytes output to @buf.
 */
static ssize_t runtime_pm_delay_show(struct device *dev,
...
/**
 * runtime_pm_delay_store - Store callback for the runtime_pm_delay sysfs
 * file.
 *
 * @dev: The device this sysfs file is for.
 * @attr: The attributes of the sysfs file.
 ...
 * Return: count if the function succeeded. An error code on failure.
 */
static ssize_t runtime_pm_delay_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (runtime_pm_delay_ms != RUNTIME_PM_DELAY_SHORT &&
		...
		return -EINVAL;
	atomic_set(&kbdev->gpu_dev_data.runtime_pm_delay_ms,
		runtime_pm_delay_ms);
 ...
 */
static DEVICE_ATTR_RW(runtime_pm_delay);
		...
		const char *buf, size_t count)
{
	...
	kbdev->gpu_dev_data.error_num.soft_reset = 0;
