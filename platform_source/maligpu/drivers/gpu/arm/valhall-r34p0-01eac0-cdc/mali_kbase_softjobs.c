...
void print_atom_info(struct kbase_jd_atom *katom)
{
	...
	pr_err("mali gpu: jc %llx, ec 0x%x, (will_fail 0x%x), status 0x%x, rb_state = 0x%x\n",
		(unsigned long long)katom->jc, katom->event_code, katom->will_fail_event_code,
		katom->status, katom->gpu_rb_state);
	pr_err("mali gpu: protect_state 0x%x, slot %d, prio %d, tick %u, blocked %d\n",
		katom->protected_state.enter, katom->slot_nr, katom->sched_priority,
		katom->ticks, atomic_read(&katom->blocked));
	if (katom->pre_dep)
...
int kbase_process_soft_job(struct kbase_jd_atom *katom)
{
	...
	case BASE_JD_REQ_SOFT_FENCE_WAIT:
	{
		...
		if (ret == 1) {
			...
			hrtimer_start(&kctx->gpu_ctx_data.fence_wait_timer,
				HR_TIMER_DELAY_MSEC(FENCE_TIMEOUT_TIME),
				HRTIMER_MODE_REL);
			kctx->gpu_ctx_data.timer_started = 1;
#ifdef CONFIG_MALI_FENCE_DEBUG
			kbasep_add_waiting_with_timeout(katom);
#else
			kbasep_add_waiting_soft_job(katom);
...
void kbase_finish_soft_job(struct kbase_jd_atom *katom)
{
	...
	case BASE_JD_REQ_SOFT_FENCE_WAIT:
		...
		/* BASE_DEBUG_FENCE_TIMEOUT */
		if (katom->kctx && katom->kctx->gpu_ctx_data.timer_started == 1) {
			hrtimer_cancel(&katom->kctx->gpu_ctx_data.fence_wait_timer);
			katom->kctx->gpu_ctx_data.timer_started = 0;
		}
		break;
#endif /* CONFIG_SYNC || CONFIG_SYNC_FILE */
	case BASE_JD_REQ_SOFT_DEBUG_COPY:
