...
int kbasep_js_devdata_init(struct kbase_device * const kbdev)
{
	...
	memset(jsdd->runpool_irq.ctx_attr_ref_count, 0,
			sizeof(jsdd->runpool_irq.ctx_attr_ref_count));/* unsafe_function_ignore: memset */
	memset(jsdd->runpool_irq.slot_affinities, 0,
			sizeof(jsdd->runpool_irq.slot_affinities));/* unsafe_function_ignore: memset */
	memset(jsdd->runpool_irq.slot_affinity_refcount, 0,
			sizeof(jsdd->runpool_irq.slot_affinity_refcount));/* unsafe_function_ignore: memset */
...
int kbasep_js_kctx_init(struct kbase_context * const kctx)
{
	...
	memset(js_kctx_info->ctx.ctx_attr_ref_count, 0,
			sizeof(js_kctx_info->ctx.ctx_attr_ref_count));/* unsafe_function_ignore: memset */
