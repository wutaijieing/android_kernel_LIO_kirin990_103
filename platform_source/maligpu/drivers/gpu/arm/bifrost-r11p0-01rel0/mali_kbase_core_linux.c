...
static int kbase_common_reg_map(struct kbase_device *kbdev)
{
	...
	if (!kbdev->sctrlreg) {
		dev_err(kbdev->dev, "Can't remap sctrl register window on platform phoenix\n");
		err = -EINVAL;
...
#ifdef CONFIG_MALI_TRYM
	/*if logic is B010, do not load GPU kernel.because fpga-gpu-exist is not exist in phoenix B010*/
	if(readl(kbdev->sctrlreg+ SCTRL_OFFSET) == MASK_PHOENIX_FPGA_VERSION_B010 )
	{
