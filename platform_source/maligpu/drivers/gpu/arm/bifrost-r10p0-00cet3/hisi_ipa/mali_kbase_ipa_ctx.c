...
#define TOTAL_POWER_MW					1000
#define TOTAL_TIME						4000
		...
		char *buf)
{
	...
	for (i = 0; i < ARRAY_SIZE(ctx->groups); i++) {
		if (!strncmp(ctx->groups[i].name, attr->attr.name, strlen(ctx->groups[i].name))) {
...
static int update_ipa_groups_from_dt(struct kbase_ipa_context *ctx)
{
	...
	for_each_available_child_of_node(np, child) {
		...
		for (i = 0; i < ARRAY_SIZE(ctx->groups); i++) {
			group = &ctx->groups[i];
			if (!strncmp(group->name, name, strlen(group->name))) {
...
struct kbase_ipa_context *kbase_dynipa_init(struct kbase_device *kbdev)
{
	...
	if (bound_detection_enable_mode) {
		if (!strncmp("disable", bound_detection_enable_mode, sizeof("disable") - 1))
...
void kbase_dynipa_term(struct kbase_device *kbdev)
{
	struct kbase_ipa_context *ctx = kbdev->ipa_ctx;
	cancel_work_sync(&kbdev->bound_detect_work);
	if (ctx)
	{
		detach_vinstr(ctx);
...
void kbase_ipa_hwcounts(struct kbase_ipa_context *ctx, int *err)
{
	...
	*/
	...
	time_drua = (ktime_to_ns(timestamp_diff) >> KBASE_PM_TIME_SHIFT);
	if (time_drua <= U32_MAX)
	{
		ctx->time_diff[statistics_idx % NR_IPA_UNIT] = (u32)time_drua;
...
u32 kbase_ipa_dynamic_bound_measure(struct kbase_ipa_context *ctx, int *err)
{
	...
	total_power = total_power * TOTAL_POWER_MW;
...
{
	u32 bound_measure = 0;
	unsigned long mhz = cur_freq / FREQ_MHZ;
