			       ...
			       int size)
{
	...
	snprintf(str, size, "%d", atomic_read(&mtl->signaled)); /* unsafe_function_ignore: snprintf */
}
...
static void pt_value_str(struct sync_pt *pt, char *str, int size)
{
	...
	snprintf(str, size, "%d(%d)", mpt->order, mpt->result); /* unsafe_function_ignore: snprintf */
}
...
void kbase_sync_fence_in_cancel_wait(struct kbase_jd_atom *katom)
{
	if (sync_fence_cancel_async(katom->fence, &katom->sync_waiter) != 0) {
		...
		dev_err(katom->kctx->kbdev->dev, "kctx %pK atom %pK wait couldn't be cancelled\n", (void *)katom->kctx, katom);
		return;
	}

	dev_err(katom->kctx->kbdev->dev, "kctx %pK katom %pK wait was cancelled\n", (void *)katom->kctx, katom);
