		...
		const char *str)
{
	...
	strcpy(str_copy, str); /* unsafe_function_ignore: strcpy */
		...
		const char *val_name, u64 val)
{
	...
	ret = snprintf(msgbuf, NAMED_U64_VAL_BUF_SZ, "%s=0x%llx", val_name, val); /* unsafe_function_ignore: snprintf */
		...
		const char *val_str)
{
	...
	/* name */
	memcpy(copy_ptr, val_name, val_name_len); /* unsafe_function_ignore: memcpy */
	copy_ptr += val_name_len;

	/* str start delimiter */
	memcpy(copy_ptr, NAMED_STR_START_DELIM, start_delim_len); /* unsafe_function_ignore: memcpy */
	copy_ptr += start_delim_len;

	/* str value */
	memcpy(copy_ptr, val_str, val_str_len); /* unsafe_function_ignore: memcpy */
	copy_ptr += val_str_len;

	/* str end delimiter */
	memcpy(copy_ptr, NAMED_STR_END_DELIM, end_delim_len); /* unsafe_function_ignore: memcpy */
	...
	case KUTF_HELPER_VALTYPE_STR:
		{
			...
			strcpy(str, DUMMY_MSG); /* unsafe_function_ignore: strcpy */
