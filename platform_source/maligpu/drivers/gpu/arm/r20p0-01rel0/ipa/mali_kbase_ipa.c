...
static struct device_node *get_model_dt_node(struct kbase_ipa_model *model)
{
	...
	snprintf(compat_string, sizeof(compat_string), "arm,%s",/* unsafe_function_ignore: snprintf */
		 model->ops->name);
				  ...
				  size_t num_elems, bool dt_required)
{
	...
	if (err && dt_required) {
		memset(addr, 0, sizeof(s32) * num_elems);/* unsafe_function_ignore: memset */
		dev_warn(model->kbdev->dev,
	...
	for (i = 0; i < num_elems; ++i) {
		...
		if (num_elems == 1)
			snprintf(elem_name, sizeof(elem_name), "%s", name);/* unsafe_function_ignore: snprintf */
		else
			snprintf(elem_name, sizeof(elem_name), "%s.%d",/* unsafe_function_ignore: snprintf */
				name, i);
				     ...
				     size_t size, bool dt_required)
{
	...
	err = of_property_read_string(model_dt_node, name,
				      &string_prop_value);
	if (err && dt_required) {
		strncpy(addr, "", size - 1);/* unsafe_function_ignore: strncpy */
		dev_warn(model->kbdev->dev,
		...
		strncpy(addr, string_prop_value, size - 1);/* unsafe_function_ignore: strncpy */
		origin = "DT";
	}
