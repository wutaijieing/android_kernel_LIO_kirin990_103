					      ...
					      struct base_jd_atom_v2 *f_atom)
{
	...
	if (KBASE_API_VERSION(10, 3) > replay_atom->kctx->api_version) {
		...
		memcpy(&tiler_core_req, &payload_uk10_2->tiler_core_req,/* unsafe_function_ignore: memcpy */
				sizeof(tiler_core_req));
		memcpy(&fragment_core_req, &payload_uk10_2->fragment_core_req,/* unsafe_function_ignore: memcpy */
