...
static size_t collect_line(struct kutf_helper_textbuf *textbuf, int newline_off)
{
	 ...
	 * it with '\0' */
	strncpy(str_start, scratch, str_buf_size - 1);/* unsafe_function_ignore: strncpy */
	...
	memmove(scratch, scratch + nextline_off, bytes_remain);/* unsafe_function_ignore: memmove */
		...
		char *enqueue_str, int buf_max_size)
{
	...
	/* Copy in string */
	strncpy(str_start, enqueue_str, str_size);/* unsafe_function_ignore: strncpy */
