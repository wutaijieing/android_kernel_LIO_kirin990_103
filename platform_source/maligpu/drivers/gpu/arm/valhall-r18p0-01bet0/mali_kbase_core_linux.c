...
int hisi_verify_fpga_exist(struct platform_device *pdev, struct kbase_device *kbdev)
{
	int ret = 0;
	bool logic_ver = false;
#ifdef CONFIG_MALI_BORR
	unsigned int sctrl_value = 0;
	sctrl_value = readl(kbdev->sctrlreg + SCTRL_OFFSET);
	dev_err(&pdev->dev, "mali gpu sctrl_value =0x%x\n",sctrl_value);
	...
	logic_ver = (sctrl_value == MASK_BALTIMORE_FPGA_B020) || (sctrl_value == MASK_BALTIMORE_FPGA_B021);
	if (logic_ver) {
		dev_err(&pdev->dev, "logic is B020 or B021, load GPU kernel\n");
		return ret;
	}
#endif /* CONFIG_MALI_BORR */
...
static int kbase_platform_device_probe(struct platform_device *pdev)
{
	...
	err = hisi_verify_fpga_exist(pdev, kbdev);
	if (err)
	{
		dev_err(&pdev->dev, "do not load GPU kernel\n");
		kbase_platform_device_remove(pdev);
		return err;
	}
