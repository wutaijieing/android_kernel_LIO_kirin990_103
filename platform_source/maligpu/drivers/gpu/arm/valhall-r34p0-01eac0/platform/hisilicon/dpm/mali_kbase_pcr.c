...
void gpu_pcr_enable(struct kbase_device *kbdev, bool flag_for_bypass)
{
	...
	gpu_set_bits(PCR_CRG_PERI_CLKEN_ADDR_MASK,
		PCR_CRG_PERI_CLKDIS_ADDR(kbdev->gpu_dev_data.crgreg));

	/* set clk_gpu_pcr frequency dividing ratio (only for mali_trym) */
	if (PCR_CRG_PERCLKDIV_ADDR(kbdev->gpu_dev_data.crgreg) != NULL)
		gpu_set_bits(PCR_CRG_PERCLKDIV_ADDR_MASK,
			PCR_CRG_PERCLKDIV_ADDR(kbdev->gpu_dev_data.crgreg));

	/* [core] set ip_arst_gpupcr */
	gpu_set_bits(PCR_CRG_CORE_RESET_ADDR_MASK,
		PCR_CRG_CORE_DIS_RESET_ADDR(kbdev->gpu_dev_data.crgreg));

	/* open gt_clk_gpu_pcr */
	gpu_set_bits(PCR_CRG_PERI_CLKEN_ADDR_MASK,
		PCR_CRG_PERI_CLKEN_ADDR(kbdev->gpu_dev_data.crgreg));

	/* [core] tpram exit shutdown mode:mask-0x0850 */
	writel(PCR_PCTRL_TPRAM_MEM_ADDR_EXIT_SHUTDOWN_MASK,
		PCR_PCTRL_CORE_TPRAM_MEM_ADDR(kbdev->gpu_dev_data.pctrlreg));
...
void gpu_pcr_disable(struct kbase_device *kbdev)
{
	...
	writel(PCR_BYPASS_DISABLE,
		PCR_CFG_BYPASS_CORE_ADDR(kbdev->gpu_dev_data.dpm_ctrl.dpmreg +
			GPU_PCR_OFFSET));

	/* disable pcr */
	writel(PCR_DISABLE,
		PCR_CTRL_CORE_ADDR(kbdev->gpu_dev_data.dpm_ctrl.dpmreg +
			GPU_PCR_OFFSET));
	...
	gpu_set_bits(PCR_CRG_CORE_RESET_ADDR_MASK,
		PCR_CRG_CORE_EN_RESET_ADDR(kbdev->gpu_dev_data.crgreg));
