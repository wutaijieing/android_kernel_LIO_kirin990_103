	...
	struct kbase_va_region *reg)
{
	...
	if (is_region_growable(reg))
		seq_printf(sfile, "%7zu  ", reg->extension);
	else
		seq_puts(sfile, "      -  ");
	...
	struct kbase_device *kbdev, int group_id)
{
	...
	err = gmc_storage_get_page(
		kbdev->gpu_dev_data.kbase_gmc_device.storage, page, handle);
	if (err) {
				...
				struct kbase_device *kbdev)
{
	...
	handle = gmc_storage_put_page(
		kbdev->gpu_dev_data.kbase_gmc_device.storage, page);
	if (IS_ERR(handle)) {
		dma_addr_t dma_addr;
	...
	for (i = 0; i < pages_num; i++) {
		...
		if (as_phys_addr_t(*p) && kbase_is_entry_compressed(*p)) {
			...
			gmc_storage_invalidate_page(
				kctx->kbdev->gpu_dev_data.kbase_gmc_device.storage,
				kbase_get_gmc_handle(p));
			p->tagged_addr = 0;
			kbase_clear_entry_compressed(p);
					...
					size_t nr)
{
	...
	if (kctx->gpu_ctx_data.set_pt_flag) {
	...
	struct kbase_gmc_arg arg, struct rb_root *va_rb_root, int *nr_pages_to_compressed)
{
	...
	if (kbdev->gpu_dev_data.gmc_workqueue != NULL)
		local_workqueue = kbdev->gpu_dev_data.gmc_workqueue;
	else
	...
	struct kbase_gmc_arg arg)
{
	...
	if (arg.op == GMC_DECOMPRESS)
		kctx->gpu_ctx_data.set_pt_flag = true;
	...
	if ((arg.op == GMC_COMPRESS) && (kbdev->gpu_dev_data.gmc_cancel != 0))
		kbdev->gpu_dev_data.gmc_cancel = 0;
	...
	if (arg.op == GMC_DECOMPRESS)
		kctx->gpu_ctx_data.set_pt_flag = false;
...
int kbase_gmc_compress(pid_t pid, struct gmc_device *gmc_dev,
			long compress_size)
{
	struct kbase_gpu_device_data *gpu_dev = container_of(gmc_dev,
		struct kbase_gpu_device_data, kbase_gmc_device);
	struct kbase_device *kbdev = container_of(gpu_dev, struct kbase_device,
		gpu_dev_data);
	struct kbase_gmc_arg arg;
...
int kbase_gmc_cancel(long cancel, struct gmc_device *gmc_dev)
{
	...
	struct kbase_device *kbdev = container_of(gpu_dev, struct kbase_device,
		gpu_dev_data);
	if (cancel == 1)
		kbdev->gpu_dev_data.gmc_cancel = 1;
	else
		kbdev->gpu_dev_data.gmc_cancel = 0;
	return 0;
}
...
void kbase_gmc_device_term(struct kbase_device *kbdev)
{
	if (kbdev == NULL)
		return;
	if (kbdev->gpu_dev_data.gmc_workqueue)
		destroy_workqueue(kbdev->gpu_dev_data.gmc_workqueue);
}
