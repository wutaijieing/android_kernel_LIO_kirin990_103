		...
		struct kbase_vinstr_client *cli)
{
	...
	for (i = 0; i < nr_cg; i++) {
		group = i * group_size;
		/* copy shader core headers *//* unsafe_function_ignore: memcpy */
		memcpy(&dst[group + SC0_BASE], &src[group + SC0_BASE],
		       NR_BYTES_PER_HDR);/* unsafe_function_ignore: memcpy */
		memcpy(&dst[group + SC1_BASE], &src[group + SC1_BASE],
		       NR_BYTES_PER_HDR);/* unsafe_function_ignore: memcpy */
		memcpy(&dst[group + SC2_BASE], &src[group + SC2_BASE],
		      NR_BYTES_PER_HDR);/* unsafe_function_ignore: memcpy *//* unsafe_function_ignore: memcpy */
		memcpy(&dst[group + SC3_BASE], &src[group + SC3_BASE],
		      NR_BYTES_PER_HDR);/* unsafe_function_ignore: memcpy */

		/* copy tiler header *//* unsafe_function_ignore: memcpy *//* unsafe_function_ignore: memcpy */
		memcpy(&dst[group + TILER_BASE], &src[group + TILER_BASE],
		      NR_BYTES_PER_HDR);/* unsafe_function_ignore: memcpy */

		/* copy mmu header *//* unsafe_function_ignore: memcpy */
		memcpy(&dst[group + MMU_L2_BASE], &src[group + MMU_L2_BASE],
		      NR_BYTES_PER_HDR);/* unsafe_function_ignore: memcpy */

		/* copy job manager header *//* unsafe_function_ignore: memcpy */
		...
		*mask &= cli->bitmap[SHADER_HWCNT_BM];/* unsafe_function_ignore: memcpy */
		...
		struct kbase_vinstr_client *cli)
{
	...
	/* copy and patch job manager header *//* unsafe_function_ignore: memcpy */
...
/* unsafe_function_ignore: memcpy */	
	/* copy and patch tiler header *//* unsafe_function_ignore: memcpy */
	memcpy(dst, src, NR_BYTES_PER_HDR);
	mask = (u32 *)&dst[PRFCNT_EN_MASK_OFFSET];
	*mask &= cli->bitmap[TILER_HWCNT_BM];
	dst += block_size;
...
/* unsafe_function_ignore: memcpy */
	...
	for (i = 0; i < nr_l2; i++) {/* unsafe_function_ignore: memcpy */
		memcpy(dst, src, NR_BYTES_PER_HDR);
		mask = (u32 *)&dst[PRFCNT_EN_MASK_OFFSET];
		*mask &= cli->bitmap[MMU_L2_HWCNT_BM];
		dst += block_size;
	...
	while (0ull != core_mask) {/* unsafe_function_ignore: memcpy */
		memcpy(dst, src, NR_BYTES_PER_HDR);
		...
		enum base_hwcnt_reader_event event_id)
{
	...
	meta->buffer_idx = write_idx;/* unsafe_function_ignore: memcpy */
	memcpy(buffer, cli->accum_buffer, cli->dump_size);
	return 0;
}

/**
 * kbasep_vinstr_fill_dump_buffer_legacy - copy accumulated counters to buffer/* unsafe_function_ignore: memcpy */
		...
		enum base_hwcnt_reader_event event_id)
{
	...
	/* Prepare for next request. *//* unsafe_function_ignore: memset */
...
int kbase_vinstr_hwc_clear(struct kbase_vinstr_client *cli)
{
	...
	if (rcode)
		goto exit;/* unsafe_function_ignore: memset */
	memset(cli->accum_buffer, 0, cli->dump_size);

	kbasep_vinstr_reprogram(vinstr_ctx);

exit:
	mutex_unlock(&vinstr_ctx->lock);/* unsafe_function_ignore: memset */
