		...
		unsigned int fixture_index)
{
	...
	snprintf(name, sizeof(name), "%d", fixture_index);/* unsafe_function_ignore: snprintf */
		...
		struct kutf_userdata_ops *userdata_ops)
{
	...
	memcpy(&test_func->userdata_ops, userdata_ops, sizeof(*userdata_ops));/* unsafe_function_ignore: snprintf */
...
#else	/* defined(CONFIG_DFX_DEBUG_FS) */
...
#endif	/* defined(CONFIG_DFX_DEBUG_FS) */
