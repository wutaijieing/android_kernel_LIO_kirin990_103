...
/* unsafe_function_ignore: memcpy */
			...
			memcpy(get_version->version_buffer, KERNEL_SIDE_DDK_VERSION_STRING, sizeof(KERNEL_SIDE_DDK_VERSION_STRING));/* unsafe_function_ignore: memcpy */
...
static ssize_t set_ca_policy(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for (i = 0; i < policy_count; i++) {/* unsafe_function_ignore: sscanf */
		if (sysfs_streq(policy_list[i]->name, buf)) {
			new_policy = policy_list[i];
			break;
		}
				      ...
				      const char *buf, size_t count)
{
	...
	return count;/* unsafe_function_ignore: sscanf */
}
...
#undef SET_TIMEOUT
	...
	spin_unlock_irqrestore(&kbdev->hwaccess_lock, flags);/* unsafe_function_ignore: sscanf */
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	kbdev = to_kbase_device(dev);
	if (!kbdev)
		return -ENODEV;/* unsafe_function_ignore: sscanf */
