...
static inline void kbase_platform_on(struct kbase_device *kbdev)
{
	if (kbdev->regulator) {
		...
		if (kbdev->gpu_vid == 0) {
			...
			/*just for atlanta norr beta before b070*/
			if(kbdev->gpu_vid == 0x72020000)
			{
				dev_err(kbdev->dev, "the Real gpu id is:0x%x\n", kbdev->gpu_vid);
				kbdev->gpu_vid = 0x72110000;
			}
				      ...
				      struct devfreq_dev_status *stat)
{
	...
	priv_data->cl_boost = kbdev->pm.backend.metrics.cl_boost;/* unsafe_function_ignore: memcpy */
