		...
		struct kbase_context *kctx, u64 evt, unsigned char new_status)
{
	...
	if ((new_status != BASE_JD_SOFT_EVENT_SET) &&/* unsafe_function_ignore: memset */
		...
		unsigned int *target_page_nr, size_t offset, size_t *to_copy)
{
	...
	*to_copy -= chunk;/* unsafe_function_ignore: memcpy */

	kunmap(pages[*target_page_nr]);/* unsafe_function_ignore: memcpy */
