...
#else
			p_task = pid_task(find_vpid(info->task[i].pid),
				PIDTYPE_PID);
#endif
			if (p_task != NULL) {
...
#else
		rcu_read_lock();
		p_task = pid_task(find_vpid(info->task[i].pid), PIDTYPE_PID);
		if (!p_task) {
			rcu_read_unlock();
			pr_err("Mali setaffinity not find task  pid : %d\n",
					info->task[i].pid);
			continue;
		}
		rcu_read_unlock();

		if (set_cpus_allowed_ptr(p_task, &cpu_mask))
			pr_err("Mali setaffinity err\n");
#endif
	}
