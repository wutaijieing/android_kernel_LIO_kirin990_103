...
#else
			tsk = pid_task(find_vpid(kctx->tgid), PIDTYPE_PID);
#endif
			if (tsk != NULL)
...
/*
 * kbase_gmc_unlock_task - Unlock function, which unlocks mmap_lock
 ...
 * mmap_lock and kbase_gpu_vm_lock of appropriate task and takes
 ...
 * the mmap_lock and mm, task reference counters should be taken into
...
#else
		tsk = pid_task(find_vpid(gmc_tsk->kctx->tgid), PIDTYPE_PID);
#endif
		if (tsk == NULL) {
...
#else
	down_write(&gmc_tsk->mm->mmap_lock);
#endif
		kbase_gpu_vm_lock(gmc_tsk->kctx);
		gmc_tsk->task = tsk;
	}
