...
static int kbasep_jd_debugfs_atoms_show(struct seq_file *sfile, void *data)
{
	...
	seq_puts(sfile, " active_count,suspending,gpu_powered, l2_desired,shader_desired\n");
	seq_printf(sfile, " %3u, %3u, %3u, %3u, %3u ", kbdev->pm.active_count, kbdev->pm.suspending,
	...
	if (kbdev->pm.backend.gpu_powered) {
		...
		seq_printf(sfile, " %8x %8x %8x %8x %8x %8x", kbase_pm_get_trans_cores(kbdev,KBASE_PM_CORE_L2), kbase_pm_get_ready_cores(kbdev, KBASE_PM_CORE_L2),
		...
		seq_printf(sfile, " %8x %8x", kbase_pm_get_trans_cores(kbdev, KBASE_PM_CORE_SHADER), kbase_pm_get_ready_cores(kbdev, KBASE_PM_CORE_SHADER));
		seq_puts(sfile, "\n");
	}
