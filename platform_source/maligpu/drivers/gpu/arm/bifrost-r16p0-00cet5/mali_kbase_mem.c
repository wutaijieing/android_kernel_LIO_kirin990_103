...
#ifdef CONFIG_MALI_LAST_BUFFER
	pools_cbs = kbase_hisi_get_lb_pools_cbs(kbdev);
...
struct kbase_mem_pool* kbase_mem_select_pool(struct kbase_context *kctx, unsigned int lb_policy_id, bool large_page)
{
	...
	if (lb_policy_id != 0) {
		...
		pool_list = large_page ? &lb_pools->lp_mem_pools : &lb_pools->mem_pools;
		...
		u32 bin_id, u32 max_allocations)
{
	...
	KBASE_TLSTREAM_AUX_JIT_STATS(kctx->id, bin_id, max_allocations,
		alloc_count, va_pages, ph_pages); //lint !e648
}
...
static void kbase_jd_user_buf_unpin_pages(struct kbase_mem_phy_alloc *alloc)
{
       if (alloc->nents) { //lint !e548
               struct page **pages = alloc->imported.user_buf.pages;
               long i;

               WARN_ON(alloc->nents != alloc->imported.user_buf.nr_pages); //lint !e665

               for (i = 0; i < alloc->nents; i++) { //lint !e737, !e574
		...
		struct kbase_va_region *reg)
{
	...
	if (WARN_ON(alloc->type != KBASE_MEM_TYPE_IMPORTED_USER_BUF)) //lint !e548, !e665
		return -EINVAL;

	if (alloc->nents) {
		if (WARN_ON(alloc->nents != alloc->imported.user_buf.nr_pages)) //lint !e548, !e665
			return -EINVAL;
		else
			return 0;
	}

	if (WARN_ON(reg->gpu_alloc->imported.user_buf.mm != current->mm)) //lint !e548, !e665
		return -EINVAL;
	...
	if (pinned_pages <= 0)
		return pinned_pages; //lint !e712

	if (pinned_pages != alloc->imported.user_buf.nr_pages) { //lint !e737
		for (i = 0; i < pinned_pages; i++)
			put_page(pages[i]);
		return -ENOMEM;
	}

	alloc->nents = pinned_pages; //lint !e732
	return 0;
}
               ...
               struct kbase_va_region *reg)
{
	...
	pinned_pages = alloc->nents; //lint !e713
		...
		struct kbase_va_region *reg, struct kbase_mem_phy_alloc *alloc)
{
	...
	case KBASE_MEM_TYPE_IMPORTED_UMM: {
		...
		if (0 == alloc->imported.umm.current_mapping_usage_count) {
			if (!kbase_is_region_invalid_or_free(reg) && //lint !e548
					reg->gpu_alloc == alloc) {
				int err;
