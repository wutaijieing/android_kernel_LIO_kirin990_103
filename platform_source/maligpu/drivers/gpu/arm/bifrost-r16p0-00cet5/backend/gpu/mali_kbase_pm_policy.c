...
void kbase_pm_update_cores_state_nolock(struct kbase_device *kbdev, bool dynamic_shadercore_onoff)
{
	...
	if (dynamic_shadercore_onoff) {
		if(shaders_desired &&
			kbdev->pm.backend.gpu_powered &&
			kbdev->pm.backend.l2_desired)
			kbase_pm_update_state(kbdev);
	} else if (kbdev->pm.backend.shaders_desired != shaders_desired) {
...
void kbase_pm_update_cores_state(struct kbase_device *kbdev)
{
	...
	kbase_pm_update_cores_state_nolock(kbdev, false);
