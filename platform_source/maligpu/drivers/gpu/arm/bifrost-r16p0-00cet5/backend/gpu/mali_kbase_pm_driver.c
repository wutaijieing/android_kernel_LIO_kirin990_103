			...
			if (kbase_pm_is_l2_desired(kbdev)) {
				...
				if (!l2_trans || l2_ready == l2_present || !tiler_trans ||tiler_ready == tiler_present) {
					...
					KBASE_TRACE_ADD(kbdev,
							...
							NULL, NULL, 44u,
							(u32)kbase_pm_get_trans_cores(kbdev,KBASE_PM_CORE_TILER));

					KBASE_TRACE_ADD(kbdev,
							PM_CORES_CHANGE_AVAILABLE_TILER,
							NULL, NULL, 55u,
							(u32)kbase_pm_get_ready_cores(kbdev, KBASE_PM_CORE_TILER));
				}
			}
			break;
...
static int kbase_pm_do_reset(struct kbase_device *kbdev)
{
	...
	if (kbase_has_hi_feature(kbdev, KBASE_FEATURE_HI0002)) {
		...
		kbdev->error_num.ts = dfx_getcurtime();
		kbase_pm_reset_instead(kbdev);
	} else {
