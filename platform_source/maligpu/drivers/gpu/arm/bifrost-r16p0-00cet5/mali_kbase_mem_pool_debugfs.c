...
int kbase_lb_mem_pool_debugfs_init(struct kbase_device *kbdev, struct dentry * parent, struct kbase_hisi_lb_pools *lb_pools)
{
	...
	pool_list = &lb_pools->mem_pools;
	lp_pool_list = &lb_pools->lp_mem_pools;
	list_for_each_entry(walker, pool_list, pool_entry) {
		char pool_name[64];
		struct dentry *debugfs_dev_lb_sub_dir;
		struct kbase_mem_pool *lp_pool;
		snprintf(pool_name, 64, "policy_id_%d", walker->lb_policy_id); /* unsafe_function_ignore: snprintf */

		debugfs_dev_lb_sub_dir = debugfs_create_dir(pool_name,
		...
		lp_pool = pools_cbs->find_pool(walker->lb_policy_id, lp_pool_list);
		if (!lp_pool) {
