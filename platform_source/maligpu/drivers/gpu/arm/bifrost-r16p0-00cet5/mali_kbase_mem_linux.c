...
static unsigned int kbase_mem_get_policy_id(struct kbase_context *kctx, u64 *flags)
{
	...
	cache_policy_callbacks *callback;
	...
	*flags &= ~BASE_MEM_LB_POLICY_MASK;
	...
	callback = kbase_hisi_get_lb_cache_cbs(kbdev);
...
#ifdef CONFIG_MALI_LAST_BUFFER
	lb_policy_id = kbase_mem_get_policy_id(kctx, flags);
#else
	*flags &= ~BASE_MEM_LB_POLICY_MASK;
#endif
...
static int kbase_cpu_vm_fault(struct vm_fault *vmf)
{
	...
	while (i < map->alloc->nents && (addr < vma->vm_end >> PAGE_SHIFT)) {
		...
		KBASE_DEBUG_ASSERT(kbdev);
		mgm_ops = kbdev->hisi_dev_data.mgm_ops;
		KBASE_DEBUG_ASSERT(mgm_ops);
		p = as_page(map->alloc->pages[i]);
		...
		struct kbase_vmap_struct *map)
{
	...
	mgm_ops = kbdev->hisi_dev_data.mgm_ops;
	KBASE_DEBUG_ASSERT(mgm_ops);
	err = mgm_ops->vm_insert_pfn_prot(pages, page_count, &prot);
	WARN_ON(err);
#endif
