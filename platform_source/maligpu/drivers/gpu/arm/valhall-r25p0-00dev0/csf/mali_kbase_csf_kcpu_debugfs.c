		...
		struct kbase_kcpu_command_cqs_wait_info *waits)
{
	...
	for (i = 0; i < waits->nr_objs; i++) {
		...
		u32 val;
		void *cpu_ptr = kbase_phy_alloc_mapping_get(kctx,
		...
		seq_printf(file, "   %llx(%u > %u)", waits->objs[i].addr, val,
				waits->objs[i].val);
	}
}
...
static int kbasep_csf_kcpu_debugfs_show(struct seq_file *file, void *data)
{
	...
	seq_puts(file, "Queue Idx, Pending Commands, Enqueue err, Blocked, Fence context  &  seqno, (Wait Type, Additional info)\n");
...
void kbase_csf_kcpu_debugfs_init(struct kbase_context *kctx)
{
	...
	file = debugfs_create_file("kcpu_queues", 0444, kctx->kctx_dentry,
