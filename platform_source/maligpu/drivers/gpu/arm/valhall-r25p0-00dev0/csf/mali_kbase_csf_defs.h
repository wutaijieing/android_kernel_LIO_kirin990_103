 ...
 * @lock:             Lock to serialize the execution of csf ioctls and access
 *                    to CSF-related fields inside kbase_context.
 ...
 * @complete_callback: Callback to be invoked on GPU reset completion.
 */
struct kbase_csf_reset_gpu {
	...
	atomic_t state;
	/**
	 * complete_callback - Do the post GPU reset completion work
	 *
 ...
 * @protected_mode_in_use:  Indicates if firmware has been permitted to let
 *                          GPU enter protected mode. On exit from protected
...
struct kbase_csf_scheduler {
	...
	unsigned long csgs_events_enable_mask;
