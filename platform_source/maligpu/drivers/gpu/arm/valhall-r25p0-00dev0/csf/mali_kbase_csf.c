		   ...
		   struct kbase_csf_cmd_stream_info const *const stream)
{
	...
	if (cs_fault_exception_type != CS_FAULT_EXCEPTION_TYPE_OK &&
	    cs_fault_exception_type != CS_FAULT_EXCEPTION_TYPE_DONE &&
	    cs_fault_exception_type != CS_FAULT_EXCEPTION_TYPE_TERMINATED &&
	    cs_fault_exception_type != CS_FAULT_EXCEPTION_TYPE_EUREKA &&
	    cs_fault_exception_type != CS_FAULT_EXCEPTION_TYPE_ACTIVE) {
 ...
 *         code.
 */
static int handle_fatal_exception_event(struct kbase_queue const *const queue,
		   struct kbase_csf_cmd_stream_info const *const stream)
{
	...
	const u64 cs_fatal_info =
		...
		((u64)kbase_csf_firmware_cs_output(stream, CS_FATAL_INFO_HI)
		 << 32);
	const u8 cs_fatal_exception_type =
	...
	if (cs_fatal_exception_type != CS_FATAL_EXCEPTION_TYPE_OK &&
	    cs_fatal_exception_type != CS_FATAL_EXCEPTION_TYPE_DONE &&
	    cs_fatal_exception_type != CS_FATAL_EXCEPTION_TYPE_TERMINATED &&
	    cs_fatal_exception_type != CS_FATAL_EXCEPTION_TYPE_EUREKA &&
	    cs_fatal_exception_type != CS_FATAL_EXCEPTION_TYPE_ACTIVE) {
		dev_warn(kbdev->dev, "CSI: %d\n"
			"CS_FATAL.EXCEPTION_TYPE: 0x%x (%s)\n"
			"CS_FATAL.EXCEPTION_DATA: 0x%x\n"
			"CS_FATAL_INFO.EXCEPTION_DATA: 0x%llx\n",
			queue->csi_index, cs_fatal_exception_type,
	...
	dev_dbg(kbdev->dev, "CSI: %d\n"
		"CS_FATAL.EXCEPTION_TYPE: 0x%x (%s)\n"
		"CS_FATAL.EXCEPTION_DATA: 0x%x\n"
		"CS_FATL_INFO.EXCEPTION_DATA: 0x%llx\n",
		queue->csi_index, cs_fatal_exception_type,
		kbase_gpu_fatal_exception_name(cs_fatal_exception_type),
		cs_fatal_exception_data, cs_fatal_info_exception_data);
		      ...
		      u32 const irqreq, u32 const irqack)
{
	...
	/* Set IRQ_ACK bits same as the IRQ_REQ bits */
	...
	if (val & JOB_IRQ_GLOBAL_IF) {
		...
		if (global_iface->output) {
			...
			if ((glb_req ^ glb_ack) & GLB_REQ_PROTM_EXIT_MASK) {
				...
				WARN_ON(scheduler->protected_mode_in_use ==
						false);
