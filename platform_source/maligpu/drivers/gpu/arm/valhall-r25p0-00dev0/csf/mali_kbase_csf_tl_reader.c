	...
	u64 *gpu_ts)
{
	...
	kbase_backend_get_gpu_time(kbdev, NULL, gpu_ts, &ts);
	...
	struct kbase_device *kbdev)
{
	...
	get_cpu_gpu_time(kbdev, &cpu_ts, &gpu_ts);
...
{
	struct kbase_device *kbdev = self->kbdev;
	const bool has_overflow = self->got_first_event
		&& self->expected_event_id != event_id;

	if (has_overflow)
		dev_warn(kbdev->dev,
			 "CSFFW stream overflow, event_id: %u does not match expected: %u.",
...
static void kbasep_csf_tl_reader_read_callback(struct timer_list *timer)
{
	 ...
	 */
	const u32 bytes_read = kbase_csf_firmware_trace_buffer_read_data(
		self->trace_buffer, read_buffer, read_buffer_size);
	u8 *const csffw_data_begin = read_buffer;
	u8 *const csffw_data_end   = read_buffer + bytes_read;
	u8 *csffw_data_it;
	...
	rcode = mod_timer(&self->read_timer,
		jiffies + msecs_to_jiffies(KBASE_CSF_TL_READ_INTERVAL));
