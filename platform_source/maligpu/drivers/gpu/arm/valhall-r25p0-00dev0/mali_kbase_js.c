 ...
 * Functions private to KBase ('Protected' functions)
 */
int kbasep_js_minimal_devdata_init(struct kbase_device * const kbdev)
{
	struct kbasep_js_device_data *jsdd;
			...
			struct kbase_context *kctx)
{
	...
	kctx_as_nr = kctx->as_nr;
	if (WARN_ON(kctx_as_nr == KBASEP_AS_NR_INVALID))
		return;

	if (WARN_ON(atomic_read(&kctx->refcount) <= 0))
		return;

	/* We need not check for if kctx is containing flags
	 * KCTX_SCHEDULED, since this flag is set and cleared part
	 * kbasep_js_schedule_ctx function which is part of JM
	 * functionality now.
	 */
	mutex_lock(&kbdev->pm.lock);
	spin_lock_irqsave(&kbdev->hwaccess_lock, flags);

	if (!WARN_ON(kctx_as_nr != kctx->as_nr) &&
			!WARN_ON(atomic_read(&kctx->refcount) <= 0))
		kbase_ctx_sched_release_ctx(kctx);	/* Update refcount */
