		...
		union kbase_ioctl_cs_get_glb_iface *param)
{
	...
	if (!err) {
		...
		err = copy_to_user(user_groups, group_data,
			MIN(max_group_num, param->out.group_num) *
				sizeof(*group_data));
	}

	if (!err)
		err = copy_to_user(user_streams, stream_data,
...
static ssize_t kbase_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (!atomic_read(&kctx->event_count)) {
		dev_err(kctx->kbdev->dev, "No events signaled\n");
		return -EINVAL;
	}

	/* There is no actual data to return */
	if (copy_to_user(buf, &dummy_data, data_size) == 0) {
		/* Acknowledge or consume the event */
		atomic_set(&kctx->event_count, 0);
		ret = data_size;
	} else {
		dev_warn(kctx->kbdev->dev, "No data to read\n");
		ret = -EFAULT;
	}
