...
{
	struct kbase_gpu_callbacks *callbacks = NULL;

	KBASE_DEBUG_ASSERT(kbdev != NULL);
	callbacks = kbdev->gpu_dev_data.callbacks;
...
static int kbase_platform_lb_alloc(struct kbase_device *kbdev)
{
	...
	manager = kbdev->gpu_dev_data.quota_manager;
...
/* The gpu_lb_callbacks instance. */
gpu_lb_callbacks lb_callbacks = {
