...
#ifdef CONFIG_DEVFREQ_THERMAL
	if (kbdev->gpu_dev_data.devfreq_cooling)
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,1,15)
		memcpy(&kbdev->gpu_dev_data.devfreq->last_status, stat, /* unsafe_function_ignore: memcpy */
			sizeof(*stat));
#else
		memcpy(&kbdev->gpu_dev_data.devfreq_cooling->last_status, /* unsafe_function_ignore: memcpy */
			stat, sizeof(*stat));
#endif
#endif
