...
int perf_ctrl_enable_gpu_lb(void __user *uarg __maybe_unused)
{
	return 0;
}
#endif
...
static void hisi_gpu_devfreq_cooling_init(struct kbase_device *kbdev)
{
	...
	ithermal_gpu_cdev_register(kbdev->dev->of_node, kbdev->devfreq);
	kbdev->devfreq_cooling = ithermal_get_gpu_cdev();
