...
int power_control_init(struct kbase_device *kbdev)
{
	...
	kbdev->reference_dev_data.clock = clk_get(kbdev->dev, "clk_g3d");
	if (IS_ERR_OR_NULL(kbdev->reference_dev_data.clock)) {
		err = PTR_ERR(kbdev->reference_dev_data.clock);
		kbdev->reference_dev_data.clock = NULL;
		if (err == -EPROBE_DEFER) {
		...
		err = clk_prepare_enable(kbdev->reference_dev_data.clock);
		if (err) {
			dev_err(kbdev->dev,
	...
	if (err)
		dev_err(kbdev->dev, "OPP table not found\n");
...
fail:
	...
	if (kbdev->reference_dev_data.clock != NULL) {
		clk_put(kbdev->reference_dev_data.clock);
		kbdev->reference_dev_data.clock = NULL;
	}
...
void power_control_term(struct kbase_device *kbdev)
{
	...
	if (kbdev->reference_dev_data.clock) {
		clk_disable_unprepare(kbdev->reference_dev_data.clock);
		clk_put(kbdev->reference_dev_data.clock);
		kbdev->reference_dev_data.clock = NULL;
	}
}
