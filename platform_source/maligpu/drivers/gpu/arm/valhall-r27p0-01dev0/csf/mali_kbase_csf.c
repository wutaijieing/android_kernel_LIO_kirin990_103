		...
		struct kbase_normal_suspend_buffer *s_buf)
{
	...
	err = kbase_mmu_insert_pages(kctx->kbdev, &kctx->kbdev->csf.mcu_mmu,
				     ...
				     nr_pages, mem_flags,
				     MCU_AS_NR, KBASE_MEM_GROUP_CSF_FW, nr_pages);
	if (err)
		...
		struct kbase_protected_suspend_buffer *s_buf)
{
	...
	err = kbase_mmu_insert_pages(kbdev, &kbdev->csf.mcu_mmu,
				     ...
				     KBASE_MEM_GROUP_CSF_FW, nr_pages);
	if (err)
		goto mmu_insert_failed;
...
/**
 * build_group_fatal_error - Build CSG Group fatal error.
 *
 * @group: Pointer to GPU command queue group.
 * @fault: Pointer to mmu or bus fault data.
 *
 * Build MMU_PAGE_FAULT or GPU_BUS_FAULT data using the information provided
 * by fault and add it to the per-context error list.
 */
static void build_group_fatal_error(struct kbase_queue_group *group,
		struct kbase_fault *fault)
{
	/* Build MMU_PAGE_FAULT or GPU_BUS_FAULT using collected information.
	 */
	 ...
	 * subsequent fatal error on the group should never take place.
	 */
...
/**
 * build_protm_group_fatal_error - Build fatal error for protected mode group
 *
 * @group: Pointer to GPU command queue group running in protected mode.
 */
static void build_protm_group_fatal_error(struct kbase_queue_group *group)
{
	struct base_csf_notification const error = {
		.type = BASE_CSF_NOTIFICATION_GPU_QUEUE_GROUP_ERROR,
		.payload = {
			.csg_error = {
				.handle = group->handle,
	...
	lockdep_assert_held(&kctx->csf.lock);
	lockdep_assert_held(&kctx->kbdev->csf.scheduler.interrupt_lock);

	WARN_ON(!kbase_csf_scheduler_protected_mode_in_use(kctx->kbdev));
					 ...
					 struct kbase_fault *fault)
{
	...
	while (!list_empty(&evicted_groups)) {
		...
		if (fault) {
			build_group_fatal_error(group, fault);
			...
			if ((group == scheduler->active_protm_grp) &&
			    group->faulted) {
				build_protm_group_fatal_error(group);
				fatal_error_built = true;
			}
	...
	/* Wake up event consumer in user space */
...
void kbase_csf_event_signal(struct kbase_context *kctx, bool notify_gpu)
{
	...
	if (notify_gpu) {
		 ...
		 * for CSF firmware.
		 */
		WARN_ON(!kctx->kbdev->pm.active_count);
 ...
 *
 * Terminates the GPU command queue group for which the timer event was raised.
 * See term_queue_group() for details.
 */
static void timer_event_worker(struct work_struct *data)
{
	struct kbase_queue_group *const group =
	...
	dev_warn(kbdev->dev,
		 "Queue group to be terminated, forward progress timeout\n");
	term_queue_group(group);
...
/**
 * handle_fault_event - Handler for firmware faults reported.
 *
 ...
 *
 * Prints meaningful firmware fault description.
 *
 * Return: 0 if successfully handled the fault, otherwise a negative error
 *         code on failure.
 */
...
/**
 * handle_fatal_exception_event - Handler for firmware fatal exceptions
 *                                reported.
 *
 ...
 *          firmware about the command stream interface.
 *
 * Prints meaningful firmware fatal exception description.
 *
 * Return: 0 if successfully handled the exception, otherwise a negative error
 *         code.
 */
static int handle_fatal_exception_event(struct kbase_queue *const queue,
	struct kbase_csf_cmd_stream_info const *const stream)
{
	...
	/* Build command stream fatal error using all collected information. */
	struct base_csf_notification error = {
		.type = BASE_CSF_NOTIFICATION_GPU_QUEUE_GROUP_ERROR,
		.payload = {
			.csg_error = {
	...
	dev_warn(kbdev->dev,
		 ...
		 kbase_gpu_fatal_exception_name(cs_fatal_exception_type),
...
/**
 * kbase_queue_fault_event - Handle faults for a GPU command queue.
 *
 ...
 * virtual address space. Do not call in interrupt context.
 *
 * Reports the fault and fatal exception for a GPU command queue and then
 * clears the corresponding notification fields to allow the firmware to
 * report other faults in future. It may also terminate the GPU command queue
 * group in which the fault/fatal exception occurred. See term_queue_group()
 * for details.
 */
static void kbase_queue_fault_event(struct kbase_queue *const queue)
{
	struct kbase_context *const kctx = queue->kctx;
	struct kbase_device *const kbdev = kctx->kbdev;
	struct kbase_queue_group *group;
	...
	if ((cs_ack & CS_ACK_FATAL_MASK) != (cs_req & CS_REQ_FATAL_MASK)) {
		err = handle_fatal_exception_event(queue, stream);
...
/**
 * fault_event_worker - Fault event handler called from a workqueue.
 *
 * @data: Pointer to a work_struct embedded in GPU command queue data.
 *
 * Handles the fault and fatal exception for a GPU command queue and then
 * releases a reference that was added to prevent the queue being destroyed
 * while this work item was pending on a workqueue.
 */
		      ...
		      u32 const irqreq, u32 const irqack)
{
	...
	while (remaining != 0) {
		 ...
		 * cannot disappear since interrupt_lock is held and before
		 * freeing a bound queue it has to be first unbound which
		 * requires interrupt_lock.
		 */
	...
	int const csg_nr)
{
	 ...
	 * terminate the group whilst this loop is running as interrupt_lock
	 * is held and for freeing a group that is resident on a CSG slot
	 * interrupt_lock is required.
	 */
	if (!group)
		return;
