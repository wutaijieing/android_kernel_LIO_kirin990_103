 ...
 *             stream front-end interface.
 * @kctx:      The kbase context.
 * @fault:     Fault information if MMU fault or GPU bus fault notification
 *             is needed Otherwise NULL.
 *
 * This function will iterate through all the active/scheduled GPU command
 * queue groups associated with the context, build fatal error information
 * for reporting to user space, deschedule them and mark them as terminated
 * (which will then lead to unbinding of all the queues bound to them)
 * and also no more work would be allowed to execute for them.
 *
...
void kbase_csf_active_queue_groups_reset(struct kbase_device *kbdev,
		struct kbase_context *kctx, struct kbase_fault *fault);
