...
static inline void kbase_platform_on(struct kbase_device *kbdev)
{
	...
	if (kbdev->hisi_dev_data.regulator && !refcount) {
		...
		dev_err(kbdev->dev, "regulator enable success");
		ret = regulator_is_enabled(kbdev->hisi_dev_data.regulator);
		dev_err(kbdev->dev, "regulator_is_enabled return : %d", ret);
...
static void hisi_gpu_devfreq_cooling_init(struct kbase_device *kbdev)
{
	...
	ithermal_gpu_cdev_register(kbdev->dev->of_node, kbdev->hisi_dev_data.devfreq);
	kbdev->hisi_dev_data.devfreq_cooling = ithermal_get_gpu_cdev();
