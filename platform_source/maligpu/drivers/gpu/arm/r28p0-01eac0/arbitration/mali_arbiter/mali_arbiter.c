 ...
 * @current_domain_id: Current domain ID
 ...
 * @force_gpu_required: true if GPU lost has occurred and GPU should be
 *                      forced to another VM
 * @host_dom: Contains mali_arb_dom struct for the host while it is
 *        registered with the Arbiter.
 * @hyp_ctx: Private data to be provided during hypervisor callbacks.
 *           Not used by the Arbiter.
 * @arb_hyp_assign_vm_gpu: Callback function to assign GPU to VM in
 *                         hypervisor
 * @arb_hyp_force_assign_vm_gpu: Callback function to force assign GPU
 *                               to VM in hypervisor
 * @arb_gpu_power_on: Callback to provide GPU power on functionality
 * @arb_gpu_power_off: Callback to providate GPU power off
 *                     functionality
 * @aggr_lost_work: stores the pending work when GPU is lost
 * Main arbiter struct which contains references to all the resources
 * and interfaces needed at runtime
 */
struct mali_arb {
...
/**
 * remove_active_domain() - Remove Active Domain
 * @arb: Arbiter data
 *
 * Removes the active domain on the arbiter
 */
...
/**
 * assign_to_domain() - Assign to domain
 * @arb: Arbiter data
 * @domain_id: Domain id
 *
 * Assigns the given domain to the arbiter
 */
...
static void run_next_guest_locked(struct mali_arb *arb)
{
	...
	if (arb->state != ARB_NO_REQ) {
		if (list_empty(&arb->req_list)) {
			...
			if (!arb->host_dom || !arb->host_dom->gpu_lost)
 ...
 * @dom: Internal Arbiter data for domain
 *
 ...
 */
static void handle_gpu_lost_locked(struct mali_arb_dom *dom)
{
	struct mali_arb *arb;
	...
	arb = dom->arb;
	dev_info(arb->dev, "GPU took too long to yield for dom%u\n",
			dom->info.domain_id);
 ...
 */
void gpu_lost_complete_locked(struct mali_arb_dom *dom,
		bool req_again)
{
	struct mali_arb *arb;
	...
	if (dom->gpu_lost) {
		dom->gpu_lost = false;
		dev_dbg(arb->dev, "Dom%u GPU lost complete\n",
				dom->info.domain_id);

		if (dom == arb->host_dom) {
			if (!req_again && list_empty(&arb->req_list)) {
				assign_to_domain(arb, 0);
				dev_dbg(arb->dev,
						"GPU no longer required after GPU lost\n");
			}
		}
	}
}
 ...
 *
 * Call back function to request GPU time
 */
static void gpu_request(struct mali_arb_dom *dom)
{
	...
	mutex_lock(&arb->mutex);
	gpu_lost_complete_locked(dom, true);
	 ...
	 */
	if (arb->state == ARB_GPU_STOPPING && !arb->timer_running) {
		dev_dbg(arb->dev, "New VM request, GPU lost now required\n");
		handle_gpu_lost_locked(arb->active_dom);
	}
...
/**
 * gpu_stopped() - VM KBase driver is in stopped state
 *                        GPU is not being used
 * @dom: Domain structure returned by registration.
 * @req_again: VM has work pending and still wants the GPU
 *
 * Callback function to acknowledge GPU is stopped
 */
static void gpu_stopped(struct mali_arb_dom *dom, bool req_again)
{
	...
	if (!list_empty(&dom->entry)) {
		...
		list_add_tail(&dom->entry, &arb->req_list);
		if (arb->state == ARB_NO_REQ || arb->state == ARB_SINGLE_REQ) {
			WARN_ON(arb->timer_running);
			run_next_guest_locked(arb);
		}
	}

	gpu_lost_complete_locked(dom, req_again);
	mutex_unlock(&arb->mutex);
}
...
static void timeout_worker(struct work_struct *data)
{
	...
	case ARB_RUNNING:
		...
		break;
	case ARB_GPU_STOPPING:
		/* Only forcibly remove the GPU if there is another VM
		 * waiting.  Otherwise, allow the VM to take longer.
		 * Active VM will still be in the list, so only
		 * proceed with the GPU lost if the list has more than one
		 * entry present.
		 */
		if (!list_is_singular(&arb->req_list))
			handle_gpu_lost_locked(dom);
		else
...
/**
 * register_hyp() - Register hypervisor callbacks for use by the Arbiter.
 * @arb_dev: Arbiter device
 * @ctx: Caller context (not used by the Arbiter)
 * @cbs: Function callbacks
 *
 * Register Hypervisor callbacks for use by Arbiter
 *
...
/**
 * unregister_hyp() - Unregister hypervisor callbacks
 * @arb_dev: Arbiter device
 *
 * Arbiter callback function which ensure that the hypervisor callbacks
 * are not called once this function has been completed.
 */
...
/**
 * register_dom() - Register a domain comms device
 ...
 *
 * This call back function must be called first before a domain
 * comms device instance can use the arbiter functionality.
 *
 * Return: 0 on success else a Linux error code
 */
...
/**
 * unregister_dom() - Unregister a domain comms device
 * @dom: Domain structure returned by registration.
 *
 * This call back function must be called before the domain comms device
...
static void unregister_dom(struct mali_arb_dom *dom)
{
	...
	gpu_lost_complete_locked(dom, false);
	if (dom == arb->host_dom)
		arb->host_dom = NULL;
...
/**
 * gpu_idle() - Notify Arbiter that GPU is Idle
 * @dom: Domain structure returned by registration.
 *
 * Called by a domain comms device to notify the Arbiter that
 * the VM is no longer using the GPU
 */
...
/**
 * arbiter_init_plat() - Arbiter platform initialization
 * @arb: Arbiter data
 *
 * Initialise the arbiter platform module and
...
#if MALI_ARBITER_TEST_API
/**
 * test_set_callbacks() - Test Call back functions
 * @arb_dev: arbiter device
 * @cbs: call back functions.
 */
...
/**
 * test_utilisation() - Test GPU time utilisation
 * @arb_dev: arbiter device
 * @gpu_busytime: GPU busy time
 * @gpu_totaltime: GPU total time
 *
 * Tests the GPU utilisation time and logs the results
 */
...
static int arbiter_probe(struct platform_device *pdev)
{
	...
	/* register arbiter callback functions */
