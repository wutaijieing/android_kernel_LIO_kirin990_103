...
static int kbasep_jd_atoms_show(struct kbase_context *kctx)
{
	...
	dev_err(kbdev->dev, "dump kctx %pK\n", kctx);
	/* Print table heading */
	dev_err(kbdev->dev, " ID, Core req, St, CR,   Predeps,           Start time, Additional info...\n");
...
void kbase_cancel_soft_job(struct kbase_jd_atom *katom)
{
	...
	default:
		...
		pr_err("mali gpu: %s soft-job cancel request katom is %pK,job req is %x",
