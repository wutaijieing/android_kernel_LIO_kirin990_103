...
static int npu_manager_get_devinfo(unsigned long arg)
{
	...
	devinfo.ctrl_cpu_core_num = num_possible_cpus();
	...
	devinfo.ctrl_cpu_endian_little = 0;
#endif
	...
	npu_drv_debug("print npu dev info msg :"
		...
		devinfo.ai_core_id, devinfo.ctrl_cpu_core_num, devinfo.ctrl_cpu_id,
		devinfo.ctrl_cpu_ip, devinfo.ctrl_cpu_endian_little, devinfo.env_type);
	...
	unsigned int cmd, unsigned long arg)
{
	...
	if (copy_to_user_safe((void __user *)(uintptr_t)arg, &msg, sizeof(msg))) {
		...
		cond_return_error(ret != 0, -EFAULT,
			"npu iova unmap ion error, ret = %d\n", ret);
	}
#endif
