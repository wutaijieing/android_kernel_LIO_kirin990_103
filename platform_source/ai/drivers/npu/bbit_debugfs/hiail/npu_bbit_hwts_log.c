...
u64 get_aicore_base_addr(u8 aic_idx)
{
	...
	npu_drv_debug("aicore_base =0x%llx\n", aicore_base);
...
{
	SOC_NPU_HWTS_DFX_LOG_CTRL_UNION log_ctrl = {0};
	SOC_NPU_HWTS_DFX_LOG_BASE_ADDR_UNION log_buff_base = {0};
	...
	npu_drv_debug("alloc log buf_size=%llu, virt_addr=0x%llx, iova_addr=0x%lx\n",
		log_buff.size, log_buff.virt_addr, log_buff.iova_addr);

	read_uint64(log_ctrl.value, SOC_NPU_HWTS_DFX_LOG_CTRL_ADDR(hwts_base));
	log_ctrl.reg.task_log_en = 0x1;
	log_ctrl.reg.block_log_en = 0x1;
	log_ctrl.reg.pmu_log_en = 0x1;
	log_ctrl.reg.log_buf_length = log_buff.size / HWTS_LOG_SIZE;
	log_ctrl.reg.log_af_threshold = log_buff.size / HWTS_LOG_SIZE;
	write_uint64(log_ctrl.value, SOC_NPU_HWTS_DFX_LOG_CTRL_ADDR(hwts_base));

	read_uint64(log_buff_base.value,
		SOC_NPU_HWTS_DFX_LOG_BASE_ADDR_ADDR(hwts_base));
	log_buff_base.reg.log_base_addr_is_virtual = 0x1;
	log_buff_base.reg.log_base_addr = log_buff.iova_addr;
	write_uint64(log_buff_base.value,
		SOC_NPU_HWTS_DFX_LOG_BASE_ADDR_ADDR(hwts_base));
}
...
void disable_hwts_prof_log(void)
{
	...
	npu_drv_debug("free log buf_size=%llu, virt_addr=0x%llx, iova_addr=0x%lx\n",
		log_buff.size, log_buff.virt_addr, log_buff.iova_addr);

	read_uint64(log_ctrl.value, SOC_NPU_HWTS_DFX_LOG_CTRL_ADDR(hwts_base));
	log_ctrl.reg.task_log_en = 0x0;
	log_ctrl.reg.block_log_en = 0x0;
	log_ctrl.reg.pmu_log_en = 0x0;
	log_ctrl.reg.log_buf_length = 0x0;
	log_ctrl.reg.log_af_threshold = 0x0;
	write_uint64(log_ctrl.value, SOC_NPU_HWTS_DFX_LOG_CTRL_ADDR(hwts_base));

	read_uint64(log_buff_base.value,
		SOC_NPU_HWTS_DFX_LOG_BASE_ADDR_ADDR(hwts_base));
	log_buff_base.reg.log_base_addr_is_virtual = 0x0;
	log_buff_base.reg.log_base_addr = 0x0;
	write_uint64(log_buff_base.value,
		SOC_NPU_HWTS_DFX_LOG_BASE_ADDR_ADDR(hwts_base));
}
...
void set_aicore_pmu_events(u64 aicore_base, pmu_config_t pmu)
{
	...
	read_uint64(cnt0_idx1.value, SOC_NPU_AICORE_PMU_CNT0_IDX_ADDR(aicore_base));
	cnt0_idx1.reg.pmu_cnt0_idx = pmu.events[0];
	write_uint64(cnt0_idx1.value,
		SOC_NPU_AICORE_PMU_CNT0_IDX_ADDR(aicore_base));

	read_uint64(cnt1_idx1.value, SOC_NPU_AICORE_PMU_CNT1_IDX_ADDR(aicore_base));
	cnt1_idx1.reg.pmu_cnt1_idx = pmu.events[1];
	write_uint64(cnt1_idx1.value,
		SOC_NPU_AICORE_PMU_CNT1_IDX_ADDR(aicore_base));

	read_uint64(cnt2_idx1.value, SOC_NPU_AICORE_PMU_CNT2_IDX_ADDR(aicore_base));
	cnt2_idx1.reg.pmu_cnt2_idx = pmu.events[2];
	write_uint64(cnt2_idx1.value,
		SOC_NPU_AICORE_PMU_CNT2_IDX_ADDR(aicore_base));

	read_uint64(cnt3_idx1.value, SOC_NPU_AICORE_PMU_CNT3_IDX_ADDR(aicore_base));
	cnt3_idx1.reg.pmu_cnt3_idx = pmu.events[3];
	write_uint64(cnt3_idx1.value,
		SOC_NPU_AICORE_PMU_CNT3_IDX_ADDR(aicore_base));

	read_uint64(cnt4_idx1.value, SOC_NPU_AICORE_PMU_CNT4_IDX_ADDR(aicore_base));
	cnt4_idx1.reg.pmu_cnt4_idx = pmu.events[4];
	write_uint64(cnt4_idx1.value,
		SOC_NPU_AICORE_PMU_CNT4_IDX_ADDR(aicore_base));

	read_uint64(cnt5_idx1.value, SOC_NPU_AICORE_PMU_CNT5_IDX_ADDR(aicore_base));
	cnt5_idx1.reg.pmu_cnt5_idx = pmu.events[5];
	write_uint64(cnt5_idx1.value,
		SOC_NPU_AICORE_PMU_CNT5_IDX_ADDR(aicore_base));

	read_uint64(cnt6_idx1.value, SOC_NPU_AICORE_PMU_CNT6_IDX_ADDR(aicore_base));
	cnt6_idx1.reg.pmu_cnt6_idx = pmu.events[6];
	write_uint64(cnt6_idx1.value,
		SOC_NPU_AICORE_PMU_CNT6_IDX_ADDR(aicore_base));

	read_uint64(cnt7_idx1.value, SOC_NPU_AICORE_PMU_CNT7_IDX_ADDR(aicore_base));
	cnt7_idx1.reg.pmu_cnt7_idx = pmu.events[7];
	write_uint64(cnt7_idx1.value,
		SOC_NPU_AICORE_PMU_CNT7_IDX_ADDR(aicore_base));
}
...
void enable_aicore_pmu_ctrl(u64 aicore_base)
{
	...
	read_uint64(pmu_ctrl.value, SOC_NPU_AICORE_PMU_CTRL_ADDR(aicore_base));
	pmu_ctrl.reg.pmu_en = 0x1;
	write_uint64(pmu_ctrl.value, SOC_NPU_AICORE_PMU_CTRL_ADDR(aicore_base));

	read_uint64(start_cnt.value,
		SOC_NPU_AICORE_PMU_START_CNT_CYC_ADDR(aicore_base));
	start_cnt.reg.pmu_start_cnt_cyc = 0x0;
	write_uint64(start_cnt.value,
		SOC_NPU_AICORE_PMU_START_CNT_CYC_ADDR(aicore_base));

	read_uint64(stop_cnt.value,
		SOC_NPU_AICORE_PMU_STOP_CNT_CYC_ADDR(aicore_base));
	stop_cnt.reg.pmu_stop_cnt_cyc = 0xFFFFFFFFFFFFFFFF;
	write_uint64(stop_cnt.value,
		SOC_NPU_AICORE_PMU_STOP_CNT_CYC_ADDR(aicore_base));
}
...
void enable_aicore_pmu(pmu_config_t pmu)
{
	...
	for (i = 0; i < AICORE_COUNT; i++) {
		...
		if (aicore_base == 0) {
			npu_drv_err("get aicore%d base failed.\n", i);
			return;
		}
...
void disable_aicore_pmu_ctrl(u64 aicore_base)
{
	...
	read_uint64(stop_cnt.value,
		SOC_NPU_AICORE_PMU_STOP_CNT_CYC_ADDR(aicore_base));
	stop_cnt.reg.pmu_stop_cnt_cyc = 0x0;
	write_uint64(stop_cnt.value,
		SOC_NPU_AICORE_PMU_STOP_CNT_CYC_ADDR(aicore_base));
}
...
int get_ispnn_profiling_log(const char __user *prof_buf)
{
	...
	u32 log_buf_len = 0;
	SOC_NPU_HWTS_DFX_LOG_CTRL_UNION log_ctrl = {0};
	SOC_NPU_HWTS_DFX_LOG_PTR_UNION log_pos_ptr = {0};
	u64 hwts_base = get_hwts_base_addr();
	...
	read_uint64(log_pos_ptr.value, SOC_NPU_HWTS_DFX_LOG_PTR_ADDR(hwts_base));
	...
	npu_drv_debug("copy hwts log to user, from=%xllx, to=%xllx", log_addr,
		prof_buf);
	...
	write_uint64(log_pos_ptr.value, SOC_NPU_HWTS_DFX_LOG_PTR_ADDR(hwts_base));
...
int print_ispnn_profiling_log(void)
{
	...
	npu_drv_debug("log_rptr=%u, log_wptr=%u, log_buf_len=%u, log_task_en=%u, log_base_addr=0x%llx\n",
		log_rptr, log_wptr, log_buf_len, log_task_en, log_base_addr);
