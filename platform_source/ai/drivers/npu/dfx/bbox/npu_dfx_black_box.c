...
static struct rdr_exception_info g_rdr_exception_info[RDR_EXCEPTION_TYPES_SUM];
...
void npu_rdr_exception_report(uint32_t error_code)
{
	if (npu_rdr_exception_is_count_exceeding(error_code) == 0) {
		npu_drv_err("call rdr_system_error, error_code: %u\n", error_code);
...
#elif defined (CONFIG_DFX_SEC_QIC_MODID_REGISTER)
