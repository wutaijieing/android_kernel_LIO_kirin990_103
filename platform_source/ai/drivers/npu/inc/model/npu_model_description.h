...
typedef struct npu_model_desc {
	...
	u16 model_id;
	u16 stream_cnt;
	u64 aicore_stack_base;
	u64 aiv_csw_buff_addr; // aiv slow context switch buffer base addr
...
typedef struct npu_model_desc_info {
	...
	uint32_t model_id;
	uint32_t stream_cnt;
	uint64_t aicore_stack_base;
	uint64_t aiv_csw_buff_addr; // aiv slow context switch buffer base addr
