	...
	struct orb_undistort_cfg_t *undistort_cfg)
{
	...
	union U_UNDISTORT_FIRST    temp_first;
	union U_UNDISTORT_SECOND    temp_second;
	union U_UNDISTORT_THIRD    temp_third;
	...
	temp_first.bits.first = undistort_cfg->first;
	temp_second.bits.second = undistort_cfg->second;
	temp_third.bits.third = undistort_cfg->third;
	...
	CMDLST_SET_REG(dev->base_addr + ORB_UNDISTORT_INVFY_REG,
		temp_invfy.u32);
	CMDLST_SET_REG(dev->base_addr + ORB_UNDISTORT_FIRST_REG,
		temp_first.u32);
	CMDLST_SET_REG(dev->base_addr + ORB_UNDISTORT_SECOND_REG,
		temp_second.u32);
	CMDLST_SET_REG(dev->base_addr + ORB_UNDISTORT_THIRD_REG,
		temp_third.u32);
