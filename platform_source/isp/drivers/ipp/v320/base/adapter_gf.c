	...
	struct ipp_stripe_info_t *gf_stripe_info, unsigned int stripe_cnt)
{
	...
	cfg_tab->coeff_cfg.filter_val = req->gf_hw_cfg.coeff_cfg.filter_val;
	...
	struct msg_req_gf_request_t *ctrl_gf)
{
	...
	ctrl_gf->gf_hw_cfg.coeff_cfg.filter_val =
		adjust_y->adjust_col_gf.y_gf_filter_val;
...
static void gf_dump_request(struct msg_req_gf_request_t *req)
{
	...
	D("[GF_FILTER+COEFF] y_gf_radius = %d, y_gf_filter_val = %d\n",
		req->gf_hw_cfg.coeff_cfg.radius,
		req->gf_hw_cfg.coeff_cfg.filter_val);
#endif
}
