...
#define IPP_ORB_SIZE_MAX 2048
#define IPP_ORB_ENH_SIZE_MAX 1280
...
{
	if (enh_cfg_tab == NULL || req_orb == NULL) {
		E("failed : null pointer");
		return;
	}
	...
	unsigned int* orb_layer_stripe_num)
{
	...
	if (layer_stripe_tmp == 0 || orb_stripe_num <= 1) {
...
static int df_size_split_ipp_orb_stripe(struct ipp_stream_t *stream,
	struct ipp_orb_stripe_info_t *p_size_info)
{
	if (stream == NULL || p_size_info == NULL) {
		E(" failed : df_size_split_ipp_orb_stripe is null");
		return ISP_IPP_ERR;
	}
...
static int orb_check_size_para(struct msg_req_orb_request_t *req)
{
	unsigned int i = 0;
	...
	for (i = 0; i < orb_pyramid_layer; i++) {
		...
		if ((req->req_orb_cur.streams[i][BI_ORB_Y].width > IPP_ORB_SIZE_MAX)
		|| (req->req_orb_cur.streams[i][BI_ORB_Y].height > IPP_ORB_SIZE_MAX)
		|| (req->req_orb_cur.streams[i][BI_ORB_Y].buffer == 0)) {
			E(" req_orb_cur_streams is incorrect");
			return ISP_IPP_ERR;
		}

		if (req->work_frame == CUR_REF) {
			if ((req->req_orb_ref.streams[i][BI_ORB_Y].width > IPP_ORB_SIZE_MAX)
			|| (req->req_orb_ref.streams[i][BI_ORB_Y].height > IPP_ORB_SIZE_MAX)
			|| (req->req_orb_ref.streams[i][BI_ORB_Y].buffer == 0)) {
				E(" req_orb_ref_streams is incorrect");
				return ISP_IPP_ERR;
			}
		}
	}
...
static int enh_check_ref_size_para(struct msg_req_orb_request_t *req)
{
	if (req == NULL) {
		E(" failed : orb_request is null");
		return ISP_IPP_ERR;
	...
	if ((req->req_orb_ref.req_enh.streams[BI_ENH_YHIST].width > IPP_ORB_ENH_SIZE_MAX)
	|| (req->req_orb_ref.req_enh.streams[BI_ENH_YHIST].height > IPP_ORB_ENH_SIZE_MAX)
	|| (req->req_orb_ref.req_enh.streams[BI_ENH_YHIST].buffer == 0)) {
		E(" req_orb_enh_ref_streams is incorrect");
		return ISP_IPP_ERR;
	}

	if ((req->req_orb_ref.req_enh.streams[BI_ENH_YIMAGE].width > IPP_ORB_ENH_SIZE_MAX)
	|| (req->req_orb_ref.req_enh.streams[BI_ENH_YIMAGE].height > IPP_ORB_ENH_SIZE_MAX)
...
static int orb_enh_check_size_para(struct msg_req_orb_request_t *req)
{
	int ret = 0;
	if (req == NULL) {
		E(" failed : orb_request is null");
	...
	if ((req->req_orb_cur.req_enh.streams[BI_ENH_YHIST].width > IPP_ORB_ENH_SIZE_MAX)
	|| (req->req_orb_cur.req_enh.streams[BI_ENH_YHIST].height > IPP_ORB_ENH_SIZE_MAX)
	|| (req->req_orb_cur.req_enh.streams[BI_ENH_YHIST].buffer == 0)) {
		E(" req_orb_enh_cur_streams is incorrect");
		return ISP_IPP_ERR;
	}

	if ((req->req_orb_cur.req_enh.streams[BI_ENH_YIMAGE].width > IPP_ORB_ENH_SIZE_MAX)
	|| (req->req_orb_cur.req_enh.streams[BI_ENH_YIMAGE].height > IPP_ORB_ENH_SIZE_MAX)
	...
	if (req->work_frame == CUR_REF) {
		ret = enh_check_ref_size_para(req);
		if (ret != 0) {
			E(" enh_check_ref_size_para is incorrect");
			return ISP_IPP_ERR;
		}
	}
...
static int orb_check_request_para(struct msg_req_orb_request_t *req)
{
	...
	|| work_module == ORB_MATCHER_MC) {
		ret = orb_check_size_para(req);
		if (ret != 0) {
			E(" failed : orb_check_size_para");
			return ret;
		}
	}
	...
	if(work_module == ENH_ORB
		...
		ret = orb_enh_check_size_para(req);
		if (ret != 0) {
			E(" failed : orb_enh_check_size_para");
			return ret;
		}
	}
