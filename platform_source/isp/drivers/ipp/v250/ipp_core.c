...
int hispcpe_smmu_nontrus_init(void)
{
	...
	if (pgd_base == 0) {
		e("Failed : kernel_domain_get_ttbr\n");
		return -EINVAL;
	}
...
{
	if (dev->hipp_refs[REFS_TYP_MCF_VBK] > 0) {
		pr_err("[%s] : mcf_vbk refs Opened\n", __func__);
		return -ENOMEM;
	}
...
static int hispcpe_map_kernel_check_param(struct hispcpe_s *dev, struct memory_block_s *frame_buf)
{
	...
	if (dev->devbuf->size != (size_t)(frame_buf->size)) {
...
static int hispcpe_map_kernel(struct hispcpe_s *dev, unsigned long args, unsigned int *ipp_buff_size)
{
	...
	if ((ret = (int)(copy_from_user(&frame_buf, args_mapkernel, sizeof(struct memory_block_s)))) != 0) {
	...
	iova = kernel_iommu_map_dmabuf(&dev->pdev->dev, dev->devbuf, frame_buf.prot, &iova_size);
	if (iova == 0) {
		pr_err("[%s] Failed : mm_iommu_map_sg\n", __func__);
		goto err_dma_buf_get;
	}

	if ((unsigned long)(frame_buf.size) != iova_size) {
...
err_hisi_iommu_check:
	if ((ret = kernel_iommu_unmap_dmabuf(&dev->pdev->dev, dev->devbuf, iova)) < 0)
...
static int vbk_processv_vbk_req(struct hispcpe_s *dev, int *ret, msg_req_vbk_request_t *ctrl_vbk)
{
	if ((*ret = ipp_set_rate(dev)) != 0) {
		pr_err("[%s] Failed : ipp_set_rate.%d\n", __func__, *ret);
		return -1;
...
static int matcher_process_matcher_req(struct hispcpe_s *dev, msg_req_matcher_request_t *ctrl_matcher, int *ret)
{
						...
						unsigned long args, struct memory_block_s *buf)
{
	...
	case HISP_CPE_MCF_REQ:
		...
		dev->hipp_refs[REFS_TYP_MCF_VBK] = 1;
		mutex_unlock(&dev->dev_lock);

		if ((ret = mcf_process(dev, args)) != 0) {
			pr_err("[%s] Failed : hispcpe_mcf_request.%d\n", __func__, ret);
			dev->hipp_refs[REFS_TYP_MCF_VBK] = 0;
			return ret;
		}
	...
	unsigned long args, struct memory_block_s *buf)
{
	...
	case HISP_CPE_PWRUP:
		...
		ret = (int)(copy_from_user(&para, args_pwup, sizeof(para)));
		if (ret != 0) {
			pr_err("[%s] copy_from_user.%d\n", __func__, ret);
			return -EFAULT;
		}
