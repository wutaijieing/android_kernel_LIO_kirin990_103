...
static int mcf_cfg_cmdlst_buff(
	msg_req_mcf_request_t *mcf_request, cmdlst_para_t *cmdlst_para, mcf_config_table_t *mcf_cfg_tab,
	cfg_tab_cvdr_t *cvdr_cfg_tab, cpe_top_config_table_t *top_cfg_tab, isp_stripe_info_t *stripe_info,
	isp_stripe_info_t *stripe_info_ds4)
{
	unsigned int i = 0;
	...
	cpe_set_cmdlst_stripe(&cmdlst_para->cmd_stripe_info[0], stripe_cnt);
...
static int mcf_request_cpe_mem_get(msg_req_mcf_request_t *mcf_request, cmdlst_para_t **cmdlst_para,
	isp_stripe_info_t *stripe_info, isp_stripe_info_t *stripe_info_ds4)
{
	int ret = CPE_FW_OK;
	...
	if ((ret = cpe_mem_get(MEM_ID_CVDR_CFG_TAB_MM, &va, &da)) != 0) {
		...
		goto mcf_get_cvdr_cfg_tab_mem_fail;
	}
	...
	if ((ret = cpe_mem_get(MEM_ID_CPETOP_CFG_TAB_MM, &va, &da)) != 0) {
		...
		goto mcf_get_cpetop_cfg_tab_mem_fail;
	}
	...
	if ((ret = cpe_mem_get(MEM_ID_CMDLST_PARA_MM, &va, &da)) != 0) {
		...
		goto mcf_get_cmdlst_para_mem_fail;
	}
	*cmdlst_para = (cmdlst_para_t *)(uintptr_t)(va);

	mcf_cfg_cmdlst_buff(mcf_request, *cmdlst_para, mcf_cfg_tab,
						cvdr_cfg_tab, top_cfg_tab, stripe_info, stripe_info_ds4);

mcf_get_cmdlst_para_mem_fail:
	cpe_mem_free(MEM_ID_CPETOP_CFG_TAB_MM);
mcf_get_cpetop_cfg_tab_mem_fail:
	cpe_mem_free(MEM_ID_CVDR_CFG_TAB_MM);
mcf_get_cvdr_cfg_tab_mem_fail:
	cpe_mem_free(MEM_ID_MCF_CFG_TAB);
	return ret;
}

static int mcf_request_check_para(msg_req_mcf_request_t *mcf_request)
{
	if(mcf_request->mcf_cfg.mode != MCF_Y_GF_MODE) // only support GF
		return CPE_FW_ERR;

	y_gf_mode_t *para = (y_gf_mode_t *) & (mcf_request->u.y_lf_gf_mode_param);

	if(mcf_request->streams[BI_MONO].buf == 0) {
		e("[%s] mcf check input para err", __func__);
		return CPE_FW_ERR;
	}

	if(para->input_mode == DUAL_INPUT) {
		if(mcf_request->streams[BI_WARP_COL_Y_DS4].buf == 0) {
			e("[%s] mcf check input ds4 para err", __func__);
			return CPE_FW_ERR;
		}
	}

	if(mcf_request->streams[BO_RESULT_Y].buf == 0) {
		e("[%s] mcf check output y para err", __func__);
		return CPE_FW_ERR;
	}

	if(para->output_mode == A_B || para->output_mode == LF_HF) {
		if(mcf_request->streams[BO_RESULT_UV].buf == 0) {
			e("[%s] mcf check output uv para err", __func__);
			return CPE_FW_ERR;
		}
	}
...
int mcf_request_handler(msg_req_mcf_request_t *mcf_request)
{
	...
	ret = mcf_request_check_para(mcf_request);
	if (ret != 0) {
		e("[%s] Failed : mcf_request_handler check.%d\n", __func__, ret);
		return ret;
	}
	...
	if (mcf_request_cpe_mem_get(mcf_request, &cmdlst_para, stripe_info, stripe_info_ds4)) {
		ret = CPE_FW_ERR;
		goto mcf_fail_get_mem;
	}
	ipp_update_cmdlst_cfg_tab(cmdlst_para);
	loge_if(df_sched_start(cmdlst_para));
	i("mcf_request_handler done\n");
