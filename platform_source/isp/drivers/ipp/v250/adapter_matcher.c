...
static int matcher_check_request_para(msg_req_matcher_request_t *matcher_request)
{
	unsigned int i;

	for(i = 0; i < matcher_request->rdr_pyramid_layer; i++) {
		if(matcher_request->req_rdr.streams[i][BI_RDR_FP].buf == 0 ||
			matcher_request->req_rdr.streams[i][BO_RDR_FP_BLOCK].buf == 0 ) {
			e("[%s] failed : rdr request para err", __func__);
			return CPE_FW_ERR;
		}
	}
	for(i = 0; i < matcher_request->cmp_pyramid_layer; i++) {
		if(matcher_request->req_cmp.streams[i][BI_CMP_REF_FP].buf == 0 ||
			matcher_request->req_cmp.streams[i][BI_CMP_CUR_FP].buf == 0 ||
			matcher_request->req_cmp.streams[i][BO_CMP_MATCHED_OUT].buf == 0) {
			e("[%s] failed : cmp request para err", __func__);
			return CPE_FW_ERR;
		}
	}
...
int matcher_request_handler(msg_req_matcher_request_t *matcher_request)
{
	...
	ret = matcher_check_request_para(matcher_request);
	if(ret != CPE_FW_OK) {
		e("[%s] failed : matcher_check_request_para", __func__);
		return ret;
	}
	...
	for (i = 0; i < matcher_request->rdr_pyramid_layer; i++) {
		if (matcher_request->req_rdr.ctrl_cfg[i].total_kpt == 0) {
			e("Check rdr[%d].total_kpt is 0", i);
			return 1;
		}
	}
