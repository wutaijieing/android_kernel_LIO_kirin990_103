...
static int vbk_check_input_para(msg_req_vbk_request_t *vbk_request)
{
	if(vbk_request->streams[BI_VBK_SRC_Y].buf == 0) {
		e("vbk stream src y buf err");
		return CPE_FW_ERR;
	}

	if(vbk_request->streams[BI_VBK_SRC_Y_DS16].buf == 0) {
		e("vbk stream src y ds16 buf err");
		return CPE_FW_ERR;
	}

	if(vbk_request->streams[BI_VBK_SRC_UV].buf == 0) {
		e("vbk stream src uv buf err");
		return CPE_FW_ERR;
	}

	if(vbk_request->streams[BI_VBK_SRC_UV_DS16].buf == 0) {
		e("vbk stream src uv ds16 buf err");
		return CPE_FW_ERR;
	}

	if(vbk_request->streams[BI_VBK_SIGMA].buf == 0) {
		e("vbk stream sigma buf err");
		return CPE_FW_ERR;
	}

	if(vbk_request->vbk_hw_config.op_mode == VBK_BOKEH_MODE) {
		if(vbk_request->streams[BI_VBK_FOREMASK].buf == 0) {
			e("vbk stream foremask buf err");
			return CPE_FW_ERR;
		}
	}
	return CPE_FW_OK;
...
static int vbk_check_output_para(msg_req_vbk_request_t *vbk_request)
{
	if(vbk_request->streams[BO_VBK_RESULT_Y].buf == 0) {
		e("vbk stream result y buf err");
		return CPE_FW_ERR;
	}

	if(vbk_request->streams[BO_VBK_RESULT_UV].buf == 0) {
		e("vbk stream result uv buf err");
		return CPE_FW_ERR;
	}

	if(vbk_request->vbk_hw_config.op_mode == VBK_BOKEH_MODE) {
		if(vbk_request->streams[BO_VBK_RESULT_FOREMASK].buf == 0) {
			e("vbk stream result foremask buf err");
			return CPE_FW_ERR;
		}
	}
...
static int vbk_check_request_para(msg_req_vbk_request_t *vbk_request)
{
	if(vbk_request->vbk_hw_config.op_mode != VBK_BOKEH_MODE &&
	...
	if(vbk_check_input_para(vbk_request) != CPE_FW_OK) {
		e("vbk check input stream y err");
		return CPE_FW_ERR;
	}

	if(vbk_check_output_para(vbk_request) != CPE_FW_OK) {
...
int vbk_request_handler(msg_req_vbk_request_t *vbk_request)
{
	...
	if(vbk_check_request_para(vbk_request) != CPE_FW_OK) {
