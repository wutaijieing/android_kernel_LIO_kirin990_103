...
static int hisp_dynamic_memory_unmap(size_t addr, size_t size)
{
	struct hisp_mem_pool_s *dev = &g_hisp_mem_pool_info;
	struct iommu_domain *domain = NULL;
	size_t phy_len = 0;
				...
				unsigned int align)
{
	...
	if (iova == 0) {
		pr_err("[ISP Mem] Fail:hisp_dynamic_memory_map,da.0x%x, size.0x%x\n",
				da, size);
				...
				unsigned int size)
{
	...
	ret = hisp_dynamic_memory_unmap(iova, size);
	if (ret < 0)
		pr_err("[%s]Fail: hisp_dynamic_memory_unmap, iova.0x%x, size.0x%x\n",
			__func__, iova, size);
...
static void hisp_mempool_destroy(unsigned int pool_num)
{
	...
	if (hisp_smmuv3_mode()) {
		...
		if (pool_info->dynamic_mem_flag) {
			ret = hisp_dynamic_memory_unmap(pool_info->addr,
					...
					unsigned int align)
{
	...
	ispcpu_vaddr = iova;
	if ((flag == MAP_TYPE_RAW2YUV) && (!hisp_smmuv3_mode())) {
	...
	va = hisp_mempool_setup(sgl, ispcpu_vaddr, size, prot,
...
static int hisp_mempool_info_init(int use_mempool, unsigned int addr,
		unsigned int size)
{
	struct hisp_mem_pool_s *info = &g_hisp_mem_pool_info;
	int ret = 0;
