...
extern unsigned long *atf_fuzz_shmem;
			...
			int sequence)
{
	...
	if (t->kcov_area) {
		...
		// 4 byte kcov_enable_flag, 4 byte kcov_cpuid, 8 byte kcov_length
		WRITE_ONCE(((unsigned int *)(t->kcov_area))[0], 1);
		// reset coverage length
		WRITE_ONCE(((unsigned long *)(t->kcov_area))[1], 0);
	}
}
...
static void kcov_stop(struct task_struct *t)
{
	...
	if (t->kcov_area)
		WRITE_ONCE(((unsigned int *)(t->kcov_area))[0], 0);
	barrier();
	t->kcov = NULL;
	t->kcov_size = 0;
	t->kcov_area = NULL;
...
static int kcov_mmap(struct file *filep, struct vm_area_struct *vma)
{
	...
	if (!atf_fuzz_shmem) {
		pr_err("atf-fuzz: %s:%d, atf_fuzz_shmem is null\n", __func__, __LINE__);
		return -ENOMEM;
	}
	mutex_lock(&kcov->mutex_lock);
	...
	if (!kcov->area) {
		...
		res = remap_pfn_range(vma, vma->vm_start,
			((phys_addr_t)RESERVED_BL31_FUZZ_SMEM_BASE)>>PAGE_SHIFT,
			vma->vm_end - vma->vm_start, vma->vm_page_prot);
		if (res) {
			pr_err("atf-fuzz: remap_pfn_range failed! errno=%d\n", res);
			goto exit;
		}
		kcov->area = atf_fuzz_shmem;
		mutex_unlock(&kcov->mutex_lock);
		return 0;
	}
exit:
	mutex_unlock(&kcov->mutex_lock);
...
static bool kcov_check_handle(u64 handle, bool common_valid,
...
void kcov_remote_start(u64 handle)
{
	...
	kcov_debug("handle = %pK, context: %s\n", handle,
