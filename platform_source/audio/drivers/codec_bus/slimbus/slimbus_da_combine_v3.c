...
/* segment length */
static uint16_t sl_table[SLIMBUS_SCENE_CONFIG_MAX][DA_COMBINE_V3_BUS_TRACK_TYPE_MAX][SLIMBUS_CHANNELS_MAX] = {
	/* 6.144M FPGA */
	{
		{ 6, 6 },                    /* audio playback */
		{ 4, 4 },                    /* audio capture */
...
/* presence rate */
static enum slimbus_presence_rate pr_table[DA_COMBINE_V3_BUS_TRACK_TYPE_MAX] = {
...
/* channel index */
static uint8_t cn_table[DA_COMBINE_V3_BUS_TRACK_TYPE_MAX][SLIMBUS_CHANNELS_MAX] = {
...
/* source port number */
static uint8_t source_pn_table[DA_COMBINE_V3_BUS_TRACK_TYPE_MAX][SLIMBUS_CHANNELS_MAX] = {
...
/* sink port number */
static uint8_t sink_pn_table[DA_COMBINE_V3_BUS_TRACK_TYPE_MAX][SLIMBUS_CHANNELS_MAX] = {
...
static uint16_t sd_table[SLIMBUS_SCENE_CONFIG_MAX][DA_COMBINE_V3_BUS_TRACK_TYPE_MAX][SLIMBUS_CHANNELS_MAX] = {
	/* 6.144M FPGA */
	{
	...
	uint8_t source_la, uint8_t sink_la, enum slimbus_transport_protocol tp)
{
	...
	for (ch = 0; ch < track_cfg[track].params.channels; ch++) {
		...
		if (track == DA_COMBINE_V3_BUS_TRACK_ECREF) {
			pchprop->sink_num = 2;
...
static void slimbus_da_combine_v3_param_init(const struct slimbus_device_info *dev)
{
	...
	for (track = 0; track < DA_COMBINE_V3_BUS_TRACK_TYPE_MAX; track++) {
	...
	uint32_t track_type, struct scene_param *params)
{
	...
	if (track_type >= DA_COMBINE_V3_BUS_TRACK_TYPE_MAX) {
	...
	if (track_type == DA_COMBINE_V3_BUS_TRACK_VOICE_UP)
		dev->voice_up_chnum = params->channels;
	else if (track_type == DA_COMBINE_V3_BUS_TRACK_AUDIO_CAPTURE)
		dev->audio_up_chnum = params->channels;
	else if (track_type == DA_COMBINE_V3_BUS_TRACK_VOICE_DOWN)
...
static int32_t slimbus_da_combine_v3_create_device(struct slimbus_device_info **device)
{
	...
	dev_info->slimbus_da_combine_para->slimbus_track_max = DA_COMBINE_V3_BUS_TRACK_TYPE_MAX;
...
static void slimbus_da_combine_v3_get_st_params(struct slimbus_sound_trigger_params *params)
{
	if (params != NULL) {
		...
		params->channels = 2;
		params->track_type = DA_COMBINE_V3_BUS_TRACK_AUDIO_CAPTURE;
	}
}
