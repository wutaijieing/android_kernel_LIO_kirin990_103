...
static void slimbus_io_config(const struct slimbus_device_info *dev,
	enum slimbus_rf_type rf, void __iomem *ioc_base_addr)
{
	/* For FPGA */
...
void slimbus_utils_module_enable(const struct slimbus_device_info *dev, bool enable)
{
	...
	if (enable) {
		ioc_base_addr = ioremap(get_phy_addr(IOC_BASE_ADDR, PLT_IOC_MEM),
			IOC_REG_SIZE);
