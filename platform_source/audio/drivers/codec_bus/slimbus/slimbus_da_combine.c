		...
		uint32_t track_type, const struct scene_param *params)
{
	...
	if (params) {
		if (params->rate == SAMPLE_RATE_8K)
			pr_table[track_type] = SLIMBUS_PR_8K;
		else if (params->rate == SAMPLE_RATE_16K)
			pr_table[track_type] = SLIMBUS_PR_16K;
		else if (params->rate == SAMPLE_RATE_32K)
			pr_table[track_type] = SLIMBUS_PR_32K;
		else if (params->rate == SAMPLE_RATE_48K)
			pr_table[track_type] = SLIMBUS_PR_48K;
		else if (params->rate == SAMPLE_RATE_96K)
			pr_table[track_type] = SLIMBUS_PR_96K;
		else if (params->rate == SAMPLE_RATE_192K)
			pr_table[track_type] = SLIMBUS_PR_192K;
		else if (params->rate == SAMPLE_RATE_768K)
			pr_table[track_type] = SLIMBUS_PR_768K;
		else if (params->rate == SAMPLE_RATE_44K1)
			pr_table[track_type] = SLIMBUS_PR_44100;
		else if (params->rate == SAMPLE_RATE_88K2)
			pr_table[track_type] = SLIMBUS_PR_88200;
		else if (params->rate == SAMPLE_RATE_176K4)
