...
#ifndef __SLIMBUS_DRV_H__
#define __SLIMBUS_DRV_H__
...
extern int slimbus_drv_init(enum platform_type platform_type,
	const void *slimbus_reg, const void *asp_reg, const void *sctrl_reg, int irq);
...
extern int slimbus_drv_switch_framer(uint8_t laif, uint16_t nco, uint16_t nci,
	const struct slimbus_bus_config *bus_config);

extern int slimbus_drv_element_read(uint8_t target_la, uint16_t address,
	enum slimbus_slice_size slice_size, void *value_read);

extern int slimbus_drv_element_write(uint8_t target_la, uint16_t address,
	enum slimbus_slice_size slice_size, void *value_write);

extern int slimbus_drv_request_info(uint8_t target_la, uint16_t address,
	enum slimbus_slice_size slice_size, void *value_read);
...
extern int slimbus_drv_track_update(int cg, int sm, int track, struct slimbus_device_info *dev,
	uint32_t ch_num, struct slimbus_channel_property *channel);

extern void slimbus_drv_get_params_la(int track_type, uint8_t *source_la,
