...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0))
	cnf_msg->kernel_time_us = time.tv_nsec / MSEC_PER_SEC;
#else
...
static int fake_msg_proc(uintptr_t arg)
{
	struct socdsp_om_ap_data om_data_info;
	struct socdsp_om_ap_data *om_data_content = NULL;

	if (copy_from_user(&om_data_info, (void *)arg, sizeof(om_data_info))) {
		loge("copy om info from user fail\n");
		return ERROR;
	}

	if (om_data_info.data_len >= MAIL_LEN_MAX) {
		loge("data len is invalid %u more than %d\n",
			om_data_info.data_len, MAIL_LEN_MAX);
		return ERROR;
	}

	om_data_content = kzalloc(om_data_info.data_len +
		sizeof(om_data_info), GFP_KERNEL);
	if (!om_data_content) {
		loge("kzalloc failed\n");
		return -ENOMEM;
	}

	if (copy_from_user(om_data_content, (void *)arg, om_data_info.data_len +
			sizeof(om_data_info))) {
		loge("copy data from user fail\n");
		kfree(om_data_content);
		return ERROR;
	}

	dsp_misc_msg_process(om_data_content);
	kfree(om_data_content);

	return 0;
}

static int fake_wtd_proc(uintptr_t arg)
{
	struct socdsp_str_cmd *cmd = NULL;
	unsigned int cmd_len;
	int ret;

	(void)arg;

	cmd_len = sizeof(*cmd) + strlen("auto_reset") + 1;
	cmd = kzalloc(cmd_len, GFP_KERNEL);
	if (!cmd) {
		loge("alloc cmd buffer fail\n");
		return -ENOMEM;
	}

	cmd->msg_id = ID_AP_AUDIO_STR_CMD;
	cmd->str_len = strlen("auto_reset");
	strncpy(cmd->str, "auto_reset", strlen("auto_reset"));

	ret = (int)mailbox_send_msg(MAILBOX_MAILCODE_ACPU_TO_HIFI_MISC, cmd, cmd_len);
...
static const struct dsp_ioctl_cmd cmd_func[] = {
	...
	msg_func(HIFI_MISC_IOCTL_DISPLAY_MSG, socdsp_get_dmesg),
#endif
#ifdef CONFIG_DP_MULTI_CHANNELS
...
#endif
#ifdef CLT_VOICE
	msg_func(CLT_HIFI_MISC_IOCTL_SEND_VOICE, hifi_dsp_async_voice_cmd),
#endif
#ifdef ENABLE_AUDIO_KCOV
	msg_func(HIFI_MISC_IOCTL_KCOV_FAKE_MSG, fake_msg_proc),
	msg_func(HIFI_MISC_IOCTL_KCOV_FAKE_WTD, fake_wtd_proc),
#endif
};
