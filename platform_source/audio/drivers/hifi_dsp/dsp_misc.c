...
void dsp_watchdog_send_event(void)
{
	...
	if (om_get_resetting_state()) {
		loge("soc dsp reset, reset all system by reset option");
		BUG_ON(true);
	}
#endif
...
#ifdef ENABLE_HIFI_DEBUG
	{ ID_DSP_AP_HOOK_DATA_CMD, 0, socdsp_debug_data_handle },
#endif
#ifdef CONFIG_HUAWEI_AUDIO
...
#endif
#ifdef EXTERNAL_MODEM
	{ ID_HIFI_AP_OM_DATA_IND, HIFI_OM_WORK_PROXY, proxy_rev_data_handle },
	{ ID_HIFI_AP_PCIE_CLOSE_IND, HIFI_PCIE_CLOSE, proxy_rev_data_handle },
	{ ID_HIFI_AP_PCIE_REOPEN_IND, HIFI_PCIE_REOPEN, proxy_rev_data_handle },
#endif
...
#else
	phy_data_size = get_platform_mem_size(PLT_HIFI_MEM);
	g_misc_data.base_phy = (unsigned char *)get_platform_mem_base_addr(PLT_HIFI_MEM);
#endif
