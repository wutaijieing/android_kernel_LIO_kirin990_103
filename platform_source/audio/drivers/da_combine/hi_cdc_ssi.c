...
static int get_cdcssi_clk(struct device *dev, struct hi_cdcssi_priv *priv)
{
	...
	priv->codec_data_clk = devm_clk_get(dev, "clk_codecssi");
	if (IS_ERR(priv->codec_data_clk)) {
		AUDIO_LOGE("clk get: ssi clk not found");
		ret = PTR_ERR(priv->codec_data_clk);
		return ret;
	}

	ret = clk_prepare_enable(priv->codec_data_clk);
	if (ret != 0) {
...
{
	clk_disable_unprepare(priv->codec_data_clk);
}
...
static int hi_cdcssi_suspend(struct device *device)
{
	...
	AUDIO_LOGI("usage count: 0x%x status: 0x%x disable depth: %u clk: %d",
		...
		clk_get_enable_count(priv->codec_data_clk));
