...
#include <linux/platform_drivers/da_combine/hi_cdc_ssi.h>
...
#define CLK_DRV_FORCE 0x5
#define DATA_DRV_FORCE 0x5
#define SDI_DRV_FORCE 0x5
...
{
	return g_cdc_ctrl_priv != NULL ? g_cdc_ctrl_priv->cdc_ctrl.bus_sel : BUSTYPE_SELECT_SLIMBUS;
}
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0))
	ktime_get_boottime_ts64(&item->tm);
#else
...
static void record_reg_dump(struct hi_cdc_ctrl_priv *priv)
{
	...
	for (i = priv->rec_wr_idx; i < ARRAY_SIZE(priv->op_rec); i++) {
		item = &priv->op_rec[i];
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,4,0))
		AUDIO_LOGI("[%6lld.%09ld]%c: %08x, %08x", item->tm.tv_sec,
	...
	struct device *dev)
{
	...
	bus_sel = BUSTYPE_SELECT_SLIMBUS;
	if (!of_property_read_u32(dev->of_node, "bus-sel", &bus_sel)) {
		if (bus_sel >= BUSTYPE_SELECT_BUTT)
			bus_sel = BUSTYPE_SELECT_SLIMBUS;
	}
	priv->cdc_ctrl.bus_sel = bus_sel;
...
void hi_cdc_ioparam_read(struct platform_device *pdev, struct hi_cdc_ctrl_priv *priv)
{
	...
	priv->cdc_ctrl.clk_cdc_drv = CLK_DRV_FORCE;
	priv->cdc_ctrl.data_cdc_drv = DATA_DRV_FORCE;
	priv->cdc_ctrl.sdi_cdc_drv = SDI_DRV_FORCE;
	...
	if (of_property_read_u32(pdev->dev.of_node, "clk_io_driver",
		&clk_drv) == 0)
		priv->cdc_ctrl.clk_cdc_drv = clk_drv;

	if (of_property_read_u32(pdev->dev.of_node, "data_io_driver",
		&data_drv) == 0)
		priv->cdc_ctrl.data_cdc_drv = data_drv;

	if (of_property_read_u32(pdev->dev.of_node, "sdi_io_driver",
		&sdi_drv) == 0)
		priv->cdc_ctrl.sdi_cdc_drv = sdi_drv;
}
...
static int hi_cdc_get_regulator(struct device *dev, struct hi_cdc_ctrl_priv *priv)
{
	...
	if (of_property_read_u32(np, "ldo8_supply", &supply_val)) {
...
static int chip_reset(struct platform_device *pdev)
{
	...
	bool need_reset_irq = false;
	const char *codec_type = NULL;

	ret = of_property_read_string(node, "codec-sel", &codec_type);
	if(ret == 0 && (strcmp(codec_type, "da_combine_v5") == 0))
		need_reset_irq = true;
	...
	if (need_reset_irq) {
		...
		AUDIO_LOGI("irq gpio: %u", gpio_irq);
	...
	AUDIO_LOGI("reset gpio: %u", gpio_reset);
...
static int get_resource(struct platform_device *pdev, struct hi_cdc_ctrl_priv *priv)
{
	...
	ret = of_property_read_u32(np, "reg-8bit-begin-addr", &priv->reg_addr8_begin);
	if (ret != 0) {
		AUDIO_LOGE("read reg-8bit-begin-addr error");
		return ret;
	}

	ret = of_property_read_u32(np, "reg-8bit-end-addr", &priv->reg_addr8_end);
	if (ret != 0) {
...
static int hi_cdcctrl_probe(struct platform_device *pdev)
{
	...
	AUDIO_LOGI("controller probe ok, clk drv: %d, data drv: %d, sdi drv: %d, pm runtime support: %d",
		priv->cdc_ctrl.clk_cdc_drv, priv->cdc_ctrl.data_cdc_drv,
		priv->cdc_ctrl.sdi_cdc_drv, priv->cdc_ctrl.pm_runtime_support);
