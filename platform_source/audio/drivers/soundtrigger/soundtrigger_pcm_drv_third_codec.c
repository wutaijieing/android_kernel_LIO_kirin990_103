/*
 * soundtrigger_pcm_drv_113x.c
 *
 * soundtrigger_pcm_drv_third_codec is a kernel driver which is used to manager third_codec dma
 *
...
static void set_third_codec_dma_int_flag(struct soundtrigger_pcm *dma_drv_info)
{
...
static void init_fast_info(struct soundtrigger_pcm *dma_drv_info)
{
	dma_drv_info->dma_int_fast_right_flag = 0;
...
static void stop_third_codec_dma(struct soundtrigger_pcm *dma_drv_info)
{
...
static struct soundtrigger_pcm_ops g_dma_third_codec_ops = {
	.open = init_fast_info,
...
static int pcm_drv_third_codec_probe(struct platform_device *pdev)
{
	...
	set_pcm_ops(&g_dma_third_codec_ops);

	return 0;
}

static const struct of_device_id pcm_drv_third_codec_match[] = {
	{ .compatible = "hisilicon,pcm_drv_third_codec", },
	{},
};

static struct platform_driver pcm_drv_third_codec_driver = {
	.driver = {
		.name  = "pcm_drv_third_codec",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(pcm_drv_third_codec_match),
	},
	.probe = pcm_drv_third_codec_probe,
};

static int __init pcm_drv_third_codec_init(void)
{
	return platform_driver_register(&pcm_drv_third_codec_driver);
}
fs_initcall(pcm_drv_third_codec_init);

static void __exit pcm_drv_third_codec_exit(void)
{
	platform_driver_unregister(&pcm_drv_third_codec_driver);
}
module_exit(pcm_drv_third_codec_exit);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("pcm third_codec driver");
