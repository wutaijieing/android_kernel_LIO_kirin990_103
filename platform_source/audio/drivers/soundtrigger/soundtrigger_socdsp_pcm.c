	...
	uint32_t *elapsed_buf, uint32_t *elapsed_size)
{
	...
	uint32_t pyh_base_addr = get_platform_mem_base_addr(PLT_HIFI_MEM);
#endif
...
#else
		*fast_buf = AP_AUDIO_WAKEUP_RINGBUFFER_ADDR + pyh_base_addr;
		*fast_size = AP_AUDIO_WAKEUP_RINGBUFEER_SIZE;
		*elapsed_buf = AP_AUDIO_WAKEUP_CAPTURE_ADDR + pyh_base_addr;
		*elapsed_size = AP_AUDIO_WAKEUP_CAPTURE_SIZE;
	} else {
		*fast_buf = AP_AUDIO_LP_WAKEUP_RINGBUFFER_ADDR + pyh_base_addr;
		*fast_size = AP_AUDIO_LP_WAKEUP_RINGBUFEER_SIZE;
		*elapsed_buf = AP_AUDIO_LP_WAKEUP_CAPTURE_ADDR + pyh_base_addr;
		*elapsed_size = AP_AUDIO_LP_WAKEUP_CAPTURE_SIZE;
#endif
	}
}
