			      ...
			      struct arch_hw_breakpoint *hw)
{
	...
	/* Address Mask */
	if (attr->bp_addr_mask != ARM_WATCHPOINT_ADDR_MASK_0) {
		if ((attr->bp_addr_mask < ARM_WATCHPOINT_ADDR_MASK_3) ||
			(attr->bp_addr_mask > ARM_WATCHPOINT_ADDR_MASK_MAX))
	...
	/* Address Mask */
	hw->ctrl.mask = attr->bp_addr_mask;
	hw->ctrl.ssc = ARM_SSC_NON_SECURE;
#endif
...
#ifdef CONFIG_HAVE_HW_BREAKPOINT_ADDR_MASK
	/* Address Mask */
	if (hw->ctrl.mask != ARM_WATCHPOINT_ADDR_MASK_0) {
		if (hw->address & ((1<< hw->ctrl.mask) - 1))
