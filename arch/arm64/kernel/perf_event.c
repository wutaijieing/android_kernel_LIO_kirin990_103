...
static int armv8pmu_get_single_idx(struct pmu_hw_events *cpuc,
				    struct arm_pmu *cpu_pmu, struct perf_event *event)
{
	int idx;
	int num_events;
	int cpu;
				  ...
				  struct perf_event *event)
{
	...
	if (armv8pmu_event_is_64bit(event) &&
	    !armv8pmu_has_long_event(cpu_pmu))
		return	armv8pmu_get_chain_idx(cpuc, cpu_pmu, event);
	else
		return armv8pmu_get_single_idx(cpuc, cpu_pmu, event);
}
...
static void __armv8pmu_probe_pmu(void *info)
{
	...
	*num_events += 1;
	...
	cpumask_set_cpu(cpu, &probe->supported_cpus);
...
static int armv8pmu_probe_pmu(struct arm_pmu *cpu_pmu)
{
	...
	cpumask_clear(&probe.supported_cpus);
