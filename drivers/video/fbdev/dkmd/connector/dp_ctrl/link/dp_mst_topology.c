...
static void dptx_print_vcpid_table(struct dp_ctrl *dptx)
{
	...
	if (retval)
		dpu_pr_err("failed to dptx_read_bytes_from_dpcd Binfo, retval=%d", retval);
...
static int dptx_remote_i2c_read(struct dp_ctrl *dptx, uint8_t port, int port1)
{
	...
	if (len < (MST_MSG_BUF_LENGTH - 9)) {
		...
		dpu_pr_info(" msg length = %d", g_mst_msg.msg_len);
...
int dptx_sideband_get_up_req(struct dp_ctrl *dptx)
{
	...
	dpu_check_and_return(!sideband_msg, -EINVAL, err, "alloc failed!");
	...
	struct drm_dp_sideband_msg_reply_body *repmsg)
{
	...
	return true;
fail_len:
	dpu_pr_err("link addr reply parse length fail %d %d", idx, raw->curlen);
	return false;
}
...
int dptx_get_topology(struct dp_ctrl *dptx)
{
	...
	dpu_pr_info("--------------- Sending link_addr 0");
	retval = dptx_aux_msg_link_address(dptx, &raw, &rep, -1);
	if (retval)
		return retval;

	dpu_pr_info("NPORTS = %d", rep.u.link_addr.nports);
	for (i = 0; i < rep.u.link_addr.nports; i++)
		dpu_pr_info("input=%d, pdt=%d, pnum=%d, mcs=%d, ldps = %d, ddps=%d",
