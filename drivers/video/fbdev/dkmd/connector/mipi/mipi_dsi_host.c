...
static void mipi_transfer_lock_init(void)
{
...
static bool mipi_dsi_cmd_is_read(struct dsi_cmd_desc *cm)
{
...
int32_t mipi_dsi_lread_reg(uint32_t *out, struct dsi_cmd_desc *cm, uint32_t len, char *dsi_base)
{
	int32_t ret = 0;
	uint32_t i = 0;
 ...
 */
static int32_t mipi_dsi_cmd_add(struct dsi_cmd_desc *cm, char __iomem *dsi_base)
{
	int32_t len = 0;
