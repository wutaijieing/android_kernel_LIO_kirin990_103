	...
	struct device_node *parent_node)
{
	...
	conn_mgr->dss_lite1_regulator = devm_regulator_get(&pdev->dev, "regulator_dss_lite1");
	if (IS_ERR_OR_NULL(conn_mgr->dss_lite1_regulator)) {
		dpu_pr_err("get dss_lite1_regulator error\n");
		return PTR_ERR(conn_mgr->dss_lite1_regulator);
	}
...
static int32_t dpu_conn_manager_probe(struct platform_device *pdev)
{
	...
	dpu_pr_info("conn manager probe v741!\n");
...
static const struct of_device_id g_conn_manager_match_table[] = {
	{
		.compatible = "dkmd,connector_v741",
		.data = NULL,
	},
	{},
};
