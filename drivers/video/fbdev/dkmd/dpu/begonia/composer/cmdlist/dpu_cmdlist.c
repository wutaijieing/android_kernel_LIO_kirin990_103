...
static void dpu_cmdlist_config_on(char __iomem *dpu_base, uint32_t scene_id)
{
	...
	outp32(DPU_CMD_CMDLIST_CH_INTE_ADDR(dpu_base + DPU_CMDLIST_OFFSET, scene_id), 0xFFFFFFFF);
	outp32(DPU_CMD_CMDLIST_CH_INTC_ADDR(dpu_base + DPU_CMDLIST_OFFSET, scene_id), 0xFFFFFFFF);

	/* ch_enable */
	set_reg(DPU_CMD_CMDLIST_CH_CTRL_ADDR(dpu_base + DPU_CMDLIST_OFFSET, scene_id), 1, 1, 0);
	outp32(DPU_CMD_CMDLIST_CH_DBG_ADDR(dpu_base + DPU_CMDLIST_OFFSET, scene_id), 0);
...
void dpu_cmdlist_present_commit(char __iomem *dpu_base, uint32_t scene_id, uint32_t cmdlist_id)
{
	...
	dpu_cmdlist_config_on(dpu_base, scene_id);
