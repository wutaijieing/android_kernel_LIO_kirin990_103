	...
	if (pblock_info && (pblock_info->src_rect.w != 0) && (pblock_info->dst_rect.w != 0)) {
		...
		*dst_rect = wb_layer->dst_rect;
		...
		if (wb_layer->transform ==
			(DPU_FB_TRANSFORM_ROT_90 | DPU_FB_TRANSFORM_FLIP_V)) {
			temp.w = dst_rect->w;
...
int dpu_wb_scl_config(struct dpu_fb_data_type *dpufd,
	dss_wb_layer_t *wb_layer)
{
	...
	scl_info.h_ratio = SCF_INC_FACTOR;
	scl_info.v_ratio = SCF_INC_FACTOR;
	if (!dpufd || !wb_layer)
		return -EINVAL;
	...
	if (wb_scl_h_ratio_set(pblock_info, &scl_info, wb_layer, src_rect, dst_rect) < 0)
		return -EINVAL;
	if (wb_scl_v_ratio_set(pblock_info, &scl_info, wb_layer, src_rect, dst_rect) < 0)
		return -EINVAL;
