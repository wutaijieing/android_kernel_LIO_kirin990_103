...
#define DPU_FB_NUM	(3)
#define DPU_FB_XRES	(540)
#define DPU_FB_YRES	(960)

static struct fb_info *dpu_fbi;
...
static int dpu_fb_probe(struct platform_device *pdev)
{
	...
	var->xres = DPU_FB_XRES;
	var->yres = DPU_FB_YRES;
	var->xres_virtual = DPU_FB_XRES;
	var->yres_virtual = DPU_FB_YRES * DPU_FB_NUM;
	var->bits_per_pixel = bpp * 8;

	snprintf(fix->id, sizeof(fix->id), "dpufb_fake");
...
static int dpu_fb_remove(struct platform_device *pdev)
{
	...
	/* remove /dev/fb* */
	if (dpu_fbi)
		unregister_framebuffer(dpu_fbi);
...
static void __exit dpu_fb_exit(void)
{
