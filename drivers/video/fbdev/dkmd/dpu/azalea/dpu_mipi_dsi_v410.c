...
void mipi_init(struct dpu_fb_data_type *dpufd, char __iomem *mipi_dsi_base)
{
	...
	if (pinfo->mipi.phy_mode == CPHY_MODE) {
		...
		if ((g_fpga_flag==0) && (g_dss_version_tag & (FB_ACCEL_DPUV410 | FB_ACCEL_DSSV501))) {
			/*enable BTA*/
...
int mipi_dsi_clk_enable(struct dpu_fb_data_type *dpufd)
{
	...
	if (clk_tmp) {
		ret = clk_prepare(clk_tmp);
		if (ret) {
			DPU_FB_ERR("fb%d dss_pclk_pctrl_clk clk_prepare failed, error=%d!\n",
				dpufd->index, ret);
			return -EINVAL;
		}
		...
		ret = clk_enable(clk_tmp);
		if (ret) {
			DPU_FB_ERR("fb%d dss_pclk_pctrl_clk clk_enable failed, error=%d!\n",
				dpufd->index, ret);
			return -EINVAL;
		}
	}
