...
#include "overlay/rdma/dpu_rdma.h"
	...
	struct ov_block_rect_param *blk_rect_param)
{
	...
	for (i = 0; i < pov_h_block->layer_nums; i++) {
		layer = &(pov_h_block->layer_infos[i]);
		if ((((int32_t)(blk_rect->last_offset) + pov_req->wb_ov_rect.x) <= (layer->dst_rect.x + layer->dst_rect.w - 1)) && \
			(layer->dst_rect.x < ((int32_t)(blk_rect->current_offset) + pov_req->wb_ov_rect.x))) {
	...
	int *block_num, dss_rect_t *ov_block_rects[], bool has_wb_scl)
{
	...
	*block_num = 0;
	blk_rect_param.blk_rect.copybit_wch_scl = false;
	...
	for (blk_rect_param.blk_rect.current_offset = blk_rect_param.blk_rect.block_size;
		blk_rect_param.blk_rect.last_offset < (uint32_t)(blk_rect_param.blk_rect.w);  /*lint !e574*/
	...
	dss_rect_t *dst_cross_rect, struct block_layer_info *blk)
{
	...
	if (h_layer->src_rect.w < h_layer->dst_rect.w ||
		...
		if (blk->scf_read_end > DSS_WIDTH((uint32_t)(blk->src_rect.w)))  /*lint !e574*/
			blk->scf_read_end = DSS_WIDTH((uint32_t)(blk->src_rect.w));
	...
	dss_layer_t *h_v_layer, struct block_layer_info *blk)
{
	...
	if (blk->scf_read_end > DSS_WIDTH((uint32_t)(h_layer->src_rect.w)))  /*lint !e574*/
		blk->scf_read_end = DSS_WIDTH((uint32_t)(h_layer->src_rect.w));
	...
	dss_layer_t *h_layer, dss_rect_t ov_block_rect, struct wb_block_layer_info *wb_blk)
{
	...
	if ((h_layer->src_rect.w != wb_blk->scf_dst_w) && (DSS_WIDTH(wb_blk->scf_dst_w) != 0)) {
		...
		if (wb_blk->scf_read_end > (uint32_t)(DSS_WIDTH(h_layer->src_rect.w))) /*lint !e574*/
			wb_blk->scf_read_end = (uint32_t)(DSS_WIDTH(h_layer->src_rect.w));
	...
	dss_rect_t ov_block_rect, dss_overlay_t *pov_req_h_v, int block_num)
{
	...
	dpu_check_and_return((ov_block_rect.w == 0 || ov_block_rect.h == 0), -1, ERR,
