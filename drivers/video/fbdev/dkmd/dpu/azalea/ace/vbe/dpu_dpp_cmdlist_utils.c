...
static void dpp_dump_cmdlist_node_items(const struct cmd_item *item, uint32_t count)
{
	...
	for (index = 0; index < count; index += inter) {
		...
		if ((addr & 0xFF000) >= 0xDE000 && (addr & 0xFF000) <= 0xF7000 &&
			...
			continue;
		DPU_FB_DEBUG("set addr:0x%x value:0x%x add1:0x%x value:0x%x "
	...
	uint32_t item_num, uint8_t region, uint8_t buffer)
{
	...
	DPU_FB_DEBUG("\t qos  | flag | pending | tast_end | last  | event_list | list_addr  | next_list  | count | "
		"id | is_used | exec| intr | nop\n");
	DPU_FB_DEBUG("\t------+---------+------------+------------+------------+------------\n");
	DPU_FB_DEBUG("\t 0x%2x | 0x%2x |0x%6x | 0x%6x | 0x%3x | 0x%8x | 0x%8x | 0x%8x | 0x%3x | 0x%1x| 0x%4x  | 0x%1x | "
		"0x%1x | 0x%x\n",
	...
	struct dss_rect roi)
{
	...
	if (is_roi_valid) {
		if (roi.y == 0) {
			...
			dpufd->dpp_cmdlist_isr_vactive_cnt = dpufd->panel_info.yres - roi.h;
	...
	uint8_t config_region)
{
	...
	if (g_debug_dpp_cmdlist_debug > 0)
		DPU_FB_INFO("config_dpp=%d, buffer_rd=%d, stutas=0x%x, config_region=%d",
			config_dpp, buffer_rd,
			dpufd->dpp_cmdlist_data[config_dpp][buffer_rd]->buffer_status, config_region);
	...
	if (dpp_cmdlist_config_reset(dpufd, reset) == 0) {
		dpp_cmdlist_node_dump(dpufd, 0, config_dpp, buffer_rd);
		if (dpp_cmdlist_config_start(dpufd, config_dpp, buffer_rd) != 0)
	...
	uint32_t modules, struct dpp_buf_info *buf_info, bool in_roi)
{
	...
	dpu_check_and_return(buf_info == NULL, -EINVAL, ERR, "dpufd is NULL\n");
