	...
	dss_vote_cmd_t target_vote)
{
	...
	if (get_dss_clock_value(dpe_get_voltage_level(current_peri_voltage), &supportable_cmd))
		return false;
...
static int dss_core_clk_enable(struct dpu_fb_data_type *dpufd)
{
	...
	if (dpufd->dss_pri_clk != NULL) {
		ret = enable_disp_clk(dpufd->dss_pri_clk, dpufd->dss_pri_clk_name);
		dpu_check_and_return((ret != 0), -EINVAL, ERR,
			"fb%d dss_pri_clk clk_enable failed, error=%d!\n", dpufd->index, ret);
	}

	if (dpufd->dss_axi_clk != NULL) {
		ret = enable_disp_clk(dpufd->dss_axi_clk, dpufd->dss_axi_clk_name);
		dpu_check_and_return((ret != 0), -EINVAL, ERR,
			"fb%d dss_axi_clk enable failed, error=%d!\n", dpufd->index, ret);
	}

	if (dpufd->dss_mmbuf_clk != NULL) {
		ret = enable_disp_clk(dpufd->dss_mmbuf_clk, dpufd->dss_mmbuf_clk_name);
		dpu_check_and_return((ret != 0), -EINVAL, ERR,
...
{
	if (dpufd->dss_pri_clk != NULL)
		disable_disp_clk(dpufd->dss_pri_clk, dpufd->dss_pri_clk_name);

	if (dpufd->dss_axi_clk != NULL)
		disable_disp_clk(dpufd->dss_axi_clk, dpufd->dss_axi_clk_name);

	if (dpufd->dss_mmbuf_clk != NULL)
		disable_disp_clk(dpufd->dss_mmbuf_clk, dpufd->dss_mmbuf_clk_name);
}
	...
	dss_vote_cmd_t target_vote)
{
	...
	ret = set_disp_clk_rate(dpufd->dss_pri_clk, target_vote.dss_pri_clk_rate, dpufd->dss_pri_clk_name);
	...
	ret = set_disp_clk_rate(dpufd->dss_mmbuf_clk, target_vote.dss_mmbuf_rate, dpufd->dss_mmbuf_clk_name);
	dpu_check_and_return((ret < 0), -1, ERR,
...
int dpufb_set_mmbuf_clk_rate(struct dpu_fb_data_type *dpufd)
{
	...
	ret = set_disp_clk_rate(dpufd->dss_mmbuf_clk, vote_cmd_tmp.dss_mmbuf_rate, dpufd->dss_mmbuf_clk_name);
	if (ret < 0) {
...
int dpe_set_clk_rate(struct platform_device *pdev)
{
	...
	ret = set_disp_clk_rate(dpufd->dss_pri_clk, DEFAULT_DSS_CORE_CLK_RATE_L3, dpufd->dss_pri_clk_name);
	if (ret < 0) {
...
int dpe_set_common_clk_rate_on_pll0(struct dpu_fb_data_type *dpufd)
{
	...
	ret = set_disp_clk_rate(dpufd->dss_mmbuf_clk, clk_rate, dpufd->dss_mmbuf_clk_name);
	if (ret < 0) {
		DPU_FB_ERR("fb%d dss_mmbuf clk_set_rate(%llu) failed, error=%d!\n",
	...
	ret = set_disp_clk_rate(dpufd->dss_pri_clk, clk_rate, dpufd->dss_pri_clk_name);
	if (ret < 0) {
		DPU_FB_ERR("fb%d dss_pri_clk clk_set_rate(%llu) failed, error=%d!\n",
			dpufd->index, clk_rate, ret);
		ret = -EINVAL;
