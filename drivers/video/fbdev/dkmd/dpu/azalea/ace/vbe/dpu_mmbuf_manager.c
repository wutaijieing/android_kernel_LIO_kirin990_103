	...
	for (i = 0; i < DSS_CHN_MAX_DEFINE; i++) {
		for (j = 0; j < DSS_CHN_MAX_DEFINE; j++) {
			if ((((offline_mmbuf[i].addr < online_mmbuf[j].mmbuf.addr + online_mmbuf[j].mmbuf.size) &&
				...
				(online_mmbuf[j].mmbuf.addr >= offline_mmbuf[i].addr))) && offline_mmbuf[i].size && use_comm_mmbuf) {
	...
	int *use_comm_mmbuf, dss_mmbuf_t *offline_mmbuf, bool has_rot)
{
	...
	dpu_check_and_no_retval(!use_comm_mmbuf, ERR, "use_comm_mmbuf is null pointer\n");
	...
	if (g_debug_dump_mmbuf > 0 && use_comm_mmbuf != NULL) {
		for (i = 0; i < DSS_CHN_MAX_DEFINE; i++)
