...
static struct kswapd_reclaim_info g_kswapd_info;
...
struct kswapd_reclaim_info *get_kswapd_reclaim_info(void)
{
	return &g_kswapd_info;
}
...
static void reclaimacct_end(struct reclaim_acct *ra, int type,
	const char *func_name, const struct shrinker *shrinker)
{
	u64 now, ns, start;
	...
	ns = now - start;
	if (ns < DELAY_LV5 || current_is_kswapd()) {
		ra->delay[type] += ns;
		ra->count[type]++;
	}
...
void reclaimacct_directreclaim_end(void)
{
	...
	reclaimacct_end(current->reclaim_acct, RA_DIRECTRECLAIM, NULL, NULL);
	...
	(void)memset_s(current->reclaim_acct, sizeof(struct reclaim_acct),
		0, sizeof(struct reclaim_acct));
	reclaimacct_free(current->reclaim_acct);
	current->reclaim_acct = NULL;
}

void reclaimacct_kswapd_start(void)
{
	...
	if (!current->reclaim_acct) {
		current->reclaim_acct = kzalloc(sizeof(struct reclaim_acct),
			GFP_KERNEL);
		if (!current->reclaim_acct)
			return;
	}

	current->reclaim_acct->start[RA_KSWAPDRECLAIM] = ktime_get_ns();
}

static void collect_kswapd_reclaim_info(void)
{
	int i;

	for (i = 0; i < NR_RA_STUBS; i++) {
		g_kswapd_info.time[i] += current->reclaim_acct->delay[i];
		g_kswapd_info.freed[i] += current->reclaim_acct->freed[i];
	}
}

void reclaimacct_kswapd_end(void)
{
	if (!current_is_kswapd() || !current->reclaim_acct)
		return;

	reclaimacct_end(current->reclaim_acct, RA_KSWAPDRECLAIM, NULL, NULL);
	current->reclaim_acct->freed[RA_KSWAPDRECLAIM] =
		current->reclaim_acct->freed[RA_SHRINKFILE] +
		current->reclaim_acct->freed[RA_SHRINKANON];

	collect_kswapd_reclaim_info();
	(void)memset_s(current->reclaim_acct, sizeof(struct reclaim_acct),
		0, sizeof(struct reclaim_acct));
}

void reclaimacct_kswapd_destroy(void)
{
	if (!current_is_kswapd() || !current->reclaim_acct)
		return;
...
void reclaimacct_drainallpages_end(void)
{
	...
	reclaimacct_end(current->reclaim_acct, RA_DRAINALLPAGES,
		__func__, NULL);
}
...
void reclaimacct_shrinklist_end(int file, unsigned long freed)
{
	int type;
	...
	reclaimacct_end(current->reclaim_acct, type, __func__, NULL),
	current->reclaim_acct->freed[type] += freed;
}
...
void reclaimacct_shrinkslab_end(const struct shrinker *shrinker,
	unsigned long freed)
{
	if (!current->reclaim_acct)
		return;

	current->reclaim_acct->freed[RA_SHRINKSLAB] += freed;
	reclaimacct_end(current->reclaim_acct, RA_SHRINKSLAB,
		__func__, shrinker);
}
