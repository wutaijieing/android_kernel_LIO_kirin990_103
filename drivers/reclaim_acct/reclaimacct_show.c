...
static int kswapd_reclaim_info_proc_show(struct seq_file *m, void *v)
{
	int i;
	struct kswapd_reclaim_info *p = NULL;
	const char *stub_name[NR_RA_STUBS] = {
		"total_process", "",
		"shrink_file  ",
		"shrink_anon  ",
	...
	p = get_kswapd_reclaim_info();

	seq_puts(m, "stage         time    freed\n");
	for (i = 0; i < NR_RA_STUBS; i++) {
		if (i == RA_DRAINALLPAGES)
			continue;
...
static int kswapd_reclaim_info_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, kswapd_reclaim_info_proc_show, NULL);
}

static const struct proc_ops kswapd_reclaim_info_proc_fops = {
	.proc_open = kswapd_reclaim_info_proc_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
...
static int __init proc_reclaimacct_init(void)
{
	...
	proc_create("kswapd_reclaim_info", 0440, NULL,
		&kswapd_reclaim_info_proc_fops);
	return 0;
}
