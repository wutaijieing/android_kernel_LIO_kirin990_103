...
static bool hyperhold_pre_enable(void)
{
	...
	set_fs(KERNEL_DS);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	if (!ksys_access(HP_PARTITION_PATH, 0)) {
#else
...
void hyperhold_init(struct zram *zram)
{
	...
	set_fs(KERNEL_DS);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	ret = ksys_access(HP_PARTITION_PATH, 0);
#else
...
static int hyperhold_write_test_thread(void *data)
{
	...
	/* 2. open file */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	fd = (int)ksys_open(HP_FILE_PATH, O_WRONLY, S_IRUSR | S_IWUSR | S_IWGRP);
#else
	...
	/* 3. write data */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	ret = (int)ksys_lseek((unsigned int)fd, 0, SEEK_SET);
#else
