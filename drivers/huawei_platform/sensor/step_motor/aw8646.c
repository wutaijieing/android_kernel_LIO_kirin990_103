 ...
 * @return void.
 */
static void aw8646_step_stop(struct motor_drv_data *di)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
...
static void start_motor(struct motor_drv_data *di)
{
	if (!di) {
		hwlog_err("%s di is null\n", __func__);
		return;
...
static int aw8646_probe(struct platform_device *pdev)
{
	...
	di->motor_wake_lock = wakeup_source_register(di->pdev, "motor_wakelock");
#else
	di->motor_wake_lock = wakeup_source_register("motor_wakelock");
#endif
...
aw8646_sysfs_fail:
	...
	wakeup_source_unregister(di->motor_wake_lock);
	di->motor_wake_lock = NULL;
aw8646_init_fail:
