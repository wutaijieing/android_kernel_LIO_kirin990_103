...
static void gpio_keyplay_timer(struct timer_list* t)
{
	struct huawei_gpio_key *gpio_key = from_timer(gpio_key, t, key_play_timer);
	if (!gpio_key) {
...
static int huawei_gpio_key_probe(struct platform_device* pdev)
{
	...
	if (support_play_key) {
		...
		play_key_lock = wakeup_source_register(&pdev->dev, "key_play_wake_lock");
#else
		play_key_lock = wakeup_source_register("key_play_wake_lock");
#endif
	}
	...
	if (support_play_key) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
		timer_setup(&(gpio_key->key_play_timer), gpio_keyplay_timer, 0);
#else
...
err_gpio_play_req:
	...
	if (support_play_key) {
		wakeup_source_unregister(play_key_lock);
		play_key_lock = NULL;
	}
