			     ...
			     uint16_t devid, const uint8_t *certz)
{
	...
	nxp_sha224_update(&context, &certz[A1007_CERTZ_SUBCN_OFF],
			A1007_CERT_SUBCN_SIZE);
	idx = A1007_CERT_SUBCN_OFF + A1007_CERT_SUBCN_SIZE;
	nxp_sha224_update(&context, &cert_template[idx],
			A1007_CERT_PUBKEY_OFF - idx);
	nxp_sha224_update(&context, &certz[A1007_CERTZ_PUBKEY_OFF],
			A1007_CERT_PUBKEY_SIZE);
	nxp_sha224_final(&context, digest);
}
