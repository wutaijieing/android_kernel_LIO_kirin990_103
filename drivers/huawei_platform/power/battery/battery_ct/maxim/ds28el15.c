	...
	enum batt_safe_info_t type, void *value)
{
	...
	power_wakeup_unlock(ds28el15.write_lock, true);
...
static int ds28el15_driver_probe(struct platform_device *pdev)
{
	...
	ds28el15.write_lock = power_wakeup_source_register(&pdev->dev, pdev->name);
...
ds28el15_probe_fail:
	power_wakeup_source_unregister(ds28el15.write_lock);
	return DS28EL15_FAIL;
}
