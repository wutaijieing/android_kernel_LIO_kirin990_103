...
static int dp_get_pd_event_result_in_out(char *buffer, int size)
{
	...
	for (i = 0; i < count; i++) {
		dp_timespec64_to_datetime_str(typec_in_out->in_time[i],
			"tpyec_in", datetime, DP_DSM_DATETIME_SIZE);
		ret += dp_debug_append_info(buffer, size,
			"[%02d] %-19s", i, datetime);

		dp_timespec64_to_datetime_str(typec_in_out->out_time[i],
	...
	char *buffer, int size)
{
	...
	list_for_each(pos, &(priv->event_list_head)) {
		...
		dp_timespec64_to_datetime_str(event->time,
	...
	char *buffer, int size)
{
	...
	dp_timespec64_to_datetime_str(priv->hdcp_time,
	...
	char *buffer, int size, char *datetime, int count)
{
	...
	dp_timespec64_to_time_str(hotplug->time[index],
