...
int tcpci_event_init(struct tcpc_device *tcpc_dev)
{
	...
	tcpc_dev->tcpci_event_wakelock = wakeup_source_register(&tcpc_dev->dev, "tcpci_event_wakelock");
	if (!tcpc_dev->tcpci_event_wakelock) {
...
int tcpci_event_deinit(struct tcpc_device *tcpc_dev)
{
	...
	wakeup_source_unregister(tcpc_dev->tcpci_event_wakelock);
	return 0;
}
