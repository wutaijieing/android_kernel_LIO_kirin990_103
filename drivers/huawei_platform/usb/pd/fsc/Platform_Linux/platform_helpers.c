...
#ifdef FSC_INTERRUPT_TRIGGERED
    wakeup_source_unregister(chip->fusb302_wakelock);
#endif // FSC_INTERRUPT_TRIGGERED
...
FSC_S32 fusb_EnableInterrupts(void)
{
    ...
    chip->fusb302_wakelock = wakeup_source_register(&chip->client->dev, "fusb302wakelock");
    if (!chip->fusb302_wakelock) {
        dev_err(&chip->client->dev, "%s wakeup source register failed\n", __func__);
        return -ENOMEM;
    }
