...
static void rt8979_auto_restart_accp_det(void)
{
	...
	hwlog_info("%s-%d +++\n", __func__, __LINE__);
	if ((switch_read_reg(di->client, FSA9685_REG_CONTROL2) &
		FSA9685_ACCP_AUTO_ENABLE) == 0) {
...
#endif
		if ((usb_switch_wakelock_flag == USB_SWITCH_NEED_WAKE_UNLOCK) &&
			(invalid_times == 0))
			switch_usb_handover_wake_unlock();
#ifdef CONFIG_TCPC_CLASS
	}
...
static int fsa9685_parse_dts(struct device_node* np, struct fsa9685_device_info *di)
{
	...
	ret = of_property_read_u32(np, "dcd_timeout_force_enable",
	...
	const struct i2c_device_id *id)
{
	...
	di->usb_switch_lock = wakeup_source_register(new_dev, "usb_switch_wakelock");
	if (!di->usb_switch_lock) {
