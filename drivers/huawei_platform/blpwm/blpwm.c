...
void blpwm_notify_by_sns(pkt_blpwm_sensor_req_t *blpwm_event)
{
	...
	g_blpwm_dev->p_ops->notiby_by_sns(blpwm_event);
}

static int blpwm_cover_detect(void)
{
	if (!g_blpwm_dev->p_ops->cover_detect)
		return -1;

	return g_blpwm_dev->p_ops->cover_detect();
}

static void blpwm_set_duty(int duty)
{
	if (!g_blpwm_dev->p_ops->set_duty)
		return;

	g_blpwm_dev->p_ops->set_duty(duty);
}

static void blpwm_set_usage(int usage)
{
	if (!g_blpwm_dev->p_ops->set_usage)
		return;

	g_blpwm_dev->p_ops->set_usage(usage);
}

static void blpwm_set_usage_stop(int usage_stop)
{
	if (!g_blpwm_dev->p_ops->set_usage_stop)
		return;

	g_blpwm_dev->p_ops->set_usage_stop(usage_stop);
}

static void blpwm_set_gesture(int gesture_status)
{
	if (!g_blpwm_dev->p_ops->set_gesture)
		return;

	g_blpwm_dev->p_ops->set_gesture(gesture_status);
}
...
bool blpwm_is_support(void)
{
	...
	if (g_blpwm_dev->cover_status == COVER_INFO_NOT_READ) {
		...
		g_blpwm_dev->support = g_blpwm_dev->p_ops->cover_detect();
...
static long blpwm_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_COVER_LOOKUP_CMD:
		...
		break;
	case IOCTL_SET_GESTURE_CMD:
		if (copy_from_user(&g_blpwm_dev->gesture_status, (void __user *)arg, sizeof(int))) {
			hwlog_err("%s: copy from user fail\n", __func__);
			return -EFAULT;
		}
		hwlog_info("g_blpwm_dev->gesture_status is:%d\n", g_blpwm_dev->gesture_status);
		blpwm_set_gesture(g_blpwm_dev->gesture_status);
		break;
	default:
