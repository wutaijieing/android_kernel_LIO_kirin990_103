...
static void bl_sns_set_gesture(int gesture_status)
{
	if (!blpwm_is_support())
		return;

	if (gesture_status)
		blpwm_config_sensorhub(SUB_CMD_BLPWM_GESTURE_OPEN);
	else
		blpwm_config_sensorhub(SUB_CMD_BLPWM_GESTURE_CLOSE);
}
...
static struct blpwm_ops g_ops = {
	.cover_detect = bl_sns_cover_detect,
	.set_duty = bl_sns_set_duty,
	.set_usage = bl_sns_set_usage,
	.set_usage_stop = bl_sns_set_usage_stop,
	.notiby_by_sns = bl_sns_notify,
	.set_gesture = bl_sns_set_gesture,
};
