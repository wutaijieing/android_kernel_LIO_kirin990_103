...
#define IOCTL_SET_GESTURE_CMD          _IO('Q', 0x10)
...
struct blpwm_ops {
	int (*cover_detect)(void);
	void (*set_duty)(int duty);
	void (*set_usage)(int usage);
	void (*set_usage_stop)(int usage_stop);
	void (*notiby_by_sns)(pkt_blpwm_sensor_req_t *blpwm_event);
	void (*set_gesture)(int gesture_status);
};
...
static inline void blpwm_open_sensorhub(void)
{
	return;
}
...
static inline void blpwm_close_sensorhub(void)
{
	return;
}
