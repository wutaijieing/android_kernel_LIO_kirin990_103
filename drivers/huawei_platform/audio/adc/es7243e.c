...
static int es7243e_get_adc_array(struct device_node *node,
				const char *name, void *out_array)
{
	int ret;
	int num = of_property_count_u32_elems(node, name);
	if (num < 0) {
		hwlog_err("module %s  of_property_count_u32_elems fail!\n", name);
		return -1;
	}
...
static int es7243e_i2c_write(struct i2c_client *client, u8 adc_addr, u8 addr, u8 value)
{
	...
	hwlog_info("%s [0x%02x], [0x%02x] = 0x%02x\n", __func__, adc_addr, addr, value);
