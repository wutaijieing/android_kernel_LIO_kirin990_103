...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
static void st_nfc_record_dmd_probe_timerhandler(struct timer_list *data)
#else
...
static int st21nfc_loc_set_polaritymode(struct st21nfc_dev *st21nfc_dev, unsigned int mode)
{
			 ...
			 const struct i2c_device_id *id)
{
	...
	client->adapter->timeout = (int)msecs_to_jiffies(3 * 10);	/* 30ms */
