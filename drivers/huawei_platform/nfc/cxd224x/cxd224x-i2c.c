...
#ifdef CONFIG_WAKELOCK
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	cxd224x_dev->wakelock = wakeup_source_register(&client->dev, CXD224X_WAKE_LOCK_NAME);
	cxd224x_dev->wakelock_lp = wakeup_source_register(&client->dev, CXD224X_WAKE_LOCK_NAME_LP);
#else
...
#ifdef CONFIG_WAKELOCK
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	wakeup_source_unregister(cxd224x_dev->wakelock);
	cxd224x_dev->wakelock = NULL;
	wakeup_source_unregister(cxd224x_dev->wakelock_lp);
	cxd224x_dev->wakelock_lp = NULL;
#else
