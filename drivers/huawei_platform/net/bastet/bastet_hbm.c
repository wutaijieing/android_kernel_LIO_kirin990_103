...
static int bastet_drop_hb_reply(struct sock *sk, struct msghdr *msg, int len)
{
...
#else
	if (!access_ok(msg->msg_iter.iov->iov_base, len))
#endif
		return -EFAULT;
	min_len = len > (signed)bsk->hbm.reply_len ? (signed)bsk->hbm.reply_len : len;
	...
	if (bsk->hbm.reply_content) {
		...
		if (!not_matched) {
			 ...
			 */
			if (bsk->hbm.reply_offset == (signed)bsk->hbm.reply_len) {
