...
#else
	enum ifs_direction direction, int proto, int bytes)
#endif
{
}
...
#else
	case IFS_PROTOCOL_TCP:
		return IPPROTO_TCP;
	case IFS_PROTOCOL_UDP:
		return IPPROTO_UDP;
	case IFS_PROTOCOL_OTHER:
#endif
...
#else
		return IFS_PROTOCOL_TCP;
	case IPPROTO_UDP:
		return IFS_PROTOCOL_UDP;
	default:
		bastet_loge("other protocol");
		return IFS_PROTOCOL_OTHER;
#endif
	}
}
...
#else
	bastet_update_if_tag_stat(name, uid, sk, IFS_DIRECTION_TX, proto, tx_bytes);
	bastet_update_if_tag_stat(name, uid, sk, IFS_DIRECTION_RX, proto, rx_bytes);
#endif
}
...
static int adjust_traffic_flow_generic(struct bst_traffic_flow_prop *flow_p)
{
	...
	tx_bytes = flow_p->tx;
	rx_bytes = flow_p->rx;
	protocol = (signed)flow_p->protocol;
	uid = (signed)flow_p->uid;
...
void bastet_wait_traffic_flow(void)
{
	...
	wait_event_interruptible_timeout(bastet_flow_queue,
		bastet_flow_wait, (signed)msecs_to_jiffies(BST_FLOW_WAIT_TIMEOUT));
	bastet_flow_wait = 0;
}
