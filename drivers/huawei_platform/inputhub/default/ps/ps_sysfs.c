...
static void ps_test_name_join_with_device(void)
{
	...
	for (i = 0; i < PS_CAL_JOIN_NUM; i++) {
		ret = strcat_s(ps_test_with_device_name[i], sizeof(char), "_");
		ret += strncat_s(ps_test_with_device_name[i],
			PS_TEST_NAME_MAX_LEN - strlen(ps_test_with_device_name[i]) - 1,
			device_name,
			strlen(device_name));
		if (ret)
