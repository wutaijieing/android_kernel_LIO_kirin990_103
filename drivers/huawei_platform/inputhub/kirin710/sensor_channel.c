...
static const struct sensors_cmd_map sensors_cmd_map_tab[] = {
	...
	{ SENSORHUB_TYPE_RELATIVE_HUMIDITY, TAG_HUMITURE },
	{ SENSORHUB_TYPE_TOF, TAG_TOF },
	{ SENSORHUB_TYPE_TOF1, TAG_TOF1 },
};
...
static void get_psensor_calibrate_data(void)
{
	...
	if (tof_dev_info->tof_first_start_flag == 0 && tof_dev_info->tof_enable_flag == 1) {
		hwlog_info("[%s-%d]:send_multi_tof_calibrate_data_to_mcu\n", __func__, __LINE__);
		ret = send_multi_tof_calibrate_data_to_mcu();
		if (ret < 0)
			hwlog_err("[%s-%d]:send_multi_tof_calibrate_data_to_mcu fail\n",
				__func__, __LINE__);
		else
			hwlog_info("[%s-%d]:send_multi_tof_calibrate_data_to_mcu succ\n",
				__func__, __LINE__);
	}
}
