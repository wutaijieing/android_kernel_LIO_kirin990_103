...
SENSOR_SHOW_INFO(HUMITURE);
static DEVICE_ATTR(humiture_info, S_IRUGO, sensor_show_HUMITURE_info, NULL);

SENSOR_SHOW_INFO(TOF);
static DEVICE_ATTR(tof_info, S_IRUGO, sensor_show_TOF_info, NULL);

SENSOR_SHOW_INFO(TOF1);
static DEVICE_ATTR(tof1_info, S_IRUGO, sensor_show_TOF1_info, NULL);
...
SENSOR_SHOW_VALUE(TAG_HUMITURE);
static DEVICE_ATTR(humiture_read_data, 0664, sensor_show_TAG_HUMITURE_read_data, NULL);
...
static int tof_vi5300_calibrate_data_save(uint8_t tag, const uint8_t *buf, int length)
{
	struct tof_vi5300_calibrate_data tmp_calibrate_data;
	struct tof_vi5300_calibrate_data calibrate_data[MAX_TOF_NUM] = {};
	struct tof_vi5300_calibrate_data *data = NULL;
	const int *nv_calibrate_data = (const int *)user_info.nv_data;

	if (memcpy_s((void *)&tmp_calibrate_data, sizeof(struct tof_vi5300_calibrate_data),
		(void *)buf, length) != EOK) {
		hwlog_err("[%s-%d]:memcpy_s fail\n", __func__, __LINE__);
		return -1;
	}
	if (read_calibrate_data_from_nv(RSV452_NV_NUM, sizeof(calibrate_data), "RSV452") < 0) {
		hwlog_err("[%s-%d]:read calibrate fail\n", __func__, __LINE__);
		return -1;
	}
	if (memcpy_s(&calibrate_data, sizeof(calibrate_data), nv_calibrate_data,
		sizeof(struct tof_vi5300_calibrate_data) * MAX_TOF_NUM) != EOK) {
		hwlog_err("[%s-%d]:memcpy_s fail\n", __func__, __LINE__);
		return -1;
	}
	if (tag == TAG_TOF)
		data = &calibrate_data[0];
	else if (tag == TAG_TOF1)
		data = &calibrate_data[1];

	if (data && tmp_calibrate_data.xtalk_check == CALIBRATE_CHECK_VALUE) {
		data->xtalk_check = tmp_calibrate_data.xtalk_check;
		data->xtalk_calib = tmp_calibrate_data.xtalk_calib;
		data->xtalk_peak = tmp_calibrate_data.xtalk_peak;
	}
	if (data && tmp_calibrate_data.offset_check == CALIBRATE_CHECK_VALUE) {
		data->offset_check = tmp_calibrate_data.offset_check;
		data->offset_calib = tmp_calibrate_data.offset_calib;
	}
	if (write_calibrate_data_to_nv(RSV452_NV_NUM, sizeof(calibrate_data),
		"RSV452", calibrate_data) < 0) {
		hwlog_err("[%s-%d]:write_calibrate_data_to_nv fail\n", __func__, __LINE__);
		return -1;
	}
	hwlog_info("[%s-%d]:tag:%u, xtalk_check:%u, xtalk_calib:%d, xtalk_peak:%d, \
		offset_check:%u, offset_calib:%d\n", __func__, __LINE__, tag,
		data->xtalk_check, data->xtalk_calib, data->xtalk_peak, data->offset_check,
		data->offset_calib);
	return 0;
}

static int vl53l3_write_calibrate_data_to_nv(const struct tof_vl53l3_calibrate_data *calib_data,
	int begin, int end)
{
	int i;

	for (i = begin; i < end; i++) {
		if (write_calibrate_data_to_nv(tof_nv_info[i].nv_number,
			tof_nv_info[i].length, tof_nv_info[i].nv_name,
			(void *)((char *)calib_data + tof_nv_info[i].bias)) < 0) {
			hwlog_err("[%s-%d]:write_calibrate_data_to_nv fail\n", __func__, __LINE__);
			return -1;
		}
	}
...
static int save_vl53l3_calibrate_data_to_nv(uint8_t tag,
	const struct tof_vl53l3_calibrate_data *calib_data)
{
	if (tag == TAG_TOF) {
		if (vl53l3_write_calibrate_data_to_nv(calib_data, 0,
			VL53L3_CALIB_DATA_BUF_NUM) < 0) {
			hwlog_err("[%s-%d]:vl53l3_write_calibrate_data_to_nv fail\n",
				__func__, __LINE__);
			return -1;
		}
		hwlog_info("[%s-%d]:write_calibrate_data_to_nv TAG_TOF success\n",
			__func__, __LINE__);
	} else if (tag == TAG_TOF1) {
		if (vl53l3_write_calibrate_data_to_nv(calib_data, VL53L3_CALIB_DATA_BUF_NUM,
			DOUBLE_TOF_CALIB_DATA_BUF_NUM) < 0) {
			hwlog_err("[%s-%d]:vl53l3_write_calibrate_data_to_nv fail\n",
				__func__, __LINE__);
			return -1;
		}
		hwlog_info("[%s-%d]:write_calibrate_data_to_nv TAG_TOF1 success\n",
			__func__, __LINE__);
	}
	return 0;
}

static int tof_vl53l3_calibrate_data_save(uint8_t tag, const uint8_t *buf, int length)
{
	struct tof_vl53l3_calibrate_data tmp_calibrate_data;
	struct tof_vl53l3_calibrate_data calibrate_data[MAX_TOF_NUM] = {};
	struct tof_vl53l3_calibrate_data test_calibrate_data[MAX_TOF_NUM] = {};
	struct tof_vl53l3_calibrate_data *data = NULL;

	if (length > sizeof(struct tof_vl53l3_calibrate_data)) {
		hwlog_err("[%s-%d]:length:%d is out of range\n", __func__, __LINE__, length);
		return -1;
	}

	if (memcpy_s((void *)&tmp_calibrate_data, sizeof(struct tof_vl53l3_calibrate_data),
		(void *)buf, length) != EOK) {
		hwlog_err("[%s-%d]:memcpy_s fail\n", __func__, __LINE__);
		return -1;
	}

	hwlog_info("[%s-%d]:tag:%u\n", __func__, __LINE__, tag);
	if (tag == TAG_TOF)
		data = &calibrate_data[0];
	else if (tag == TAG_TOF1)
		data = &calibrate_data[1];

	if (tmp_calibrate_data.xtalk_check != CALIBRATE_CHECK_VALUE ||
		tmp_calibrate_data.offset_check != CALIBRATE_CHECK_VALUE)
		return 0;

	if (memcpy_s(data, sizeof(struct tof_vl53l3_calibrate_data), &tmp_calibrate_data,
		sizeof(tmp_calibrate_data)) != EOK) {
		hwlog_err("[%s-%d]:memcpy_s fail\n", __func__, __LINE__);
		return -1;
	}
	tof_vl53l3_calib_data_dump(data, sizeof(struct tof_vl53l3_calibrate_data));
	if (save_vl53l3_calibrate_data_to_nv(tag, data) < 0) {
		hwlog_err("[%s-%d]:save_vl53l3_calibrate_data_to_nv fail, tag:%u\n",
			__func__, __LINE__, tag);
		return -1;
	}
...
static int tof_calibrate_save_ext(uint8_t tag, const uint8_t *buf, int length)
{
	int32_t ret = 0;
	int32_t sensor_type = TOF_CHIP_INVALID;

	if (!buf || length <= 0 || length > VL53L3_CALIB_DATA_SIZE) {
		hwlog_err("[%s-%d]:invalid argument", __func__, __LINE__);
		tof_calibration_res = EXEC_FAIL;
		return -1;
	}

	if (memcpy_s((void *)&sensor_type, sizeof(sensor_type),
		(void *)buf, sizeof(int32_t)) != EOK) {
		hwlog_err("[%s-%d]:memcpy_s fail\n", __func__, __LINE__);
		return -1;
	}
	hwlog_info("[%s-%d]:sensor_type:%d\n", __func__, __LINE__, sensor_type);
	if (sensor_type == TOF_CHIP_VI5300) {
		ret = tof_vi5300_calibrate_data_save(tag, buf, length);
		if (ret < 0) {
			hwlog_err("[%s-%d]:tof_vi5300_calibrate_data_save fail, ret:0x%x\n",
				__func__, __LINE__, ret);
			return -1;
		}
	} else if (sensor_type == TOF_CHIP_VL53L3) {
		ret = tof_vl53l3_calibrate_data_save(tag, buf, length);
		if (ret < 0) {
			hwlog_err("[%s-%d]:tof_vl53l3_calibrate_data_save fail, ret:0x%x\n",
				__func__, __LINE__, ret);
			return -1;
		}
	} else {
		hwlog_err("[%s-%d]:tof sensor type:%d is not vi5300 or vl53l3\n",
			__func__, __LINE__, sensor_type);
		return -1;
	}
...
static int tof_vi5300_calibrate_data_show(char *buf, int length)
{
	int ret;
	struct tof_vi5300_calibrate_data tmp_calibrate_data[MAX_TOF_NUM];
	const int *calibrate_data = (const int *)user_info.nv_data;

	if (read_calibrate_data_from_nv(RSV452_NV_NUM, sizeof(tmp_calibrate_data), "RSV452") < 0) {
		hwlog_err("[%s-%d]:read calibrate fail\n", __func__, __LINE__);
		return -1;
	}

	if (memcpy_s(&tmp_calibrate_data, sizeof(tmp_calibrate_data),
		calibrate_data, sizeof(struct tof_vi5300_calibrate_data) * MAX_TOF_NUM) != EOK) {
		hwlog_err("[%s-%d]:memcpy_s fail\n", __func__, __LINE__);
		return -1;
	}
	ret = snprintf_s(buf, PAGE_SIZE, length,
		"%u, %d, %u, %u, %d, %u, %d, %u, %u, %d\n",
		tmp_calibrate_data[0].xtalk_check, tmp_calibrate_data[0].xtalk_calib,
		tmp_calibrate_data[0].xtalk_peak, tmp_calibrate_data[0].offset_check,
		tmp_calibrate_data[0].offset_calib, tmp_calibrate_data[1].xtalk_check,
		tmp_calibrate_data[1].xtalk_calib, tmp_calibrate_data[1].xtalk_peak,
		tmp_calibrate_data[1].offset_check, tmp_calibrate_data[1].offset_calib);
	if (ret < 0) {
		hwlog_err("[%s-%d]:snprintf_s fail\n", __func__, __LINE__);
		return -1;
	}
	hwlog_info("[%s-%d]:vi5300 calibrate data:%s\n", __func__, __LINE__, buf);
	return ret;
}

static int read_vl53l3_calibrate_data_from_nv(struct tof_vl53l3_calibrate_data *calib_data,
	int begin, int end)
{
	int i;

	for (i = begin; i < end; i++) {
		if (read_calibrate_data_from_nv(tof_nv_info[i].nv_number, tof_nv_info[i].length,
			tof_nv_info[i].nv_name) < 0) {
			hwlog_err("[%s-%d]:read calibrate fail\n", __func__, __LINE__);
			return -1;
		}
		if (memcpy_s((void *)((char *)calib_data + tof_nv_info[i].bias),
			tof_nv_info[i].length, (void *)user_info.nv_data,
			tof_nv_info[i].length) != EOK) {
			hwlog_err("[%s-%d]:memcpy_s fail\n", __func__, __LINE__);
			return -1;
		}
	}
...
static int tof_vl53l3_calibrate_data_show(char *buf, int length)
{
	int ret;
	struct tof_vl53l3_calibrate_data tmp_calibrate_data[MAX_TOF_NUM];

	if (read_vl53l3_calibrate_data_from_nv(&tmp_calibrate_data[0], 0,
		VL53L3_CALIB_DATA_BUF_NUM) < 0) {
		hwlog_err("[%s-%d]:read_vl53l3_calibrate_data_from_nv tof fail, ret:0x%x\n",
			__func__, __LINE__, ret);
		return -1;
	}
	if (read_vl53l3_calibrate_data_from_nv(&tmp_calibrate_data[1], VL53L3_CALIB_DATA_BUF_NUM,
		DOUBLE_TOF_CALIB_DATA_BUF_NUM) < 0) {
		hwlog_err("[%s-%d]:read_vl53l3_calibrate_data_from_nv tof1 fail, ret:0x%x\n",
			__func__, __LINE__, ret);
		return -1;
	}

	tof_vl53l3_calib_data_dump(&tmp_calibrate_data[0],
		sizeof(struct tof_vl53l3_calibrate_data));
	tof_vl53l3_calib_data_dump(&tmp_calibrate_data[1],
		sizeof(struct tof_vl53l3_calibrate_data));
	ret = snprintf_s(buf, PAGE_SIZE, length,
		"%u, %u, %d, %d, %u, %d, %d, %d, %u, %u, %d, %d, %u, %d, %d, %d\n",
		tmp_calibrate_data[0].xtalk_check,
		tmp_calibrate_data[0].algo_crosstalk_compensation_plane_offset_kcps,
		tmp_calibrate_data[0].algo_crosstalk_compensation_x_plane_gradient_kcps,
		tmp_calibrate_data[0].algo_crosstalk_compensation_y_plane_gradient_kcps,
		tmp_calibrate_data[0].offset_check,
		tmp_calibrate_data[0].algo_part_to_part_range_offset_mm,
		tmp_calibrate_data[0].mm_config_inner_offset_mm,
		tmp_calibrate_data[0].mm_config_outer_offset_mm,
		tmp_calibrate_data[1].xtalk_check,
		tmp_calibrate_data[1].algo_crosstalk_compensation_plane_offset_kcps,
		tmp_calibrate_data[1].algo_crosstalk_compensation_x_plane_gradient_kcps,
		tmp_calibrate_data[1].algo_crosstalk_compensation_y_plane_gradient_kcps,
		tmp_calibrate_data[1].offset_check,
		tmp_calibrate_data[1].algo_part_to_part_range_offset_mm,
		tmp_calibrate_data[1].mm_config_inner_offset_mm,
		tmp_calibrate_data[1].mm_config_outer_offset_mm);
	if (ret < 0) {
		hwlog_err("[%s-%d]:vl53l3 snprintf_s fail ret:0x%x\n", __func__, __LINE__, ret);
		return -1;
	}
	hwlog_info("[%s-%d]:vl53l3 calibrate data:%s\n", __func__, __LINE__, buf);
	return ret;
}

static ssize_t attr_tof_calibrate_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int ret;
	int32_t sensor_type = TOF_CHIP_INVALID;
	const int *data = (const int *)user_info.nv_data;
	...
	if (read_calibrate_data_from_nv(RSV452_NV_NUM,
		sizeof(sensor_type), "RSV452") < 0) {
		hwlog_err("[%s-%d]:read calibrate fail\n", __func__, __LINE__);
		return -1;
	}
	if (memcpy_s(&sensor_type, sizeof(data), data, sizeof(int32_t)) != EOK) {
		hwlog_err("[%s-%d]:memcpy_s fail\n", __func__, __LINE__);
		return -1;
	}

	hwlog_info("[%s-%d]:current sensor_type:0x%x\n", __func__, __LINE__, sensor_type);
	if (sensor_type == TOF_CHIP_VI5300) {
		ret = tof_vi5300_calibrate_data_show(buf, VI5300_CALIB_SHOW_BUF_SIZE);
		if (ret < 0) {
			hwlog_err("[%s-%d]:tof_vi5300_calibrate_data_show fail, ret:0x%x\n",
				__func__, __LINE__, ret);
			return -1;
		}
	} else if (sensor_type == TOF_CHIP_VL53L3) {
		ret = tof_vl53l3_calibrate_data_show(buf, VL53L3_CALIB_SHOW_BUF_SIZE);
		if (ret < 0) {
			hwlog_err("[%s-%d]:tof_vl53l3_calibrate_data_show fail, ret:0x%x\n",
				__func__, __LINE__, ret);
			return -1;
		}
	}
...
static void tof_calibrate_save_log(void)
{
	int ret;
	char content[CLI_CONTENT_LEN_MAX] = {0};
	char date_str[CLI_TIME_STR_LEN] = {0};
	...
	if (memset_s(&content, CLI_CONTENT_LEN_MAX, 0, sizeof(content)) != EOK) {
		hwlog_err("[%s-%d]:memset_s fail\n", __func__, __LINE__);
		return;
	}
	ret = snprintf_s(content, CLI_CONTENT_LEN_MAX, TOF_LOG_BUF_SIZE, TOF_CALI_RAW_DATA, 0,
...
static enum ret_type send_calibrate_cmd_ext(uint8_t tag, int val, char *pkg_mcu_buf, int *length)
{
	int ret;
	int i;
	struct write_info pkg_ap[VL53L3_CALIB_DATA_BUF_NUM] = { 0 };
	struct read_info pkg_mcu[VL53L3_CALIB_DATA_BUF_NUM] = { 0 };
	pkt_parameter_req_t spkt;
	struct pkt_header *shd = (struct pkt_header *)&spkt;
	struct tof_vl53l3_calibrate_cmd_to_mcu pkg_cmd[VL53L3_CALIB_DATA_BUF_NUM] = {
		{ tag, CALIB_DATA_PKG_FIRST_PART, val },
		{ tag, CALIB_DATA_PKG_SECOND_PART, val },
		{ tag, CALIB_DATA_PKG_THIRD_PART, val },
	};

	*length = 0;
	for (i = 0; i < VL53L3_CALIB_DATA_BUF_NUM; i++) {
		pkg_ap[i].tag = tag;
		pkg_ap[i].cmd = CMD_CMN_CONFIG_REQ;
		spkt.subcmd = SUB_CMD_SELFCALI_REQ;
		pkg_ap[i].wr_buf = &shd[1];
		pkg_ap[i].wr_len = sizeof(struct tof_vl53l3_calibrate_cmd_to_mcu) + SUBCMD_LEN;
		if (memcpy_s(spkt.para, sizeof(struct tof_vl53l3_calibrate_cmd_to_mcu),
			&pkg_cmd[i], sizeof(pkg_cmd[i])) != EOK) {
			hwlog_err("send_calibrate_cmd_ext:memcpy_s fail\n");
			return COMMU_FAIL;
		}
		ret = write_customize_cmd(&pkg_ap[i], &pkg_mcu[i], true);
		if (ret) {
			hwlog_err("send tag %u calibrate cmd to mcu fail, ret:%d\n", tag, ret);
			return COMMU_FAIL;
		} else if (pkg_mcu[i].errno != 0) {
			hwlog_err("send tag %u calibrate fail, %d\n", tag, pkg_mcu[i].errno);
			return EXEC_FAIL;
		}
		if (memcpy_s((void *)(pkg_mcu_buf + *length), CALIB_DATA_MAX_BUF_SIZE,
			(void *)pkg_mcu[i].data, pkg_mcu[i].data_length) != EOK) {
			hwlog_err("memcpy_s pkg_mcu_buf fail, tag:%u, i:%d\n", tag, i);
			return COMMU_FAIL;
		}
		hwlog_info("[%s-%d]:pkg_num:%d, data_length:%d\n", __func__, __LINE__,
			i, pkg_mcu[i].data_length);
		*length += pkg_mcu[i].data_length;
	}
	return SUC;
}

static int send_tof_calibrate_cmd(const struct tof_calibrate_cmd_packet *calibrate_cmd,
	uint8_t *tag_ptr, uint8_t *buffer, int *length)
{
	int ret;
	...
	if (calibrate_cmd->index == DEV_TOF1_INDEX) {
		*tag_ptr = TAG_TOF;
	} else if (calibrate_cmd->index == DEV_TOF2_INDEX) {
		*tag_ptr = TAG_TOF1;
	} else {
		hwlog_err("[%s-%d]:calibrate cmd index:%d, is out of range\n",
			__func__, __LINE__, calibrate_cmd->index);
		return -1;
	}
	...
	hwlog_info("[%s-%d]:calibrate cmd index:%d, type:%d\n", __func__, __LINE__,
		calibrate_cmd->index, calibrate_cmd->cmd_type);
	if (chip_info->chip_type == TOF_CHIP_VI5300) {
		tmp_pkg_mcu = send_calibrate_cmd(*tag_ptr, calibrate_cmd->cmd_type,
			&tof_calibration_res);
		if (memcpy_s(buffer, VL53L3_CALIB_DATA_SIZE, tmp_pkg_mcu.data,
			tmp_pkg_mcu.data_length) != EOK) {
			hwlog_err("[%s-%d]:memcpy_s fail\n", __func__, __LINE__);
			return -1;
		}
		*length = tmp_pkg_mcu.data_length;
	} else if (chip_info->chip_type == TOF_CHIP_VL53L3) {
		tof_calibration_res = send_calibrate_cmd_ext(*tag_ptr, calibrate_cmd->cmd_type,
			buffer, length);
	} else {
		hwlog_err("[%s-%d]:chip_type:%d is not tof\n",
			__func__, __LINE__, chip_info->chip_type);
		tof_calibration_res == COMMU_FAIL;
	}
	if (tof_calibration_res == COMMU_FAIL || tof_calibration_res == EXEC_FAIL) {
		hwlog_err("[%s-%d]:tof_calibration_res:%d\n",
			__func__, __LINE__, tof_calibration_res);
		return -1;
	}
	return 0;
}

static ssize_t attr_tof_calibrate_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int ret;
	struct tof_calibrate_cmd_packet calibrate_cmd = {};
	uint8_t tag = 0;
	uint8_t buffer[VL53L3_CALIB_DATA_SIZE] = { 0 };
	int length = 0;
	...
	if (get_sensor_tof_flag() != 1) {
		hwlog_info("[%s-%d]:no tof sensor, no need calibrate\n", __func__, __LINE__);
		return -1;
	}
	if (count != sizeof(struct tof_calibrate_cmd_packet)) {
		hwlog_err("[%s-%d]:tof calibrate cmd size is wrong\n", __func__, __LINE__);
		return -1;
	}
	if (memcpy_s(&calibrate_cmd, sizeof(struct tof_calibrate_cmd_packet), buf, count) != EOK) {
		hwlog_err("[%s-%d]:memcpy_s fail\n", __func__, __LINE__);
		return -1;
	}
	ret = send_tof_calibrate_cmd(&calibrate_cmd, &tag, buffer, &length);
	if (ret < 0) {
		hwlog_err("[%s-%d]:send_tof_calibrate_cmd fail, ret:0x%x\n",
			__func__, __LINE__, ret);
		goto TOF_CALIBRATE_STORE_SAVE_LOG;
	}
	hwlog_info("[%s-%d]:buffer data length:%d\n", __func__, __LINE__, length);
	tof_calibrate_save_ext(tag, buffer, length);

TOF_CALIBRATE_STORE_SAVE_LOG:
	tof_calibrate_save_log();
	return count;
}
...
static struct attribute *sensor_attributes[] = {
	...
	&dev_attr_humiture_info.attr,
	&dev_attr_tof1_info.attr,
	&dev_attr_tof_info.attr,
	...
	&dev_attr_humiture_read_data.attr,
