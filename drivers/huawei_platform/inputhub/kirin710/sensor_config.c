...
static int send_tof_vi5300_calibrate_data_to_mcu(struct tof_device_info *dev_info_tof,
	struct tof_device_info *dev_info_tof1)
{
	int ret = 0;
	...
	if (memcpy_s(calibrate_data, sizeof(calibrate_data), user_info.nv_data,
	...
	if (dev_info_tof && dev_info_tof->tof_enable_flag == 1 &&
		...
		ret = send_calibrate_data_to_mcu(TAG_TOF, SUB_CMD_SET_OFFSET_REQ,
			&calibrate_data[0], sizeof(struct tof_vi5300_calibrate_data), false);
		if (ret < 0)
			hwlog_err("[%s-%d]:send_calibrate_data_to_mcu tag:%d fail\n",
				__func__, __LINE__, TAG_TOF);
	}
	if (dev_info_tof1 && dev_info_tof1->tof_enable_flag == 1 &&
		dev_info_tof1->tof_first_start_flag == 0) {
		dev_info_tof1->tof_first_start_flag = 1;
		hwlog_info("[%s-%d]:tof1 set calibrate data\n", __func__, __LINE__);
		ret = send_calibrate_data_to_mcu(TAG_TOF1, SUB_CMD_SET_OFFSET_REQ,
			&calibrate_data[1], sizeof(struct tof_vi5300_calibrate_data), false);
		if (ret < 0)
			hwlog_err("[%s-%d]:send_calibrate_data_to_mcu tag:%d fail\n",
				__func__, __LINE__, TAG_TOF1);
	}
	return ret;
}

static int send_vl53l3_calibrate_data_pkg_to_mcu(int tag,
	const struct tof_vl53l3_calibrate_data *calibrate_data)
{
	int ret;
	int i;
	struct vl53l3_calib_data_pkg pkg_info[VL53L3_CALIB_DATA_BUF_NUM] = {};

	for (i = 0; i < VL53L3_CALIB_DATA_BUF_NUM; i++) {
		pkg_info[i].pkg_num = i;
		pkg_info[i].tag = tag;
		pkg_info[i].sensor_type = TOF_CHIP_VL53L3;
		if (i == VL53L3_CALIB_DATA_BUF_NUM - 1)
			pkg_info[i].length = VL53L3_CALIB_DATA_PKG_OFFSET;
		else
			pkg_info[i].length = VL53L3_CALIB_DATA_PKG_SIZE;
		if (memcpy_s((void *)pkg_info[i].buffer, pkg_info[i].length,
			(void *)((char *)calibrate_data + (VL53L3_CALIB_DATA_PKG_SIZE * i)),
			pkg_info[i].length) != EOK) {
			hwlog_err("[%s-%d]:memcpy_s tag:%d, pkg_num:%d fail\n",
				__func__, __LINE__, tag, i);
			return -1;
		}
		ret = send_calibrate_data_to_mcu(tag, SUB_CMD_SET_OFFSET_REQ,
			(void *)&pkg_info[i], sizeof(struct vl53l3_calib_data_pkg), false);
		if (ret < 0) {
			hwlog_err("[%s-%d]:send_calibrate_data_to_mcu tag:%d fail, ret:0x%x\n",
				__func__, __LINE__, tag, ret);
			return -1;
		}
	}
	return 0;
...
static int send_vl53l3_calibrate_data(uint8_t tag, int begin, int end)
{
	int ret;
	int i;
	struct tof_vl53l3_calibrate_data calib_data = { 0 };
	...
	tof_vl53l3_calib_data_dump(&calib_data, sizeof(struct tof_vl53l3_calibrate_data));
	ret = send_vl53l3_calibrate_data_pkg_to_mcu(tag, &calib_data);
	if (ret < 0) {
		hwlog_err("[%s-%d]:send_vl53l3_calibrate_data_pkg_to_mcu tag:%u fail ret:0x%x\n",
			__func__, __LINE__, tag, ret);
		return -1;
	}
	return 0;
...
static int send_tof_vl53l3_calibrate_data_to_mcu(struct tof_device_info *dev_info_tof,
	struct tof_device_info *dev_info_tof1)
{
	int ret = 0;
	...
	if (dev_info_tof && dev_info_tof->tof_enable_flag == 1 &&
		...
		dev_info_tof->tof_first_start_flag = 1;
		hwlog_info("[%s-%d]:tof set calibrate data\n", __func__, __LINE__);
		ret = send_vl53l3_calibrate_data(TAG_TOF, 0, VL53L3_CALIB_DATA_BUF_NUM);
		if (ret < 0) {
			hwlog_err("[%s-%d]:send_tof_vl53l3_calibrate_data fail, ret:0x%x\n",
	...
	if (dev_info_tof1 && dev_info_tof1->tof_enable_flag == 1 &&
		...
		dev_info_tof1->tof_first_start_flag = 1;
		hwlog_info("[%s-%d]:tof1 set calibrate data\n", __func__, __LINE__);
		ret = send_vl53l3_calibrate_data(TAG_TOF1, VL53L3_CALIB_DATA_BUF_NUM,
			DOUBLE_TOF_CALIB_DATA_BUF_NUM);
		if (ret < 0) {
			hwlog_err("[%s-%d]:send_tof1_vl53l3_calibrate_data fail, ret:0x%x\n",
				__func__, __LINE__, ret);
			return -1;
		}
	}
...
int send_multi_tof_calibrate_data_to_mcu(void)
{
	int ret = 0;
	struct tof_vi5300_calibrate_data vi5300_calibrate_data[MAX_TOF_NUM] = {};
	struct tof_vl53l3_calibrate_data vl53l3_calibrate_data[MAX_TOF_NUM] = {};
	struct tof_device_info *dev_info_tof = tof_get_device_info(TAG_TOF);
	struct tof_device_info *dev_info_tof1 = tof_get_device_info(TAG_TOF1);

	if (!dev_info_tof || !dev_info_tof1) {
		hwlog_err("[%s-%d]:params are null\n", __func__, __LINE__);
		return -1;
	}

	if (dev_info_tof->chip_type == TOF_CHIP_VI5300 || dev_info_tof1 == TOF_CHIP_VI5300) {
		ret = send_tof_vi5300_calibrate_data_to_mcu(dev_info_tof, dev_info_tof1);
		if (ret < 0) {
			hwlog_err("[%s-%d]:send_tof_vi5300_calibrate_data_to_mcu fail, ret:0x%x\n",
				__func__, __LINE__, ret);
			return -1;
		}
	} else if (dev_info_tof->chip_type == TOF_CHIP_VL53L3 ||
		dev_info_tof1 == TOF_CHIP_VL53L3) {
		ret = send_tof_vl53l3_calibrate_data_to_mcu(dev_info_tof, dev_info_tof1);
		if (ret < 0) {
			hwlog_err("[%s-%d]:send_tof_vl53l3_calibrate_data_to_mcu fail, ret:0x%x\n",
				__func__, __LINE__, ret);
			return -1;
		}
	} else {
		hwlog_err("[%s-%d]:tof sensor type:%d is not vi5300 or vl53l3\n",
			__func__, __LINE__, dev_info_tof->chip_type);
		return -1;
	}
...
static void reset_als_calibrate_data(void)
{
	...
	if (t == ALS_CHIP_ROHM_RGB || t == ALS_CHIP_AVAGO_RGB ||
		...
		t == ALS_CHIP_SY3133 || t == ALS_CHIP_TMD2755)
	...
	struct als_device_info *dev_info)
{
	...
	for (i = 0; i < table_size; i++) {
		if ((sy3133_tab[i].phone_type == als_data.als_phone_type) &&
			...
			sy3133_tab[i].tp_lcd_manufacture == DEFAULT_TPLCD) &&
