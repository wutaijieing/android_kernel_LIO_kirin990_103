...
static int get_volt_show(struct seq_file *m, void *v)
{
	...
	arm_smccc_1_1_smc(GET_CPU_VOLT_FID_VALUE,
...
static int __init cpufreq_get_volt_init(void)
{
	...
	np = of_find_compatible_node(NULL, NULL, "lowpower, get_val");
	if (np == NULL) {
		pr_err("%s: no compatible node found\n", __func__);
		return -EPERM;
	}

	ret = of_property_read_u32_array(np, "lowpower,bl31-share-mem",
					 &data[0], 2UL);
	if (ret != 0) {
