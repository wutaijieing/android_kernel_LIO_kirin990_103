...
int ssi_set_gpio_pins(int32_t clk, int32_t data)
{
    ...
    (void)ssi_request_gpio(g_st_board_info.ssi_gpio_clk, g_st_board_info.ssi_gpio_data);
...
#define SSI_READ_RETTY 1000
    ...
    for (i = 0; i < SSI_DATA_LEN; i++) {
        ssi_clk_output();
        ssi_delay(g_interval_time);
        rx = (uint16_t)oal_gpio_get_value(g_ssi_data_gpio);
                                   ...
                                   int seg_index, int32_t seg_size)
{
    ...
    for (j = 0; j < seg_size; j += step) {
        ssi_address = pst_reg_info->base_addr + (uint32_t)(seg_index * buf_len) + (uint32_t)j;
...
static int ssi_setup_clk_enhance(uint32_t clk_type)
{
    ...
    if (get_hi110x_subchip_type() == BOARD_VERSION_SHENKUO) {
        tcxo_available = shenkuo_ssi_check_tcxo_available();
    } else if (get_hi110x_subchip_type() == BOARD_VERSION_BISHENG) {
        tcxo_available = bisheng_ssi_check_tcxo_available();
    } else if (get_hi110x_subchip_type() == BOARD_VERSION_HI1161) {
        tcxo_available = hi1161_ssi_check_tcxo_available();
    } else {
        tcxo_available = ssi_check_tcxo_available();
    }

    /* TCXO不在位, 需要切换到ssi clk */
    if (tcxo_available != BOARD_SUCC) {
        return ssi_setup_clk(clk_type);
    }
...
int ssi_read_reg_info(ssi_reg_info *pst_reg_info, void *buf, int32_t size, int32_t is_file)
{
    ...
    ps_print_info("dump reg info 0x%8x:%u, buf len:%u \n", pst_reg_info->base_addr, pst_reg_info->len, buf_len);
...
int ssi_dump_device_regs(unsigned long long module_set)
{
    ...
    if (get_hi110x_subchip_type() == BOARD_VERSION_SHENKUO) {
        ret = shenkuo_ssi_device_regs_dump(module_set);
    } else if (get_hi110x_subchip_type() == BOARD_VERSION_BISHENG) {
        ret = bisheng_ssi_device_regs_dump(module_set);
    } else if (get_hi110x_subchip_type() == BOARD_VERSION_HI1161) {
        ret = hi1161_ssi_device_regs_dump(module_set);
    } else {
