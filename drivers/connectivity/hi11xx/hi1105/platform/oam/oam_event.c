...
void oam_set_ota_board_type(oam_ota_stru *ota_data)
{
    ...
    switch (chip_type) {
        ...
        case BOARD_VERSION_BISHENG:
            ota_data->st_ota_hdr.auc_resv[0] = OAM_OTA_TYPE_BISHENG_HOST;
            break;
        case BOARD_VERSION_HI1161:
            ota_data->st_ota_hdr.auc_resv[0] = OAM_OTA_TYPE_1161_HOST;
            break;
        default:
    ...
    switch (en_ota_type) {
        ...
        case OAM_OTA_TYPE_MEMBLOCK:
            ...
            if (puc_param_two_addr != NULL) {
                if (memcpy_s((void *)(pst_ota_data->auc_ota_data + us_param_one_len),
                             (uint32_t)(us_param_two_len),
                             (const void *)puc_param_two_addr,
                             (uint32_t)us_param_two_len) != EOK) {
                    oal_mem_sdt_netbuf_free(pst_netbuf, OAL_TRUE);
                    oal_io_print("oam_ota_report_to_sdt::memcpy_s failed.\n");
                    return OAL_FAIL;
                }
