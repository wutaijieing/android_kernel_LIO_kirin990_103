...
OAL_STATIC uint32_t hcc_bus_chan_set(hcc_bus_dev *pst_bus_dev, const char *start, uint32_t len)
{
    ...
    if (bus_chan_init_string_cmp(start, len, "usb") == OAL_SUCC) {
        ...
        oal_print_hi11xx_log(HI11XX_LOG_INFO, "hisi wifi support usb");
        return HCC_BUS_USB;
    }
#endif
...
hcc_bus *hcc_get_current_110x_bus(void)
{
    ...
    hcc_bus_dev *pst_bus_dev = hcc_get_bus_dev(hcc_get_main_dev());
#else
...
int32_t hi110x_hcc_ip_pm_ctrl(uint32_t enable, uint8_t dev_id)
{
...
int32_t hi110x_bind_hcc_thread(uint8_t is_bind, uint8_t thread_bind, uint8_t irq_bind)
{
    return hcc_bind_thread(is_bind, thread_bind, irq_bind, HCC_EP_WIFI_DEV);
}
EXPORT_SYMBOL_GPL(hi110x_bind_hcc_thread);
...
#ifdef _PRE_PLAT_FEATURE_MULTI_HCC
OAL_STATIC int32_t hcc_dev_res_init_116x(void)
{
    return hcc_dev_res_init(g_bus_devs.dev_list, g_bus_devs.dev_num);
}
#else
OAL_STATIC int32_t hcc_dev_res_init_110x(void)
{
...
int32_t hcc_add_bus(hcc_bus *pst_bus, const char *bus_name)
{
    ...
    if (oal_dlist_is_empty(&g_hcc_bus_res_hdr) == OAL_TRUE) {
        oal_spin_unlock_irq_restore(&g_hcc_bus_res_lock, &irqsave);
#ifdef _PRE_PLAT_FEATURE_MULTI_HCC
        ret = hcc_dev_res_init_116x();
#else
        ret = hcc_dev_res_init_110x();
#endif
        oal_spin_lock_irq_save(&g_hcc_bus_res_lock, &irqsave);
        if (ret) {
...
void hcc_get_cpus(cpu_cap_mask *cpu_mask, uint8_t is_lowest_load)
{
    if (is_lowest_load == 1) {
        ...
        if (cpu < 0) {
            oal_print_hi11xx_log(HI11XX_LOG_INFO, "find lowest load cpu failed!");
            return;
        }
...
int32_t hcc_bus_bindcpu(hcc_bus *hi_bus, uint32_t chan, int32_t is_bind)
{
    ...
    oal_print_hi11xx_log(HI11XX_LOG_INFO, "hcc %s cpu, thread[%d]irq[%d]",
    ...
    if (hi_bus->opt_ops->bindcpu == NULL) {
        oal_print_hi11xx_log(HI11XX_LOG_INFO, "bindcpu NULL!");
    }
...
{
    hcc_bus_exception_submit(hcc_get_bus(HCC_EP_WIFI_DEV), WIFI_TRANS_FAIL);
}
...
OAL_STATIC int32_t wait_wlan_wkup_gpio_level(int32_t gpio_level, unsigned long timeout)
{
    ...
    forever_loop() {
        gpio_value = board_get_dev_wkup_host_state(W_SYS);
...
OAL_STATIC uint32_t hcc_bus_get_exception_mem_info_1161(hcc_bus *hi_bus)
{
    uint32_t size;
    struct st_wifi_dump_mem_info *mem_info = NULL;
    uint32_t is_gt = (hi_bus->dev_id == HCC_EP_GT_DEV) ? OAL_TRUE : OAL_FALSE;

    oal_print_hi11xx_log(HI11XX_LOG_INFO, "1161 %s memdump ", is_gt ? "gt" : "wifi");

    if (is_gt) {
        mem_info = g_hi161_pilot_gt_meminfo;
        size = sizeof(g_hi161_pilot_gt_meminfo) / sizeof(struct st_wifi_dump_mem_info);
    } else {
        mem_info = g_hi161_pilot_wifi_meminfo;
        size = sizeof(g_hi161_pilot_wifi_meminfo) / sizeof(struct st_wifi_dump_mem_info);
    }
...
OAL_STATIC int32_t hcc_bus_get_exception_mem_info(hcc_bus *hi_bus)
{
    ...
    if (get_hi110x_subchip_type() == BOARD_VERSION_HI1105) {
        ...
        oal_print_hi11xx_log(HI11XX_LOG_INFO, "shenkuo %s memdump ", hi110x_is_asic() ? "asic" : "fpga");
        if (hi110x_is_asic()) {
            /* shenkuo Asic */
            mem_info = g_shenkuo_pilot_asic_meminfo;
            size = sizeof(g_shenkuo_pilot_asic_meminfo) / sizeof(struct st_wifi_dump_mem_info);
        } else {
            /* shenkuo Fpga */
            mem_info = g_shenkuo_pilot_fpga_meminfo;
            size = sizeof(g_shenkuo_pilot_fpga_meminfo) / sizeof(struct st_wifi_dump_mem_info);
        }
    } else if (get_hi110x_subchip_type() == BOARD_VERSION_BISHENG) {
        oal_print_hi11xx_log(HI11XX_LOG_INFO, "bisheng memdump");

        /* Bisheng Memdump */
        mem_info = g_bisheng_meminfo;
        size = sizeof(g_bisheng_meminfo) / sizeof(struct st_wifi_dump_mem_info);
    } else if (get_hi110x_subchip_type() == BOARD_VERSION_HI1161) {
            return hcc_bus_get_exception_mem_info_1161(hi_bus);
    } else {
...
OAL_STATIC void hcc_bus_exception_handler(oal_work_stru *work)
{
    ...
    wifi_exception_work_submit(excp_type, atomic_long_read(&work->data));
...
OAL_STATIC int32_t oal_register_gpio_intr(hcc_bus_dev *pst_bus_dev)
{
    ...
    unsigned int wlan_irq = (unsigned int)pst_board->wakeup_irq[W_SYS];
    ...
    pst_bus_dev->wlan_irq = pst_board->wakeup_irq[W_SYS];
    ...
    ret = request_irq(gpio_to_irq(pst_board->dev_wakeup_host[W_SYS]), hcc_bus_wlan_gpio_irq,
...
void oal_wlan_gpio_intr_enable(hcc_bus_dev *pst_bus_dev, uint32_t ul_en)
{
    ...
    if (ul_en) {
...
#ifdef _PRE_HI_DRV_GPIO
            enable_irq(gpio_to_irq(get_hi110x_board_info()->dev_wakeup_host[W_SYS]));
#else
        ...
        if (!pst_bus_dev->irq_stat) {
#ifdef _PRE_HI_DRV_GPIO
            disable_irq_nosync(gpio_to_irq(get_hi110x_board_info()->dev_wakeup_host[W_SYS]));
#else
...
OAL_STATIC int32_t hcc_bus_resource_alloc(hcc_bus *pst_bus)
{
    ...
    if (unlikely(ret != EOK)) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "memcpy_s failed ret=%d", ret);
...
#endif
    oal_print_hi11xx_log(HI11XX_LOG_INFO, "hcc_dev_init we had %d devs", dev_num);
...
void resume_wlan_wakeup_host_gpio(void)
{
    ...
    pst_bus_dev->wlan_irq = board->wakeup_irq[W_SYS];
    wlan_irq = (uint32_t)pst_bus_dev->wlan_irq;
    oal_wlan_gpio_intr_enable(pst_bus_dev, OAL_FALSE);
    free_irq(wlan_irq, pst_bus_dev);
    oal_gpio_free(g_st_board_info.dev_wakeup_host[W_SYS]);

    ret = gpio_request(g_st_board_info.dev_wakeup_host[W_SYS], PROC_NAME_GPIO_WLAN_WAKEUP_HOST);
    if (ret != 0) {
    ...
    gpio_direction_input(g_st_board_info.dev_wakeup_host[W_SYS]);
    wlan_irq = gpio_to_irq(g_st_board_info.dev_wakeup_host[W_SYS]);
    ret = request_irq(wlan_irq, hcc_bus_wlan_gpio_irq,
        IRQF_NO_SUSPEND | IRQF_TRIGGER_RISING | IRQF_DISABLED,
        "wifi_gpio_intr", pst_bus_dev);
    if (ret != 0) {
...
int32_t oal_wifi_platform_load_dev(void)
{
    ...
    if (ret != OAL_SUCC) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "hisi bus param init fail");
        return ret;
    }
    for (i = 0; i < g_bus_devs.dev_num; i++) {
    ...
    if (usb_cap == OAL_TRUE) {
        ...
        if (ret != OAL_SUCC) {
            oal_print_hi11xx_log(HI11XX_LOG_ERR, "load usb failed ret=%d", ret);
            return ret;
        }
    }
...
int32_t hcc_dev_init(void)
{
    ...
    hcc_bus_dev *pst_bus_dev = NULL;
#ifdef _PRE_GT_SUPPORT
    pst_bus_dev = hcc_get_bus_dev(HCC_EP_GT_DEV);
                             ...
                             long nice)
{
    ...
    if (oal_warn_on(p == NULL) || (param == NULL)) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "param set thread scheduler policy fail, para is null");
        return;
    };
...
void hcc_bus_try_to_dump_device_mem(hcc_bus *hi_bus, int32_t is_sync)
{
    ...
    if (oal_trigger_bus_exception(hi_bus, is_sync) != OAL_TRUE) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "hcc_bus_try_to_dump_device_mem fail\n");
    }
}
