...
OAL_STATIC int32_t oal_sdio_bindcpu(hcc_bus *pst_bus, uint32_t chan, int32_t is_bind)
{
    ...
    if (pst_bus->pst_rx_tsk != NULL) {
        ...
        oal_print_hi11xx_log(HI11XX_LOG_INFO, "wlan bus rx thread to cpu 0x%lx",
...
OAL_STATIC struct sdio_device_id const g_oal_sdio_ids[] = {
    ...
    { SDIO_DEVICE(HISDIO_VENDOR_ID_HISI,    HISDIO_PRODUCT_ID_SHENKUO_HISI) },
    { SDIO_DEVICE(HISDIO_VENDOR_ID_HISI,    HISDIO_PRODUCT_ID_BISHENG) },
    {},
};
...
void oal_sdio_tc_msg_001(int msg)
{
    ...
    oal_print_hi11xx_log(HI11XX_LOG_INFO, "send msg 0x%8X, ret = %d", msg, ret);
    wlan_pm_enable();
}
...
#else
struct oal_sdio g_hi110x_sdio_ut;
int32_t oal_wifi_platform_load_sdio(void)
{
    return OAL_SUCC;
}
