...
OAL_STATIC const oal_pci_device_id_stru g_pcie_id_tab[] = {
    ...
    { PCIE_HISI_VENDOR_ID,   PCIE_HISI_DEVICE_ID_SHENKUOFPGA, PCI_ANY_ID, PCI_ANY_ID }, /* shenkuo FPGA PCIE */
    { PCIE_HISI_VENDOR_ID_0, PCIE_HISI_DEVICE_ID_SHENKUO, PCI_ANY_ID, PCI_ANY_ID }, /* shenkuo PCIE */
    { PCIE_HISI_VENDOR_ID,   PCIE_HISI_DEVICE_ID_SHENKUO, PCI_ANY_ID, PCI_ANY_ID }, /* shenkuo tmp PCIE */
    { PCIE_HISI_VENDOR_ID_0, PCIE_HISI_DEVICE_ID_BISHENG, PCI_ANY_ID, PCI_ANY_ID }, /* bisheng tmp PCIE */
...
{
    int32_t ret;
    oal_print_hi11xx_log(HI11XX_LOG_INFO, "before set, dma mask:0x%llx coherent mask:0x%llx",
    ...
    if (ret != 0) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "dma set mask failed=%d", ret);
        return ret;
    }
    oal_print_hi11xx_log(HI11XX_LOG_INFO, "after set, dma mask:0x%llx coherent mask:0x%llx",
                         dma_get_mask(&pst_pci_dev->dev), pst_pci_dev->dev.coherent_dma_mask);
    return OAL_SUCC;
}
                                         ...
                                         int32_t is_bind)
{
    ...
    if (bindcpu_cfg->irq_cpu && pci_dev->irq != 0) {
        ...
        oal_print_hi11xx_log(HI11XX_LOG_INFO, "pcie bind irq[%d] to cpu %d",
    ...
    if (bindcpu_cfg->thread_cmd && pci_lres->pst_pci_res->pst_rx_hi_task != NULL) {
        if (bindcpu_cfg->is_bind) {
            /* 中断所在核会影响线程迁移 */
            cpumask_clear_cpu(first_big_cpu, &fast_cpus);
...
int32_t oal_pcie_get_sleep_state(hcc_bus *pst_bus)
{
    ...
    return (board_get_host_wakeup_dev_state(W_SYS) == 1) ? DISALLOW_TO_SLEEP_VALUE : ALLOW_TO_SLEEP_VALUE;
}
