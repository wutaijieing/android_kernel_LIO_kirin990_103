...
OAL_STATIC OAL_INLINE int32_t ete_send_check_param(oal_pcie_linux_res *pcie_res, oal_netbuf_head_stru *pst_head)
{
    ...
    /* pcie is link */
    if (oal_unlikely(pcie_res->comm_res->link_state <= PCI_WLAN_LINK_DOWN)) {
                                                             ...
                                                             oal_pci_dev_stru *pst_pci_dev)
{
    ...
    for (i = 0; i < (int32_t)send_cnt; i++) {
        ...
        /* Debug */
        if (oal_unlikely(pcie_res->comm_res->link_state < PCI_WLAN_LINK_WORK_UP)) {
...
int32_t oal_ete_rx_cnt_host_ddr_mode_init(oal_pcie_linux_res *pcie_res)
{
    ...
    if (ret != OAL_SUCC) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "pcie_ete_rx_cnt_host_ddr_mode_res_init failed!");
        return ret;
    }
    ret = pcie_ete_rx_cnt_host_ddr_mode_enable((uintptr_t)ep_res->pst_ete_base, OAL_TRUE);
    if (ret != OAL_SUCC) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "pcie_ete_rx_cnt_host_ddr_mode_enable failed!");
        return ret;
    }
    return OAL_SUCC;
}
...
int32_t oal_ete_rx_cnt_host_ddr_mode_exit(oal_pcie_linux_res *pcie_res)
{
    ...
    if (ret != OAL_SUCC) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "pcie_ete_rx_cnt_host_ddr_mode disable failed!");
    }
...
int32_t oal_ete_ip_init(oal_pcie_linux_res *pcie_res)
{
    ...
    if (ret != OAL_SUCC) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "oal_ete_rx_write_end_mode_init failed!");
        return ret;
    }
    return OAL_SUCC;
}
...
void oal_ete_ip_exit(oal_pcie_linux_res *pcie_res)
{
    ...
    if (ret != OAL_SUCC) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "oal_ete_rx_cnt_host_ddr_mode_exit failed!");
    }
    /* stop ete */
...
OAL_STATIC void oal_ete_int_mask_init(oal_pcie_ep_res *ep_res)
{
    ...
    oal_print_hi11xx_log(HI11XX_LOG_INFO, "host_intr_mask:0x%x", ep_res->ete_info.host_intr_mask);
    ...
    oal_print_hi11xx_log(HI11XX_LOG_INFO, "pcie_ctl_ete_intr_mask:0x%x", ep_res->ete_info.pcie_ctl_ete_intr_mask);
...
OAL_STATIC int32_t oal_ete_rx_hi_thread(void *data)
{
    ...
    if (oal_warn_on(ep_res == NULL)) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "error: ep_res is null");
        return -EFAIL;
    }

    pcie_res = oal_list_get_entry(ep_res, oal_pcie_linux_res, ep_res);
    if (oal_warn_on(pcie_res == NULL)) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "error: pcie_res is null");
        return -EFAIL;
    }
    ...
    forever_loop() {
        ...
        if (oal_unlikely(comm_res->link_state < PCI_WLAN_LINK_WORK_UP || !comm_res->regions.inited)) {
            oal_print_hi11xx_log(HI11XX_LOG_WARN,
                                 "hi thread link invaild, stop supply mem, link_state:%s, region:%d",
                                 oal_pcie_get_link_state_str(comm_res->link_state),
                                 comm_res->regions.inited);
        } else {
            ret = OAL_SUCC;
...
void oal_ete_print_transfer_info(oal_pcie_linux_res *pcie_res, uint64_t print_flag)
{
    ...
    if (oal_unlikely(pcie_res->comm_res->link_state >= PCI_WLAN_LINK_RES_UP)) {
        ...
        oal_print_hi11xx_log(HI11XX_LOG_INFO, "pcie is %s", g_pcie_link_state_str[pcie_res->comm_res->link_state]);
    }
}
