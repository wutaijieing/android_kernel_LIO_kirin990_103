...
static struct pcie_rx_cnt_host_res g_rx_cnt_host_res = {
    .dma_handle = (dma_addr_t)NULL,
    .cpu_addr = (void *)NULL,
};

/*lint -e651*/
static const host_ctrl_addr g_shenkuo_hostctrl_addr = {
...
/*lint +e651*/
host_ctrl_addr* g_pst_ete_addr = (host_ctrl_addr*)&g_shenkuo_hostctrl_addr;
...
static uint32_t pcie_ete_rx_get_cnt_from_host_ddr(uint32_t chan, uint32_t pcie_id)
{
    uint32_t *cpu_addr = NULL;
    if ((chan >= ETE_RX_CHANN_MAX_NUMS) || (pcie_id >= RX_CHANNEL_CNT_HOST_DDR_MODE_PCIE_NUM)) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "pcie_ete_rx_get_cnt_from_host_ddr pcie %d chan %d get failed", chan,
                             pcie_id);
        return 0;
    }
    if (g_rx_cnt_host_res.cpu_addr == NULL) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "pcie_ete_rx_get_cnt_from_host_ddr failed, not init");
        return 0;
    }
    cpu_addr = (uint32_t *)(g_rx_cnt_host_res.cpu_addr + pcie_id * ETE_RX_CHANN_MAX_NUMS * sizeof(uint32_t) +
                            chan * sizeof(uint32_t));
    return oal_readl(cpu_addr);
}

static void pcie_ete_rx_set_cnt_to_host_ddr(uint32_t chan, uint32_t pcie_id, uint32_t val)
{
    uint32_t *cpu_addr = NULL;
    if ((chan >= ETE_RX_CHANN_MAX_NUMS) || (pcie_id >= RX_CHANNEL_CNT_HOST_DDR_MODE_PCIE_NUM)) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "pcie_ete_rx_set_cnt_to_host_ddr pcie %d chan %d get failed", chan,
                             pcie_id);
        return;
    }
    if (g_rx_cnt_host_res.cpu_addr == NULL) {
...
int32_t pcie_ete_rx_cnt_host_ddr_mode_res_init(oal_pcie_linux_res *pcie_res)
{
    ...
    size_t alloc_size = sizeof(uint32_t) * RX_CHANNEL_CNT_HOST_DDR_MODE_PCIE_NUM * ETE_RX_CHANN_MAX_NUMS;

    g_rx_cnt_host_res.cpu_addr =
        dma_alloc_coherent(&pst_pci_dev->dev, alloc_size, &g_rx_cnt_host_res.dma_handle, GFP_KERNEL);
    if (g_rx_cnt_host_res.cpu_addr == NULL) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "pcie_ete_rx_cnt_host_ddr_mode_res_init init failed");
        return -OAL_ENOMEM;
    }
    for (j = 0; j < RX_CHANNEL_CNT_HOST_DDR_MODE_PCIE_NUM; j++) {
        for (i = 0; i < ETE_RX_CHANN_MAX_NUMS; i++) {
            ret = oal_pcie_host_slave_address_switch(pcie_res,
                                                     (uint64_t)(g_rx_cnt_host_res.dma_handle +
                                                                j * sizeof(uint32_t) * ETE_RX_CHANN_MAX_NUMS +
                                                                i * sizeof(uint32_t)),
                                                     &dst_addr, OAL_TRUE);
            if (ret != OAL_SUCC) {
                oal_print_hi11xx_log(HI11XX_LOG_ERR, "slave address switch failed ret=%d iova=%llx", ret,
                                     (uint64_t)g_rx_cnt_host_res.dma_handle);
                dma_free_coherent(&pst_pci_dev->dev, alloc_size, g_rx_cnt_host_res.cpu_addr,
                                  g_rx_cnt_host_res.dma_handle);
                return ret;
            }
            pcie_ete_set_rx_cnt_host_addr((uintptr_t)ep_res->pst_ete_base, i, j, dst_addr);
            pcie_ete_rx_set_cnt_to_host_ddr(i, j, PCIE_ETE_INVALID_RD_VALUE);
        }
    }
    return OAL_SUCC;
}
...
int32_t pcie_ete_rx_cnt_host_ddr_mode_res_release(oal_pcie_linux_res *pcie_res)
{
    oal_pci_dev_stru *pst_pci_dev = pcie_res->comm_res->pcie_dev;
    size_t alloc_size = sizeof(uint32_t) * RX_CHANNEL_CNT_HOST_DDR_MODE_PCIE_NUM * ETE_RX_CHANN_MAX_NUMS;
    dma_free_coherent(&pst_pci_dev->dev, alloc_size, g_rx_cnt_host_res.cpu_addr, g_rx_cnt_host_res.dma_handle);
    g_rx_cnt_host_res.cpu_addr = (void *)NULL;
    g_rx_cnt_host_res.dma_handle = (dma_addr_t)NULL;
    return OAL_SUCC;
}

unsigned int pcie_ete_get_rx_done_total_cnt(uintptr_t base_address, uint32_t chan_idx)
{
    return pcie_ete_rx_get_cnt_from_host_ddr(chan_idx, 0);
}
