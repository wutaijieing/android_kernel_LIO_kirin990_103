...
int32_t oal_pcie_ip_factory_test(hcc_bus *pst_bus, int32_t test_count)
{
    ...
    if (oal_warn_on(pst_pci_lres->ep_res.chip_info.cb.pcie_ip_factory_test == NULL)) {
        return -OAL_EIO;
    }

    return pst_pci_lres->ep_res.chip_info.cb.pcie_ip_factory_test(pst_bus, test_count);
}
...
static pcie_chip_id_stru g_pcie_chiptest_id[] = {
    {PCIE_HISI_DEVICE_ID_HI1161, oal_pcie_chiptest_init_hi1161}
};

int32_t oal_pcie_chiptest_init(oal_pcie_linux_res *pst_pci_res, int32_t device_id)
{
    int32_t i;
