                                         ...
                                         oal_pcie_linux_res *pst_pcie_res)
{
    ...
    ret = oal_pcie_inbound_ca_to_va(pst_pcie_res->comm_res, cpu_address, &addr_map);
    if (ret != OAL_SUCC) {
...
OAL_STATIC struct pm_test_trace_info* oal_pcie_pm_get_trace_info(oal_pcie_linux_res *pst_pcie_res, uint32_t subsys_reg)
{
    ...
    ret = oal_pcie_inbound_ca_to_va(pst_pcie_res->comm_res, subsys_reg, &addr_map);
    if (ret != OAL_SUCC) {
        oal_io_print("pcie_read32 0x%8x unmap, read failed!\n", subsys_reg);
        return NULL;
    }
    ...
    ret = oal_pcie_inbound_ca_to_va(pst_pcie_res->comm_res, trace_addr, &addr_map);
    if (ret != OAL_SUCC) {
        oal_io_print("pcie_read32 0x%8x unmap, read failed!\n", trace_addr);
        return NULL;
    }
...
#define GLE_TRACE_INFO_REG 0x40001c48
                                    ...
                                    uint32_t length)
{
    ...
    for (index = 0; index < length; index += sizeof(uint32_t)) { /* 每次偏移4字节 */
        ret = oal_pcie_inbound_ca_to_va(pst_pcie_res->comm_res, cpu_address + index, &addr_map);
        if (ret != OAL_SUCC) {
...
OAL_STATIC int32_t oal_pcie_send_test_pkt(int32_t num)
{
    ...
    hcc_bus_dev *bus_dev = hcc_get_bus_dev(hcc_get_main_dev());
                                            ...
                                            uint32_t outbound_size, dma_addr_t outbound_dma_addr)
{
    ...
    if (pst_pcie_res->ep_res.chip_info.ete_support == OAL_TRUE) {
        ...
        if (ret == OAL_SUCC) {
            ...
            if (ret == OAL_SUCC) {
                oal_print_hi11xx_log(HI11XX_LOG_INFO, "pcie_if_devva_to_hostca devva=0x%llx -> iova=0x%llx", devva,
                                     host_iova);
            } else {
...
OAL_STATIC int32_t oal_pcie_addr_switch_test(oal_pcie_linux_res *pst_pcie_res, const char *buf)
{
    ...
    if (pst_pcie_res->ep_res.chip_info.addr_info.glb_ctrl != 0) {
        ...
        if (ret == OAL_SUCC) {
            ...
            if (ret == OAL_SUCC) {
                oal_print_hi11xx_log(HI11XX_LOG_INFO, "oal_pcie_get_dev_ca 0x%llx -> 0x%x", host_va,
                                     (uint32_t)dev_cpuaddr);
            } else {
                oal_print_hi11xx_log(HI11XX_LOG_ERR, "oal_pcie_get_dev_ca 0x%llx  convert failed", host_va);
            }
        } else {
                                            ...
                                            uint32_t data)
{
    ...
    ret = oal_pcie_inbound_ca_to_va(pst_pcie_res->comm_res, cpuaddr + size - 1, &addr_map);
    if (ret != OAL_SUCC) {
        pci_print_log(PCI_LOG_ERR, "unmap device address 0x%x, size:%u", cpuaddr, size);
        return -OAL_EINVAL;
    }

    ret = oal_pcie_inbound_ca_to_va(pst_pcie_res->comm_res, cpuaddr, &addr_map);
    if (ret != OAL_SUCC) {
                                             ...
                                             uint32_t times, uint32_t burst_size)
{
    ...
    ret = oal_pcie_inbound_ca_to_va(pst_pcie_res->comm_res, cpu_address, &addr_map_start);
    if (ret != OAL_SUCC) {
        pci_print_log(PCI_LOG_ERR, "invalid device cpu address 0x%x", cpu_address);
        return -OAL_EINVAL;
    }

    ret = oal_pcie_inbound_ca_to_va(pst_pcie_res->comm_res, cpu_address + length - 1, &addr_map_end);
    if (ret != OAL_SUCC) {
                                                           ...
                                                           const char *buf, oal_pcie_linux_res *pst_pcie_res)
{
    ...
    /* Just for debug */
    if ((sscanf_s(buf, "%u %d", &test_time, &alloc_count) != 2)) {
        return -OAL_EINVAL;
    }
                                     ...
                                     int32_t performance)
{
    ...
    forever_loop()
    {
        ...
        fp->f_pos += rlen;
        ret = oal_pcie_inbound_ca_to_va(pst_pcie_res->comm_res, cpu_address + total_len, &addr_map);
        if (ret != OAL_SUCC) {
