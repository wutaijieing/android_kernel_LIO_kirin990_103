...
OAL_STATIC int32_t oal_dft_dump_key_info(char *buf, int32_t key_type, uint32_t print_tag)
{
    ...
    oal_list_search_for_each(pst_pos, &g_dft_head)
    {
        ...
        memcpy_s(&first_tv, sizeof(first_tv), &pst_dft_item->first_timestamp, sizeof(pst_dft_item->first_timestamp));
        ...
        memcpy_s(&last_tv, sizeof(last_tv), &pst_dft_item->last_timestamp, sizeof(pst_dft_item->last_timestamp));
        if (last_tv.tv_sec) {
...
void oal_dft_print_error_key_info(void)
{
    ...
    for (i = (OAL_DFT_TRACE_BUTT - 1); i >= OAL_DFT_TRACE_FAIL; i--) {
        ret = oal_dft_dump_key_info(NULL, i, (KERN_PRINT));
        if (ret < 0) {
