...
module_param(g_ft_pcie_loopback_bias_cldo3, uint, S_IRUGO | S_IWUSR);
module_param(g_ft_pcie_loopback_bias_vptx, uint, S_IRUGO | S_IWUSR);
module_param(g_ft_pcie_loopback_bias_vph, uint, S_IRUGO | S_IWUSR);
#endif
...
int32_t hcc_test_init_module(struct hcc_handler *hcc)
{
    ...
    if (hcc->bus_dev->dev_id == HCC_EP_GT_DEV) {
