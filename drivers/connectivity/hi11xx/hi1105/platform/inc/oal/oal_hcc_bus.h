...
extern void hcc_bus_performance_core_schedule(uint32_t dev_id);
...
extern int32_t hi110x_hcc_ip_pm_ctrl(uint32_t enable, uint8_t dev_id);
...
OAL_STATIC OAL_INLINE int32_t hcc_bus_wakeup_request(hcc_bus *hi_bus)
{
    ...
    if (oal_warn_on(hi_bus->opt_ops == NULL)) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "hcc_bus_wakeup_request: opt_ops NULL");
        return -OAL_ENODEV;
    }

    if (hi_bus->opt_ops->wakeup_request == NULL) {
        oal_print_hi11xx_log(HI11XX_LOG_ERR, "hcc_bus_wakeup_request: opt_ops->wakeup_request NULL");
        return -OAL_EIO;
    }
