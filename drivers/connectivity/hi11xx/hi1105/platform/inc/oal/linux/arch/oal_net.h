...
/* Rx Delay */
#define RX_SKB_OFFSET(_trace_id) (_trace_id << 2)

/* data指向hcc hdr: [5+1+3+31+ 24(time)] */
#define ETE_RX_SKB_DELAY(_rx_skb, _trace_id) \
    (*(uint32_t*)((uint8_t*)oal_netbuf_data(_rx_skb) + 40 + RX_SKB_OFFSET(_trace_id)))
#define SET_RX_SKB_ETE_DELAY_TRACE_TIME(_rx_skb, _trace_id, _new_time) \
    ETE_RX_SKB_DELAY(_rx_skb,  _trace_id) = _new_time

/* data指向payload: 40 + 24(time)[] */
#define RX_SKB_DELAY(_rx_skb, _trace_id) \
    (*(uint32_t*)((uint8_t*)oal_netbuf_data(_rx_skb) - 24 + RX_SKB_OFFSET(_trace_id)))
#define SET_RX_SKB_HCC_DELAY_TRACE_TIME(_rx_skb, _trace_id, _new_time) \
    RX_SKB_DELAY(_rx_skb,  _trace_id) = _new_time

/* napi_poll:data指向payload: 31+ 24(time)+12+2(ether+type) */
/* net_if:data指向payload: 31+ 24(time)+12+2(ether+type) */
#define HOST_RX_SKB_DELAY(_rx_skb, _trace_id) \
    (*(uint32_t*)((uint8_t*)oal_netbuf_data(_rx_skb) - 38 + RX_SKB_OFFSET(_trace_id)))
...
/* data指向了hcc hdr, CB头地址 5+1+3+19字节 */
#define ETE_TX_SKB_DELAY(_tx_skb, _trace_id) \
    (*(uint32_t*)((uint8_t*)oal_netbuf_data(_tx_skb) + 28 + RX_SKB_OFFSET(_trace_id))) /* why 32? */
#define SET_TX_SKB_ETE_DELAY_TRACE_TIME(_tx_skb, _trace_id, _new_time) \
    (ETE_TX_SKB_DELAY(_tx_skb,  _trace_id) = _new_time)
#define GET_TX_SKB_ETE_DELAY_TRACE_TIME(_tx_skb, _trace_id)    ETE_TX_SKB_DELAY(_tx_skb, _trace_id)
#endif
...
OAL_STATIC OAL_INLINE int32_t oal_ieee80211_frequency_to_channel(int32_t center_freq)
{
    ...
    } else if (center_freq >= 58320 && center_freq <= 64800) { // 60G频段下中心频率在[58320, 64800]对应的信道号 =(频率 - 5616)/ 2160;
...
OAL_STATIC OAL_INLINE int32_t oal_net_register_netdev(oal_net_device_stru *p_net_device)
{
    ...
    const uint16_t us_needed_headroom_size = 84;
#else
