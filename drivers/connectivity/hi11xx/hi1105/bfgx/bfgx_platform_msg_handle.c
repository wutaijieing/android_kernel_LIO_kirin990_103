...
#endif
    bfgx_thread_msg_handle,      // SYS_INF_GLE_INIT = 0x23,                GLE软件初始化完成
    bfgx_thread_msg_handle,      // SYS_INF_GLE_EXIT = 0x24,                GLE软件初始化完成
};
                                   ...
                                   uint8_t *buf_ptr)
{
    ...
    switch (msg) {
        ...
        case SYS_INF_ENABLE_GPIO_WKUP_BFG_RSP:
            ...
            break;
#endif
        case SYS_INF_GLE_INIT:
            thread_completion_handle(ps_core_d, THREAD_FLAG_INIT, BFGX_GLE);
            break;
        case SYS_INF_GLE_EXIT:
            thread_completion_handle(ps_core_d, THREAD_FLAG_EXIT, BFGX_GLE);
            break;
        default:
            break;
    };
...
STATIC void heartbeat_debug_print(struct ps_core_s *ps_core_d, uint8_t syschar)
{
    ...
    ps_print_info("[%s]pkt_rx:BT=%lu,GNSS=%lu,FM=%lu,SYS_MSG=%lu,OML=%lu,timer=%d,mod=%d,sleep_flag=%d,frc_awake=%d\n",
