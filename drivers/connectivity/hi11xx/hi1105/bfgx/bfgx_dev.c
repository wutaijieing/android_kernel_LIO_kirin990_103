...
STATIC struct service_attribute g_hi110x_srv_desc[BFGX_BUTT] = {
    ...
    {"NFC", OAL_TRUE, BUART, SYS_CFG_OPEN_NFC, SYS_CFG_CLOSE_NFC, WAIT_NFC_OPEN_TIME, WAIT_NFC_CLOSE_TIME},
    {"GNSS_ME", OAL_TRUE, GUART, SYS_CFG_OPEN_GNSS, SYS_CFG_CLOSE_GNSS, WAIT_GNSS_OPEN_TIME, WAIT_GNSS_CLOSE_TIME}
};

STATIC uint8_t g_hi110x_bus_en_map[UART_BUTT] = {B_SYS, B_SYS};

STATIC struct service_attribute g_bisheng_srv_desc[BFGX_BUTT] = {
    {"BT", OAL_TRUE, BUART, SYS_CFG_OPEN_BT, SYS_CFG_CLOSE_BT, WAIT_BT_OPEN_TIME, WAIT_BT_CLOSE_TIME},
    ...
    {"IR", OAL_TRUE, BUART, SYS_CFG_OPEN_IR, SYS_CFG_CLOSE_IR, WAIT_IR_OPEN_TIME, WAIT_IR_CLOSE_TIME},
    {"NFC", OAL_TRUE, BUART, SYS_CFG_OPEN_NFC, SYS_CFG_CLOSE_NFC, WAIT_NFC_OPEN_TIME, WAIT_NFC_CLOSE_TIME},
    {"GLE", OAL_TRUE, GUART, SYS_CFG_OPEN_GLE, SYS_CFG_CLOSE_GLE, WAIT_GLE_OPEN_TIME, WAIT_GLE_CLOSE_TIME}
};

STATIC uint8_t g_bisheng_bus_en_map[UART_BUTT] = {B_SYS, G_SYS};

STATIC struct service_attribute g_hi1161_srv_desc[BFGX_BUTT] = {
    {"BT", OAL_TRUE, BUART, SYS_CFG_OPEN_BT, SYS_CFG_CLOSE_BT, WAIT_BT_OPEN_TIME, WAIT_BT_CLOSE_TIME},
...
STATIC uint8_t g_hi1161_bus_en_map[UART_BUTT] = {B_SYS, G_SYS};
...
void service_attribute_load(void)
{
    ...
    chip_type = get_hi110x_subchip_type();
    if (chip_type == BOARD_VERSION_BISHENG) {
        top->srv_desc = g_bisheng_srv_desc;
        top->bus_en_map = g_bisheng_bus_en_map;
        ps_print_info("%s load bisheng service attribute\n", __func__);
    } else if (chip_type == BOARD_VERSION_HI1161) {
        top->srv_desc = g_hi1161_srv_desc;
        top->bus_en_map = g_hi1161_bus_en_map;
        ps_print_info("%s load hi1161 service attribute\n", __func__);
    } else {
        top->srv_desc = g_hi110x_srv_desc;
        top->bus_en_map = g_hi110x_bus_en_map;
        ps_print_info("%s load hi110x service attribute\n", __func__);
    }
}
...
uint8_t service_get_valid(uint8_t service)
{
    ...
    return top->srv_desc[service].valid;
}
...
void service_set_bus_id(uint8_t service, uint8_t bus_id)
{
    if (service > BFGX_BUTT || bus_id > UART_BUTT) {
        ps_print_err("para error %d %d\n", service, bus_id);
        return;
    }
    ps_print_err("change sevice %d  bus to %d\n", service, bus_id);
    if (get_hi110x_subchip_type() == BOARD_VERSION_BISHENG) {
        g_bisheng_srv_desc[service].bus_id = bus_id;
    } else if (get_hi110x_subchip_type() == BOARD_VERSION_HI1161) {
        g_hi1161_srv_desc[service].bus_id = bus_id;
    } else {
        g_hi110x_srv_desc[service].bus_id = bus_id;
    }
}
...
{
    struct ps_core_s *ps_core_d = ps_get_core_reference(service_get_bus_id(subsys));
...
void ps_kfree_skb(struct ps_core_s *ps_core_d, uint8_t type)
{
    ...
    switch (type) {
        ...
        case RX_IR_QUEUE:
            ...
            break;
        case RX_GLE_QUEUE:
            skb_queue_purge(&ps_core_d->bfgx_info[BFGX_GLE].rx_queue);
            break;
        default:
...
int32_t ps_restore_skbqueue(struct ps_core_s *ps_core_d, struct sk_buff *skb, uint8_t type)
{
    ...
    switch (type) {
        ...
        case RX_NFC_QUEUE:
            ...
            break;
        case RX_GLE_QUEUE:
            skb_queue_head(&ps_core_d->bfgx_info[BFGX_GLE].rx_queue, skb);
            break;
        case RX_DBG_QUEUE:
...
int32_t alloc_seperted_rx_buf(struct ps_core_s *ps_core_d, uint8_t subsys, uint32_t len, uint8_t alloctype)
{
    ...
    if (subsys == BFGX_BT || subsys == BFGX_GLE) {
...
#define WAIT_CLOSE_TIMES 100
    ...
    for (i = 0; i < WAIT_CLOSE_TIMES; i++) {
        bwkup_gpio_val = oal_gpio_get_value(ps_core_d->pm_data->wakeup_host_gpio);
        if (bwkup_gpio_val == 0) {
            ret = SUCCESS;
            break;
        }
...
STATIC int32_t hw_bfgx_input_check(uint32_t subsys)
{
    ...
    if (oal_warn_on(is_bfgx_support() != OAL_TRUE) || (service_get_valid(subsys) == OAL_FALSE)) {
...
STATIC int32_t hw_bfgx_start(struct ps_core_s *ps_core_d, uint32_t subsys)
{
    ...
    if (bfgx_open_cmd_send(subsys, pm_data->index) != BFGX_POWER_SUCCESS) {
...
int32_t hw_bfgx_open(uint32_t subsys)
{
    ...
    ps_core_d = ps_get_core_reference(service_get_bus_id(subsys));
...
int32_t hw_bfgx_close(uint32_t subsys)
{
    ...
    if (subsys >= BFGX_BUTT || (service_get_valid(subsys) == OAL_FALSE)) {
        ps_print_err("subsys is err, subsys is [%d]\n", subsys);
        return -EINVAL;
    }
    ...
    ret = bfgx_close_cmd_send(subsys, pm_data->index);
    if (ret < 0) {
                             ...
                             struct bt_data_combination *data_comb)
{
    ...
    struct ps_core_s *ps_core_d = ps_get_core_reference(service_get_bus_id(BFGX_BT));
    if (hw_bfgx_write_check(filp, buf, ps_core_d, BFGX_BT) < 0) {
        return -EINVAL;
    }
...
{
    struct pm_drv_data *pm_data = pm_get_drvdata(service_get_bus_id(BFGX_BT));
    struct pm_top* pm_top_data = pm_get_top();
...
STATIC ssize_t hw_ir_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
    ...
    ps_core_d = ps_get_core_reference(service_get_bus_id(BFGX_IR));
    if (unlikely((ps_core_d == NULL) || (buf == NULL) || (filp == NULL))) {
        ps_print_err("ps_core_d is NULL\n");
        return -EINVAL;
    }
...
STATIC int32_t hw_fm_open(struct inode *inode, struct file *filp)
{
    ...
    ps_core_d = ps_get_core_reference(service_get_bus_id(BFGX_FM));
 ...
 * 功能描述  : 不支持NFC的情况下，平台利用该接口单独上电，用于平台功能调试
               uart:指定命令通过哪个uart下发。bisheng支持双uart独立启动
 */
int32_t hw_plat_open(uint8_t uart)
{
    ...
    if (get_hi110x_subchip_type() == BOARD_VERSION_BISHENG) {
        g_bisheng_srv_desc[BFGX_NFC].bus_id = uart;
    } else if (get_hi110x_subchip_type() == BOARD_VERSION_HI1161) {
        g_hi1161_srv_desc[BFGX_NFC].bus_id = uart;
    } else {
        g_hi110x_srv_desc[BFGX_NFC].bus_id = BUART;
    }
...
{
    int8_t index = ps_core_d->dev_index;
...
{
    return plat_excp_dump_rotate_cmd_read(arg, &g_gt_memdump_cfg);
}
/*
...
STATIC void uart_loop_result(uint32_t count, uint16_t pkt_len, unsigned long long total_time)
{
...
int32_t uart_loop_test(void)
{
    ...
    uart_loop_result(count, pkt_len, total_time);
...
STATIC int32_t ps_probe(struct platform_device *pdev)
{
    ...
    strncpy_s(g_hisi_platform_data.dev_name, sizeof(g_hisi_platform_data.dev_name),
              bd_info->uart_port[BUART], sizeof(g_hisi_platform_data.dev_name) - 1);
    ...
    ps_plat_d->core_data->skb_retry_count = get_skb_retry_count();
...
int32_t hw_ps_init(void)
{
    ...
    /* vfb version fastboot bugfix: after bt cali and close, b/g_wakeup_host gpio is high */
    ret = hw_ps_me_init();
    ret = hw_ps_gle_init();
    ret = hw_ps_gt_init();
