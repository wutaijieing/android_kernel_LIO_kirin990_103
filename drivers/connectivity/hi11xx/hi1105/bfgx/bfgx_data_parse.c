...
int32_t (*gle_tty_recv)(void *, const uint8_t *, int32_t);
...
STATIC struct ps_tx_manage_info g_ps_tx_info[BFGX_BUTT] = {
    ...
    {PKG_NOT_SEPRETED, TX_HIGH_QUEUE, GLE_MSG, GLE_MSG, GLE_MSG}
};
...
STATIC ps_core_msg_handler g_ps_core_msg_handle[MSG_BUTT] = {
    ...
    ps_recv_no_sepreated_data,   // GLE_MSG = 0x015,         GLE串口消息
};
...
STATIC void ps_device_last_word_check_process(struct ps_core_s *ps_core_d, uint8_t *buf_ptr)
{
    ...
    if ((dbg_pkt_lenth == rx_pkt_total_len) && (dbg_pkt_lenth == LAST_WORD_LEN) &&
        ...
        /* Shenkuo联调阶段, 尝试dump寄存器辅助分析 */
        if (is_hi110x_debug_type() && (get_hi110x_subchip_type() == BOARD_VERSION_SHENKUO)) {
...
void ps_skb_enqueue(struct ps_core_s *ps_core_d, struct sk_buff *skb, uint8_t type)
{
    ...
    switch (type) {
        ...
        case RX_IR_QUEUE:
            ...
            break;
        case RX_GLE_QUEUE:
            skb_queue_tail(&ps_core_d->bfgx_info[BFGX_GLE].rx_queue, skb);
            break;
        default:
...
struct sk_buff *ps_skb_dequeue(struct ps_core_s *ps_core_d, uint8_t type)
{
    ...
    switch (type) {
        ...
        case RX_NFC_QUEUE:
            ...
            break;
        case RX_GLE_QUEUE:
            curr_skb = skb_dequeue(&ps_core_d->bfgx_info[BFGX_GLE].rx_queue);
            break;
        default:
            ps_print_err("queue type is error, type=%d\n", type);
            break;
...
int32_t ps_skb_queue_len(struct ps_core_s *ps_core_d, uint8_t type)
{
    ...
    switch (type) {
        ...
        case RX_NFC_QUEUE:
            ...
            break;
        case RX_GLE_QUEUE:
            curr_queue = &ps_core_d->bfgx_info[BFGX_GLE].rx_queue;
            break;
        default:
            ps_print_err("queue type is error, type=%d\n", type);
            break;
...
int32_t ps_core_exit(struct ps_core_s *ps_core_d, int index)
{
    ...
    if (ps_core_d != NULL) {
        ...
        ps_kfree_skb(ps_core_d, RX_GLE_QUEUE);
