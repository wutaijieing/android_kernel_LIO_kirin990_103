...
OAL_STATIC int32_t wifi_subsystem_reset(void)
{
    ...
    if (firmware_download_function(WIFI_CFG, hcc_get_bus(HCC_EP_WIFI_DEV)) != EXCEPTION_SUCCESS) {
...
OAL_STATIC int32_t wifi_system_reset(void)
{
    ...
    ret = board_power_reset(W_SYS);
    if (ret) {
        ps_print_err("board_power_reset wlan failed=%d\n", ret);
        return -EXCEPTION_FAIL;
    }

    /* 独立加载模式且单wifi场景下，仅加载wifi， 否则，wifi + bfg一起加载 */
    if (bfgx_is_shutdown() && (hi110x_firmware_download_mode() == MODE_SEPEREATE)) {
...
OAL_STATIC int32_t gt_device_reset(void)
{
 ...
 *             count              : 需要读取的内存块个数
 */
OAL_STATIC int32_t wifi_exception_mem_dump(hcc_bus *hi_bus)
{
    int32_t ret = -OAL_EIO;
    int32_t reset_flag;
...
retry:
    ...
    ret = (is_gt == OAL_TRUE) ? gt_device_mem_dump(pst_mem_dump_info, count)
                              : wifi_device_mem_dump(pst_mem_dump_info, count);
 ...
 * 功能描述  : 异常恢复动作触发接口
 */
int32_t wifi_exception_work_submit(uint32_t wifi_excp_type, uint8_t dev_id)
{
...
OAL_STATIC int32_t bfgx_power_reset(void)
{
    ...
    if (firmware_download_function(BFGX_CFG, hcc_get_bus(HCC_EP_WIFI_DEV)) != EXCEPTION_SUCCESS) {
        hcc_bus_disable_state(pm_top_data->wlan_pm_info->pst_bus, OAL_BUS_STATE_ALL);
