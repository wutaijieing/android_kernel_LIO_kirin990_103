...
#undef THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_HOST_HAL_FTM_SHENKUO_C


static uint32_t shenkuo_mac_regs_ftm_transfer(hal_host_device_stru *hal_device, uint32_t reg_addr)
{
    uint64_t host_va = 0;
    ...
    if (oal_pcie_devca_to_hostva(HCC_EP_WIFI_DEV, reg_addr + offset, &host_va) != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{shenkuo_mac_regs_ftm_transfer::regaddr[%x] devca2hostva fail.}", reg_addr);
        return OAL_FAIL;
    }
#ifndef _PRE_LINUX_TEST
    switch (reg_addr) {
        case SHENKUO_MAC_FTM_PROCESS_REG:
            hal_device->ftm_regs.ftm_pro_reg = host_va;
            break;
        case SHENKUO_MAC_FTM_INFO_ADDR_MSB_REG:
            hal_device->ftm_regs.ftm_info_msb = host_va;
            break;
        case SHENKUO_MAC_FTM_INFO_ADDR_LSB_REG:
            hal_device->ftm_regs.ftm_info_lsb = host_va;
            break;
        case SHENKUO_MAC_FTM_BUF_BASE_ADDR_LSB_REG:
            hal_device->ftm_regs.base_lsb = host_va;
            break;
        case SHENKUO_MAC_FTM_BUF_BASE_ADDR_MSB_REG:
            hal_device->ftm_regs.base_msb = host_va;
            break;
        case SHENKUO_MAC_FTM_WHITELIST_ADDR_LSB_0_REG:
            hal_device->ftm_regs.white_addr_lsb = host_va;
            break;
        case SHENKUO_MAC_FTM_WHITELIST_ADDR_MSB_0_REG:
            hal_device->ftm_regs.white_addr_msb = host_va;
            break;
        default:
            oam_warning_log1(0, OAM_SF_FTM, "{shenkuo_mac_regs_ftm_transfer::reg addr[%x] not used.}", reg_addr);
            return OAL_FAIL;
    }
#endif
    return OAL_SUCC;
...
static uint32_t shenkuo_phy_regs_ftm_transfer(hal_host_device_stru *hal_device, uint32_t reg_addr)
{
    uint64_t host_va = 0;
    uint32_t offset  = shenkuo_regs_addr_get_offset(hal_device->device_id, reg_addr);
    if (oal_pcie_devca_to_hostva(HCC_EP_WIFI_DEV, reg_addr + offset, &host_va) != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{shenkuo_phy_regs_ftm_transfer::regaddr[%x] devca2hostva fail.}", reg_addr);
        return OAL_FAIL;
    }
#ifndef _PRE_LINUX_TEST
    switch (reg_addr) {
        ...
        case SHENKUO_RX_AGC_REG_BANK_CFG_AGC_WEAK_SIG_EN_REG:
            hal_device->ftm_regs.ftm_gc_weak_sig_en = host_va;
            break;
        case SHENKUO_RF_CTRL_REG_BANK_0_CFG_ONLINE_CALI_USE_RF_REG_EN_REG:
            hal_device->ftm_regs.ftm_cali_reg = host_va;
            break;
        default:
            oam_warning_log1(0, OAM_SF_FTM, "{shenkuo_phy_regs_ftm_transfer::reg addr[%x] not used.}", reg_addr);
            return OAL_FAIL;
    }
#endif
    return OAL_SUCC;
...
uint32_t shenkuo_host_ftm_reg_init(uint8_t hal_dev_id)
{
    ...
    uint32_t hal_mac_regs[] = {
        SHENKUO_MAC_FTM_PROCESS_REG,
        SHENKUO_MAC_FTM_BUF_BASE_ADDR_LSB_REG,
        SHENKUO_MAC_FTM_BUF_BASE_ADDR_MSB_REG,
        SHENKUO_MAC_FTM_WHITELIST_ADDR_LSB_0_REG,
        SHENKUO_MAC_FTM_WHITELIST_ADDR_MSB_0_REG,
        SHENKUO_MAC_FTM_INFO_ADDR_LSB_REG,
        SHENKUO_MAC_FTM_INFO_ADDR_MSB_REG,
    };
    uint32_t hal_phy_regs[] = {
        SHENKUO_PHY_GLB_REG_BANK_FTM_CFG_REG,
        SHENKUO_RF_CTRL_REG_BANK_0_CFG_ONLINE_CALI_USE_RF_REG_EN_REG,
        SHENKUO_CALI_TEST_REG_BANK_0_FIFO_FORCE_EN_REG,
        SHENKUO_PHY_GLB_REG_BANK_PHY_BW_MODE_REG,
        SHENKUO_RX_AGC_REG_BANK_CFG_AGC_WEAK_SIG_EN_REG,
    };
    hal_host_device_stru *hal_device  = hal_get_host_device(hal_dev_id);
    if (hal_device == NULL) {
        oam_error_log0(0, 0, "shenkuo_host_ftm_reg_init::haldev null!");
        return OAL_FAIL;
    }
    if (hal_device->device_id >= HAL_DEVICE_ID_BUTT) {
        oam_error_log1(0, OAM_SF_FTM, "{shenkuo_host_ftm_reg_init::device_id[%d].}", hal_device->device_id);
        return OAL_FAIL;
    }
    if (!hal_device->ftm_regs.inited) {
        for (idx = 0; idx < sizeof(hal_mac_regs) / sizeof(uint32_t); idx++) {
            if (shenkuo_mac_regs_ftm_transfer(hal_device, hal_mac_regs[idx]) != OAL_SUCC) {
                return OAL_FAIL;
            }
        }
        for (idx = 0; idx < sizeof(hal_phy_regs) / sizeof(uint32_t); idx++) {
            if (shenkuo_phy_regs_ftm_transfer(hal_device, hal_phy_regs[idx]) != OAL_SUCC) {
                return OAL_FAIL;
            }
        }
    }
    hal_device->ftm_regs.inited = OAL_TRUE;
    oam_warning_log0(0, OAM_SF_ANY, "{shenkuo_host_ftm_reg_init::regs addr trans succ.}");

    return OAL_SUCC;
}


static void shenkuo_ftm_get_dscr_addr(uint64_t value, uint8_t *addr)
{
...
hmac_vap_stru *shenkuo_host_vap_get_by_hal_vap_id(uint8_t hal_device_id, uint8_t hal_vap_id)
{
    ...
    if (hal_device == NULL) {
        oam_error_log0(0, 0, "{shenkuo_host_vap_get_by_hal_vap_id::hal_device null.}");
        return NULL;
    }
    if ((hal_vap_id == SHENKUO_OTHER_BSS_ID) || (hal_vap_id < SHENKUO_HAL_VAP_OFFSET) ||
        (hal_vap_id >= (HAL_MAX_VAP_NUM + SHENKUO_HAL_VAP_OFFSET))) {
        return NULL;
    }

    hal_vap_id -= SHENKUO_HAL_VAP_OFFSET;
...
uint32_t shenkuo_host_ftm_get_info(hal_ftm_info_stru *hal_ftm_info, uint8_t *addr)
{
    ...
    shenkuo_ftm_dscr_stru *shenkuo_ftm_dscr = NULL;

    /* 获取hal_ftm_info的地址 */
    shenkuo_ftm_dscr = (shenkuo_ftm_dscr_stru *)addr;
    if (shenkuo_ftm_dscr == NULL) {
        return OAL_FAIL;
    }

    /* 2. 存储部分结果 */
    hal_ftm_info->ftm_vap_index = shenkuo_ftm_dscr->word4.bit_vap_index;
    hal_ftm_info->ftm_init_resp_flag = shenkuo_ftm_dscr->word10.bit_init_resp_flag;
    hal_ftm_info->ftm_intp_time = shenkuo_ftm_dscr->word10.bit_intp_time;
    hal_ftm_info->ftm_rx_time = shenkuo_ftm_dscr->word9.rx_time;
    hal_ftm_info->ftm_tx_time = shenkuo_ftm_dscr->word8.tx_time;
    hal_ftm_info->ftm_dialog_token = shenkuo_ftm_dscr->word4.bit_dialog_token;
    hal_ftm_info->ftm_timestamp = shenkuo_ftm_dscr->word7.timestamp;

    shenkuo_ftm_get_dscr_addr(shenkuo_ftm_dscr->word5.ra, &hal_ftm_info->ftm_ra[MAC_ADDR_0]);
    shenkuo_ftm_get_dscr_addr(shenkuo_ftm_dscr->word6.ta, &hal_ftm_info->ftm_ta[MAC_ADDR_0]);

    for (idx = 0; idx < HAL_HOST_MAX_RF_NUM; idx++) {
        hal_ftm_info->ftm_snr_ant[idx] = shenkuo_ftm_dscr->word1.snr_ant[idx];
        hal_ftm_info->ftm_rssi_ant[idx] = shenkuo_ftm_dscr->word1.rssi_hltf[idx];
    }
    hal_ftm_info->ftm_phase_incr_ant = shenkuo_ftm_dscr->word4.bit_phase_incr;
    ftm_power_input = (uint32_t *)((uint8_t *)shenkuo_ftm_dscr + sizeof(shenkuo_ftm_dscr_stru));
...
void shenkuo_host_ftm_set_sample(hal_host_device_stru *hal_device, oal_bool_enum_uint8 ftm_status)
{
    ...
    oal_setl_bits((void *)(uintptr_t)hal_device->ftm_regs.ftm_pro_reg,
                  SHENKUO_MAC_CFG_FTM_EN_OFFSET, SHENKUO_MAC_CFG_FTM_EN_LEN,
                  ftm_status);
}
...
void shenkuo_host_ftm_set_enable(hal_host_device_stru *hal_device, oal_bool_enum_uint8 ftm_status)
{
    ...
    if (ftm_status == OAL_TRUE) {
        oal_setl_bits((void *)(uintptr_t)hal_device->ftm_regs.ftm_gc_weak_sig_en,
                      SHENKUO_RX_AGC_REG_BANK_CFG_WEAK_SIG_IMMUNITY_EN_OFFSET,
                      SHENKUO_RX_AGC_REG_BANK_CFG_WEAK_SIG_IMMUNITY_EN_LEN,
                      OAL_FALSE);
    }

    /* 置位 PHY FTM enable位 */
    oal_setl_bits((void *)(uintptr_t)hal_device->ftm_regs.ftm_cfg_reg,
                  SHENKUO_PHY_GLB_REG_BANK_CFG_FTM_EN_OFFSET, SHENKUO_PHY_GLB_REG_BANK_CFG_FTM_EN_LEN,
                  ftm_status);

    /* 置位 PHY FTM cali enable位 */
    oal_setl_bits((void *)(uintptr_t)hal_device->ftm_regs.ftm_cali_reg,
                  SHENKUO_RF_CTRL_REG_BANK_0_CFG_FTM_CALI_WR_RF_REG_EN_OFFSET,
                  SHENKUO_RF_CTRL_REG_BANK_0_CFG_FTM_CALI_WR_RF_REG_EN_LEN,
                  ftm_status);
}
                                  ...
                                  uint8_t tx_rssi_selection)
{
    ...
    oal_setl_bits((void *)(uintptr_t)hal_device->ftm_regs.ftm_cfg_reg,
                  SHENKUO_PHY_GLB_REG_BANK_CFG_FTM_CALI_CH_SEL_OFFSET,
                  SHENKUO_PHY_GLB_REG_BANK_CFG_FTM_CALI_CH_SEL_LEN,
                  tx_selection);
    oal_setl_bits((void *)(uintptr_t)hal_device->ftm_regs.fifo_force_reg,
                  SHENKUO_CALI_TEST_REG_BANK_0_CFG_REG_TX_FIFO_EN_OFFSET,
                  SHENKUO_CALI_TEST_REG_BANK_0_CFG_REG_TX_FIFO_EN_LEN, BIT(tx_selection));
    oam_warning_log1(0, OAM_SF_ANY,
                     "shenkuo_set_ftm_m2s_phy:: tx_selection=[%d]", tx_selection);
}
...
{
    if (devva == 0) {
        oam_error_log0(0, 0, "shenkuo_host_ftm_set_buf_base_addr::devva is empty");
        return;
    }
...
void shenkuo_host_ftm_set_buf_size(hal_host_device_stru *hal_device, uint16_t cfg_ftm_buf_size)
{
    /* 设置FTM采样上报片外循环BUF深度 */
    oal_setl_bits((void *)(uintptr_t)hal_device->ftm_regs.ftm_pro_reg,
                  SHENKUO_MAC_CFG_FTM_BUF_SIZE_OFFSET,
                  SHENKUO_MAC_CFG_FTM_BUF_SIZE_LEN, cfg_ftm_buf_size);
}
