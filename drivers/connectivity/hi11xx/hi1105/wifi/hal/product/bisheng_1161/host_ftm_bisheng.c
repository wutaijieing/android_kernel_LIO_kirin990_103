...
#undef THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_HOST_HAL_FTM_BISHENG_C


static uint32_t bisheng_regs_ftm_transfer(hal_host_device_stru *hal_device, uint32_t reg_addr)
{
    uint64_t host_va = 0;
    ...
    if (oal_pcie_devca_to_hostva(HCC_EP_WIFI_DEV, reg_addr + offset, &host_va) != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{bisheng_regs_ftm_transfer::regaddr[%x] devca2hostva fail.}", reg_addr);
        return OAL_FAIL;
    }
    ...
    switch (reg_addr) {
        ...
        case PHY_GLB_REG_BANK_PHY_BW_MODE_REG:
            ...
            break;
        default:
            oam_warning_log1(0, OAM_SF_FTM, "{bisheng_regs_ftm_transfer::reg addr[%x] not used.}", reg_addr);
            return OAL_FAIL;
    }
#endif
    return OAL_SUCC;
...
uint32_t bisheng_host_ftm_reg_init(uint8_t hal_dev_id)
{
    ...
    if (hal_device == NULL) {
        oam_error_log0(0, 0, "bisheng_host_ftm_reg_init::haldev null!");
        return OAL_FAIL;
    }
    if (hal_device->device_id >= HAL_DEVICE_ID_BUTT) {
        oam_error_log1(0, OAM_SF_FTM, "{bisheng_host_ftm_reg_init::device_id[%d].}", hal_device->device_id);
        return OAL_FAIL;
    }
    if (!hal_device->ftm_regs.inited) {
        for (idx = 0; idx < sizeof(hal_regs) / sizeof(uint32_t); idx++) {
            if (bisheng_regs_ftm_transfer(hal_device, hal_regs[idx]) != OAL_SUCC) {
                return OAL_FAIL;
            }
        }
    }
    hal_device->ftm_regs.inited = OAL_TRUE;
    oam_warning_log0(0, OAM_SF_ANY, "{bisheng_host_ftm_reg_init::regs addr trans succ.}");

    return OAL_SUCC;
}


static void bisheng_ftm_get_dscr_addr(uint64_t value, uint8_t *addr)
{
...
hmac_vap_stru *bisheng_host_vap_get_by_hal_vap_id(uint8_t hal_device_id, uint8_t hal_vap_id)
{
    ...
    if (hal_device == NULL) {
        oam_error_log0(0, 0, "{bisheng_host_vap_get_by_hal_vap_id::hal_device null.}");
        return NULL;
    }
    if ((hal_vap_id == BISHENG_OTHER_BSS_ID) || (hal_vap_id < BISHENG_HAL_VAP_OFFSET)) {
        return NULL;
    }

    hal_vap_id -= BISHENG_HAL_VAP_OFFSET;
...
uint32_t bisheng_host_ftm_get_info(hal_ftm_info_stru *hal_ftm_info, uint8_t *addr)
{
    ...
    bisheng_ftm_dscr_stru *bisheng_ftm_dscr = NULL;

    /* 获取hal_ftm_info的地址 */
    bisheng_ftm_dscr = (bisheng_ftm_dscr_stru *)addr;
    if (bisheng_ftm_dscr == NULL) {
        return OAL_FAIL;
    }

    /* 2. 存储部分结果 */
    hal_ftm_info->ftm_vap_index = bisheng_ftm_dscr->word4.bit_vap_index;
    hal_ftm_info->ftm_init_resp_flag = bisheng_ftm_dscr->word10.bit_init_resp_flag;
    hal_ftm_info->ftm_intp_time = bisheng_ftm_dscr->word10.bit_intp_time;
    hal_ftm_info->ftm_rx_time = bisheng_ftm_dscr->word9.rx_time;
    hal_ftm_info->ftm_tx_time = bisheng_ftm_dscr->word8.tx_time;
    hal_ftm_info->ftm_dialog_token = bisheng_ftm_dscr->word4.bit_dialog_token;
    hal_ftm_info->ftm_timestamp = bisheng_ftm_dscr->word7.timestamp;

    bisheng_ftm_get_dscr_addr(bisheng_ftm_dscr->word5.ra, &hal_ftm_info->ftm_ra[MAC_ADDR_0]);
    bisheng_ftm_get_dscr_addr(bisheng_ftm_dscr->word6.ta, &hal_ftm_info->ftm_ta[MAC_ADDR_0]);

    for (idx = 0; idx < HAL_HOST_MAX_RF_NUM; idx++) {
        hal_ftm_info->ftm_snr_ant[idx] = bisheng_ftm_dscr->word1.snr_ant[idx];
        hal_ftm_info->ftm_rssi_ant[idx] = bisheng_ftm_dscr->word1.rssi_hltf[idx];
    }
    hal_ftm_info->ftm_phase_incr_ant = bisheng_ftm_dscr->word4.bit_phase_incr;
    ftm_power_input = (uint32_t *)((uint8_t *)bisheng_ftm_dscr + sizeof(bisheng_ftm_dscr_stru));
    ...
    uint8_t tx_rssi_selection)
{
    ...
    oam_warning_log1(0, OAM_SF_ANY,
                     "bisheng_host_ftm_set_m2s_phy:: tx_selection=[%d]", tx_selection);
}
...
{
    if (devva == 0) {
        oam_error_log0(0, 0, "bisheng_host_ftm_set_buf_base_addr::devva is empty");
        return;
    }
