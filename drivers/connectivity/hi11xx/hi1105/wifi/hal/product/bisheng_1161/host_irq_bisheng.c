...
#undef THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_HOST_HAL_IRQ_BISHENG_C

#define oal_make_word64(lsw, msw) ((((uint64_t)(msw) << 32) & 0xFFFFFFFF00000000) | (lsw))


static uint32_t bisheng_get_skb_from_ring_entry(uint64_t hw_dscr_addr, hal_host_ring_ctl_stru *ring_ctl,
    ...
    *pp_skb = hal_alloc_list_delete_netbuf(hal_dev, alloc_list, host_iova, &pre_num);
    ...
    if (pre_num) {
        oam_info_log1(hal_dev->device_id, OAM_SF_RX,
            "bisheng_get_skb_from_ring_entry:not first netbuf, pre netbuf num[%d]", pre_num);
    }
    if (*pp_skb == NULL) {
        oam_error_log0(hal_dev->device_id, OAM_SF_RX, "bisheng_get_skb_from_ring_entry:not found skb");
        return OAL_FAIL;
    }
    return OAL_SUCC;
}
...
uint32_t bisheng_host_rx_ring_entry_get(hal_host_device_stru *hal_dev,
    hal_host_ring_ctl_stru *ring_ctl, oal_netbuf_head_stru *netbuf_head)
{
    ...
    while (count) {
        ...
        /* 从物理地址获取skb地址 */
        ret = bisheng_get_skb_from_ring_entry(hw_dscr_addr,  ring_ctl, hal_dev, &netbuf);
        if (oal_unlikely(ret != OAL_SUCC)) {
            count--;
            continue;
        }
...
void bisheng_host_rx_mpdu_que_push(hal_rx_mpdu_que *rx_mpdu,
...
static void bisheng_irq_host_rx_complete_notify(hal_host_device_stru *hal_device)
{
...
OAL_STATIC void bisheng_irq_host_rx_complete_isr(hal_host_device_stru *hal_device, uint32_t mask)
{
    uint32_t                     ret;
    ...
    if (!hal_device->inited) {
        ...
        bisheng_clear_host_mac_int_status(hal_device, mask);
        return;
    }
    ...
    ret = bisheng_host_rx_ring_entry_get(hal_device, &hal_device->st_host_rx_complete_ring, &rx_msdu_que);
    if (ret != OAL_SUCC) {
        bisheng_clear_host_mac_int_status(hal_device, mask);
        return;
    }
    ...
    /* 将临时链表中的元素存入乒乓队列 */
    bisheng_host_rx_mpdu_que_push(rx_mpdu, &rx_msdu_que, &ori_skb_num);
    bisheng_clear_host_mac_int_status(hal_device, mask);
    /* 如果乒乓链表没有数据，抛事件到下半部处理 */
    if (!ori_skb_num) {
        bisheng_irq_host_rx_complete_notify(hal_device);
    } else {
...
void bisheng_irq_host_rx_ring_empty_isr(hal_host_device_stru *hal_dev, uint32_t mask,
    hal_rx_dscr_queue_id_enum rx_qid)
{
    bisheng_clear_host_mac_int_status(hal_dev, mask);
}


void bisheng_irq_host_common_timer_isr(hal_host_device_stru *hal_dev)
{
    /* VSP HOST中断处理超时函数 */
}
...
static void bisheng_host_ftm_csi_event(hal_host_device_stru *hal_device, hal_host_location_isr_stru *ftm_csi_isr_event)
{
    frw_event_mem_stru *event_mem = NULL;
    ...
    if (event_mem == NULL) {
        oam_error_log0(0, OAM_SF_FTM, "{bisheng_host_ftm_csi_event::alloc event_mem failed.}\r\n");
        return;
    }
    ...
    if (memcpy_s(hmac_ftm_csi_event->auc_event_data, sizeof(hal_host_location_isr_stru), ftm_csi_isr_event,
        ...
        oam_error_log0(0, OAM_SF_FTM, "bisheng_host_ftm_csi_event::memcpy fail!");
        frw_event_free_m(event_mem);
        return;
    }
...
static uint32_t bisheng_host_ftm_csi_addr_report(uint64_t info_addr_lsb, uint64_t info_addr_msb, uint64_t *addr)
{
    *addr = oal_make_word64(oal_readl(info_addr_lsb), oal_readl(info_addr_msb));

    if (*addr == 0) {
        oam_warning_log0(0, OAM_SF_FTM, "{bisheng_host_ftm_csi_addr_report:get addr fail.}");
        return OAL_FAIL;
    }
    return OAL_SUCC;
}

void bisheng_irq_host_ftm_csi_isr(hal_host_device_stru *hal_dev, uint32_t mask)
{
    hal_host_location_isr_stru ftm_csi_isr_event;

    bisheng_clear_host_mac_int_status(hal_dev, mask);

    ftm_csi_isr_event.ftm_csi_info = bisheng_get_host_ftm_csi_locationinfo(hal_dev);
    ...
    if (!ftm_csi_isr_event.ftm_csi_info) {
        oam_error_log0(0, OAM_SF_FTM, "bisheng_irq_host_ftm_csi_isr::no ftm csi rpt!");
        return;
    }

    if (ftm_csi_isr_event.ftm_csi_info & BIT0) { // CSI上报
        if (bisheng_host_ftm_csi_addr_report(hal_dev->csi_regs.csi_info_lsb,
            hal_dev->csi_regs.csi_info_msb, &ftm_csi_isr_event.csi_info_addr) != OAL_SUCC) {
            return;
        }
        bisheng_host_ftm_csi_event(hal_dev, &ftm_csi_isr_event);
    }

    if (ftm_csi_isr_event.ftm_csi_info & BIT1) { // FTM上报
        if (bisheng_host_ftm_csi_addr_report(hal_dev->ftm_regs.ftm_info_lsb,
 ...
 *   修改内容   : 新生成函数
 */
void bisheng_irq_host_tx_complete_isr(hal_host_device_stru *hal_dev, uint32_t mask)
{
...
void bisheng_irq_host_mac_isr(hal_host_device_stru *hal_dev)
{
    uint32_t               state = 0;
    uint32_t               mask = 0;
    ...
    /* 读取MAC中断状态,中断mask寄存器 */
    bisheng_get_host_mac_int_status(hal_dev, &state);
    bisheng_get_host_mac_int_mask(hal_dev, &mask);
    ...
    while (state) {
        ...
        if (int_state.bits.rpt_host_intr_rx_complete != 0) {
            /* 接收数据报文中断 */
            bisheng_irq_host_rx_complete_isr(hal_dev, int_state.u32);
        } else if (int_state.bits.rpt_host_intr_tx_complete) {
            /* 发送完成上报中断 */
            bisheng_irq_host_tx_complete_isr(hal_dev, int_state.u32);
#ifdef _PRE_WLAN_FEATURE_CSI
        } else if (int_state.bits.rpt_host_intr_location_complete != 0) {
            bisheng_irq_host_ftm_csi_isr(hal_dev, int_state.u32);
#endif
        } else if (int_state.bits.rpt_host_intr_norm_ring_empty != 0) {
            bisheng_irq_host_rx_ring_empty_isr(hal_dev, int_state.u32, HAL_RX_DSCR_NORMAL_PRI_QUEUE);
        } else if (int_state.bits.rpt_host_intr_small_ring_empty != 0) {
            bisheng_irq_host_rx_ring_empty_isr(hal_dev, int_state.u32, HAL_RX_DSCR_SMALL_QUEUE);
        } else if (int_state.bits.rpt_host_intr_ba_win_exceed != 0) {
            /* 接收到的BA窗口越界异常中断 */
            bisheng_clear_host_mac_int_status(hal_dev, int_state.u32);
            ...
            /* 通用计时器中断 */
            bisheng_irq_host_common_timer_isr(hal_dev);
            bisheng_clear_host_mac_int_status(hal_dev, int_state.u32);
        } else {
...
void bisheng_mac_irq_host_all(void *p_dev)
{
    hal_host_device_stru *hal_dev = (hal_host_device_stru *)p_dev;
    bisheng_irq_host_mac_isr(hal_dev);
}


void bisheng_irq_host_phy_isr(hal_host_device_stru *hal_dev)
{
    uint32_t               state = 0;
    uint32_t               mask = 0;
    ...
    /* 读取MAC中断状态寄存器 */
    bisheng_get_host_phy_int_status(hal_dev, &state);

    /* 读取中断mask寄存器 */
    bisheng_get_host_phy_int_mask(hal_dev, &mask);

    state &= ~mask;
    int_state.u32 = state;
    bisheng_clear_host_phy_int_status(hal_dev, int_state.u32);
}

void bisheng_phy_irq_host_all(void *p_dev)
{
    hal_host_device_stru *hal_dev = (hal_host_device_stru *)p_dev;
    bisheng_irq_host_phy_isr(hal_dev);
}


void bisheng_host_chip_irq_init(void)
{
    ...
    st_isr_cb.pcie_mac_2g_isr_callback.pf_func = bisheng_mac_irq_host_all;
    st_isr_cb.pcie_phy_2g_isr_callback.para = (void *)hal_get_host_device(0);
    st_isr_cb.pcie_phy_2g_isr_callback.pf_func = bisheng_phy_irq_host_all;

    st_isr_cb.pcie_mac_5g_isr_callback.para = (void *)hal_get_host_device(1);
    st_isr_cb.pcie_mac_5g_isr_callback.pf_func = bisheng_mac_irq_host_all;
    st_isr_cb.pcie_phy_5g_isr_callback.para = (void *)hal_get_host_device(1);
    st_isr_cb.pcie_phy_5g_isr_callback.pf_func = bisheng_phy_irq_host_all;
