...
#undef THIS_FILE_ID
#define THIS_FILE_ID OAM_FILE_ID_HOST_HAL_CSI_BISHENG_C


static uint32_t bisheng_regs_csi_transfer(hal_host_device_stru *hal_device, uint32_t reg_addr)
{
    uint64_t host_va = 0;
    uint32_t offset = bisheng_regs_addr_get_offset(hal_device->device_id, reg_addr);
    if (oal_pcie_devca_to_hostva(HCC_EP_WIFI_DEV, reg_addr + offset, &host_va) != OAL_SUCC) {
        oam_error_log1(0, OAM_SF_ANY, "{bisheng_regs_csi_transfer::regaddr[%x] devca2hostva fail.}", reg_addr);
        return OAL_FAIL;
    }
    ...
    switch (reg_addr) {
        ...
        case MAC_PPU_REG_CSI_INFO_ADDR_MSB_REG:
            ...
            break;
        default:
            oam_warning_log1(0, OAM_SF_ANY, "{bisheng_regs_csi_transfer::reg addr[%x] not used.}", reg_addr);
            return OAL_FAIL;
    }
#endif
    return OAL_SUCC;
...
void bisheng_csi_base_addr_init(hal_host_device_stru *hal_device)
{
...
uint32_t bisheng_csi_reg_init(hal_host_device_stru *hal_device)
{
    uint32_t idx;
    uint32_t hal_regs[] = {
    ...
    if (hal_device->device_id >= HAL_DEVICE_ID_BUTT) {
        oam_error_log1(0, OAM_SF_ANY, "{bisheng_csi_reg_init::device_id[%d].}", hal_device->device_id);
        return OAL_FAIL;
    }

    if (hal_device->csi_regs.inited == OAL_FALSE) {
        for (idx = 0; idx < sizeof(hal_regs) / sizeof(uint32_t); idx++) {
            if (OAL_SUCC != bisheng_regs_csi_transfer(hal_device, hal_regs[idx])) {
                hal_device->csi_regs.inited = OAL_FALSE;
                return OAL_FAIL;
            }
        }
        hal_device->csi_regs.inited = OAL_TRUE;
        oam_warning_log0(0, OAM_SF_ANY, "{bisheng_csi_reg_init::regs addr trans succ.}");
    }
    return OAL_SUCC;
}


void bisheng_host_set_csi_buf_base_addr(hal_host_device_stru *hal_device, uint64_t devva)
{
    if (devva == 0) {
        oam_error_log0(0, 0, "bisheng_host_set_csi_buf_base_addr::devva is empty");
        return;
    }
...
void bisheng_host_set_csi_buf_size(hal_host_device_stru *hal_device, uint16_t cfg_csi_buf_size)
{
    oal_writel(cfg_csi_buf_size, (uintptr_t)hal_device->csi_regs.size);
}


void bisheng_host_set_csi_en(hal_host_device_stru *hal_device, uint32_t reg_value)
{
...
void bisheng_host_set_phy_info_en(hal_host_device_stru *hal_device, uint32_t reg_value)
{
...
void bisheng_host_set_csi_bw(hal_host_device_stru *hal_device, uint32_t reg_value)
{
    oal_setl_bits((void *)(uintptr_t)hal_device->csi_regs.csi_pro_reg,
...
void bisheng_host_set_csi_frame_type(hal_host_device_stru *hal_device, uint32_t reg_value)
{
    oal_setl_bits((void *)(uintptr_t)hal_device->csi_regs.csi_pro_reg,
...
void bisheng_host_set_csi_extrat_en(hal_host_device_stru *hal_device, uint32_t reg_value)
{
    oal_setl_bits((void *)(uintptr_t)hal_device->csi_regs.csi_pro_reg,
...
void bisheng_host_set_csi_sample_period(hal_host_device_stru *hal_device, uint32_t reg_value)
{
    oal_setl_bits((void *)(uintptr_t)hal_device->csi_regs.csi_pro_reg,
                  MAC_PPU_REG_CFG_CSI_SAMPLE_PERIOD_OFFSET, MAC_PPU_REG_CFG_CSI_SAMPLE_PERIOD_LEN,
                  reg_value);
...
void bisheng_host_set_csi_ta(hal_host_device_stru *hal_device, uint8_t *mac_addr)
{
...
static uint32_t bisheng_config_csi_buffer_addr(hal_host_device_stru *hal_device, uint32_t cfg_csi_on)
{
    int32_t ret;
    ...
    if (cfg_csi_on == OAL_TRUE) {
        if (hal_device->csi_regs.rbase_vaddr == NULL) {
            ...
            bisheng_host_set_csi_buf_base_addr(hal_device, hal_device->csi_regs.devva);
            bisheng_host_set_csi_buf_size(hal_device, HMAC_CSI_BUFF_SIZE);
        }
    } else {
        if (hal_device->csi_regs.rbase_vaddr != NULL) {
            dma_free_coherent(&pcie_dev->dev,
...
uint32_t bisheng_csi_config(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    ...
    if (oal_any_null_ptr2(cfg_csi_param, hal_device)) {
        oam_error_log0(0, OAM_SF_CFG, "{bisheng_csi_config::cfg_csi_param and hal_device null.}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    cfg_csi_on = cfg_csi_param->en_cfg_csi_on;

    if (bisheng_csi_reg_init(hal_device) != OAL_SUCC) {
        oam_error_log0(0, OAM_SF_CFG, "{bisheng_csi_config::csi init regs failed.}");
        return OAL_ERR_CODE_PTR_NULL;
    }

    /* 唤醒低功耗内存 */
    wlan_pm_wakeup_dev_lock();
    bisheng_host_set_csi_en(hal_device, OAL_FALSE);
    bisheng_host_set_phy_info_en(hal_device, OAL_FALSE);

    /* 设置带宽 */
    bisheng_host_set_csi_bw(hal_device, (uint32_t)cfg_csi_param->uc_csi_bw);

    /* 设置CSI采集的帧类型 */
    bisheng_host_set_csi_frame_type(hal_device, (uint32_t)cfg_csi_param->uc_csi_frame_type);

    /* 设置CSI采样周期 */
    bisheng_host_set_csi_sample_period(hal_device, (uint32_t)cfg_csi_param->uc_csi_sample_period);

    if (!mac_addr_is_zero(cfg_csi_param->auc_mac_addr)) {
        /* 设置上报csi帧的TA mac地址 */
        bisheng_host_set_csi_ta(hal_device, cfg_csi_param->auc_mac_addr);
    }

    if (bisheng_config_csi_buffer_addr(hal_device, !!cfg_csi_on) != OAL_SUCC) {
        return OAL_FAIL;
    }

    bisheng_host_set_csi_en(hal_device, !!cfg_csi_on); /* !! 非零传入1, 0传入0 */
    bisheng_host_set_phy_info_en(hal_device, !!cfg_csi_on);

    oam_warning_log4(0, OAM_SF_CFG,
        "{bisheng_csi_config::cfg_csi_on:%d, bw:%d, csi_frame_type:%d, "
...
{
    bisheng_csi_ppu_stru *csi_ppu = NULL;
    uint32_t timestamp;
    csi_ppu = (bisheng_csi_ppu_stru *)addr;
...
{
    bisheng_csi_base_addr_init(hal_device);
