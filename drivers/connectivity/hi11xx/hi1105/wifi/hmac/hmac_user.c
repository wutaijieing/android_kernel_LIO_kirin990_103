...
static void hmac_set_user_lut_index_tbl(hmac_user_stru *hmac_user, uint8_t lut_idx)
{
    hmac_lut_index_tbl_stru *entry = &g_hmac_lut_index_tbl[lut_idx];
...
OAL_STATIC void hmac_user_tid_init(hmac_user_stru *hmac_user)
{
    ...
    for (tid = 0; tid < WLAN_TID_MAX_NUM; tid++) {
        ...
        hmac_init_tx_ring(hmac_user, tid);
...
uint32_t hmac_user_del_reset_wapi(mac_device_stru *mac_device, mac_vap_stru *mac_vap, hmac_user_stru *hmac_user)
{
    ...
    hmac_wapi_deinit(&hmac_user->st_wapi);
    /* STA模式下，清组播wapi加密端口 */
...
uint32_t hmac_user_del(mac_vap_stru *mac_vap, hmac_user_stru *hmac_user)
{
    ...
    mac_user = (mac_user_stru *)(&hmac_user->st_user_base_info);
    mac_device = mac_res_get_dev(mac_vap->uc_device_id);
    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    mac_chip = hmac_res_get_mac_chip(mac_vap->uc_chip_id);
    if (oal_unlikely(oal_any_null_ptr3(mac_chip, mac_device, hmac_vap))) {
        return OAL_ERR_CODE_PTR_NULL;
    }
    ...
    oam_warning_log4(mac_vap->uc_vap_id, OAM_SF_UM, "{hmac_user_del::del user[%d] start,is multi user[%d], "
        "user mac:XX:XX:XX:XX:%02X:%02X}", mac_user->us_assoc_id, mac_user->en_is_multi_user,
        mac_user->auc_user_mac_addr[MAC_ADDR_4], mac_user->auc_user_mac_addr[MAC_ADDR_5]);
    if (hmac_user_del_feature_process(mac_device, hmac_vap, hmac_user) != OAL_SUCC) {
        return OAL_FAIL;
    }
    hmac_sta_del_user(mac_device, hmac_vap);
    if (hmac_user_del_sync_dmac(mac_vap, mac_user) != OAL_SUCC) {
        return OAL_FAIL;
    }
    ...
    hmac_user_clear_defrag_res(hmac_user);
#ifdef _PRE_WLAN_CHBA_MGMT
    hmac_chba_del_user_proc(hmac_vap, hmac_user);
#endif
    /* 从vap中删除用户 */
    mac_vap_del_user(mac_vap, user_index);
    hmac_arp_probe_destory(hmac_vap, hmac_user);
    hmac_chan_update_40m_intol_user(mac_vap);
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
    hmac_custom_set_mult_user_flowctrl(mac_vap);
#endif
    hmac_user_del_refresh_lut(mac_chip, hmac_user);
    /* 清除tx switch ring list中的TID节点 */
    hmac_tid_ring_switch_disable(hmac_user);
#ifdef _PRE_WLAN_CHBA_MGMT
    hmac_update_chba_vap_info(hmac_vap, hmac_user);
    hmac_user_del_clear_vap_user_bitmap(hmac_vap, user_index);
#endif
    /* 释放用户内存 */
    hmac_user_del_free_res(mac_chip, user_index);
    hmac_user_del_vap_init(hmac_vap, mac_device, hmac_user);
    return OAL_SUCC;
}
...
uint32_t hmac_user_add(mac_vap_stru *mac_vap, uint8_t *mac_addr, uint16_t *user_index)
{
    ...
    ***************************************************************************/
    ...
    /* sta需额外增加一个组播用户 */
    wlan_chip_mcast_ampdu_sta_add_multi_user(mac_vap, mac_addr);
#endif
    return OAL_SUCC;
}
