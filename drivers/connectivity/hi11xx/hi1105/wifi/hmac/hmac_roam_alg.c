...
uint32_t hmac_roam_set_sae_mode(mac_vap_stru *mac_vap, mac_bss_dscr_stru *bss_dscr)
{
    ...
    if (is_only_support_sae(rsn_akm_suites) || is_only_support_sae(crypto.aul_akm_suite) ||
        is_only_support_tbpeke(rsn_akm_suites) || is_only_support_tbpeke(crypto.aul_akm_suite)) {
                                                ...
                                                mac_bss_dscr_stru *bss_dscr)
{
    ...
    if (hmac_chba_coex_other_is_dbac(mac_device, &bss_dscr->st_channel) == OAL_TRUE) {
        oam_warning_log0(0, OAM_SF_ROAM, "{hmac_roam_alg_check_common_info::bss not supp chba coex!}");
        return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
    }
#endif
    return OAL_SUCC;
...
uint32_t hmac_roam_alg_bss_check(hmac_roam_info_stru *roam_info, mac_bss_dscr_stru *bss_dscr)
{
    ...
    if (is_roam_weak_rssi_condition(bss_dscr->c_rssi, bss_dscr)) {
        return OAL_ERR_CODE_ROAM_NO_VALID_BSS;
    }
