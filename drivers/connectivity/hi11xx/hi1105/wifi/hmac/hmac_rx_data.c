    ...
    oal_netbuf_stru *buf, mac_vap_stru *vap, uint32_t len)
{
    ...
    oam_warning_log3(vap->uc_vap_id, OAM_SF_ANY, "{hmac_rx_vip_info::EAPOL type=%u, len==%u, \
        user[%d]}[1:1/4 2:2/4 3:3/4 4:4/4]", en_eapol_type, oal_netbuf_len(buf), MAC_GET_RX_CB_TA_USER_IDX(rx_ctrl));
}
...
static void hmac_rx_lan_frame_no_user_log(hmac_vap_stru *hmac_vap, oal_netbuf_stru *netbuf, mac_rx_ctl_stru *rx_ctrl)
{
    ...
    oam_error_log1(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_RX,
        "{hmac_rx_lan_frame_classify::user_idx=%d}", MAC_GET_RX_CB_TA_USER_IDX(rx_ctrl));
#ifdef _PRE_WLAN_DFT_DUMP_FRAME
    ...
    mac_rx_ctl_stru *rx_ctl, mac_header_frame_control_stru *frame_control)
{
    ...
    if (MAC_GET_RX_CB_IS_REO_TIMEOUT(rx_ctl) == OAL_TRUE || rx_ctl->is_before_last_release == OAL_TRUE) {
        oam_warning_log3(0, OAM_SF_RX, "{hmac_rx_pn_param_check::SEQ NUM[%d] reo_timeout[%d] before_last_release[%d].}",
...
uint32_t hmac_rx_pn_check(oal_netbuf_stru *netbuf)
{
    ...
    if (oal_unlikely((qos_flg == OAL_TRUE) && (rx_ctl->bit_rx_tid >= WLAN_TID_MAX_NUM))) {
        oam_warning_log3(0, OAM_SF_RX, "{hmac_rx_pn_check::uc_qos_flg %d user id[%d] tid[%d].}",
                         qos_flg, MAC_RXCB_TA_USR_ID(rx_ctl), rx_ctl->bit_rx_tid);
        return OAL_SUCC;
    }
    ...
    if (hmac_rx_pn_is_invalid(rx_pn_val, last_pn_val, rx_ctl->bit_mcast_bcast,
        ...
        oam_warning_log4(0, OAM_SF_RX, "{hmac_rx_pn_check::last pn[%ld] send pn[%ld] user id[%d] tid[%d].}",
                         last_pn_val, rx_pn_val, MAC_RXCB_TA_USR_ID(rx_ctl), rx_ctl->bit_rx_tid);
...
void hmac_rx_enqueue(oal_netbuf_stru *netbuf, oal_net_device_stru *net_dev)
{
    ...
    hmac_rx_skb_netif_rpt(net_dev, netbuf);
}
...
uint32_t hmac_device_rx_amsdu_proc(mac_rx_ctl_stru *cb, hmac_user_stru *hmac_user, oal_netbuf_stru  **netbuf)
{
    ...
    if (MAC_RXCB_IS_FIRST_AMSDU(cb) && (!MAC_RXCB_IS_LAST_MSDU(cb))) {
        if (!oal_netbuf_list_empty(list_head)) {
            ...
            while ((temp_netbuf = oal_netbuf_delist(list_head)) != NULL) {
                ...
                oam_error_log3(0, OAM_SF_RX, "{hmac_device_rx_amsdu_proc::sn = [%d], first = [%d], last = [%d]}",
                    MAC_GET_RX_CB_SEQ_NUM(temp_cb), MAC_RXCB_IS_FIRST_AMSDU(temp_cb), MAC_RXCB_IS_LAST_MSDU(temp_cb));
...
uint32_t hmac_dev_rx_process_data_event(frw_event_mem_stru *event_mem)
{
    ...
    hmac_user = (hmac_user_stru *)mac_res_get_hmac_user(MAC_GET_RX_CB_TA_USER_IDX(cb));
    if (hmac_user == NULL) {
...
#ifdef _PRE_WLAN_CHBA_MGMT
        cb = (mac_rx_ctl_stru *)oal_netbuf_cb(pst_temp_netbuf);
        pst_hmac_user = (hmac_user_stru *)mac_res_get_hmac_user(MAC_GET_RX_CB_TA_USER_IDX(cb));
        hmac_stat_user_rx_netbuf(pst_hmac_user, pst_temp_netbuf);
#endif
...
static int32_t hmac_dhcp_get_option_uint32(uint32_t *option_val, const uint8_t *opt_buff, uint32_t len, uint8_t option)
{
    ...
    if (EOK != memcpy_s(&val, sizeof(val), puc_val, sizeof(uint32_t))) {
...
OAL_STATIC void hmac_parse_dns_answer(uint8_t *puc_msg, uint16_t us_msg_len, uint16_t us_qdcount)
{
    ...
    while (us_qdcount > 0 && puc_buff < puc_end) {
        ...
        if (uc_qlen > 0) {
            ...
            us_qdcount--;
            l_ret += memcpy_s(auc_domain_s, sizeof(auc_domain_s), auc_domain, sizeof(auc_domain));
    ...
    while (puc_buff + BYTE_OFFSET_12 < puc_end) {
        ...
        if (us_type == OAL_NS_T_A && us_rdlen == HMAC_DNS_RDLEN) {
            l_ret += memcpy_s(&ul_ipv4, sizeof(ul_ipv4), puc_buff, us_rdlen);
...
void hmac_parse_special_ipv4_packet(uint8_t *puc_pktdata, uint32_t datalen, hmac_pkt_direction_enum en_pkt_direction)
{
     ...
     */
    if (datalen < (us_tot_len + ETH_HLEN) || datalen < (iphdr_len + ETH_HLEN) || (iphdr_len > us_tot_len)) {
        return;
    }
