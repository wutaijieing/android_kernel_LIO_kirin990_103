...
void hmac_csi_handle(hal_host_location_isr_stru *ftm_csi_isr)
{
    ...
    if (OAL_SUCC != pcie_if_devva_to_hostca(HCC_EP_WIFI_DEV, csi_base_addr, (uint64_t *)&host_iova)) {
    ...
    hmac_vap = hal_host_vap_get_hal_vap_id(ftm_csi_isr->hal_device_id, hmac_csi_info.vap_index);
    if (hmac_vap == NULL) {
        return ;
    }
