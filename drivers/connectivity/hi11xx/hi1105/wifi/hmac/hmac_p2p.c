...
{
    mac_vap_stru *all_mac_vap[WLAN_SERVICE_VAP_MAX_NUM_PER_DEVICE] = {NULL};
#ifdef _PRE_WLAN_CHBA_MGMT
    if (g_optimized_feature_switch_bitmap & BIT(CUSTOM_OPTIMIZE_CHBA)) {
        return  mac_device_find_up_p2p_go(mac_device, p2p_vap);
    } else {
#endif
...
OAL_STATIC uint8_t mac_csa_need_downgrade_protocol(mac_vap_stru *other_vap, mac_vap_stru *p2p_vap, uint8_t new_band,
    wlan_channel_bandwidth_enum_uint8 go_new_bw)
{
    /* sta(chbaa)+p2p共存时，且p2p要跨频段时，有带宽能力和协议限制 */
    if (other_vap != NULL && new_band != p2p_vap->st_channel.en_band) {
        /* sta(chba)和p2p都在11ax，有一个vap需要切到辅路没法在11ax，需要降协议 */
        ...
        if (mac_csa_check_can_run_dbdc(other_vap, p2p_vap, new_band, go_new_bw) == OAL_FALSE) {
            return OAL_TRUE;
        }
    }
    return OAL_FALSE;
...
OAL_STATIC uint8_t hmac_go_csa_need_downgrade_protocol(mac_device_stru *mac_device, mac_vap_stru *sta_vap,
    mac_vap_stru *p2p_vap, uint8_t new_band, wlan_channel_bandwidth_enum_uint8 go_new_bw)
{
#ifdef _PRE_WLAN_CHBA_MGMT
    hmac_vap_stru *hmac_chba_vap = hmac_chba_find_chba_vap(mac_device);
#endif
    uint8_t ret = OAL_FALSE;

    if (sta_vap != NULL) {
        ret = mac_csa_need_downgrade_protocol(sta_vap, p2p_vap, new_band, go_new_bw);
#ifdef _PRE_WLAN_CHBA_MGMT
    } else if (hmac_chba_vap != NULL) {
        ret = mac_csa_need_downgrade_protocol(&hmac_chba_vap->st_vap_base_info, p2p_vap, new_band, go_new_bw);
#endif
    }
    ...
    mac_device_stru *mac_device, uint8_t new_band, uint8_t new_channel)
{
    ...
    if (mac_device->en_dbac_running && new_band == p2p_vap->st_channel.en_band &&
        ...
        return OAL_SUCC;
    } else if (mac_is_dfs_channel(check_band, new_channel) == OAL_TRUE) {
...
uint32_t hmac_p2p_change_go_channel(mac_vap_stru *mac_vap, uint8_t len, uint8_t *param)
{
    ...
    if (hmac_find_up_p2p_and_sta_vap(mac_device, &p2p_vap, &sta_vap) != OAL_SUCC) {
        oam_warning_log0(0, OAM_SF_ANY, "{hmac_p2p_change_go_channel::hmac_find_up_p2p_vap fail.}");
        return OAL_FAIL;
    }
    if (p2p_vap == NULL) {
    ...
    /* 需要降协议，无法通过csa切换协议 */
    if (hmac_go_csa_need_downgrade_protocol(mac_device, sta_vap, p2p_vap, new_band, go_new_bw) == OAL_TRUE) {
        return OAL_FAIL;
    }
#ifdef _PRE_WLAN_CHBA_MGMT
    if (hmac_chba_coex_go_csa_chan_is_dbac(mac_device, new_channel, go_new_bw) == OAL_TRUE) {
        return OAL_FAIL;
    }
#endif
