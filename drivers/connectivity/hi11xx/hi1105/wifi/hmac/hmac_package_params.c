    ...
    mac_vap_stru *mac_vap, uint32_t *params, uint8_t *new_params, uint16_t *new_params_len)
{
    ...
    p2p_noa_param->start_time = (uint32_t)params[0] * HMAC_MS_TO_US;
    p2p_noa_param->duration = (uint32_t)params[1] * HMAC_MS_TO_US;
    p2p_noa_param->interval = (uint32_t)params[2] * HMAC_MS_TO_US;
    p2p_noa_param->uc_count = (uint8_t)params[3];
    ...
    uint8_t *new_params, uint16_t *new_params_len)
{
    ...
    chba_channel_seq->data.test_params.channel_seq_level = params[0];
    chba_channel_seq->data.test_params.first_work_channel = params[1];
    chba_channel_seq->data.test_params.sec_work_channel = params[2];
