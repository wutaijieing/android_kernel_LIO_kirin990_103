...
OAL_STATIC uint32_t hmac_config_d2h_hal_vap_syn(mac_vap_stru *mac_vap, uint8_t len, uint8_t *p_param)
{
    ...
    if (!p_hal_vap_info->valid) {
        ...
        if (p_hal_vap_info->hal_dev_id == HAL_DEVICE_ID_SLAVE) {
            ...
            wlan_chip_send_dbdc_scan_cali_data(mac_vap, OAL_TRUE);
#ifdef _PRE_WLAN_FEATURE_FTM
...
static uint32_t hmac_config_d2h_rx_mode_syn(mac_vap_stru *mac_vap, uint8_t len, uint8_t *p_param)
{
    ...
    if (rx_switch->new_rx_mode >= HAL_SWTICH_ING || hal_device->rx_mode == rx_switch->new_rx_mode) {
        oam_error_log2(mac_vap->uc_vap_id, OAM_SF_RX, "hmac_config_d2h_rx_mode_syn::new[%d]==old[%d][0:ddr, 1:ram]",
                       rx_switch->new_rx_mode, hal_device->rx_mode);
    }

    oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_RX, "hmac_config_d2h_rx_mode_syn:: \
        switch rx from %d to %d [0:ddr, 1:ram]once reset smac%d",
        hal_device->rx_mode, rx_switch->new_rx_mode, rx_switch->once_ddr_reset_smac);
...
uint32_t hmac_config_down_vap(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    if (pst_mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
        ...
        hmac_down_sta_clean_ssid_mib(pst_mac_vap);
    ...
    /***************************************************************************
                         抛事件到DMAC层, 同步DMAC数据
    ***************************************************************************/
    if (hmac_config_send_event(pst_mac_vap, WLAN_CFGID_DOWN_VAP, us_len, puc_param) != OAL_SUCC) {
        oam_warning_log0(pst_mac_vap->uc_vap_id, 0, "{hmac_config_down_vap::hmac_config_send_event fail.}");
        return OAL_FAIL;
    }
    ...
    mac_device_stru *mac_device, mac_vap_stru *mac_vap, mac_cfg_mode_param_stru *mode_param)
{
    ...
    /* 根据device能力对参数进行检查 */
    if (hmac_config_check_dev_protocol_cap_para(mode_param, mac_device) != OAL_SUCC) {
        return OAL_ERR_CODE_CONFIG_EXCEED_SPEC;
    }
...
uint32_t hmac_config_set_sniffer_shenkuo(mac_vap_stru *mac_vap, uint16_t len, uint8_t *param)
{
    uint32_t ret;
    mac_cfg_sniffer_param_stru *cfg_sniffer_param = NULL;
    ...
    if (hmac_device->sniffer_mode == WLAN_SINFFER_ON) {
        ...
        if (hal_host_sniffer_rx_ppdu_free_ring_init(hal_device) != OAL_SUCC) {
            oam_error_log0(0, 0, "hmac_config_set_sniffer_shenkuo::init_rx_ppdu_free_ring fail.");
            return OAL_FAIL;
        }
    } else {
...
OAL_STATIC oal_bool_enum_uint8 hmac_check_up_vap_num_is_valid(mac_vap_stru *mac_vap)
{
    mac_device_stru *mac_device = mac_res_get_dev(mac_vap->uc_device_id);
    if (mac_device == NULL) {
        return OAL_FALSE;
    ...
    /* max_p2p_group_num在不同的芯片中有不同的定义，支持3vap时2，否则为1. */
    if (mac_device_calc_up_vap_num(mac_device) >= g_wlan_spec_cfg->max_p2p_group_num + 1) {
        return OAL_FALSE;
    }
    return OAL_TRUE;
}
uint32_t hmac_config_set_channel_check(mac_vap_stru *mac_vap, mac_cfg_channel_param_stru *channel_param)
{
    ...
    st_set_mac_channel.en_bandwidth = channel_param->en_bandwidth;
    st_set_mac_channel.uc_chan_number = channel_param->uc_channel;
    if (hmac_chba_coex_channel_is_dbac(mac_vap, &st_set_mac_channel) == OAL_TRUE) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_CHAN, "{hmac_config_set_channel::dbac check failed.}");
        return OAL_FAIL;
    }
#endif
    return OAL_SUCC;
...
uint32_t hmac_config_set_beacon(mac_vap_stru *mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    hmac_vap_stru *hmac_vap = NULL;
    uint32_t ret;
    uint32_t aul_akm_suite[WLAN_AUTHENTICATION_SUITES] = { 0 };
    ...
    oam_warning_log2(mac_vap->uc_vap_id, OAM_SF_CFG,
        "{hmac_config_set_beacon::mac_mib_get_rsn_akm_suites_s 0x[%x], 0x[%x].}", aul_akm_suite[0], aul_akm_suite[1]);
...
uint32_t hmac_config_kick_user(mac_vap_stru *mac_vap, uint16_t us_len, uint8_t *param)
{
    ...
    oam_warning_log4(mac_vap->uc_vap_id, OAM_SF_UM,
        ...
        ku_param->auc_mac_addr[0], ku_param->auc_mac_addr[4], ku_param->auc_mac_addr[5], ku_param->us_reason_code);
    ...
    /* 发去认证帧 */
    hmac_mgmt_send_disassoc_frame(mac_vap, mac_user->auc_user_mac_addr, ku_param->us_reason_code, en_pmf);

    hmac_user->assoc_err_code = ku_param->us_reason_code;
#ifdef _PRE_WLAN_CHBA_MGMT
    if (mac_is_chba_mode(mac_vap) == OAL_TRUE) {
...
#endif
    /* 修改 state & 删除 user */
    hmac_handle_disconnect_rsp(hmac_vap, hmac_user, ku_param->us_reason_code);
    /* 删除用户 */
...
uint32_t hmac_config_always_tx(mac_vap_stru *mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    if (((mac_cfg_tx_comp_stru *)puc_param)->uc_param == HAL_ALWAYS_TX_MPDU) {
        ...
        hmac_tx_host_ring_release(&g_always_tx_ring[hmac_vap->hal_dev_id]);
    }
#endif
    /***************************************************************************
        抛事件到DMAC层, 同步DMAC数据
...
#ifndef _PRE_LINUX_TEST
    /* 设置host常收状态 */
    hal_set_alrx_status(hmac_vap->hal_dev_id, OAL_TRUE);
#endif
#endif
    return ret;
}
...
uint32_t hmac_config_get_sta_list(mac_vap_stru *pst_mac_vap, uint16_t *us_len, uint8_t *puc_param)
{
    ...
    *(uintptr_t *)puc_param = (uintptr_t)NULL;
    ...
    if (pst_netbuf != NULL) {
        ret = memcpy_s(oal_netbuf_data(pst_netbuf), netbuf_len, pc_sta_list_buff, netbuf_len);
        if (ret != EOK) {
            oam_error_log2(0, OAM_SF_CFG, "hmac_config_get_sta_list::memcpy fail! ret[%d], len[%d]", ret, netbuf_len);
            oal_mem_free_m(pc_sta_list_buff, OAL_TRUE);
...
uint32_t hmac_config_connect_param_check(mac_vap_stru *mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    if (us_len != sizeof(hmac_conn_param_stru)) {
        oam_error_log1(0, OAM_SF_ANY,
            "{hmac_config_connect:: connect failed, unexpected param len ! [%d]!}", us_len);
        hmac_free_connect_param_resource((hmac_conn_param_stru *)puc_param);
        return OAL_ERR_CODE_INVALID_CONFIG;
    }
    if (!mac_vap_is_up(mac_vap) && hmac_check_up_vap_num_is_valid(mac_vap) == OAL_FALSE) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_CHAN, "{hmac_check_up_vap_num_is_valid::vap num is invalid.}");
        return OAL_FAIL;
    }
    return OAL_SUCC;
                                           ...
                                           hmac_conn_param_stru *connect_param)
{
    ...
    if (mac_vap->en_vap_state == MAC_VAP_STATE_UP) {
        ...
        if (!oal_memcmp(mac_mib_get_DesiredSSID(mac_vap), connect_param->auc_ssid, connect_param->uc_ssid_len) &&
            ...
            oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_CFG,
                "{hmac_process_connect_by_vap_state::roaming AP with ressoc frame, %02X:XX:XX:XX:%02X:%02X}",
                connect_param->auc_bssid[MAC_ADDR_0],
...
uint32_t hmac_config_connect(mac_vap_stru *mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    if (connect_param->ie_len > WLAN_WPS_IE_MAX_SIZE) {
        oam_error_log1(0, OAM_SF_ANY, "{hmac_config_connect::connect failed, \
                       connect_param ie_len[%d] error!}", connect_param->ie_len);
        hmac_free_connect_param_resource(connect_param);
        return OAL_ERR_CODE_INVALID_CONFIG;
    }
    ...
    if (bss_dscr == NULL) {
        oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_CFG,
            "{hmac_config_connect::find bss failed by bssid:%02X:XX:XX:XX:%02X:%02X}", connect_param->auc_bssid[0],
            connect_param->auc_bssid[4], connect_param->auc_bssid[5]);
        hmac_free_connect_param_resource(connect_param);
        return OAL_FAIL;
    }
#ifdef _PRE_WLAN_FEATURE_MBO
    ...
    mac_device = mac_res_get_dev(mac_vap->uc_device_id);
    if (mac_device != NULL) {
        if (hmac_chba_coex_other_is_dbac(mac_device, &bss_dscr->st_channel) == OAL_TRUE) {
            oam_error_log0(0, 0, "hmac_config_connect::sta channel dbac");
            hmac_free_connect_param_resource(connect_param);
            return OAL_FAIL;
        }
    }
...
#ifdef _PRE_WLAN_FEATURE_WAPI
    ret = hmac_config_connect_wapi_proc(mac_vap, connect_param, bss_dscr, &conn_sec);
    if (ret != OAL_SUCC) {
        return ret;
    }
#endif
...
static uint32_t hmac_app_ie_remove_ext_caps(oal_app_ie_stru *app_ie)
{
    ...
    if (memmove_s(ie, remain_len + (ie[1] + MAC_IE_HDR_LEN), ie + (ie[1] + MAC_IE_HDR_LEN), remain_len) != EOK) {
        oam_error_log0(0, OAM_SF_CFG, "hmac_app_ie_remove_ext_caps:memmove fail");
    }
    return OAL_SUCC;
}
static uint32_t hmac_app_ie_remove_operating_class(mac_vap_stru *mac_vap, oal_app_ie_stru *app_ie)
{
    ...
    if ((ie == NULL) || (mac_mib_get_dot11ExtendedChannelSwitchActivated(mac_vap) == OAL_TRUE)) {
        return OAL_SUCC;
    }
    if (app_ie->ie_len < ie[1] + MAC_IE_HDR_LEN) {
        return OAL_FAIL;
    ...
    remain_len = app_ie->ie_len - (uint32_t)(ie - app_ie->auc_ie);
    if (memmove_s(ie, remain_len + (ie[1] + MAC_IE_HDR_LEN), ie + (ie[1] + MAC_IE_HDR_LEN), remain_len) != EOK) {
        oam_error_log0(0, OAM_SF_CFG, "hmac_app_ie_remove_operating_class:memmove fail");
    }
    return OAL_SUCC;
}
...
uint32_t hmac_config_remain_on_channel(mac_vap_stru *mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    if (ret != OAL_SUCC) {
        /* 不能进入监听状态，返回设备忙 */
        oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_P2P, "{hmac_config_remain_on_channel::dispatch fail[%d]}", ret);
...
OAL_STATIC uint32_t hmac_config_get_dbdc_info(mac_vap_stru *pst_mac_vap, uint8_t uc_len, uint8_t *puc_param)
{
    ...
    wlan_chip_send_dbdc_scan_cali_data(pst_mac_vap, pst_mac_device->en_dbdc_running);
...
OAL_STATIC const hmac_config_syn_stru g_ast_hmac_config_syn[] = {
    ...
    { WLAN_CFGID_CHBA_NOTIFY_STA_CSA_CHAN_INFO, { 0, 0 }, hmac_chba_coex_d2h_sta_csa_dbac_rpt },
...
uint32_t hmac_config_dbdc_debug_switch(mac_vap_stru *pst_mac_vap, uint16_t us_len, uint8_t *puc_param)
{
    ...
    if (pst_dbdc_debug_switch->cmd_bit_map & BIT(MAC_DBDC_CHANGE_HAL_DEV)) {
