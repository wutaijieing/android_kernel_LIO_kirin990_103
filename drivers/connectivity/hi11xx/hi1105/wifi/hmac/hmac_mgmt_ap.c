    ...
    uint8_t *chtxt, uint16_t *auth_rsp_len)
{
    ...
    *auth_rsp_len = hmac_encap_auth_rsp(&hmac_vap->st_vap_base_info, auth_rsp, auth_req, chtxt, WLAN_CHTXT_SIZE);
    if (*auth_rsp_len == 0) {
        oam_error_log0(vap_id, OAM_SF_AUTH, "{hmac_ap_auth_rsp_handle::auth_rsp_len is 0.}");
        return OAL_FAIL;
    }
...
OAL_STATIC void hmac_ap_rx_auth_req(hmac_vap_stru *hmac_vap, oal_netbuf_stru *pst_auth_req)
{
    ...
    uint16_t uc_auth_rsp_len, us_auth_alg_num;
    uint32_t ret;
    ...
    ret = hmac_ap_rx_reset_psm(hmac_vap, auth_rsp);
    if (ret != OAL_SUCC) {
        oal_netbuf_free(auth_rsp);
        return;
    }
    ...
    ret = hmac_tx_mgmt_send_event(&hmac_vap->st_vap_base_info, auth_rsp, uc_auth_rsp_len);
    if (ret != OAL_SUCC) {
        oal_netbuf_free(auth_rsp);
...
OAL_STATIC void hmac_ap_up_rx_asoc_req_change_user_state_to_auth(mac_vap_stru *mac_vap,
    hmac_user_stru *hmac_user)
{
...
uint32_t hmac_ap_check_blacklist_result(hmac_vap_stru *hmac_vap, uint8_t *mac_hdr, uint8_t mgmt_frm_type)
{
    ...
    /* 黑名单过滤检查 */
    blacklist_result = hmac_blacklist_filter(&hmac_vap->st_vap_base_info, sa);
