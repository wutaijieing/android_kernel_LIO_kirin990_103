...
static void hmac_wapi_dump_frame(uint8_t *puc_info, uint8_t *puc_data, uint32_t len);
#endif
...
static oal_netbuf_stru *hmac_wapi_netbuff_tx_handle(hmac_wapi_stru *pst_wapi, oal_netbuf_stru *pst_buf)
{
    ...
    /* buf的初始位置在snap头的llc处 */
    if (MAC_DATA_WAPI == mac_get_data_type_from_80211(pst_buf, 0, oal_netbuf_get_len(pst_buf))) {
...
/*
 * 函 数 名  : hmac_wapi_netbuf_tx_encrypt_shenkuo
 * 功能描述  ： shenkuo host_tx场景，加密已经封装为802.11格式的数据帧
 */
oal_netbuf_stru *hmac_wapi_netbuf_tx_encrypt_shenkuo(hmac_vap_stru *hmac_vap, hmac_user_stru *hmac_user,
    oal_netbuf_stru *netbuf)
{
    ...
    if (netbuf == NULL) {
        ...
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY,
            "{hmac_wapi_netbuf_tx_encrypt_shenkuo fail!}");
        return NULL;
    }
    ...
    if (oal_unlikely(ret != OAL_SUCC)) {
        oam_error_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_WAPI,
            "hmac_wapi_netbuf_tx_encrypt_shenkuo:memmove fail");
    }
...
/*
 * 函 数 名  : hmac_wapi_netbuf_rx_decrypt_shenkuo
 * 功能描述  : shenkuo 接收，解密802.11格式的数据帧
 */
uint32_t hmac_wapi_netbuf_rx_decrypt_shenkuo(hmac_host_rx_context_stru *rx_context)
{
    ...
    hmac_user_wapi = &hmac_user->st_wapi;
    if ((WAPI_PORT_FLAG(hmac_user_wapi) != OAL_TRUE) ||
        (hmac_user_wapi->wapi_netbuff_rxhandle == NULL)) {
        return OAL_SUCC;
    }
    ...
    if (netbuf == NULL) {
        oam_warning_log0(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_RX,
            "{hmac_wapi_netbuf_rx_decrypt_shenkuo:: wapi decrypt FAIL!}");
...
/* 计算mic,wapi的数据帧一般为QOS帧 */
static uint32_t hmac_wapi_encrypt_calculate_mic(hmac_wapi_stru *wapi, oal_netbuf_stru *netbuf,
    mac_ieee80211_frame_stru *mac_hdr, uint8_t *calc_mic, uint8_t *pn_swap)
{
    uint16_t pdu_len, mic_len;
    uint32_t result;
    ...
    if (hmac_wapi_calc_mic_data(mac_hdr, wapi->uc_keyidx, payload, pdu_len, mic_data, mic_len) != OAL_SUCC) {
        hmac_wapi_mic_free(mic_data);
        return OAL_FAIL;
    }

    key_ck = wapi->ast_wapi_key[wapi->uc_keyidx].auc_wpi_ck;

    if (memcpy_s(pn_swap, SMS4_PN_LEN, wapi->ast_wapi_key[wapi->uc_keyidx].auc_pn_tx, WAPI_PN_LEN) != EOK) {
        hmac_wapi_mic_free(mic_data);
        oam_error_log0(0, OAM_SF_ANY, "hmac_wapi_encrypt_calculate_mic::memcpy fail!");
        return OAL_FAIL;
    }

    hmac_wpi_swap_pn(pn_swap, SMS4_PN_LEN);
    /* 计算mic */
    result = hmac_wpi_pmac(pn_swap, mic_data, (mic_len >> BIT_OFFSET_4), key_ck, calc_mic, SMS4_MIC_LEN);
    hmac_wapi_mic_free(mic_data);
    if (result == OAL_FAIL) {
...
/* 加密准备 */
static oal_netbuf_stru *hmac_wapi_encrypt_prepare(hmac_wapi_stru *wapi, oal_netbuf_stru *netbuf,
    mac_ieee80211_frame_stru *mac_hdr, uint8_t mac_hdr_len)
{
    int32_t ret;
    ...
    if (netbuff_des == NULL) {
        WAPI_TX_MEMALLOC_ERR(wapi);
        oam_error_log0(0, OAM_SF_ANY, "{hmac_wapi_encrypt::netbuff_des alloc err!");
        return NULL;
    }

    /* 填写cb */
    ret = memcpy_s(oal_netbuf_cb(netbuff_des), MAC_TX_CTL_SIZE, oal_netbuf_cb(netbuf), MAC_TX_CTL_SIZE);

    /* 先拷贝mac头,为了后续hcc处理，此处填写最大的空间 */
    oal_netbuf_init(netbuff_des, MAC_80211_QOS_HTC_4ADDR_FRAME_LEN);
    ret += memcpy_s(oal_netbuf_data(netbuff_des), MAC_80211_QOS_HTC_4ADDR_FRAME_LEN, mac_hdr, mac_hdr_len);
    if (ret != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "hmac_wapi_encrypt_prepare::memcpy fail!");
...
/* 加密数据 */
static uint32_t hmac_wapi_encrypt_data(hmac_wapi_stru *wapi, oal_netbuf_stru *netbuff_des, oal_netbuf_stru *netbuf,
    uint8_t *calc_mic, uint8_t *pn_swap)
{
    int32_t ret;
    uint32_t result;
    uint16_t pdu_len;
    ...
    datain = oal_mem_alloc_m(OAL_MEM_POOL_ID_LOCAL, pdu_len + SMS4_MIC_LEN, OAL_TRUE);
    if (datain == NULL) {
        oam_error_log0(0, OAM_SF_ANY, "{hmac_wapi_encrypt::datain alloc err!");
        WAPI_TX_MEMALLOC_ERR(wapi);
        return OAL_ERR_CODE_ALLOC_MEM_FAIL;
    }

    ret = memcpy_s(datain, pdu_len + SMS4_MIC_LEN, payload, pdu_len);
    /* 拷贝mic */
    ret += memcpy_s(datain + pdu_len, SMS4_MIC_LEN, calc_mic, SMS4_MIC_LEN);
    if (ret != EOK) {
        oam_error_log0(0, OAM_SF_ANY, "hmac_wapi_encrypt_data::memcpy fail!");
        return OAL_FAIL;
    }

    key_ek = wapi->ast_wapi_key[wapi->uc_keyidx].auc_wpi_ek;
    result = hmac_wpi_encrypt(pn_swap, datain, pdu_len + SMS4_MIC_LEN, key_ek,
        oal_netbuf_data(netbuff_des) + HMAC_WAPI_HDR_LEN + MAC_80211_QOS_HTC_4ADDR_FRAME_LEN);

    oal_mem_free_m(datain, OAL_TRUE);
    if (result != OAL_SUCC) {
...
/* 填写wapi头 */
static uint32_t hmac_set_wapi_head(hmac_wapi_stru *wapi, oal_netbuf_stru *netbuff_des, uint8_t key_index)
{
    int32_t ret;
    uint8_t *wapi_hdr = NULL;
    ...
    *(wapi_hdr + SMS4_KEY_IDX) = 0;
    /* 填写PN */
    ret = memcpy_s((wapi_hdr + SMS4_KEY_IDX + SMS4_WAPI_HDR_RESERVE), SMS4_PN_LEN,
        wapi->ast_wapi_key[key_index].auc_pn_tx, WAPI_PN_LEN);
    if (ret != EOK) {
...
static uint32_t hmac_wapi_encrypt(hmac_wapi_stru *wapi, oal_netbuf_stru *netbuf)
{
    mac_ieee80211_frame_stru *mac_hdr = NULL;
    uint8_t mac_hdr_len, key_index;
    uint16_t pdu_len;
    uint8_t calc_mic[SMS4_MIC_LEN];
    uint8_t *calc_mic_ptr = &calc_mic[0];
    uint8_t pn_swap[SMS4_PN_LEN]; /* 保存变换后的pn,用来计算mic和加密 */
    uint8_t *pn_swap_ptr = &pn_swap[0];
    oal_netbuf_stru *netbuff_des = NULL;
    ...
    mac_set_protectedframe((uint8_t *)mac_hdr);

    oam_warning_log2(0, OAM_SF_ANY, "{hmac_wapi_encrypt::mac_hdr_len %u! bit_more==%u.}",
        mac_hdr_len, mac_hdr->st_frame_control.bit_more_frag);
    pdu_len = MAC_GET_CB_MPDU_LEN(tx_ctl);

    /* 计算mic预备数据 */
    key_index = wapi->uc_keyidx;

    /************ 2. 计算mic,wapi的数据帧一般为QOS帧  ************/
    ret = hmac_wapi_encrypt_calculate_mic(wapi, netbuf, mac_hdr, calc_mic_ptr, pn_swap_ptr);
    if (ret != OAL_SUCC) {
        return ret;
    }
    /************ 3. 准备新的netbuff,用来存放加密后的数据, 填写cb,并准备加密前的数据 ************/
    netbuff_des = hmac_wapi_encrypt_prepare(wapi, netbuf, mac_hdr, mac_hdr_len);
    if (netbuff_des == NULL) {
        return OAL_ERR_CODE_ALLOC_MEM_FAIL;
    }
    /************************ 4. 加密 ************************/
    if (hmac_wapi_encrypt_data(wapi, netbuff_des, netbuf, calc_mic_ptr, pn_swap_ptr) != OAL_SUCC) {
        oal_netbuf_free(netbuff_des);
        return OAL_FAIL;
    }
    /***************** 5. 填写wapi头 *****************/
    ret = hmac_set_wapi_head(wapi, netbuff_des, key_index);
    if (ret != OAL_SUCC) {
        oal_netbuf_free(netbuff_des);
        return OAL_FAIL;
    }
    ...
    MAC_GET_CB_FRAME_HEADER_ADDR(tx_ctl) = (mac_ieee80211_frame_stru *)oal_netbuf_data(netbuff_des);

    /* netbuf 的data指针指向payload */
    oal_netbuf_pull(netbuff_des, MAC_80211_QOS_HTC_4ADDR_FRAME_LEN);

    /* 不包括mac hdr */
    MAC_GET_CB_MPDU_LEN(tx_ctl) = (uint16_t)(HMAC_WAPI_HDR_LEN + pdu_len + SMS4_MIC_LEN);
    oal_netbuf_next(netbuff_des) = oal_netbuf_next(netbuf);
    oal_netbuf_next(netbuf) = netbuff_des;
    /* 更新pn */
    hmac_wapi_pn_update(wapi->ast_wapi_key[wapi->uc_keyidx].auc_pn_tx, wapi->uc_pn_inc);
