...
void hmac_calc_tx_ampdu_maxsize(mac_user_stru *mac_user, wlan_protocol_enum_uint8 protocol_mode)
{
    if (protocol_mode == WLAN_HT_MODE) {
        ...
        mac_user->ampdu_maxsize =
            (uint32_t)(1U << (20 + mac_user->st_he_hdl.st_he_cap_ie.st_he_mac_cap.bit_max_ampdu_length_exponent)) - 1;
        mac_user->ampdu_maxsize = oal_min(mac_user->ampdu_maxsize, 6500631);
    } else {
        mac_user->ampdu_maxsize = 65535;
    }
...
uint32_t hmac_proc_ht_cap_ie(mac_vap_stru *pst_mac_vap, mac_user_stru *pst_mac_user, uint8_t *puc_ht_cap_ie)
{
    ...
    uint16_t us_tmp_txbf_low, us_ht_cap_info;
...
uint32_t hmac_proc_vht_cap_ie(mac_vap_stru *pst_mac_vap, hmac_user_stru *pst_hmac_user, uint8_t *puc_vht_cap_ie)
{
    ...
    hmac_calc_tx_ampdu_maxsize(pst_mac_user, WLAN_VHT_MODE);
    return OAL_SUCC;
}
    ...
    uint32_t us_rx_len, hmac_user_stru *pst_hmac_user)
{
    ...
    uint8_t uc_num_rates, uc_num_ex_rates;
