    ...
    mac_hisi_cap_vendor_ie_stru *hisi_cap_ie)
{
    ...
    if (ie_tmp != NULL) {
        /* 兼容05 C10/C20版本，解决私有IE结构体定义长度不一致问题 */
        /* 05商用版本私有IE结构体定义如下：
...
void mac_set_vht_capinfo_field_extend(mac_vap_stru *mac_vap, uint8_t *puc_buffer)
{
    ...
    if ((mac_vap->st_channel.en_bandwidth >= WLAN_BAND_WIDTH_160PLUSPLUSPLUS) &&
        ...
        g_wlan_spec_cfg->rx_stbc_160 == OAL_FALSE) {
...
uint16_t mac_get_rsn_capability(const uint8_t *rsn_ie)
{
    ...
    for (suite_idx = 0; suite_idx < pairwise_count; suite_idx++) {
        if (MAC_IE_REAMIN_LEN_IS_ENOUGH(&rsn_ie[2], &rsn_ie[index], ie_len, MAC_IE_REMAIN_LEN_4) == OAL_FALSE) {
            return 0;
        }
        index += INDEX_OFFSET_4;
    }
    ...
    /* AKM Suite Count */
    if (MAC_IE_REAMIN_LEN_IS_ENOUGH(&rsn_ie[2], &rsn_ie[index], ie_len, MAC_IE_REMAIN_LEN_2) == OAL_FALSE) {
        return 0;
    }
