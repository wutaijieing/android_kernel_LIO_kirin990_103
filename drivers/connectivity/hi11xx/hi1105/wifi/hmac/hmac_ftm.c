...
void hmac_ftm_vap_init_shenkuo(hmac_vap_stru *hmac_vap)
{
    ...
    ftm_initiator_stru *ftm_initiator_session)
{
    ...
    if (frame_len == 0) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_FTM,
            "{hmac_ftm_initiator_tx_ftm_request:fail to encap request frame!}");
    ...
    tx_ctl = (mac_tx_ctl_stru *)oal_netbuf_cb(ftm_frame);
    ...
    MAC_GET_CB_FRAME_SUBTYPE(tx_ctl) = WLAN_ACTION_FTM_REQUEST;
    ret = mac_vap_set_cb_tx_user_idx(mac_vap, tx_ctl, ftm_initiator_session->bssid);
    if (ret != OAL_SUCC) {
        oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_FTM, "(hmac_ftm_responder_send_ftm_frame::fail to find user by \
            xx:xx:xx:0x:0x:0x.}", ftm_initiator_session->bssid[MAC_ADDR_3],
                              ...
                              ftm_initiator_stru *ftm_initiator_session)
{
    ...
    mac_scan_req_h2d_stru h2d_scan_params;
    mac_vap_stru *mac_vap = NULL;
    ...
    /* 设置扫描参数 */
    memset_s(&h2d_scan_params, sizeof(mac_scan_req_h2d_stru), 0, sizeof(mac_scan_req_h2d_stru));
    h2d_scan_params.st_scan_params.en_bss_type = WLAN_MIB_DESIRED_BSSTYPE_INFRA;
    h2d_scan_params.st_scan_params.en_scan_type = WLAN_SCAN_TYPE_PASSIVE;
    h2d_scan_params.st_scan_params.en_scan_mode = WLAN_SCAN_MODE_FTM_REQ;
    h2d_scan_params.st_scan_params.uc_bssid_num = 1;
    h2d_scan_params.st_scan_params.uc_vap_id = mac_vap->uc_vap_id;
    h2d_scan_params.st_scan_params.uc_scan_func = MAC_SCAN_FUNC_MEAS;
    h2d_scan_params.st_scan_params.uc_channel_nums = 1;
    h2d_scan_params.st_scan_params.uc_probe_delay = 0;
    h2d_scan_params.st_scan_params.uc_max_scan_count_per_channel = 1;
    h2d_scan_params.st_scan_params.uc_max_send_probe_req_count_per_channel = 0;
    h2d_scan_params.st_scan_params.ull_cookie = 0;

    oal_set_mac_addr(h2d_scan_params.st_scan_params.auc_bssid[0], ftm_initiator_session->bssid);
    oal_set_mac_addr(h2d_scan_params.st_scan_params.auc_sour_mac_addr,
        mac_mib_get_StationID(mac_vap));
    ret = memcpy_s(h2d_scan_params.st_scan_params.ast_channel_list, sizeof(mac_channel_stru),
        &ftm_initiator_session->channel_ftm, sizeof(mac_channel_stru));
    if (ret != EOK) {
        oam_warning_log1(0, OAM_SF_SCAN, "{hmac_ftm_set_channel: memcpy_s failed ret is %d}", ret);
    }

    /* 扫描时间 */
    h2d_scan_params.st_scan_params.us_scan_time = scan_time;

    if (hmac_scan_proc_scan_req_event(hmac_vap, &h2d_scan_params) != OAL_SUCC) {
        oam_warning_log0(mac_vap->uc_vap_id, 0, "hmac_ftm_set_channel:hmac scan req failed");
    }
    ...
    uint16_t us_frame_len, ftm_initiator_stru *ftm_initiator_session)
{
    ...
    if (mac_ftmp != NULL && (mac_ftmp->uc_len == (sizeof(mac_ftm_parameters_ie_stru) - MAC_IE_HDR_LEN))) {
        ...
        oam_warning_log2(hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_FTM,
            "rx FTM_1, update ftm param,tsf_sync_info [0x%x], after fix partial_tsf_timer[0x%x].",
            ftm_initiator_session->tsf_sync_info, ftm_initiator_session->partial_tsf_timer);
...
uint32_t hmac_ftm_responder_send_ftm_frame(hmac_vap_stru *hmac_vap, ftm_responder_stru *ftm_responder_session)
{
    ...
    MAC_GET_CB_MPDU_LEN(tx_ctl) = frame_len + sizeof(mac_ftm_action_event_stru);
    MAC_GET_CB_FRAME_TYPE(tx_ctl) = WLAN_CB_FRAME_TYPE_ACTION;
    MAC_GET_CB_FRAME_SUBTYPE(tx_ctl) = WLAN_ACTION_FTM_RESPONE;
    ret = mac_vap_set_cb_tx_user_idx(mac_vap, tx_ctl, ftm_responder_session->mac_ra);
    if (ret != OAL_SUCC) {
        oam_warning_log3(mac_vap->uc_vap_id, OAM_SF_FTM, "(hmac_ftm_responder_send_ftm_frame::fail to find user by \
            xx:xx:xx:0x:0x:0x.}", ftm_responder_session->mac_ra[MAC_ADDR_3],
            ftm_responder_session->mac_ra[MAC_ADDR_4], ftm_responder_session->mac_ra[MAC_ADDR_5]);
    }
    ret = hmac_ftm_tx_action_event(mac_vap, ftm_frame, frame_len);
    if (ret != OAL_SUCC) {
        oal_netbuf_free(ftm_frame);
...
{
    hal_ftm_info_stru *hal_ftm_info = NULL;
    hmac_vap_stru *hmac_vap = NULL;

    hal_ftm_info = (hal_ftm_info_stru *)param;

    if (hal_ftm_info == NULL) {
        return OAL_FAIL;
    }
...
oal_bool_enum_uint8 hmac_rrm_proc_rm_request_shenkuo(hmac_vap_stru *hmac_vap_sta, oal_netbuf_stru *netbuf)
{
