...
uint32_t hmac_proc_tx_device_ring_tx_adapt(frw_event_mem_stru *frw_event)
{
    ...
    /* DEVICE ring通道 */
    if (g_wifi_delay_debug & HMAC_DELAY_TX) {
        hmac_wifi_delay_tx_hcc_device_tx(dmac_tx_event->pst_netbuf, frw_get_event_hdr(frw_event));
    }
#endif
...
void hmac_rx_wifi_pm_wkup(struct frw_hcc_extend_hdr *extend_hdr)
{
    if ((OAL_TRUE != wlan_pm_wkup_src_debug_get()) || ((extend_hdr->en_nest_type == FRW_EVENT_TYPE_WLAN_DRX))) {
        return;
    }
...
#ifdef _PRE_WLAN_WAKEUP_SRC_PARSE
    hmac_rx_wifi_pm_wkup(pst_extend_hdr);
#endif
