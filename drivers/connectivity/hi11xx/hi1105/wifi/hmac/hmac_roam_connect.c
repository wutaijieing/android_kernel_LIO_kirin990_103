...
OAL_STATIC void hmac_roam_11r_connect_fail_notify_wpas(hmac_roam_info_stru *roam_info)
{
    frw_event_mem_stru *event_mem;
    frw_event_stru *event = NULL;
    mac_vap_stru *mac_vap = &roam_info->pst_hmac_vap->st_vap_base_info;

    /* 抛11r connect fail事件到WAL */
    event_mem = frw_event_alloc_m(sizeof(uint16_t));
    if (event_mem == NULL) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "{hmac_roam_11r_connect_fail_notify_wpas::event_mem null.}");
        return;
    }
    /* 填写事件 */
    event = frw_get_event_stru(event_mem);
    frw_event_hdr_init(&(event->st_event_hdr), FRW_EVENT_TYPE_HOST_CTX, HMAC_HOST_CTX_EVENT_SUB_TYPE_FT_CONNECT_FAIL,
        sizeof(uint16_t), FRW_EVENT_PIPELINE_STAGE_0, mac_vap->uc_chip_id, mac_vap->uc_device_id, mac_vap->uc_vap_id);

    if (memcpy_s(frw_get_event_payload(event_mem), sizeof(uint16_t),
        &roam_info->st_connect.en_status_code, sizeof(uint16_t)) != EOK) {
        oam_error_log0(0, OAM_SF_FTM, "hmac_roam_11r_connect_fail_notify_wpas::memcpy fail!");
        frw_event_free_m(event_mem);
        return;
    }
    ...
    if (frw_event_dispatch_event(event_mem) != OAL_SUCC) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_FTM, "{hmac_roam_11r_connect_fail_notify_wpas::dispatch_event fail}");
    }
    frw_event_free_m(event_mem);
}
...
OAL_STATIC uint32_t hmac_roam_assoc_timeout(hmac_roam_info_stru *roam_info, void *param)
{
    ...
    if ((++roam_info->st_connect.uc_assoc_num >= MAX_ASOC_CNT) ||
        ...
        if (roam_info->pst_hmac_vap->bit_11r_enable == OAL_TRUE &&
            ...
            hmac_roam_11r_connect_fail_notify_wpas(roam_info);
        }
#endif
        return hmac_roam_connect_fail(roam_info);
    }
