    ...
    mac_vap_stru *pst_legacy_sta)
{
    ...
    if (mac_is_dfs_channel(pst_legacy_sta->st_channel.en_band, pst_legacy_sta->st_channel.uc_chan_number) == OAL_TRUE) {
...
uint32_t hmac_go_flow_sta_channel_handle(mac_device_stru *mac_device)
{
    ...
    if ((g_optimized_feature_switch_bitmap & BIT(CUSTOM_OPTIMIZE_CHBA)) &&
        mac_device_calc_up_vap_num(mac_device) > TWO_UP_VAP_NUMS) {
            oam_warning_log1(0, OAM_SF_DBAC, "{hmac_go_flow_sta_channel_handle:: find [%d]up vap > 2,return}",
...
uint32_t hmac_dbac_status_notify(frw_event_mem_stru *pst_event_mem)
{
    ...
    oam_warning_log2(pst_event->st_event_hdr.uc_vap_id, OAM_SF_DBAC,
        "hmac_dbac_status_notify::dbac switch to enable=%d, vap_id %d",
        *pen_dbac_enable, pst_event->st_event_hdr.uc_vap_id);
