...
static uint32_t hmac_config_social_channel_params(uint8_t channel_number, uint8_t bandwidth,
    ...
    mac_chba_param_stru *chba_params)
{
    ...
    /* 设置工作信道 */
    ret = hmac_chba_config_channel(mac_vap, chba_vap_info, chba_params->init_channel.uc_chan_number,
        chba_params->init_channel.en_bandwidth);
    if (ret == OAL_SUCC) {
        chba_vap_info->work_channel = mac_vap->st_channel; /* work信道配置成功, 将配置保存在chba_vap_info下 */
    }
    /* chba维测日志默认关闭 */
    ...
    uint8_t channel_number, uint8_t bandwidth)
{
    ...
    if (hmac_chba_chan_check_is_valid(hmac_vap, &mac_channel) == OAL_FALSE) {
        oam_error_log0(0, 0, "hmac_chba_config_channel::chba chan check_is not valid!");
        return OAL_FAIL;
    }
    /* 已经调度过，要使用vap信道配置接口 */
    ...
    oam_warning_log2(mac_vap->uc_vap_id, 0,
        "hmac_chba_config_channel::cur channel %d, switch to channel %d",
    ...
    mac_channel_stru *social_channel)
{
    ...
    if (band == WLAN_BAND_2G) {
        oam_error_log1(0, 0, "hmac_config_social_channel_params::social chan[%d]", channel_number);
        return OAL_ERR_CODE_INVALID_CONFIG;
    }

    ret = mac_get_channel_idx_from_num(band, channel_number, OAL_FALSE, &chan_idx);
    if (ret != OAL_SUCC) {
        oam_error_log1(0, 0, "hmac_config_social_channel_params::config social channel err[%u]", ret);
        return OAL_ERR_CODE_INVALID_CONFIG;
    }
...
void hmac_config_social_channel(hmac_chba_vap_stru *chba_vap_info, uint8_t channel_number, uint8_t bandwidth)
{
    ...
    if (hmac_vap == NULL) {
        oam_error_log0(0, 0, "hmac_config_social_channel::hmac_vap is null");
        return;
    }
    /* 根据定制化使用固定的social channel */
    if (g_chba_social_chan_cap) {
        chan_temp = g_chba_init_social_channel;
        if (chan_temp == 0) {
            hmac_config_social_channel_params(CHBA_INIT_SOCIAL_CHANNEL, WLAN_BAND_WIDTH_80PLUSPLUS, &social_channel);
        } else if (hmac_config_social_channel_params(chan_temp, bandwidth, &social_channel) != OAL_SUCC) {
            hmac_config_social_channel_params(CHBA_INIT_SOCIAL_CHANNEL, WLAN_BAND_WIDTH_80PLUSPLUS, &social_channel);
        }
    } else {
        /* social channel跟随work channel */
        if (hmac_config_social_channel_params(channel_number, bandwidth, &social_channel) != OAL_SUCC) {
            return;
        }
    }
    oam_warning_log1(hmac_vap->st_vap_base_info.uc_vap_id, 0,
        "hmac_config_social_channel::social channel switch to [%d]", social_channel.uc_chan_number);
    if (hmac_chba_chan_check_is_valid(hmac_vap, &social_channel) == OAL_FALSE) {
        oam_error_log0(0, 0, "hmac_config_social_channel::chba chan check_is not valid!");
        return;
    }
    chba_vap_info->social_channel = social_channel; /* 非法的social信道配置, 保存到chba_vap_info下 */

    /* 同步social channel到device */
