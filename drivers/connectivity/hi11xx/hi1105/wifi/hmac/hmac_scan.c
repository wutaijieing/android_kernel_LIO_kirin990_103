...
OAL_STATIC uint32_t hmac_find_rate_in_rates(uint8_t rate, uint8_t *update_rate, uint8_t rate_num)
{
    ...
    for (k = 0; k < rate_num; k++) {
        if (IS_EQUAL_RATES(update_rate[k], rate)) {
            break;
        }
    }
    return k;
    ...
    oal_netbuf_stru *netbuf, uint16_t buf_len, hmac_scanned_bss_info *scanned_bss, uint8_t ext_len)
{
    ...
    scan_result_extend_info = wlan_chip_get_scaned_result_extend_info(netbuf, frame_len);
    if (oal_unlikely((mac_device == NULL) || (scan_result_extend_info == NULL))) {
        return OAL_FAIL;
    }
...
uint32_t hmac_scan_proc_scan_comp_event(frw_event_mem_stru *event_mem)
{
    ...
    if (hmac_vap->en_wait_chan_meas_end == OAL_TRUE) {
        oam_warning_log1(0, OAM_SF_SCAN, "{hmac_scan_proc_scan_comp_event::chan meas end, scan rsp status[%d]}",
            d2h_scan_rsp_info->en_scan_rsp_status);
        oal_complete(&(hmac_vap->st_chan_meas_end_ready));
        hmac_vap->en_wait_chan_meas_end = OAL_FALSE;
    }
    return OAL_SUCC;
}
