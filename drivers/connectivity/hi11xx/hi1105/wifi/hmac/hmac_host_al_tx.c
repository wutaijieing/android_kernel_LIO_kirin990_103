...
OAL_STATIC void hmac_always_tx_set_tid_info(hmac_msdu_info_ring_stru *tx_ring)
{
    ...
    tx_ring->base_ring_info.size = hal_host_tx_tid_ring_size_get();
...
{
    oal_spin_lock_init(&tx_ring->tx_lock);
    oal_spin_lock_init(&tx_ring->tx_comp_lock);
}
...
uint32_t hmac_always_tx_proc(uint8_t *param, uint8_t hal_dev_id)
{
    ...
    ret = hmac_alloc_tx_ring(tx_ring);
    if (ret != OAL_SUCC) {
    ...
    alloc_size = hal_host_tx_tid_ring_depth_get(tx_ring->base_ring_info.size) * TX_RING_MSDU_INFO_LEN;
