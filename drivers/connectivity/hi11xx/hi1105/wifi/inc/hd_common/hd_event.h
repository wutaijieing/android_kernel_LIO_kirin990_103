...
typedef struct {
    ...
    /* 重要:回调函数指针:函数指针必须放最后否则核间通信出问题 */
    mac_scan_cb_fn p_fn_cb;
} mac_scan_req_ex_stru; /* hd_event shenkuo only */
...
typedef struct {
    uint32_t bit_uapsd : 1,
               ...
               bit_1024qam : 1,                        /* 对端支持1024qam能力: 0-不支持, 1-支持 */ /* shenkuo 不使用 */
...
/* begin:1103/1105 rom使用 */
#define WLAN_PAIRWISE_CIPHER_SUITES_1103 2
#define WLAN_AUTHENTICATION_SUITES_1103  2
typedef struct {
    uint32_t wpa_versions;
    uint32_t group_suite;
    uint32_t aul_pair_suite[WLAN_PAIRWISE_CIPHER_SUITES_1103];
    uint32_t aul_akm_suite[WLAN_AUTHENTICATION_SUITES_1103];
    uint32_t group_mgmt_suite;
} mac_crypto_settings_stru_1103; /* hd_event */
...
typedef struct {
    ...
    wlan_pmf_cap_status_uint8 en_pmf_cap;          /* 设备pmf能力 */
    oal_bool_enum_uint8 en_wps_enable;
    mac_crypto_settings_stru_1103 st_crypto; /* 密钥套件信息 */ /* 1103/1105 rom使用 */
#ifdef _PRE_WLAN_FEATURE_11R
    uint8_t auc_mde[NUM_8_BYTES]; /* MD IE信息 */
#endif                    // _PRE_WLAN_FEATURE_11R
    int8_t c_rssi;      /* 关联AP的RSSI信息 */
    ...
    oal_bool_enum_uint8 is_wapi_connect;
    uint8_t auc_rsv[NUM_2_BYTES];
} mac_conn_security_stru_1103; /* hd_event */
/* end:1103/1105 rom使用 */
typedef struct {
