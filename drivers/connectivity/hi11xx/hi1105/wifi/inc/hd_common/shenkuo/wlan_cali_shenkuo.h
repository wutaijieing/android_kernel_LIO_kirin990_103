...
#ifndef __WLAN_CALI_SHENKUO_H__
#define __WLAN_CALI_SHENKUO_H__
...
#define SHENKUO_CALI_MAX_MEM_SIZE (147120) //  reserved for cali data
#ifdef _PRE_WLAN_FEATURE_MEM_TX_BUGFIX
#define WLAN_CALI_DPD_CORRAM_DATA_LEN    6144
#else
#define WLAN_CALI_DPD_CORRAM_DATA_LEN    3072
#endif
#define WLAN_CALI_DPD_CORRAM_DATA_SIZE    (WLAN_CALI_DPD_CORRAM_DATA_LEN * sizeof(uint32_t))
...
typedef enum {
    ...
    SHENKUO_CALI_CHAIN_NUM_BUTT,
} shenkuo_rf_cali_chain_num_enum;
typedef uint8_t shenkuo_rf_cali_chain_num_enum_uint8;
...
typedef enum {
    ...
    WLAN_RF_CALI_DPD             = 22,
#ifdef _PRE_WLAN_FEATURE_LP_PATH
...
/* 校准RF带宽枚举 */
typedef enum {
    RF_CALI_BW_20M, /* 目前10M LPF BW带宽不单独校准, 用20M LPF BW校准值 */
    RF_CALI_BW_2G_BUTT,
    RF_CALI_BW_40M = RF_CALI_BW_2G_BUTT,
    RF_CALI_BW_80M,
    RF_CALI_BW_5G_BUTT,

    RF_CALI_BW_BUTT = RF_CALI_BW_5G_BUTT,
} rf_cali_bandwidth_enum;
typedef uint8_t rf_cali_bandwidth_enum_uint8;
...
typedef struct {
    ...
    uint16_t bit_temperature : 3,
               ...
               uc_5g_chan_idx2 : 5,
               bit_rev : 3;
} shenkuo_update_cali_channel_stru;

struct shenkuo_cali_param_tag {
    uint32_t dog_tag;
    shenkuo_update_cali_channel_stru st_cali_update_info;
...
typedef struct shenkuo_cali_param_tag shenkuo_cali_param_stru;
...
typedef struct {
    ...
    uint8_t trx_ppf_code;   /* shenkuo rf不使用,共平台5x需要 */
    uint8_t resv[2];  /* 对齐 2 */
...
/* 校准策略/控制参数 */
typedef struct {
    shenkuo_update_cali_channel_stru cali_update_info;
