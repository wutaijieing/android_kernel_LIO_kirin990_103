...
OAL_STATIC uint32_t wal_hipriv_feature_log_switch(oal_net_device_stru *net_dev, int8_t *pc_param)
{
    ...
    wal_tx_pow_param_enum idx)
{
    ...
    if ((idx == WAL_TX_POW_PARAM_SET_RF_REG_CTL || idx == WAL_TX_POW_PARAM_SET_NO_MARGIN) && value >= 2) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_hipriv_check_tx_pow_para_valid::para large than 2!}");
        return OAL_ERR_CODE_INVALID_CONFIG;
    }

    if (idx >= WAL_TX_POW_PARAM_SET_FIX_LEVEL && idx <= WAL_TX_POW_PARAM_SET_CTL_LEVEL && value > 4) {
...
OAL_STATIC uint32_t wal_hipriv_set_dscr_param(oal_net_device_stru *net_dev, int8_t *param, uint8_t func_type)
{
    ...
    if (param_index >= WAL_DSCR_PARAM_RATE && param_index <= WAL_DSCR_PARAM_BW) {
        ret = wal_hipriv_process_rate_params(net_dev, name, WAL_HIPRIV_CMD_NAME_MAX_LEN, set_dscr_param);
        if (ret != OAL_SUCC) {
            return ret;
        }
    } else {
    ...
    if (oal_unlikely(l_ret != OAL_SUCC)) {
        oam_warning_log1(0, OAM_SF_ANY, "{wal_hipriv_set_dscr_param::wal_send_cfg_event err[%d]!}\r\n", l_ret);
        return (uint32_t)l_ret;
    }
...
OAL_STATIC uint32_t wal_hipriv_set_ucast_data_dscr_param(oal_net_device_stru *net_dev, int8_t *param)
{
...
OAL_STATIC uint32_t wal_hipriv_set_bcast_data_dscr_param(oal_net_device_stru *net_dev, int8_t *param)
{
    return wal_hipriv_set_dscr_param(net_dev, param, WAL_HIPRIV_DSCR_PARAM_CMD_BCAST_DATA);
}


OAL_STATIC uint32_t wal_hipriv_set_ucast_mgmt_dscr_param(oal_net_device_stru *net_dev, int8_t *param)
{
    return wal_hipriv_set_dscr_param(net_dev, param, WAL_HIPRIV_DSCR_PARAM_CMD_UCAST_MGMT);
}


OAL_STATIC uint32_t wal_hipriv_set_mbcast_mgmt_dscr_param(oal_net_device_stru *net_dev, int8_t *param)
{
    ...
    int8_t *ac_name, int8_t *ac_value)
{
    ...
    /* 打印help信息 */
    ret = wal_hipriv_print_phy_debug_help_info(ac_value, strlen(ac_value));
    if (ret == OAL_SUCC) {
        return ret;
    }
...
OAL_STATIC uint32_t wal_hipriv_set_protocol_debug_analyze(
    mac_protocol_debug_switch_stru *protocol_debug, int8_t **input_param)
{
    uint32_t off_set = 0;
    int8_t ac_name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};
    int8_t ac_value[WAL_HIPRIV_CMD_VALUE_MAX_LEN] = {0};
...
OAL_STATIC uint32_t wal_hipriv_set_protocol_debug_para_prepare(
    mac_protocol_debug_switch_stru *protocol_debug, int8_t *pc_param)
{
    uint32_t off_set = 0;
    int8_t ac_name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};
    uint32_t ret;
    ...
    do {
        ...
        if (oal_strcmp("csa", ac_name) == 0) {
            ret = wal_protocol_debug_parase_csa_cmd(pc_param, protocol_debug, &off_set);
            if (ret != OAL_SUCC) {
                protocol_debug_cmd_format_info();
                return ret;
            }
            ...
            ret = wal_hipriv_set_protocol_debug_analyze(protocol_debug, &pc_param);
            if (ret != OAL_SUCC) {
                return ret;
            }
        }
    ...
    mac_dbdc_debug_switch_stru *dbdc_debug_switch,  mac_dbdc_cmd_enum_uint8 type, uint8_t status)
{
    ...
    } else if (type == MAC_DBDC_CHANGE_HAL_DEV) {
        dbdc_debug_switch->cmd_bit_map |= BIT(MAC_DBDC_CHANGE_HAL_DEV);
...
OAL_STATIC uint32_t wal_hipriv_analyze_dbdc_debug_params(mac_vap_stru *mac_vap,
    mac_dbdc_debug_switch_stru *dbdc_debug_switch, int8_t *pc_param)
{
    int8_t name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};
    ...
    do {
        ...
        if ((ret != OAL_SUCC) && (off_set != 0)) {
            oam_warning_log1(vap_id, OAM_SF_CFG, "{wal_hipriv_set_dbdc_debug_switch::cmd format err, ret:%d;!!}", ret);
            return ret;
        }
        pc_param += off_set;
    ...
    uint32_t *cmd_off_set, uint8_t *name)
{
    ...
    if (chip_type >= BOARD_VERSION_SHENKUO) {
...
uint32_t wal_hipriv_dyn_cali_cfg(oal_net_device_stru *pst_net_dev, int8_t *puc_param)
{
    ...
    wal_ioctl_cmd_tbl_stru st_cyn_cali_cfg;
    uint32_t ret;
    uint8_t uc_map_index = 0;
    ...
    /* 记录命令对应的枚举值 */
    pst_dyn_cali_param->en_dyn_cali_cfg = g_ast_dyn_cali_cfg_map[uc_map_index].idx;
    /* 获取参数配置值 */
...
OAL_STATIC uint32_t wal_hipriv_vsp_link_en(oal_net_device_stru *net_device, int8_t *param)
{
    ...
    uint32_t offset;
    uint32_t ret;
    struct hmac_vsp_cmd  *cmd;
    int i;

    cmd = (struct hmac_vsp_cmd *)write_msg.auc_value;
    ...
    if (strlen(ac_name) != 17 || sscanf_s(ac_name, "%02x:%02x:%02x:%02x:%02x:%02x\n", /* mac地址字符串长度17 */
        ...
        &cmd->user_mac[BYTE_OFFSET_3], &cmd->user_mac[BYTE_OFFSET_4], &cmd->user_mac[BYTE_OFFSET_5]) != 6) {
        return OAL_FAIL;
    }
    ...
    WAL_WRITE_MSG_HDR_INIT(&write_msg, WLAN_CFGID_VSP_DEBUG, sizeof(struct hmac_vsp_cmd));
    if (OAL_SUCC != wal_send_cfg_event(net_device, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(struct hmac_vsp_cmd), (uint8_t*)&write_msg, OAL_FALSE,
        NULL)) {
...
OAL_STATIC uint32_t wal_hipriv_vsp_dbg(oal_net_device_stru *net_device, int8_t *param)
{
    uint32_t ret;
    uint32_t offset;
    int8_t ac_name[WAL_HIPRIV_CMD_NAME_MAX_LEN];
...
OAL_STATIC uint32_t wal_hipriv_set_extlna_bypass_rssi(oal_net_device_stru *net_device, int8_t *param)
{
    wal_msg_write_stru st_write_msg;
    uint32_t off_set;
    int8_t ac_name[WAL_HIPRIV_CMD_NAME_MAX_LEN] = {0};
...
#ifdef _PRE_WLAN_FEATURE_WAPI
#ifdef _PRE_WAPI_DEBUG
    { "wapi_info", wal_hipriv_show_wapi_info }, /* wapi hipriv "vap0 wapi_info xx:xx:xx:xx:xx:xx" */
#endif
#endif                                          /* #ifdef _PRE_WLAN_FEATURE_WAPI */
