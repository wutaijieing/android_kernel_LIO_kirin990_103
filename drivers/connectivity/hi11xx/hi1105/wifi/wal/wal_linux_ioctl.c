...
#if (_PRE_OS_VERSION_LINUX == _PRE_OS_VERSION)
static int32_t wal_hipriv_inetaddr_notifier_call(struct notifier_block *this, unsigned long event, void *ptr);

OAL_STATIC struct notifier_block g_wal_hipriv_notifier = {
    .notifier_call = wal_hipriv_inetaddr_notifier_call
};

static int32_t wal_hipriv_inet6addr_notifier_call(struct notifier_block *this, unsigned long event, void *ptr);
...
#endif
OAL_STATIC uint32_t wal_hipriv_memory_monitor(oal_net_device_stru *net_dev, int8_t *pc_param);
...
OAL_STATIC uint32_t wal_hipriv_chba_log_level(oal_net_device_stru *net_dev, int8_t *param);
OAL_STATIC uint32_t wal_hipriv_chba_set_ps_thres(oal_net_device_stru *net_dev, int8_t *param);
OAL_STATIC uint32_t wal_hipriv_chba_island_chan_switch(oal_net_device_stru *net_dev, int8_t *param);
OAL_STATIC uint32_t wal_hipriv_set_chba_feature_switch(oal_net_device_stru *pst_net_dev, int8_t *pc_param);
OAL_STATIC uint32_t wal_hipriv_set_chba_battery(oal_net_device_stru *net_dev, int8_t *pc_param);
...
OAL_STATIC uint32_t wal_hipriv_start_chan_meas(oal_net_device_stru *net_dev, int8_t *pc_param);
#endif
...
OAL_STATIC int32_t wal_ext_priv_cmd_pcie_aspm(int32_t aspm)
{
    return hi110x_hcc_ip_pm_ctrl(aspm, HCC_EP_WIFI_DEV);
}
...
int32_t wal_regdomain_update(oal_net_device_stru *net_dev, int8_t *pc_country)
{
    ...
    ret = wal_regdomain_update_post_event(net_dev, pst_regdom, pc_country);
    if (ret != OAL_SUCC) {
        return ret;
    }
...
int32_t wal_selfstdy_regdomain_update(oal_net_device_stru *net_dev, int8_t *pc_country)
{
    ...
    /* 发送消息 */
    ret = wal_send_cfg_event(net_dev, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(mac_cfg_country_stru), (uint8_t *)&write_msg, OAL_FALSE, &rsp_msg);
    if (oal_unlikely(ret != OAL_SUCC)) {
        if (oal_value_ne_all2(ret, -OAL_ETIMEDOUT, -OAL_EFAUL)) {
            oal_mem_free_m(pst_mac_regdom, OAL_TRUE);
            pst_mac_regdom = NULL;
    ...
    wlan_p2p_mode_enum_uint8 p2p_mode)
{
    ...
    if (oal_unlikely(result != OAL_SUCC)) {
        ...
        if (wal_check_and_release_msg_resp(rsp_msg) == OAL_SUCC) {
            if (wal_del_vap_try_wlan_pm_close(mac_vap) != OAL_SUCC) {
...
uint32_t wal_hipriv_set_ip_filter(oal_net_device_stru *net_dev, int8_t *pc_param)
{
    ...
    if (0 == oal_strncmp(ac_cmd, CMD_SET_RX_FILTER_ENABLE, OAL_STRLEN(CMD_SET_RX_FILTER_ENABLE))) {
        ...
        /* 更新黑名单 */
        /* 获取名单条目数 */
        l_items_cnt = oal_min(MAX_HIPRIV_IP_FILTER_BTABLE_SIZE, oal_atoi(ac_cmd_param));
        /* 获取名单条目 */
        ...
        return wal_ipriv_ip_filter_items(ast_items, sizeof(ast_items), l_items_cnt);
    } else {
...
OAL_STATIC uint32_t wal_hipriv_always_tx(oal_net_device_stru *net_dev, int8_t *pc_param)
{
    ...
    ret = wal_check_and_release_msg_resp(rsp_msg);
    if (ret != OAL_SUCC) {
        oam_warning_log1(0, OAM_SF_DFR, "{wal_hipriv_always_tx fail, err code[%u]!}", ret);
        return ret;
    }
...
OAL_STATIC void wal_config_mac_filter(oal_net_device_stru *net_dev, int8_t *pc_command)
{
    ...
    uint32_t mac_mode, mac_cnt, result, i;
    uint32_t off_set = 0;
    ...
    wlan_p2p_mode_enum_uint8 en_p2p_mode)
{
    ...
    if (wal_del_vap_try_wlan_pm_close(mac_vap) != OAL_SUCC) {
        oam_error_log0(mac_vap->uc_vap_id, OAM_SF_ANY, "wal_deinit_wlan_vap:del vap post process fail");
        return ret;
    }
    if (oal_unlikely(ret != OAL_SUCC)) {
    ...
    switch (event) {
        case NETDEV_UP: {
            ...
            if (mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
                ...
                if (pst_hmac_vap == NULL) {
                    oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_vap null.vap[%d]}", mac_vap->uc_vap_id);
                    break;
                }
                /* 获取到IP地址的时候开启低功耗 */
        ...
        case NETDEV_DOWN: {
            ...
            if (mac_vap->en_vap_mode == WLAN_VAP_MODE_BSS_STA) {
                ...
                pst_hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
                if (pst_hmac_vap == NULL) {
                    oam_warning_log1(mac_vap->uc_vap_id, OAM_SF_CFG, "{hmac_vap null.[%d]}", mac_vap->uc_vap_id);
                    break;
                }
    ...
    mac_psm_query_type_enum_uint8 en_query_type, wal_wifi_priv_cmd_stru *priv_cmd)
{
    ...
    *(mac_psm_query_type_enum_uint8*)(write_msg.auc_value) = en_query_type;
    ...
    if (oal_copy_to_user(priv_cmd->puc_buf, pst_psm_stat->aul_val,
...
OAL_STATIC oal_bool_enum_uint8 wal_ioctl_chan_meas_start_check(mac_vap_stru *mac_vap)
{
    ...
    if (hmac_device->st_scan_mgmt.en_is_scanning &&
        ...
        oam_warning_log0(mac_vap->uc_vap_id, 0,
            "wal_ioctl_chan_meas_is_running_check::chan meas scan is running, return.");
        return OAL_TRUE;
    }
...
static int32_t wal_ioctl_get_current_tsf(oal_net_device_stru *net_dev, oal_ifreq_stru *ifr,
    wal_wifi_priv_cmd_stru *priv_cmd)
{
    mac_vap_stru *mac_vap = NULL;
...
OAL_STATIC uint32_t wal_hipriv_chba_set_user_bitmap(oal_net_device_stru *net_dev, int8_t *pc_param)
{
    ...
    */
    ...
    oal_strtoaddr(ac_name, sizeof(ac_name), ps_bitmap_info.auc_mac_addr, WAL_ATCMDSRV_IOCTL_MAC_LEN);
...
/* 解析chba特性开关命令 */
OAL_STATIC uint32_t wal_hipriv_analyze_chba_feature_switch_params(int8_t *pc_param,
...
#ifdef _PRE_WLAN_CHBA_MGMT
    } else if (oal_strncmp(pc_cmd, CMD_CHBA_MODULE_INIT, CMD_CHBA_MODULE_INIT_LEN) == 0) {
