...
OAL_STATIC uint32_t wal_vendor_set_change_go_channel(oal_net_device_stru *net_dev,
    wal_wifi_priv_cmd_stru *priv_cmd, uint8_t *cmd, uint32_t cmd_len)
{
    uint8_t *cmd_data;
    uint32_t offset;
...
uint32_t wal_vendor_11ax_wifi6_blacklist_handle(oal_net_device_stru *p_net_dev, uint8_t *pc_cmd, uint32_t total_len)
{
    int32_t  ret;
    ...
    if ((blacklistCnt > HMAC_WIFI6_SELF_CURE_BLACK_LIST_MAX_NUM) ||
        (blacklistCnt * sizeof(hmac_wifi6_self_cure_blacklist_stru) != remain_len)) {
        oam_warning_log3(0, OAM_SF_11AX, "{wal_vendor_11ax_wifi6_blacklist_handle::uc_blacklist_cnt=[%d],\
            max_num=[%d] remain[%d].}\r\n", blacklistCnt, HMAC_WIFI6_SELF_CURE_BLACK_LIST_MAX_NUM, remain_len);
        return OAL_FAIL;
    }

    WAL_WRITE_MSG_HDR_INIT(&writeMsg, WLAN_CFGID_SET_11AX_WIFI6_BLACKLIST, sizeof(hmac_wifi6_self_cure_info_stru));
    ...
    ret = wal_send_cfg_event(p_net_dev, WAL_MSG_TYPE_WRITE,
        WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(hmac_wifi6_self_cure_info_stru), (uint8_t *)&writeMsg, OAL_FALSE, NULL);
    if (oal_unlikely(ret != OAL_SUCC)) {
...
int32_t wal_vendor_get_ap_bandwidth(oal_net_device_stru *pst_net_dev, wal_wifi_priv_cmd_stru *priv_cmd)
{
    ...
    oal_copy_to_user(priv_cmd->puc_buf, &uc_current_bandwidth, sizeof(uint8_t));
...
int32_t wal_vendor_get_160m_supported(oal_net_device_stru *pst_net_dev, wal_wifi_priv_cmd_stru *priv_cmd)
{
    oal_bool_enum_uint8 aput_support_160m = OAL_FALSE;
#ifdef _PRE_PLAT_FEATURE_CUSTOMIZE
    aput_support_160m = wlan_chip_is_aput_support_160m();
#endif
    oal_copy_to_user(priv_cmd->puc_buf, &aput_support_160m, sizeof(oal_bool_enum_uint8));
...
OAL_STATIC int32_t wal_ioctl_set_dynamic_dbac_mode(oal_net_device_stru *p_net_dev, uint8_t *p_command)
{
    ...
    uint8_t value = (uint8_t)oal_atoi(p_data);
    if (value > 3) {   // mode最大为3
    ...
    uint8_t *pc_cmd, wal_wifi_priv_cmd_stru *p_priv_cmd)
{
    ...
    if (oal_strncasecmp(pc_cmd, CMD_PCIE_ASPM_STATE, CMD_PCIE_ASPM_STATE_LEN) == 0) {
        ...
        return (uint32_t)wal_hid2d_sleep_mode(pst_net_dev, oal_atoi(pc_cmd + CMD_PM_STATE_LEN + 1), p_priv_cmd);
    ...
    oal_ifreq_stru *pst_ifr, wal_wifi_priv_cmd_stru *p_priv_cmd, uint8_t *pc_cmd)
{
    ...
    if (0 == oal_strncasecmp(pc_cmd, CMD_SET_CHANGE_GO_CHANNEL, CMD_SET_CHANGE_GO_CHANNEL_LEN)) {
        return wal_vendor_set_change_go_channel(pst_net_dev, p_priv_cmd, pc_cmd, p_priv_cmd->total_len);
    }
    ...
    return (int32_t)wal_vendor_priv_cmd_ext3(pst_net_dev, pst_ifr, p_priv_cmd, pc_cmd);
}
...
int32_t wal_vendor_priv_cmd_prepare_command(oal_ifreq_stru *pst_ifr, wal_wifi_priv_cmd_stru *priv_cmd, int8_t **cmd)
{
    int32_t l_ret;
    int8_t *command = NULL;
