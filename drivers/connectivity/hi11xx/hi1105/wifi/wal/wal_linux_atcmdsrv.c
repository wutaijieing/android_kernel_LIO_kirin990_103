...
OAL_STATIC int32_t wal_get_hw_status_shenkuo(oal_net_device_stru *pst_net_dev,
    wal_atcmdsrv_wifi_priv_cmd_stru *pst_priv_cmd)
{
    int32_t                       l_ret;
    uint32_t                      lte_status = 0;
    shenkuo_cali_param_stru          *pst_cali_data_c0 = NULL;
    shenkuo_cali_param_stru          *pst_cali_data_c1 = NULL;
    int32_t                       *pl_fem_pa_status = &(pst_priv_cmd->pri_data.l_fem_pa_status);
    ...
    pst_cali_data_c0 = (shenkuo_cali_param_stru *)get_cali_data_buf_addr();
    if (pst_cali_data_c0 == NULL) {
        oam_error_log0(0, 0, "wal_get_hw_status_shenkuo::C0 null ptr happened");
        return -OAL_EINVAL;
    }

    pst_cali_data_c1 = pst_cali_data_c0 + 1;
    if (pst_cali_data_c1 == NULL) {
        oam_error_log0(0, 0, "wal_get_hw_status_shenkuo::null ptr happened");
        return -OAL_EINVAL;
    }
    ...
    *pl_fem_pa_status = (int32_t)((pst_cali_data_c0->check_hw_status) |
    ...
    if (*pl_fem_pa_status != 0) {
        oam_error_log1(0, OAM_SF_ANY,
            "{wal_get_hw_status_shenkuo::FEM pa status[%x].Bit:[0-1]2gc0,[2-3]5gc0,[4-5]2gc1,[6-7]5gc1,[8]lte_gpio}",
            *pl_fem_pa_status);
        return -OAL_EINVAL;
    }
    ...
    wal_atcmdsrv_wifi_priv_cmd_stru *pst_priv_cmd)
{
    ...
    if (l_chip_type >= BOARD_VERSION_SHENKUO) {
        l_ret = wal_get_hw_status_shenkuo(pst_net_dev, pst_priv_cmd);
...
OAL_STATIC void wal_get_shenkuo_fem_pa_status(oal_net_device_stru *pst_net_dev, int32_t *pl_fem_pa_status)
{
    ...
    *pl_fem_pa_status = (int32_t)(check_hw_status[WLAN_RF_CHANNEL_ZERO] |
    ...
    if (*pl_fem_pa_status != 0) {
        chr_exception_report(CHR_PLATFORM_EXCEPTION_EVENTID, CHR_SYSTEM_WIFI, CHR_LAYER_DEV,
                             CHR_WIFI_DEV_EVENT_CHIP, CHR_WIFI_DEV_ERROR_FEM_FAIL);
        oam_error_log1(0, OAM_SF_ANY,
            "{wal_get_shenkuo_fem_pa_status::FEM pa status[%x].Bit:[0-1]2gc0,[2-3]5gc0,[4-5]2gc1,[6-7]5gc1}",
            *pl_fem_pa_status);
#ifdef CONFIG_HUAWEI_DSM
        hw_110x_dsm_client_notify(SYSTEM_TYPE_WIFI, DSM_WIFI_FEMERROR, "%s: fem error",  __FUNCTION__);
#endif
...
void wal_atcmsrv_ioctl_get_fem_pa_status(oal_net_device_stru *pst_net_dev, int32_t *pl_fem_pa_status)
{
    ...
    if (l_chip_type >= BOARD_VERSION_SHENKUO) {
        wal_get_shenkuo_fem_pa_status(pst_net_dev, pl_fem_pa_status);
    } else if (l_chip_type == BOARD_VERSION_HI1105) {
        wal_get_1105_fem_pa_status(pst_net_dev, pl_fem_pa_status);
    } else if (l_chip_type == BOARD_VERSION_HI1161) {
    } else {
                                                 ...
                                                 wal_atcmdsrv_wifi_priv_cmd_stru *pst_priv_cmd)
{
    ...
    int8_t pc_param_shenkuo[][NUM_9_BYTES] = { "3 0001 0", "3 0010 0", "3 0100 0", "3 1000 0", "3 1111 0" };
    ...
    if (get_hi110x_subchip_type() <= BOARD_VERSION_HI1105) {
        ...
        l_ret = (int32_t)wal_hipriv_set_m2s_switch(pst_net_dev, pc_param_shenkuo[l_mimo_channel]);
    }
    ...
    struct wal_atcmdsrv_wifi_connect_info *connect_info)
{
    ...
    scanned_bss_info = hmac_scan_find_scanned_bss_by_bssid(bss_mgmt, connect_info->auc_bssid);
