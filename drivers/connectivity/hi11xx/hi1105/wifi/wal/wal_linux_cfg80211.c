...
OAL_STATIC uint32_t wal_cfg80211_del_vap(mac_cfg_del_vap_param_stru *pst_del_vap_param)
{
    ...
    ***************************************************************************/
    ...
    if (wal_del_vap_try_wlan_pm_close(pst_mac_vap) != OAL_SUCC) {
        oam_error_log0(pst_mac_vap->uc_vap_id, OAM_SF_ANY, "wal_cfg80211_del_vap:wal_del_vap_try_wlan_pm_close fail");
        return OAL_EFAIL;
    }
...
OAL_STATIC uint32_t wal_parse_ht_vht_ie(oal_net_device_stru *pst_netdev,
...
OAL_STATIC int32_t wal_cfg80211_sched_scan_stop(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_netdev,
    uint64_t reqid);
    ...
    hmac_device_stru *hmac_device)
{
    ...
    if ((hmac_device->st_scan_mgmt.en_is_scanning != OAL_TRUE) ||
        hmac_device->st_scan_mgmt.st_scan_record_mgmt.en_scan_mode != WLAN_SCAN_MODE_HID2D_SCAN) {
        return;
    }
    mac_vap = oal_net_dev_priv(netdev);
    hmac_vap = (hmac_vap_stru *)mac_res_get_hmac_vap(mac_vap->uc_vap_id);
    if (oal_unlikely(hmac_vap == NULL)) {
        oam_warning_log0(mac_vap->uc_vap_id, OAM_SF_P2P, "{wal_cfg80211_scan_abort_chan_meas:: hmac_vap null!}");
        return;
    }
    ...
    hmac_vap->en_wait_chan_meas_end = OAL_TRUE;
    oal_init_completion(&(hmac_vap->st_chan_meas_end_ready));
    wal_force_scan_complete(netdev, OAL_TRUE);
    /* 超时时间200ms */
    if (oal_wait_for_completion_timeout(&(hmac_vap->st_chan_meas_end_ready),
        (uint32_t)oal_msecs_to_jiffies(200)) == 0) {
...
OAL_STATIC int32_t wal_cfg80211_scan(oal_wiphy_stru *pst_wiphy, oal_cfg80211_scan_request_stru *pst_request)
{
    ...
    if (wlan_chip_is_support_hw_wapi() == OAL_FALSE && is_p2p_scan_req(pst_request) &&
...
OAL_STATIC int32_t wal_cfg80211_connect_init(oal_cfg80211_conn_stru *pst_sme, hmac_conn_param_stru *mac_conn_param)
{
    ...
    if ((mac_conn_param->ie_len > 0) && (pst_sme->ie != NULL)) {
        ...
        if (puc_ie == NULL) {
            ...
            free_connect_param_resource(mac_conn_param);
            return -OAL_EFAIL;
        }
...
OAL_STATIC wlan_channel_bandwidth_enum_uint8 wal_cfg80211_set_channel_cal_bw(oal_net_device_stru *netdev)
{
    ...
    if (wal_should_down_protocol_bw(p_mac_dev, p_mac_vap) == OAL_TRUE) {
...
OAL_STATIC int32_t wal_cfg80211_start_ap(oal_wiphy_stru *wiphy,
                                         ...
                                         oal_ap_settings_stru *ap_settings)
{
    ...
    if ((g_optimized_feature_switch_bitmap & BIT(CUSTOM_OPTIMIZE_CHBA)) &&
        ...
        (hmac_get_chba_vap())->mac_vap_id = uc_vap_id;
    }
#endif
    /*****************************************************************************
        1.设置信道
    ...
    *****************************************************************************/
    ...
    /*****************************************************************************
        2.2 设置auth mode信息
    *****************************************************************************/
    en_auth_algs = ap_settings->auth_type;

    l_ret = wal_cfg80211_set_auth_mode(netdev, en_auth_algs);
    if (l_ret != OAL_SUCC) {
        return -OAL_EFAIL;
    }
...
OAL_STATIC int32_t wal_cfg80211_stop_ap(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_netdev)
{
    mac_vap_stru *pst_mac_vap = NULL;
    oal_netdev_priv_stru *pst_netdev_priv = NULL;
    int32_t l_ret;
    ...
    l_ret = wal_cfg80211_stop_ap_event(pst_netdev, pst_mac_vap);
    if (l_ret != OAL_SUCC) {
        return l_ret;
    }
    ...
    mac_pno_scan_stru *pno_scan_info)
{
    ...
    pno_scan_info->pno_scan_interval = PNO_SCHED_SCAN_INTERVAL; /* 驱动自己定义为300s */
    ...
    oal_net_device_stru *pst_netdev, oal_cfg80211_sched_scan_request_stru *pst_request)
{
    ...
    wal_cfg80211_sched_scan_start_pno_init(pst_request, &st_pno_scan_info);
    /* 保存当前的PNO调度扫描请求指针 */
...
OAL_STATIC int32_t wal_cfg80211_add_station(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_dev,
...
OAL_STATIC int32_t wal_cfg80211_del_station(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_dev,
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 1, 0))
...
OAL_STATIC int32_t wal_cfg80211_get_station_check_left_time(oal_station_info_stru *pst_sta_info,
    hmac_vap_stru *pst_hmac_vap)
{
    ...
    if (i_leftime == 0) {
        ...
        oam_warning_log1(0, OAM_SF_ANY, "wal_cfg80211_get_station::rssi %d", pst_hmac_vap->station_info.signal);
        return OAL_SUCC;
    }
}
    ...
    oal_net_device_stru *pst_dev, const uint8_t *puc_mac, oal_station_info_stru *pst_sta_info)
{
    ...
    ********************************************************************************/
    ...
    l_ret = wal_send_cfg_event(pst_dev, WAL_MSG_TYPE_WRITE, WAL_MSG_WRITE_MSG_HDR_LENGTH +
        sizeof(dmac_query_station_info_request_event), (uint8_t *)&write_msg, OAL_FALSE, NULL);
    if (oal_unlikely(l_ret != OAL_SUCC)) {
...
OAL_STATIC int32_t wal_cfg80211_dump_station(oal_wiphy_stru *pst_wiphy, oal_net_device_stru *pst_dev,
...
#endif
        pst_net_dev->features    |= NETIF_F_IP_CSUM | NETIF_F_IPV6_CSUM | NETIF_F_RXCSUM;
        pst_net_dev->hw_features |= NETIF_F_IP_CSUM | NETIF_F_IPV6_CSUM | NETIF_F_RXCSUM;
    }
    ...
    hmac_device_stru *hmac_device, mac_vap_stru *pst_mac_vap)
{
    ...
    if (wal_check_and_release_msg_resp(rsp_msg) == OAL_SUCC) {
        if (wal_del_vap_try_wlan_pm_close(pst_mac_vap) != OAL_SUCC) {
            oam_warning_log0(0, OAM_SF_ANY, "{wal_cfg80211_del_virtual_intf::wal_del_vap_try_wlan_pm_close fail}");
        }
    } else {
...
OAL_STATIC int32_t wal_cfg80211_del_virtual_intf(oal_wiphy_stru *pst_wiphy, oal_wireless_dev_stru *pst_wdev)
{
    ...
    if (net_dev == NULL) {
        oam_warning_log0(0, OAM_SF_ANY, "{wal_cfg80211_del_virtual_intf::net_dev is null by netdev, mode[%d]!}");
        return -OAL_EINVAL;
    }
    ...
    pst_hmac_vap->pst_del_net_device = net_dev;
    oal_workqueue_schedule(&(pst_hmac_vap->st_del_virtual_inf_worker));

    return wal_cfg80211_del_virtual_intf_event(net_dev, hmac_device, pst_mac_vap);
}
//#endif
    ...
    uint64_t *pull_cookie)
{
    ...
    wait = params->wait;
    /* shenkuo FPGA信道切换时间长，需要增加监听时间以保证管理帧发送时间,回片后可删除 */
    ...
    uint8_t uc_dev_id)
{
    ...
    snprintf_s(ac_vap_netdev_name, MAC_NET_DEVICE_NAME_LENGTH, MAC_NET_DEVICE_NAME_LENGTH - 1, "wlan%u", uc_dev_id);
    ...
    snprintf_s(ac_vap_netdev_name, MAC_NET_DEVICE_NAME_LENGTH,
               MAC_NET_DEVICE_NAME_LENGTH - 1, "p2p%u", uc_dev_id);
...
static void wal_cfg80211_init_scan_info(oal_wiphy_stru *wiphy)
{
...
uint32_t wal_cfg80211_init(void)
{
    ...
    for (chip = 0; chip < oal_bus_get_chip_num(); chip++) {
        for (uc_device = 0; uc_device < pst_hmac_board->ast_chip[chip].uc_device_nums; uc_device++) {
            ...
            wal_cfg80211_init_iface(pst_wiphy);
            wal_cfg80211_init_scan_info(pst_wiphy);
            wal_cfg80211_init_cipher_info(pst_wiphy);
            ...
            wal_cfg80211_init_band_info(pst_wiphy, uc_dev_id);
...
int32_t wal_del_vap_try_wlan_pm_close(mac_vap_stru *mac_vap)
{
    ...
    chip = hmac_res_get_mac_chip(mac_vap->uc_chip_id);
    if (chip == NULL) {
        oam_error_log1(0, OAM_SF_ANY, "wal_del_sta_try_wlan_pm_close:chip_id=%d", mac_vap->uc_chip_id);
        return OAL_FAIL;
    }
