...
struct its_power {
	...
	struct timespec64 last_ts; /* record interval */
#endif
...
u64 freq_to_index(u64 core_idx, u64 freq)
{
	...
	policy = cpufreq_cpu_get(core_idx);
	if (policy == NULL)
		return freq_idx;
...
static unsigned long long ithermal_sec_its_set_window_by_core(u64 core_idx, u64 old_freq,
						   u64 freq)
{
...
static int ithermal_sec_its_get_total_power(u64 core_idx, u64 freq, u64 *dynamic_power,
				 u64 *static_power)
{
	int ret;
...
#ifdef CONFIG_ITS_IPA
static int ithermal_sec_its_get_gpu_leakage(u64 *static_power)
{
	int ret;
	u64 x1, x2, x3;
				...
				unsigned long event, void *data)
{
	...
	if (freq->new != freq->old)
		ithermal_sec_its_set_window_by_core(freq->policy->cpu, freq->old, freq->new);
...
static void update_gpu_power(void)
{
	...
	ret = ithermal_sec_its_get_gpu_leakage(&gpu_leakage);
	if (ret != 0)
...
static void its_dubai_getpower(struct work_struct *work)
{
	...
	for (idx = 0; idx < (int)nr_cpu_ids; idx++) {
		...
		if (freq > 0)
			ret = ithermal_sec_its_get_total_power(idx, freq,
							   &dynamic_power,
							   &static_power);
		else
			ret = ITS_ERR;
...
static int ithermal_its_init(void)
{
	int ret;
	...
	ret = of_property_read_u32(np, "ithermal,its_polling_timeout",
...
static void ithermal_its_exit(void)
{
	if (g_its_power_data != NULL) {
#ifdef CONFIG_ITS_DEBUG
...
module_init(ithermal_its_init);
module_exit(ithermal_its_exit);
