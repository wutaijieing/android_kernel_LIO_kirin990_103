...
#ifdef CONFIG_DPM_HWMON_PERIPCR
static unsigned int g_dpm_peri_pcr_vote;
...
static void dpm_enable_peri_pcr(void)
{
...
static void peri_pcr_vote(bool vote, unsigned int module_mask)
{
...
static void peri_pcr_resume(void)
{
	if (g_dpm_peri_pcr_vote > 0)
		dpm_enable_peri_pcr();
}
...
void dpm_enable_module(struct dpm_hwmon_ops *pos, bool dpm_switch)
{
	if (pos == NULL)
		return;
#ifdef CONFIG_DPM_HWMON_PERIPCR
	if (pos->dpm_type == DPM_PERI_MODULE || pos->dpm_type == DPM_GPU_MODULE)
		peri_pcr_vote(dpm_switch, BIT(pos->dpm_type));
#endif
