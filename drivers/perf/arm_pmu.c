...
static void armpmu_enable(struct pmu *pmu)
{
	...
	int enabled = bitmap_weight(hw_events->used_mask, num_events);
...
int perf_num_counters(void)
{
	...
	if (__oprofile_cpu_pmu != NULL)
		max_events = *this_cpu_ptr(__oprofile_cpu_pmu->num_events);
...
int armpmu_register(struct arm_pmu *pmu)
{
	...
	for_each_cpu(cpu, &pmu->supported_cpus) {
		...
		pr_info("enabled with %s PMU driver, %d counters available%s for CPU%d\n",
