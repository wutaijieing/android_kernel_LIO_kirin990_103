...
void hufs_mphy_clk_set(struct hufs_host *host)
{
	u32 reg;
	if (!host)
		return;
...
void hufs_mphy_fpga_reset(struct ufs_hba *hba)
{
...
void hufs_mphy_asic_reset(struct ufs_hba *hba)
{
	int ret;
	struct hufs_host *host = NULL;
...
void ufs_soc_init(struct ufs_hba *hba)
{
	...
	if (host->caps & USE_HUFS_MPHY_TC)
		hufs_mphy_fpga_reset(hba);
	else
		hufs_mphy_asic_reset(hba);
	...
	/* if ver is factory,we will use two lane */
	if (runmode_is_factory())
		host->caps &= BIT_1LANE_MASK;
