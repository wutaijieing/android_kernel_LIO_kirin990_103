...
static struct wakeup_source *ffu_lock = NULL;
...
static void ufshcd_ufs_set_dieid(struct ufs_hba *hba, struct ufs_dev_info *dev_info);
#endif
#endif
...
static void ufshcd_wb_toggle_fn(struct work_struct *work)
{
	...
	ret = ufshcd_query_attr_retry(hba, UPIU_QUERY_OPCODE_READ_ATTR,
				      ...
				      0, &current_buffer);
	if (ret) {
		dev_err(hba->dev, "%s dCurrentWriteBoosterBufferSize read failed %d\n",
			 __func__, ret);
		goto out;
	}
...
static void ufshcd_wb_flush_fn(struct work_struct *work)
{
	...
	ret = ufshcd_query_attr_retry(hba, UPIU_QUERY_OPCODE_READ_ATTR,
				      ...
				      0, &available);
	if (ret) {
		dev_err(hba->dev, "%s, read wb available buffer size failed!\n", __func__);
		return;
	}
...
static bool is_samsung_v5_workaround(struct ufs_hba *hba)
{
	...
	list_for_each(node, &hba->wb_flush_v5_whitelist) {
		samsung_v5 = list_entry(node, struct samsung_v5_pnm, p);
		...
		struct ufshcd_lrb *lrbp, int max_timeout)
{
	...
	if (!time_left) {
		...
		if (!ufshcd_clear_cmd(hba, lrbp->task_tag))
			...
			err = -EAGAIN;
		else if (hba->vops && hba->vops->dbg_hci_dump)
