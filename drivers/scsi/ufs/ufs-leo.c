...
static void hufs_set_memory_sd_signal_0(struct hufs_host *host)
{
	u32 reg;
...
static void hufs_fpga_device_reset_and_disreset(struct ufs_hba *hba)
{
	u32 reg = 0;
...
static void hufs_asic_device_reset_and_disreset(struct ufs_hba *hba)
{
...
static void hufs_memory_repair(struct hufs_host *host)
{
...
static void hufs_mphy_clk_set(struct hufs_host *host)
{
	u32 reg;
...
void ufs_soc_init(struct ufs_hba *hba)
{
	...
	hufs_set_memory_sd_signal_0(host);

	/* STEP 4 CLOSE MPHY REF CLOCK */
	hufs_mphy_clk_set(host);
	...
	if (host->caps & USE_HUFS_MPHY_TC)
		hufs_fpga_device_reset_and_disreset(hba);
	else
		hufs_asic_device_reset_and_disreset(hba);
...
static void hufs_wait_for_hibern8(struct ufs_hba *hba)
{
	int err = 0;
