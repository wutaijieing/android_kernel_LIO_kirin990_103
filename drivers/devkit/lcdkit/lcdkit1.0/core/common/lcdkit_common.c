...
int lcdkit_btb_check(void)
{
	...
	if (btb_info_pr->btb_check_support == 0) {
		LCDKIT_INFO("not support btb check\n");
		return CHECK_SKIPPED;
	}

	for (i = 0; i < (sizeof(btb_info_pr->btb_gpio) / sizeof(unsigned int)); i++) {
		btb_gpio = btb_info_pr->btb_gpio[i];
	...
	if (btb_status > 0) {
		...
		memcpy(buf, "LCD is connected error!\n", sizeof("LCD is connected error!\n"));
		if (btb_check_times < MAX_REPPORT_TIMES + 1)
			btb_check_times++;
		...
		if (btb_check_times <= MAX_REPPORT_TIMES) {
			ret = dsm_client_ocuppy(lcd_dclient);
			if (!ret) {
				dsm_client_record(lcd_dclient, buf);
				dsm_client_notify(lcd_dclient, DSM_LCD_BTB_CHECK_ERROR_NO);
