...
struct goodix_module_info_for_gt9966 {
	u8 mask_id[6]; /* 6 is mask id len */
	u8 mask_vid[3]; /* 3 is mask vid len */
	u8 mask_vid_reserved;
	u8 pid[8]; /* 8 is pid len */
	u8 vid[4]; /* 4 is vid len */
	u8 vid_reserved;
	u8 sensor_id;
	u8 reserved[2]; /* 4 is reserved len */
	u16 checksum;
};
#pragma pack()
...
static int touch_driver_spi_read_check(struct thp_device *tdev,
	u8 *data, unsigned int len)
{
...
#ifdef CONFIG_HUAWEI_THP_MTK
static int touch_driver_spi_read_retry_mtk(struct thp_device *tdev,
...
#ifdef CONFIG_HUAWEI_THP_MTK
	ret = touch_driver_spi_read_retry_mtk(tdev, spi, spi_msg);
	if (ret < 0) {
	...
	u8 *data, unsigned int len)
{
	...
	ret = touch_driver_spi_read_check(tdev, data, len);
	if (ret)
		return ret;
	...
	struct thp_device *tdev)
{
	...
	int retry;
	u8 temp_buf[GOODIX_MASK_ID_LEN + 1] = {0};
	u8 pid_buf[GOODIX_PID_LEN_GT9966 + 1] = {0};
	...
	for (retry = 0; retry < MODULE_INFO_READ_RETRY; retry++) {
		...
		thp_log_info("%s:ret:%d\n", __func__, ret);

		thp_log_info("module_info:%*ph\n", len,
			(u8 *)&module_info_for_gt9966);
	...
	memcpy(temp_buf, module_info_for_gt9966.mask_id,
		GOODIX_MASK_ID_LEN);
	temp_buf[GOODIX_MASK_ID_LEN] = '\0';
	thp_log_info("MASK_ID %s : ver %*ph\n", temp_buf,
		GOODIX_MASK_VID_LEN_GT9966, module_info_for_gt9966.mask_vid);
	memcpy(pid_buf, module_info_for_gt9966.pid,
		GOODIX_PID_LEN_GT9966);
	pid_buf[GOODIX_PID_LEN_GT9966] = '\0';
	thp_log_info("PID %s : ver %*ph\n", pid_buf,
		GOODIX_VID_LEN_GT9966, module_info_for_gt9966.vid);
...
static int touch_driver_power_on_for_9966(struct thp_device *tdev)
{
	...
	if (ret)
		thp_log_err("%s:vcc fail\n", __func__);
	ret = thp_power_supply_ctrl(THP_IOVDD,
		THP_POWER_ON, AFTER_IOVDD_DELAY_9966);
	if (ret)
		thp_log_err("%s:vddio fail\n", __func__);
	thp_log_info("%s pull up tp ic reset\n", __func__);
	gpio_set_value(tdev->gpios->rst_gpio, GPIO_HIGH);
	...
	char *buf, unsigned int len)
{
	...
	value = (tdev->thp_core->support_vendor_ic_type == GT9966) ||
	...
	bool delay_enable)
{
	...
	bool stylus3_uplink_switch = false;
	...
	thp_log_debug("%s: stylus3_status = %u, stylus_adsroption_status = %u, current_stylus3_work_mode = %u\n",
		__func__, stylus3_status, stylus_adsroption_status, current_stylus3_work_mode);
	stylus3_uplink_switch = ((stylus3_status & BT_STYLUS3_CONNECTED_MASK) > 0) &&
		(!stylus_adsroption_status) && (!current_stylus3_work_mode);
...
static int touch_driver_suspend(struct thp_device *tdev)
{
	...
	value = ((cd->support_vendor_ic_type == GT9966) ||
		((cd->support_vendor_ic_type == GT9897) &&
