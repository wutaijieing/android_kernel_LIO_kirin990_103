...
#if defined CONFIG_HUAWEI_THP_UDP
...
static long thp_ioctl_set_afe_status(const void __user *data)
{
	...
	case THP_AFE_STATUS_SCREEN_ON:
		...
		if (cd->lcd_need_get_afe_status &&
			...
			ktime_get_real_ts64(&cd->afe_status_record_tv);
...
#endif
	if (!cd->thp_wake_lock) {
		thp_log_err("%s wakeup source register failed\n", __func__);
		goto err_out;
	}
	...
	struct thp_core_data *cd)
{
	...
	thp_dts_parse_process_template(rc, value, &(cd->need_huge_memory_in_spi),
		__func__, "need_huge_memory_in_spi");
	...
	thp_dts_parse_process_template(rc, value, &(cd->support_ring_feature),
		__func__, "support_ring_feature");
