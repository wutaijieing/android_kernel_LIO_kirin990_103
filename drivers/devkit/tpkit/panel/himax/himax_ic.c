...
static int himax_irq_bottom_half(struct ts_cmd_node *in_cmd,struct ts_cmd_node *out_cmd)
{
	...
	if(atomic_read(&g_himax_ts_data->suspend_mode) && g_himax_ts_data->tskit_himax_data->ts_platform_data->chip_data->easy_wakeup_info.sleep_mode)
	{
		...
		__pm_wakeup_event(g_himax_ts_data->tskit_himax_data->ts_platform_data->ts_wake_lock,
...
static int himax_chip_detect(struct ts_kit_platform_data *platform_data)
{
	...
	ts->ts_flash_wake_lock = wakeup_source_register(
		&ts->ts_dev->dev, HIMAX_VENDER_NAME);
#else
	ts->ts_flash_wake_lock = wakeup_source_register(HIMAX_VENDER_NAME);
#endif
	if (!ts->ts_flash_wake_lock) {
...
#endif
	wakeup_source_unregister(ts->ts_flash_wake_lock);
	ts->ts_flash_wake_lock = NULL;
err_detect_failed:
...
static void __exit himax_module_exit(void)
{
	...
	wakeup_source_unregister(g_himax_ts_data->ts_flash_wake_lock);
	g_himax_ts_data->ts_flash_wake_lock = NULL;
	if (g_himax_ts_data){
		kfree(g_himax_ts_data);
		g_himax_ts_data = NULL;
	}
