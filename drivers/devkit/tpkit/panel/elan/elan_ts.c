...
static int elan_ktf_init_chip(void)
{
	...
	elan_ts->wake_lock = wakeup_source_register(
			&elan_ts->elan_dev->dev, "elantp_wake_lock");
#else
	elan_ts->wake_lock = wakeup_source_register("elantp_wake_lock");
#endif
	if (!elan_ts->wake_lock) {
