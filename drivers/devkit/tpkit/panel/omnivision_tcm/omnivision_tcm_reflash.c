		...
		char *buf, loff_t pos, size_t count)
{
	...
	retval = secure_cpydata(&reflash_hcd->image_buf[pos],
			IMAGE_BUF_SIZE - pos,
			buf,
			count,
			count);
		...
		struct ovt_tcm_buffer *output)
{
	...
	if (output != NULL) {
		...
		retval = secure_cpydata(output->buf,
				output->buf_size,
		...
		unsigned int datalen)
{
	...
	while (remaining_length) {
		...
		retval = secure_cpydata(&reflash_hcd->out.buf[2],
