...
static int invert_hs_create_workqueue(void)
{
	...
	invert_hs_pdata->wake_lock = wakeup_source_register(NULL, "invert_hs_wakelock");
#else
	invert_hs_pdata->wake_lock = wakeup_source_register("invert_hs_wakelock");
#endif
	if (!invert_hs_pdata->wake_lock) {
