...
static struct page *gpu_mgm_alloc_page(
...
static void gpu_mgm_update_sizes(struct memory_group_manager_device *mgm_dev,
		int group_id, unsigned int order, bool is_alloc)
{
	update_size(mgm_dev, group_id, order, is_alloc);
}

static int gpu_mgm_get_import_mem_id(
...
static u64 gpu_mgm_gpu_map_page(u8 mmu_level, u64 pte)
{
	CSTD_UNUSED(mmu_level);

#ifdef CONFIG_MALI_LAST_BUFFER
	/* L.B intergration: Get the GID info.
	 * 1. In order to get the phys's GID, we will call gpu-driver function
...
static u64 gpu_mgm_update_gpu_pte(struct memory_group_manager_device *mgm_dev,
...
static vm_fault_t gpu_mgm_vm_insert_pfn(struct memory_group_manager_device *mgm_dev,
...
static int gpu_mgm_remap_vmalloc_range(
		struct memory_group_manager_device *mgm_dev,
...
static void *gpu_mgm_vmap(struct page **pages, unsigned int count,
...
static int memory_group_manager_probe(struct platform_device *pdev)
{
	...
	mgm_dev->ops.mgm_alloc_page = gpu_mgm_alloc_page;
	mgm_dev->ops.mgm_free_page = gpu_mgm_free_page;
	mgm_dev->ops.mgm_update_sizes = gpu_mgm_update_sizes;
	mgm_dev->ops.mgm_get_import_memory_id = gpu_mgm_get_import_mem_id;
	mgm_dev->ops.gpu_map_page = gpu_mgm_gpu_map_page;
	mgm_dev->ops.mgm_update_gpu_pte = gpu_mgm_update_gpu_pte;
	mgm_dev->ops.mgm_vmf_insert_pfn_prot = gpu_mgm_vm_insert_pfn;
	mgm_dev->ops.remap_vmalloc_range = gpu_mgm_remap_vmalloc_range;
	mgm_dev->ops.vmap = gpu_mgm_vmap;
