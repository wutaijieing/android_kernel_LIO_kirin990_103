...
#include "efuse/borr/mali_kbase_efuse.h"
#elif defined(CONFIG_MALI_NORR_PHX) || defined(CONFIG_MALI_NORR_CDC)
#include "efuse/norr/mali_kbase_efuse.h"
#endif
#endif
...
extern struct gpu_virtual_devfreq gpu_virtual_devfreq_data;
#endif
...
{
#if defined (CONFIG_MALI_NORR_PHX) || defined(CONFIG_MALI_NORR) || defined(CONFIG_MALI_TRYM) || defined (CONFIG_MALI_NATT) || defined (CONFIG_MALI_NORR_CDC)
...
#endif
#if defined (CONFIG_MALI_NORR_PHX) || defined(CONFIG_MALI_NORR) || defined (CONFIG_MALI_NORR_CDC)
	if (kbase_has_hi_feature(kbdev, KBASE_FEATURE_STRIPING_GRANUL_SETTING)) {
...
static int kbase_gpu_init_device_tree(struct kbase_device * const kbdev)
{
	...
	kbdev->gpu_dev_data.lb_enabled = 0;
	kbdev->gpu_dev_data.sys_cache_capacity = 0;
	...
	if (np) {
		...
		ret = of_property_read_u32(np, "last-buffer",
			&kbdev->gpu_dev_data.lb_enabled);
		if(ret)
			dev_warn(kbdev->dev,
			"Failed to get last buffer setting.\n");

#ifdef CONFIG_MALI_LAST_BUFFER
		if (kbdev->gpu_dev_data.lb_enabled == 1) {
			kbdev->gpu_dev_data.lb_enabled = is_lb_available();
		}
#endif

		ret = of_property_read_u32(np, "sys-cache-capacity",
			&kbdev->gpu_dev_data.sys_cache_capacity);
		if(ret)
			dev_warn(kbdev->dev,
...
#ifdef CONFIG_GPU_SHADER_PRESENT_CFG
#if defined CONFIG_MALI_BORR
		enum SOC_SPEC_TYPE soc_type = get_current_chip_type(kbdev);
		if (soc_type == PC_CHIP || soc_type == PC_NORMAL_CHIP) {
			ret = get_gpu_efuse_cfg(kbdev, &kbdev->gpu_dev_data.shader_present_lo_cfg);
			if (ret) {
		...
		int lite_chip = 0;
		if (kbdev->gpu_dev_data.gpu_chip_type == 2) {
			ret = get_gpu_efuse_cfg(kbdev, &kbdev->gpu_dev_data.shader_present_lo_cfg, &lite_chip);
			if (ret) {
				kbdev->gpu_dev_data.lb_enable = 0;
				dev_err(kbdev->dev, "Failed to get_gpu_efuse_cfg!\n");
				return -ENOENT;
			} else {
				kbdev->gpu_dev_data.lb_enabled = !lite_chip;
			}
		}
#else
		ret = of_property_read_u32(np, "shader-present-lo-cfg",
