	...
	int volt_ratio)
{
	...
	for (i = 0; i < CHARGE_PATH_MAX_NUM; i++) {
		...
		if (info->thre_para[i].vbat_hth && (vbat[i] > info->thre_para[i].vbat_hth)) {
			hwlog_info("vbat is high, vbat[%d]=%d\n, vbat_hth=%d", i, vbat[i], info->thre_para[i].vbat_hth);
			goto end;
		}
		...
		if (info->thre_para[i].ibus_lth && (ibus[i] < info->thre_para[i].ibus_lth)) {
			hwlog_info("ibus is low, ibus[%d]=%d\n, ibus_lth=%d", i, ibus[i], info->thre_para[i].ibus_lth);
			goto end;
		}
