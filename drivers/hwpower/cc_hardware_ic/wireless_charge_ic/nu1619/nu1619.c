...
static struct hwqi_handle *g_nu1619_handle;
...
static int nu1619_read_byte(u16 reg, u8 *data)
{
	...
	return nu1619_read_block(di, reg, data, POWER_BYTE_LEN);
}
...
static int nu1619_read_word(u16 reg, u16 *data)
{
	...
	ret = nu1619_read_block(di, reg, buff, POWER_WORD_LEN);
	if (ret)
		return -EIO;
...
static int nu1619_write_byte(u16 reg, u8 data)
{
	...
	return nu1619_write_block(di, reg, &data, POWER_BYTE_LEN);
}
...
static int nu1619_write_word(u16 reg, u16 data)
{
	...
	return nu1619_write_block(di, reg, buff, POWER_WORD_LEN);
}
...
static int nu1619_get_fw_version(u8 *data, int len, void *dev_data)
{
	...
	data[0] = (u8)((chip_info.mtp_fw_ver >> 0) & POWER_MASK_BYTE);
	data[1] = (u8)((chip_info.mtp_fw_ver >> 8) & POWER_MASK_BYTE);
	data[2] = (u8)((chip_info.mtp_fw_ver >> 16) & POWER_MASK_BYTE);
	data[3] = (u8)((chip_info.mtp_fw_ver >> 24) & POWER_MASK_BYTE);
...
static void nu1619_get_rx_iavg(int *rx_iavg, void *dev_data)
{
	struct nu1619_dev_info *di = dev_data;
...
static void nu1619_get_rx_imax(int *rx_imax, void *dev_data)
{
	struct nu1619_dev_info *di = dev_data;
...
static int nu1619_data_received_handler(struct nu1619_dev_info *di)
{
	...
	case HWQI_CMD_ACK_BST_ERR:
		...
		if (g_nu1619_handle &&
			...
			g_nu1619_handle->hdl_non_qi_fsk_pkt(buff, HWQI_PKT_LEN, di);
		break;
	default:
		break;
	}
...
static void nu1619_irq_work(struct work_struct *work)
{
	...
	if (!di) {
		...
		power_wakeup_unlock(g_nu1619_wakelock, false);
		return;
	}
...
static irqreturn_t nu1619_interrupt(int irq, void *_di)
{
	...
	power_wakeup_lock(g_nu1619_wakelock, false);
...
static struct wlrx_ic_ops g_nu1619_rx_ic_ops = {
	...
	.get_iavg               = nu1619_get_rx_iavg,
	.get_imax               = nu1619_get_rx_imax,
...
static struct hwqi_ops nu1619_qi_protocol_ops = {
	.chip_name = "nu1619",
...
static void nu1619_shutdown(struct i2c_client *client)
{
	...
	if (wlrx_get_wireless_channel_state() == WIRELESS_CHANNEL_ON) {
		...
		(void)nu1619_set_vfc(ADAPTER_5V * POWER_MV_PER_V, di);
		(void)nu1619_set_rx_vout(ADAPTER_5V * POWER_MV_PER_V, di);
...
static int nu1619_ops_register(struct nu1619_dev_info *di)
{
	...
	ret = wireless_fw_ops_register(&g_nu1619_fw_ops, di->ic_type);
	if (ret) {
	...
	ret = hwqi_ops_register(&nu1619_qi_protocol_ops, di->ic_type);
	if (ret) {
		hwlog_err("ops_register: register qi_ops failed\n");
		return ret;
	}
	g_nu1619_handle = hwqi_get_handle();
	...
	const struct i2c_device_id *id)
{
	...
	g_nu1619_wakelock = power_wakeup_source_register(di->dev, "nu1619_wakelock");
...
dts_para_init_fail:
	power_wakeup_source_unregister(g_nu1619_wakelock);
dev_ck_fail:
