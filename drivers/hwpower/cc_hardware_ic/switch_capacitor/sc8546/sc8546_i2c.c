...
int sc8546_write_mask(struct sc8546_device_info *di, u8 reg, u8 mask, u8 shift, u8 value)
{
	...
	ret = sc8546_read_byte(di, reg, &val);
	if (ret < 0)
		return ret;
	...
	return sc8546_write_byte(di, reg, val);
}

int sc8546_write_block(struct sc8546_device_info *di, u8 reg, u8 *value,
...
int sc8546_read_block(struct sc8546_device_info *di, u8 reg, u8 *value,
	unsigned int num_bytes)
{
	if (!di || (di->chip_already_init == 0)) {
		hwlog_err("chip not init\n");
