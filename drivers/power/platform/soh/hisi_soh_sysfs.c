	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!buf) {
		soh_err("Input param buf is invalid\n");
		return -EINVAL;
	}
...
void soh_nv_printf(struct hisi_soh_device *_di, enum soh_type type)
{
	...
	switch (type) {
	case SOH_ACR:
		soh_info("[%s]ACR:order[%d]  ctrl [%d]\n",
			__func__, di->soh_acr_dev.acr_nv.order_num,
			di->soh_acr_dev.acr_nv.acr_control);
		for (i = 0; i < SOH_ACR_NV_DATA_NUM; i++)
			soh_info("[%s]Arrary[%d]: cycle:%d  acr:%d  chip_temp0:%d  chip_temp1:%d  vol:%d  bat_temp:%d\n",
	...
	case SOH_DCR:
		...
		break;
	case SOH_PD_LEAK:
		soh_info("[%s]PD_LEAK:order[%d]  ctrl [%d]\n",
			__func__, di->soh_pd_leak_dev.pd_leak_nv.order_num,
			di->soh_pd_leak_dev.pd_leak_nv.pd_control);
		for (i = 0; i < SOH_PD_NV_DATA_NUM; i++)
			soh_info("[%s]Array[%d]:cycle[%d]  leak_ma[%d]  chip_t0[%d]  chip_t1[%d]  cur0[%d]"
				  "  cur1[%d] vol0[%d] vol1[%d] rtc0[%d] rtc1[%d] sys_cc[%d]\n",
...
static int acr_nv_print (struct hisi_soh_device *di, char *buf, size_t size)
{
	...
	cnt = snprintf_s(buf, size, size - 1, "ACR:order[%d]  ctrl [%d]",
			...
			di->soh_acr_dev.acr_nv.acr_control);
	if (cnt < 0) {
		soh_err("[%s]snprintf_s fail, err=%d\n", __func__, cnt);
		return cnt;
	}
	for (i = 0; i < SOH_ACR_NV_DATA_NUM; i++) {
		if (size < (unsigned int)cnt + 1) {
			soh_err("[%s]buf out of bound cnt:%d\n", __func__, cnt);
			return cnt;
		}
...
static int pdleak_nv_print (struct hisi_soh_device *di, char *buf, size_t size)
{
	...
	for (i = 0; i < SOH_PD_NV_DATA_NUM; i++) {
		...
		ret = snprintf_s(buf + cnt, size - cnt, size - cnt - 1,
			"Array[%d]:cycle[%d]  leak_ma[%d]  chip_t0[%d]  chip_t1[%d]  cur0[%d]"
			"  cur1[%d] vol0[%d] vol1[%d] rtc0[%d] rtc1[%d] sys_cc[%d]\n",
...
ssize_t nv_read(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	case SOH_ACR:
		...
		if (ret < 0)
			soh_err("[%s]acr nv print fail, err=%d\n",
				__func__, ret);
		break;
	case SOH_DCR:
		...
		if (ret < 0)
			soh_err("[%s]dcr nv print fail, err=%d\n",
				__func__, ret);
		break;
	case SOH_PD_LEAK:
		...
		if (ret < 0)
			soh_err("[%s]pdleak nv print fail, err=%d\n",
				__func__, ret);
		break;
	default:
		break;
...
int soh_create_sysfs_file(struct hisi_soh_device *di)
{
	...
	if (!di) {
		soh_err("%s input di is null", __func__);
		return -1;
	}

	retval = sysfs_create_group(&di->dev->kobj, &soh_attr_group);
	if (retval) {
		soh_err("%s failed to create sysfs group!!!\n", __func__);
		return -1;
	}
	...
	if (power_class) {
		...
		if (soh_dev != NULL) {
			...
			if (retval != 0)
				soh_err("%s failed to create sysfs link!!!\n", __func__);
		} else {
			soh_err("%s failed to create new_dev!!!\n", __func__);
		}
	}
...
void soh_sysfs_remove_file(struct hisi_soh_device *di)
{
	...
	if (!soh_dev) {
		soh_err("%s soh_dev is null", __func__);
		return;
	}
