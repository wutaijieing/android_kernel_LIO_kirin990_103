...
void update_polar_ishort_info_time(unsigned int time_now)
{
	...
	if (di->ishort_info.time0 && di->ishort_info.last_time) {
		di->ishort_info.time0 -= (int)time_now;
		di->ishort_info.last_time -= (int)time_now;
	}
}
...
int show_polar_log_head(char *buf, size_t dest_max)
{
	unsigned int len = strlen(buf);

	return snprintf_s(buf + len, dest_max - len, dest_max - len - 1,
		"ocv_old    p_vol   p_cur    err_a    polar_vol   avg_cur    peak_cur    ishort   ");
}

int show_polar_log_content(char *buf, size_t dest_max)
{
	unsigned int len = strlen(buf);
	struct hisi_polar_device *di = get_polar_dev();
	...
	if (di == NULL)
		return snprintf_s(buf + len, dest_max - len, dest_max - len - 1,
			"%-7d    %-5d    %-5d    %-5d    %-9ld    %-7d    %-9d   %-8d  ",
			0, 0, 0, 0, 0L, 0, 0, 0);

	return snprintf_s(buf + len, dest_max - len, dest_max - len - 1,
		"%-7d    %-5d    %-5d    %-5d    %-9ld    %-7d    %-9d   %-8d  ",
		di->polar.ocv_old, di->polar.ori_vol, di->polar.ori_cur, di->polar.err_a,
		di->polar.vol, di->polar.curr_5s, di->polar.curr_peak, di->ishort_info.curr_ua);
}
	 ...
	 int time_now, int eco_ibat, unsigned int suspend_time)
{
	...
	if (eco_ibat > POLAR_ECO_IBAT_LIMIT)
		return FALSE;
	if (di->eco_info.last_sample_time - (int)suspend_time <
...
static void update_polar_ishort_dmd(
	struct hisi_polar_device *di, const char *buff, int buff_len)
{
	int ret = -1;
	...
	if (di->ishort_info.reported == 0) {
		...
		ret = power_dsm_report_dmd(
			POWER_DSM_BATTERY, POWER_DSM_ERROR_BATTERY_POLAR_ISHORT, buff);
	...
	int charging_stop_soc, int batt_soc_real, int qmax)
{
	...
	current_sec = (int)di->polar_dev_ops->get_coul_time();
	...
	if (di->ishort_info.time0 &&
		...
		buff_len = snprintf_s(buff, DSM_BUFF_SIZE_MAX,
			DSM_BUFF_SIZE_MAX - 1,
			"ishort:%d, last_ishort:%d, ocv0:%d, ocv1:%d, cc0:%ld, cc1:%ld, time0:%d,"
			" time1:%d, qmax:%d, last_cycle:%d, cycle:%d, brand:%s\n",
	...
	int sr_sleep_time, int sleep_cc)
{
	...
	sample_time_rtc = dfx_getcurtime();
...
void update_polar_ocv(struct info_to_update_polar_info *coul_info)
{
	...
	duration = di->eco_info.now_sample_time - (int)coul_info->suspend_time;
	if (duration) {
...
void update_polar_params(int ocv_soc_mv, int curr_now, int vol_now, bool update_flag)
{
	...
	if (last_calc_time == 0) {
		last_calc_time = dfx_getcurtime() / NSEC_PER_MSEC;
	} else {
		curr_time = dfx_getcurtime() / NSEC_PER_MSEC;
