...
#define hwlog_debug(fmt, args...) pr_debug("[hisi_charging_core]" fmt, ## args)
#define hwlog_info(fmt, args...) pr_info("[hisi_charging_core]" fmt, ## args)
#define hwlog_warn(fmt, args...) pr_warn("[hisi_charging_core]" fmt, ## args)
#define hwlog_err(fmt, args...) pr_err("[hisi_charging_core]" fmt, ## args)
				      ...
				      struct charge_core_data *data)
{
	...
	for (i = 0; i < VOLT_PARA_LEVEL; i++) {
		if ((vbat >= volt_data[i].vbat_min) &&
		    (vbat < volt_data[i].vbat_max)) {
			if ((last_vbat - vbat <= 0) ||
			    (volt_data[i].vbat_max - vbat >
			     volt_data[i].volt_back) ||
			    (abs(last_i - i) > 1) ||
			    (flag_running_first == 1)) {
				data->iin = data->iin < volt_data[i].iin_volt ?
						    data->iin :
						    volt_data[i].iin_volt;
				data->ichg =
					data->ichg < volt_data[i].ichg_volt ?
						data->ichg :
						volt_data[i].ichg_volt;
			} else {
				data->iin = data->iin < last_iin ? data->iin :
								   last_iin;
				data->ichg = data->ichg < last_ichg ?
						     data->ichg :
						     last_ichg;
			}
			break;
...
*  return value:  NULL
**********************************************************/
static void charge_core_vdpm_handler(int vbat,
				     ...
				     struct charge_core_data *data)
{
	...
	if (vbat > last_vbat) {
		...
		for (i = 0; i < VDPM_PARA_LEVEL; i++) {
			if (((vbat + vdpm_data[i].cap_back) >=
			     vdpm_data[i].cap_min) &&
			    ((vbat + vdpm_data[i].cap_back) <
			     vdpm_data[i].cap_max)) {
				data->vdpm = vdpm_data[i].vin_dpm;
				break;
			}
				      ...
				      struct charge_core_data *data)
{
	...
	for (i = 0; i < SEGMENT_PARA_LEVEL; i++) {
		if ((vbat >= segment_data[i].vbat_min) &&
			...
			if ((last_i - i <= 0) ||
			    (segment_data[i].vbat_max - vbat >
			     segment_data[i].volt_back) ||
			    (abs(last_i - i) > 1) ||
			    (flag_running_first == 1)) {
...
*  return value:  NULL
**********************************************************/
static void charge_core_protect_inductance_handler(
	int cbat, struct charge_inductance_data inductance_data[],
	struct charge_core_data *data)
{
	int i;
	static int last_i = 0;
	...
	for (i = 0; i < INDUCTANCE_PARA_LEVEL; i++) {
		if ((cbat >= inductance_data[i].cap_min) &&
		    (cbat < inductance_data[i].cap_max)) {
			if ((last_i - i <= 0) ||
			    (inductance_data[i].cap_max - cbat >
			     inductance_data[i].cap_back) ||
			    (abs(last_i - i) > 1) ||
			    (flag_running_first == 1)) {
				data->iin =
					data->iin < inductance_data[i]
								.iin_inductance ?
						data->iin :
						inductance_data[i]
							.iin_inductance;
	...
	hwlog_info(
		"%s : battery capacity is %%%d, last_iin set to %d ma,i = %d \n",
...
struct charge_core_data *charge_core_get_params(void)
{
	...
	int tbatt = coul_drv_battery_temperature_for_charger();
	...
	charge_core_protect_inductance_handler(cbatt, di->inductance_para,
					       &di->data);

	hwlog_info("[%s] tbatt = %d, ichg = %d, iin = %d, vterm = %d\n",
		   __func__, tbatt, di->data.ichg, di->data.iin,
		   di->data.vterm);
	return &di->data;
}

static int charge_core_init_temp_para(struct charge_core_info *di)
{
	int i;
...
static int charge_core_init_volt_para(struct charge_core_info *di)
{
	int i;
	struct chrg_para_lut *p_batt_data = NULL;
...
static int charge_core_init_segment_para(struct charge_core_info *di)
{
	int i;
	struct chrg_para_lut *p_batt_data = NULL;
	...
	for (i = 0; i < (p_batt_data->segment_len) / SEGMENT_PARA_TOTAL; i++) {
		...
		if ((di->segment_para[i].vbat_min < BATTERY_VOLTAGE_MIN_MV) ||
		    ...
		    (di->segment_para[i].ichg_segment <
		     CHARGE_CURRENT_0000_MA) ||
		    (di->segment_para[i].ichg_segment >
		     CHARGE_CURRENT_MAX_MA) ||
		    (di->segment_para[i].vterm_segment <
		     BATTERY_VOLTAGE_3200_MV) ||
		    (di->segment_para[i].vterm_segment >
		     BATTERY_VOLTAGE_4500_MV) ||
...
static int charge_core_battery_data(struct charge_core_info *di)
{
	...
	ret = charge_core_init_temp_para(di);
	if (ret) {
			hwlog_err("the temp_para init fail\n");
			return -EINVAL;
	}

	ret = charge_core_init_volt_para(di);
	if (ret) {
			hwlog_err("the volt_para init fail\n");
			return -EINVAL;
	}

	ret = charge_core_init_segment_para(di);
	if (ret) {
			hwlog_err("the segment_para init fail\n");
			return -EINVAL;
	}
...
static int charge_core_inductance_para_pares(struct device_node *np,
				 struct charge_core_info *di)
{
	...
	/* inductance_para */
	memset(di->inductance_para, 0,
		INDUCTANCE_PARA_LEVEL * sizeof(struct charge_inductance_data));
	...
	for (i = 0; i < array_len; i++) {
		ret = of_property_read_string_index(np, "inductance_para", i,
						    &chrg_data_string);
		if (ret) {
		...
		case INDUCTANCE_PARA_CAP_MIN:
			...
			di->inductance_para[i / (INDUCTANCE_PARA_TOTAL)]
				.cap_min = idata;
			break;
...
static int charge_core_current_parse(struct device_node *np,
				 struct charge_core_info *di)
{
	int ret = 0;
	...
	if (ret) {
		hwlog_info(
			"get ichg_fcp failed ,use ichg_ac's value instead \n");
				 ...
				 struct charge_core_info *di)
{
	...
	if (charge_core_current_parse(np, di)) {
			hwlog_err("charge_core_current_parse fail\n");
			return -EINVAL;
	}
	...
	memset(di->vdpm_para, 0,
	       VDPM_PARA_LEVEL *
		       sizeof(struct charge_vdpm_data)); // data reset to 0
	...
	for (i = 0; i < array_len; i++) {
		...
		hwlog_debug("di->vdpm_para[%d][%d] = %d\n",
			    i / (VDPM_PARA_TOTAL), i % (VDPM_PARA_TOTAL),
			    idata);
	}

	if (charge_core_inductance_para_pares(np, di)) {
		hwlog_err("inductance_para pares fail\n");
		return -EINVAL;
	}
	...
	di->data.iin_max = di->data.iin_ac < di->data.iin_fcp ?
				   di->data.iin_fcp :
				   di->data.iin_ac;
