...
static void isc_splash2_file_sync(struct iscd_info *iscd)
{
	...
	if (iscd->isc_splash2_ready) {
		...
		iscd->fatal_isc_hist.magic_num = (unsigned int)iscd->iscd_file_magic_num;
		write_size = coul_kernel_write(fd,
			(const void *)&iscd->fatal_isc_hist,
			sizeof(iscd->fatal_isc_hist), 0);
		if (write_size != (int)sizeof(iscd->fatal_isc_hist))
...
static void isc_config_splash2_file_sync(struct iscd_info *iscd)
{
	...
	iscd->fatal_isc_config.write_flag = (unsigned int)iscd->write_flag;
	iscd->fatal_isc_config.delay_cycles = (unsigned int)iscd->isc_valid_delay_cycles;
	iscd->fatal_isc_config.has_reported = (unsigned int)iscd->has_reported;
	iscd->fatal_isc_config.magic_num = (unsigned int)iscd->iscd_file_magic_num;
	write_size = coul_kernel_write(fd, (const void *)&iscd->fatal_isc_config,
		sizeof(iscd->fatal_isc_config), 0);
	if (write_size != (int)sizeof(iscd->fatal_isc_config))
		iscd_core_err("Write %s failed(wtire:%zd expect:%zu) in %s\n",
	...
	struct iscd_sample_info *sample_info)
{
	...
	timespec_64 sample_time = {0};
	time_t_64 delta_time;

	if (iscd_is_sample_allowed(iscd, &tbatt) == ERROR)
		return ERROR;

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0))
	ktime_get_coarse_real_ts64(&sample_time);
#else
	...
	if (iscd->size > 0) {
		...
		if ((delta_time < (time_t_64)ISCD_CALC_INTERVAL_900S) &&
	...
	iscd_core_info("ISCD sampled info: sample_cnt = %d, time_s = %ld, tbatt = %d, "
		"ocv_uV = %d, cc_uAh = %lld, ocv_soc_uAh = %lld\n",
...
{
	int size;
	time_t_64 delta_time1, delta_time2;
	...
	int index0, int index1)
{
	...
	time_t_64 delta_time, delta_time_abs;
	int isc = 0;
	...
	if ((delta_tbatt_abs <= iscd->tbatt_diff_max) &&
		((delta_time_abs >= (time_t_64)iscd->calc_time_interval_min) ||
		((delta_ocv_abs >= ISCD_OCV_DELTA_MAX) &&
		((int)delta_cc_abs <= ISCD_CC_DELTA_MAX) &&
		(delta_time_abs >= (time_t_64)ISCD_CALC_INTERVAL_900S)))) {
	...
	int time_limit, int last, int i)
{
	...
	if (time_limit) {
		...
		if ((delt_year < 0) || (delt_year > 1))
			...
			delt_day = (int)((delt_year ? ISC_DAYS_PER_YEAR : 0) +
...
static void __successive_isc_judgement(struct iscd_info *iscd, int time_limit)
{
	...
	for (j = (int)iscd->fatal_isc_trigger.valid_num; j > 0; j--) {
		for (i = last; i >= 0; i--) {
		...
		if ((last - i) >= (int)(iscd->fatal_isc_trigger.trigger_num[j - 1])) {
			iscd->isc_status = (unsigned int)j;
			iscd->fatal_isc_hist.isc_status = (unsigned int)j;
...
static void refresh_isc_hist(struct iscd_info *iscd, const struct rtc_time *tm)
{
	...
	if ((valid_num < MAX_FATAL_ISC_NUM) && (valid_num >= 0)) {
		...
		iscd->fatal_isc_hist.charge_cycles[valid_num] =
			(unsigned int)get_coul_dev_batt_chargecycles();
...
static void iscd_timer_start(struct iscd_info *iscd, time_t_64 delta_secs)
{
		...
		if (iscd_sample_battery_info(iscd, sample_info) == SUCCESS) {
			...
			if (iscd->enable == ENABLED)
				iscd_timer_start(iscd,
					(time_t_64)iscd->sample_time_interval);
		} else {
			if (iscd->enable == ENABLED)
				iscd_timer_start(iscd,
					(time_t_64)iscd->sample_time_interval /
...
static int iscd_check_splash2_mounted(void)
{
	...
	while ((read_size = coul_kernel_read(fd, fd->f_pos, buff + whence,
		...
		if (whence > (int)strlen(SPLASH2_MOUNT_INFO)) {
			...
			ret_s = memset_s(buff + strlen(SPLASH2_MOUNT_INFO),
				whence - strlen(SPLASH2_MOUNT_INFO), 0,
				whence - strlen(SPLASH2_MOUNT_INFO));
			if (ret_s != EOK)
				iscd_core_err("%s memset_s failed\n", __func__);
			whence = (int)strlen(SPLASH2_MOUNT_INFO);
		}
	}
	if (find_str == NULL) {
...
static int iscd_create_dir(void)
{
	...
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	file_des = _sys_access(ISC_DATA_DIRECTORY, 0);
	set_fs(old_fs);
	...
	if (file_des < 0) {
		...
		set_fs(KERNEL_DS);
		file_des = _sys_mkdir(ISC_DATA_DIRECTORY, 0770); /* 0770: permission code */
		set_fs(old_fs);
		if (file_des < 0) {
...
static int init_from_isc_data(struct iscd_info *iscd)
{
	...
	filp_close(fd, NULL);
	if (read_size == (int)sizeof(iscd->fatal_isc_hist)) {
		iscd_core_info("fatal isc datum file size was correct\n");
		memcpy(&iscd->fatal_isc_hist, buff, read_size);
		if (iscd->fatal_isc_hist.magic_num == (unsigned int)iscd->iscd_file_magic_num) {
			...
			iscd->fatal_isc_hist.magic_num =
				(unsigned int)iscd->iscd_file_magic_num;
		}
	} else {
...
static void init_from_isc_config_data(struct iscd_info *iscd)
{
	...
	if (read_size == (int)sizeof(iscd->fatal_isc_config)) {
		...
		ret = memcpy_s(&iscd->fatal_isc_config,
			sizeof(iscd->fatal_isc_config), buff,
			sizeof(iscd->fatal_isc_config));
		if (ret != EOK)
			iscd_core_info("call memcpy_s failed!");
		...
		if (iscd->fatal_isc_config.magic_num == (unsigned int)iscd->iscd_file_magic_num) {
			iscd->has_reported = (int)iscd->fatal_isc_config.has_reported;
			if (iscd->fatal_isc_config.write_flag == TRUE) {
				iscd->isc_valid_delay_cycles =
					(int)iscd->fatal_isc_config.delay_cycles;
				iscd->write_flag =
					(int)iscd->fatal_isc_config.write_flag;
			}
		}
	} else {
...
static void get_isc_trigger_dts(struct device_node *np, struct iscd_info *iscd)
{
	...
	case SUCCESSIVE_ISC_JUDGEMENT_TIME:
		...
		if (ret <= 0 || (ret % ELEMS_PER_CONDITION) != 0 ||
			...
			iscd->fatal_isc_trigger.valid_num = (unsigned int)ret / ELEMS_PER_CONDITION;
...
static int iscd_probe(struct platform_device *pdev)
{
	...
	g_iscd_wakelock = wakeup_source_register(&pdev->dev, "iscd_wakelock");
#else
	g_iscd_wakelock = wakeup_source_register("iscd_wakelock");
#endif
	if (!g_iscd_wakelock) {
		iscd_core_err("%s wakeup source register failed\n", __func__);
		return -EINVAL;
}
...
static int iscd_remove(struct platform_device *pdev)
{
	...
	wakeup_source_unregister(g_iscd_wakelock);
