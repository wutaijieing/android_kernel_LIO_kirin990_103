...
void coul_get_initial_ocv(struct smartstar_coul_device *di)
{
	...
	ocvreg = (unsigned short)di->coul_dev_ops->get_ocv();
...
void get_ocv_by_fcc(struct smartstar_coul_device *di)
{
	...
	new_ocv = (unsigned int)interpolate_ocv(di->batt_data->pc_temp_ocv_lut,
	...
	int time_now, int batt_temp)
{
	...
	if (g_test_delta_soc_renew_ocv == 0)
		g_delta_soc_renew_ocv = (u8)(abs(pc_now - di->batt_soc_real) / TENTH);
	else
	...
	for (i = 0; i < OCV_LEVEL_MAX; i++) {
		if ((g_ocv_level_para[i].is_enabled == TRUE) &&
			...
			(data->big_current_10min <=
				g_ocv_level_para[i].max_avg_curr_limit)) {
			coul_core_info("[SR]%s(LEVEL:%d): sleep_time=%ds, wake_time=%ds,"
				" sleep_current=%dma, duty_ratio=%d, temp=%d, big_current=%d\n",
...
static int ocv_could_update(struct smartstar_coul_device *di)
{
	...
	time_now = (int)di->coul_dev_ops->get_coul_time();
	sleep_time = time_now - (int)di->suspend_time; /* sleep time */
