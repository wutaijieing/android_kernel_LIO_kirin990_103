...
#endif
#ifdef CONFIG_COUL_POLAR
	ret = show_polar_log_content(buf, PAGE_SIZE);
	if (ret < 0) {
		coul_core_err("(%s)snprintf_s fail ret = %d\n", __func__, ret);
		return 0;
	}
...
static ssize_t get_gaugelog_head(char *buf)
{
	...
	ret = snprintf_s(buf, PAGE_SIZE, PAGE_SIZE - 1, "%s",
		"ss_VOL  ss_CUR  ss_ufSOC  ss_SOC  SOC  "
		"ss_RM  ss_FCC  Qmax     ss_UUC  ss_CC  ss_dRC"
		"  pdFlag Temp  ss_OCV   rbatt  fcc    ocv_level  fcc_flag   ");
	if (ret < 0) {
		coul_core_err("(%s)snprintf_s fail ret = %d\n", __func__, ret);
		return 0;
	}
...
#endif
#ifdef CONFIG_COUL_POLAR
	ret = show_polar_log_head(buf, PAGE_SIZE);
	if (ret < 0) {
		coul_core_err("(%s)snprintf_s fail ret = %d\n", __func__, ret);
		return 0;
	}
