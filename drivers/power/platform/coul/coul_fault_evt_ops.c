...
void coul_lock_init(struct device *dev)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
	g_coul_lock = wakeup_source_register(dev, "coul_wakelock");
#else
	g_coul_lock = wakeup_source_register("coul_wakelock");
#endif
	if (!g_coul_lock) {
...
{
	wakeup_source_unregister(g_coul_lock);
}
	...
	int ibat_ua, int vbat_uv, int cut_off_vol)
{
	...
	unsigned int count = di->low_vol_filter_cnt;
