...
void psam64_set_ad_base(void)
{
    ...
    phy_addr_write(g_psam_device->dma[0], (unsigned char *)g_psam_device->regs + HI_PSAM64_ADQ0_BASE_H_OFFSET,
                   (unsigned char *)g_psam_device->regs + HI_PSAM64_ADQ0_BASE_L_OFFSET);
    phy_addr_write(g_psam_device->dma[1], (unsigned char *)g_psam_device->regs + HI_PSAM64_ADQ1_BASE_H_OFFSET,
                   (unsigned char *)g_psam_device->regs + HI_PSAM64_ADQ1_BASE_L_OFFSET);
...
int psam64_config_ad(ipf_ad_type_e type, unsigned int n, ipf_ad_desc_s *param)
{
    ...
    offset = (IPF_AD_0 == type) ? HI_PSAM_ADQ0_WPTR_OFFSET : HI_PSAM_ADQ1_WPTR_OFFSET;

    size = (IPF_AD_0 == type) ? IPF_DLAD0_DESC_SIZE : IPF_DLAD1_DESC_SIZE;

    table = (IPF_AD_0 == type) ? psam_ad_table0 : psam_ad_table1;
    ...
    for (i = 0; i < n; i++) {
        ...
        ad[wptr].output_ptr0.bits.ptr = (unsigned long long)(uintptr_t)param[i].out_ptr0;
        ad[wptr].output_ptr1.bits.ptr = (unsigned long long)(uintptr_t)param[i].out_ptr1;
