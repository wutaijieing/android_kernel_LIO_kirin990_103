...
#include <linux/platform_drivers/l3cache_partition_ctrl.h>
#endif
...
static inline bool prio_less(struct task_struct *a, struct task_struct *b, bool in_fi)
{
	...
	trace_task_prio_compare(a, pa, a->dl.deadline, a->se.vruntime,
...
static struct task_struct *sched_core_find(struct rq *rq, unsigned long cookie)
{
	...
	trace_sched_core_find(cpu_of(rq), cookie, node_task);
...
static void __sched_core_enable(void)
{
	...
	printk_deferred("sched core enabled\n");
#endif
}
...
#ifdef CONFIG_SCHED_CORE_DEBUG
	printk_deferred("sched core disabled\n");
#endif
}
...
#ifdef CONFIG_SCHED_HISI_UTIL_CLAMP
	if (rq->uclamp[UCLAMP_MIN].value > capacity_curr_of(cpu_of(rq)))
				  ...
				  const struct sched_attr *attr)
{
	...
	if (attr->sched_flags & SCHED_FLAG_UTIL_CLAMP_MIN &&
		...
		if (task_on_rq_queued(p) &&
		    attr->sched_util_min > capacity_curr_of(task_cpu(p))) {
			uclamp_rq_reinc_id(task_rq(p), p, UCLAMP_MIN);
			sugov_mark_util_change(task_cpu(p), SET_MIN_UTIL);
			sugov_check_freq_update(task_cpu(p));
		}
		trace_set_task_uclamp(p, UCLAMP_MIN, attr->sched_util_min);
#endif
	}
...
#ifdef CONFIG_SCHED_HISI_UTIL_CLAMP
		trace_set_task_uclamp(p, UCLAMP_MAX, attr->sched_util_max);
#endif
	}
}
...
#ifdef CONFIG_HISI_EAS_SCHED
	if (wake_flags & WF_SYNC)
		trace_sched_waking_sync(wake_flags);
#endif
	       ...
	       struct task_struct *next, struct rq_flags *rf)
{
	...
	phase_account_task(prev, cpu_of(rq), PHASE_PREV_MODE | PHASE_TASK_SWITCH);
	phase_account_task(next, cpu_of(rq), PHASE_RESET_MT_INFO | PHASE_TASK_SWITCH);
...
void scheduler_tick(void)
{
	...
	phase_account_task(curr, cpu, PHASE_TASK_TICK);
...
pick_task(struct rq *rq, const struct sched_class *class, struct task_struct *max, bool in_fi)
{
	 ...
	 */
#ifdef CONFIG_PHASE_FORWARD
	if (cookie_equals(class_pick, cookie) && phase_tk_predict(class_pick, max, FORWARD_MODE))
#else
...
#ifdef CONFIG_PHASE_REVERSE
static bool is_need_force_idle(struct rq *rq, struct task_struct *curr, int cpu)
{
	struct rq *max_rq = NULL;
	struct task_struct *max_task = NULL;

	if (!cpu_online(cpu) || !rq->core || !curr->core_cookie)
		return false;

	max_task = rq->core->core_pick_max;
	max_rq = rq->core->core_max_rq;

	if (!max_rq || !max_task || max_rq == rq)
		return false;
	...
	if (max_rq->curr != max_task)
		return false;

	phase_account_task(curr, cpu, PHASE_TASK_FORCEIDLE);
...
pick_next_task(struct rq *rq, struct task_struct *prev, struct rq_flags *rf)
{
	...
	if (rq->core_flags & SCF_CHECK_REVERSE) {
		...
		/* need force idle */
		if (is_need_force_idle(rq, prev, cpu)) {
			...
			trace_reverse_force_idle(cpu, prev, next);
			return next;
		}

		/* continue run, not disturb */
		if (task_on_rq_queued(prev)) {
			rq->core_flags |= RQCF_NO_UPDATE_MT_INFO;
			return prev;
		}
	}
#endif
	...
	if (rq->core->core_pick_seq == rq->core->core_task_seq &&
		...
		rq->core_pick = NULL;
		trace_core_pick_pre_select(cpu, rq->core->core_task_seq, rq->core->core_pick_seq,
	...
	rq->core->core_prev_mode = NO_SMT_MODE;
...
again:
		for_each_cpu_wrap(i, smt_mask, cpu) {
			...
			trace_core_pick_cpu_select(cpu, p);
			...
			if (!max || !cookie_match(max, p)) {
				...
				max = p;
				rq->core->core_pick_max = p;
				rq->core->core_max_rq = rq_i;
				trace_core_pick_max_select(cpu, old_max, max);
...
static bool try_steal_cookie(int this, int that)
{
	...
	do {
		...
		trace_steal_cookie(p, this, that, dst->idle->core_occupation, cookie);
...
static void __sched notrace __schedule(bool preempt)
{
	...
	if (likely(prev != next)) {
		...
		if (rq->core_flags & SCF_SKIP_UPDATE_INFO)
			...
			phase_account_task(prev, cpu_of(rq), PHASE_RESET_MT_INFO | PHASE_PREV_MODE | PHASE_TASK_SWITCH);
#endif
...
#ifdef CONFIG_HISI_EAS_SCHED
	trace_sched_setscheduler(p, oldpolicy, oldprio, attr->sched_flags, user, pi);
#endif
		       ...
		       struct cftype *cft, s64 boost)
{
	...
	tg->boost = boost;
	trace_sched_uclamp_boost(css->cgroup->kn->name, boost);
...
static int __sched_write_tag(struct write_core_tag *tag)
{
	struct cgroup_subsys_state *css = tag->css;
	unsigned int val = tag->val;
