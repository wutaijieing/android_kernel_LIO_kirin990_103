...
static inline unsigned int cpu_capacity_margin(int cpu)
{
...
static inline bool task_fits(struct task_struct *p, int cpu)
{
	...
	if (rtg_target &&
	    capacity_orig_of(cpu) < capacity_orig_of(cpumask_first(rtg_target)))
		return false;
#endif
...
static int select_energy_cpu_idx(struct task_struct *p, int prev_cpu, struct energy_env *eenv)
{
	...
	if (cpumask_test_cpu(prev_cpu, &eenv->candidates)) {
		...
		trace_sched_compute_energy(p, base_energy, prev_cpu, prev_delta,
...
#ifdef CONFIG_SCHED_TRACEPOINT
		trace_sched_compute_energy(p, base_energy, cpu, cur_delta,
					   best_energy_cpu, best_energy_delta);
#endif
	}
...
static int find_energy_efficient_cpu(struct task_struct *p, int prev_cpu, int sync)
{
	...
	do {
		...
		cpumask_and(&search_cpus, sched_group_span(sg), p->cpus_ptr);
		...
		for_each_cpu(cpu, &search_cpus) {
#ifdef CONFIG_SCHED_TRACEPOINT
			trace_sched_cpu_util(cpu, cpu_rq(cpu)->nr_running,
					     cpu_util_next(cpu, p, cpu),
					     capacity_of(cpu), capacity_curr_of(cpu),
					     capacity_orig_of(cpu),
					     idle_cpu(cpu), walt_cpu_high_irqload(cpu),
					     cpu_isolated(cpu), is_reserved(cpu));
#endif
			if (skip_cpu(cpu))
				continue;
			...
			if (favor_smaller_capacity) {
				if (cpu_cap < min_cap_orig)
					min_cap_orig = cpu_cap;
				else if (cpu_cap > min_cap_orig)
					continue;
			}
		...
		if (!latency_sensitive) {
			if (max_spare_cap_cpu >= 0)
				cpumask_set_cpu(max_spare_cap_cpu, &eenv.candidates);
			if (shallowest_idle_cpu >= 0)
				cpumask_set_cpu(shallowest_idle_cpu, &eenv.candidates);
		}
...
#ifdef CONFIG_SCHED_TRACEPOINT
	trace_sched_find_energy_efficient_cpu(p, task_util(p), boosted, latency_sensitive,
...
#ifdef CONFIG_SCHED_TRACEPOINT
		trace_sched_load_balance_detach_tasks(p->pid, env->src_cpu,
				      ...
				      int *sg_status)
{
	...
	for_each_cpu_and(i, sched_group_span(group), env->cpus) {
		...
		if (is_vip_prio(rq->rt.highest_prio.curr) &&
		    test_tsk_thread_flag(rq->curr, TIF_EXPECTED_HEAVY) &&
				   ...
				   struct sg_lb_stats *sgs)
{
	...
	if ((env->sd->flags & SD_ASYM_CPUCAPACITY) &&
#ifdef CONFIG_HISI_EAS_SCHED
	    (sgs->sum_h_nr_running <= sgs->group_weight) &&
#else
	    (sgs->group_type <= group_fully_busy) &&
#endif
...
#ifdef CONFIG_SCHED_TRACEPOINT
	trace_sched_load_balance_fbg(sds.busiest->cpumask[0],
				     ...
				     struct sched_group *group)
{
	 ...
	 */
	if (busiest && env->migration_type == migrate_util && busiest->nr_running > 1) {
		if (busiest_util >= capacity_of(cpu_of(busiest)))
			env->imbalance = busiest_util;
		else
			env->imbalance = busiest_util / busiest->nr_running;
	}
#endif
#ifdef CONFIG_SCHED_TRACEPOINT
	trace_find_busiest_queue(busiest ? cpu_of(busiest) : -1,
...
out_unlock:
	...
	trace_sched_find_new_ilb(ilb, allow_bigger_ilb);
	return ilb;
}
#else
static inline int find_new_ilb(void)
...
#ifdef CONFIG_DFX_BB
	softirq_hook(HK_SCHED_SOFTIRQ, (u64)(uintptr_t)run_rebalance_domains, 0);
#endif
...
#ifdef CONFIG_DFX_BB
	softirq_hook(HK_SCHED_SOFTIRQ, (u64)(uintptr_t)run_rebalance_domains, 1);
#endif
}
...
#ifdef CONFIG_HISI_EAS_SCHED
static int find_max_idle_cpu(struct task_struct *p)
{
	int i, max_cap_cpu = -1;
				      ...
				      struct cpumask *preferred_cpus)
{
	...
	if (idle_backup_cpu != -1) {
		trace_find_rtg_target_cpu(p, preferred_cpus, "idle_backup", idle_backup_cpu);
		return idle_backup_cpu;
	}

	trace_find_rtg_target_cpu(p, preferred_cpus, "max_spare", max_spare_cap_cpu);
