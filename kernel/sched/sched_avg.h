...
void inc_nr_heavy_running(struct rq *update_rq, struct task_struct *p);
void dec_nr_heavy_running(struct rq *update_rq, struct task_struct *p);
void update_nr_heavy_running(struct rq *update_rq,
			     struct task_struct *p, bool is_heavy);

#else /* !CONFIG_SCHED_RUNNING_AVG */

static inline bool sched_update_running_avg(void) { return false; }

static inline void sched_get_cpus_running_avg(const struct cpumask *cpus,
					      struct sched_nr_stat *stat)
{
}

static inline void sched_update_nr_prod(struct rq *updated_rq) {}
static inline unsigned int sched_get_cpus_real_big_tasks(const struct cpumask *cpus)
{
	return 0;
}
static inline void inc_nr_heavy_running(struct rq *update_rq,
					struct task_struct *p) {}
static inline void dec_nr_heavy_running(struct rq *update_rq,
					struct task_struct *p) {}
static inline void update_nr_heavy_running(struct rq *update_rq,
					struct task_struct *p, bool is_heavy) {}
