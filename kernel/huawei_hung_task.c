...
#define TASK_ITEM_NAME_LEN (TASK_COMM_LEN + 1)
struct task_item {
	struct rb_node node;
	pid_t pid;
	pid_t tgid;
	char name[TASK_ITEM_NAME_LEN];
...
void hwhungtask_dump_blocked_tasks(struct task_item *taskitem, struct task_struct *p)
{
	...
	if (unlikely(p->flags & PF_FROZEN)) {
		if (taskitem)
			pr_err("hungtask:name=%s,PID=%d,tgid=%d,tgname=%s,frozen_by=%d,"
...
static void update_panic_task(const struct task_item *item)
{
	...
	upload_hungtask.pid = item->pid;
	upload_hungtask.tgid = item->tgid;
	(void)memcpy_s(upload_hungtask.name, TASK_ITEM_NAME_LEN,
		item->name, TASK_ITEM_NAME_LEN);
}
...
static void show_unconcerned_block_task(struct task_item *item, struct task_struct *task)
{
	if (unlikely(item->task_type & TASK_TYPE_FROZEN))
		pr_err("hungtask: Unconcerned task %s:%d is frozen by %d for %ds\n",
		   item->name, item->pid, task->frozen_by, item->time_in_d_state * HEARTBEAT_TIME);
	else
...
static void get_localtime(struct rtc_time *tm)
{
	...
	time.tv_sec -= (int)(sys_tz.tz_minuteswest * 60);
