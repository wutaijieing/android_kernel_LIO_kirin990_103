...
static void kshield_do_send_sig_info(int sig, struct task_struct *p)
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(5, 10, 0)
	do_send_sig_info(sig, SEND_SIG_FORCED, p, true);
#else
	do_send_sig_info(sig, SEND_SIG_PRIV, p, PIDTYPE_TGID);
#endif
}
...
static int take_action(struct task_struct *p, int action)
{
	...
	case PERMISSIVE:
		...
		kshield_do_send_sig_info(SIGCONT, p);
		break;
	case FREEZE:
		ks_debug("Freeze task");
		kshield_do_send_sig_info(SIGSTOP, p);
		break;
	case KILL:
		ks_debug("Kill task");
		kshield_do_send_sig_info(SIGKILL, p);
		break;
	default:
		break;
	}
...
#else
	if (is_ttbr0_addr((unsigned long)(uintptr_t)x)) {
#endif
