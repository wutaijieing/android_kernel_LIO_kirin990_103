...
#else
static void clear_hkip_counter_timer(struct timer_list *tm)
{
	if (!tm)
		return;

	struct hkip_atkinfo *atkinfo = from_timer(atkinfo, tm, timer);
...
#else
	timer_setup(&atkinfo->timer, clear_hkip_counter_timer, 0);
#endif
