...
#define KSTP_LOG_TAG "kernel_stp_scanner"
...
int kernel_stp_scanner_init(void)
{
	...
	if (g_kernel_stp_scan_wq == NULL) {
		kstp_log_error(KSTP_LOG_TAG, "kernel stp scanner wq error, no mem");
		return -ENOMEM;
	}
	...
	if (g_scanner_list_init == KSTP_SCAN_LIST_UNINIT) {
		...
		kstp_log_trace(KSTP_LOG_TAG, "kernel stp scanner init success");
		return 0;
	} else {
		kstp_log_trace(KSTP_LOG_TAG, "kernel stp scanner has init success at register");
		return 0;
	}
}
...
int kernel_stp_scanner_register(stp_cb callbackfunc)
{
	...
	if (g_scanner_list_init == KSTP_SCAN_LIST_UNINIT) {
		INIT_LIST_HEAD(&g_stp_scanner_node.stp_scanner_list);
		g_scanner_list_init = KSTP_SCAN_LIST_INIT;
		kstp_log_trace(KSTP_LOG_TAG, "kernel stp scanner init success at register");
	}

	listnode = kzalloc(sizeof(struct kernel_stp_scanner_node),
				GFP_KERNEL);
	if (listnode == NULL) {
		kstp_log_error(KSTP_LOG_TAG, "listnode init failed");
		return KSTP_ERRCODE;
	}
	...
	list_for_each_entry(p, &g_stp_scanner_node.stp_scanner_list, stp_scanner_list) {
		if (listnode->callbackfunc == p->callbackfunc) {
			repeat_count++;
			kstp_log_debug(KSTP_LOG_TAG, "callbackfunc has in scanner list,no need add again!");
			break;
		}
	}
	...
	if (repeat_count == 0) {
		...
		kstp_log_trace(KSTP_LOG_TAG, "kernel stp scanner list add node success");
	}
...
static void kernel_stp_do_callback(struct work_struct *data)
{
	...
	if (data == NULL) {
		kstp_log_error(KSTP_LOG_TAG, "kernel stp do callbackfunc param invalid");
		return;
	}
	work_node = container_of(data, struct kernel_stp_scan_work, work);

	ret = work_node->callbackfunc();
	if (ret != 0)
		kstp_log_error(KSTP_LOG_TAG, "kernel stp do callbackfunc failed!");
	kfree(work_node);
}
...
void kernel_stp_scanner(void)
{
	...
	list_for_each_entry(p, &g_stp_scanner_node.stp_scanner_list, stp_scanner_list) {
		...
		if (work_node == NULL) {
			kstp_log_error(KSTP_LOG_TAG, "work_node init failed");
			continue;
		}
...
void kernel_stp_scanner_exit(void)
{
	...
	kstp_log_trace(KSTP_LOG_TAG, "stp_scanner_list cleanup success");
}
