...
static void bt_capture_drv_enable(struct snd_soc_component *codec)
{
	...
	up_src_pre_pmu(codec, platform_data->bt_uplink_params.channels,
		SAMPLE_RATE_48K, SAMPLE_RATE_REG_CFG_48K, SRC_MODE_2);
	u56_select_pga(codec);
	ret = codec_bus_activate("BT_UP",
		&platform_data->bt_uplink_params);
	if (ret != 0)
		AUDIO_LOGE("activate track bt up err: %d", ret);
...
static void bt_capture_drv_disable(struct snd_soc_component *codec)
{
	...
	u56_select_pga(codec);
	ret = codec_bus_deactivate("BT_UP", &platform_data->bt_uplink_params);
	if (ret != 0)
		AUDIO_LOGE("track deactivate bt up err: %d", ret);
...
static void bt_play_drv_enable(struct snd_soc_component *codec)
{
	...
	ret = codec_bus_activate("BT_DOWN", &platform_data->bt_downlink_params);
	if (ret != 0)
		AUDIO_LOGE("track activate bt down err: %d", ret);
...
static void bt_play_drv_disable(struct snd_soc_component *codec)
{
	...
	ret = codec_bus_deactivate("BT_DOWN", &platform_data->bt_downlink_params);
	if (ret != 0)
		AUDIO_LOGE("track deactivate bt down err: %d", ret);
