...
#ifdef CONFIG_AUDIO_FACTORY_MODE
static const struct snd_kcontrol_new snd_audio_factory_mode_controls[] = {
...
int da_combine_v5_add_kcontrol(struct snd_soc_component *codec)
{
	...
	if (!priv->board_config.single_kcontrol_route_mode) {
...
#ifdef CONFIG_AUDIO_FACTORY_MODE
	ret = snd_soc_add_component_controls(codec, snd_audio_factory_mode_controls,
		ARRAY_SIZE(snd_audio_factory_mode_controls));
	if (ret != 0) {
		AUDIO_LOGE("add kcontrols failed, ret: %d", ret);
		return ret;
	}
