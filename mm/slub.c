...
void *kmem_cache_alloc_trace(struct kmem_cache *s, gfp_t gfpflags, size_t size)
{
	...
	kmalloc_trace_hook((unsigned char)DFX_MEM_ALLOC, _RET_IP_, (unsigned long long)ret,/*lint !e571*/
...
void kfree(const void *x)
{
	...
	if (unlikely(!PageSlab(page))) {
		...
		kfree_hook(object);
#ifdef CONFIG_DFX_MEM_TRACE
		kmalloc_trace_hook((unsigned char)DFX_MEM_FREE, _RET_IP_, (unsigned long long)x,/*lint !e571*/
...
#ifdef CONFIG_DFX_MEM_TRACE
	kmalloc_trace_hook((unsigned char)DFX_MEM_ALLOC, caller, (unsigned long long)ret,
                (unsigned long long)virt_to_phys(ret), (unsigned int)size);
#endif
	return ret;
}
