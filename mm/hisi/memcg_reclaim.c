		...
		struct mem_cgroup *memcg)
{
	...
	lru_active_anon_size = lruvec_lru_size(lruvec,
	...
	pr_debug("RAMTURBO: %s active_lru shrinked %lupages(%luBytes) takes %lums",
		__func__, lru_active_anon_size,
		lru_active_anon_size * PAGE_SIZE,
		jiffies_to_msecs(end_active - start_active));

#ifdef CONFIG_HW_RECLAIM_ACCT
	reclaimacct_shrinklist_end(false, 0);
...
#ifdef CONFIG_HW_RECLAIM_ACCT
	reclaimacct_shrinklist_end(false, nr_reclaimed);
#endif
	...
	pr_debug("RAMTURBO: %s total shrinked %lupages(%luBytes) takes %lums",
		__func__, nr_reclaimed, nr_reclaimed * PAGE_SIZE,
...
#ifdef CONFIG_HW_RECLAIM_ACCT
	reclaimacct_shrinklist_end(is_file_lru(lru), nr_reclaimed);
#endif
	return nr_reclaimed;
}
...
#ifdef CONFIG_HW_RECLAIM_ACCT
		reclaimacct_shrinklist_end(is_file_lru(lru), 0);
#endif
		return 0;
	}
