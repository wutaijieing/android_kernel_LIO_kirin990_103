...
static enum compact_result __force_compact_finished(struct compact_control *cc)
{
	...
	if (__pageblock_is_enough(cc->zone, cc->order,
...
static enum compact_result __force_compact_suitable(struct compact_control *cc)
{
	if (__pageblock_is_enough(cc->zone, cc->order,
			cc->migratetype, cc->nr_expect))
		return COMPACT_SUCCESS;
...
#ifdef CONFIG_COMPACTION_SPECIFIED
	if (cc->force_compact)
		return __force_compact_finished(cc);
#endif
	/* Compaction run completes if the migrate and free scanner meet */
	if (compact_scanners_met(cc)) {
		/* Let the next compaction start anew. */
...
#ifdef CONFIG_COMPACTION_SPECIFIED
	if (cc->force_compact)
		ret = __force_compact_suitable(cc);
#endif
