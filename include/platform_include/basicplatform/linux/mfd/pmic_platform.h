...
unsigned int hisi_pmic_reg_read(int addr);    /* for modem & endpoint */
void hisi_pmic_reg_write(int addr, int val);  /* for modem & endpoint */
extern int hisi_pmic_special_ocp_register(
	char *power_name, pmic_ocp_callback handler);   /* for modem & endpoint */
#endif
