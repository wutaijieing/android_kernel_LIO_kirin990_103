...
#if defined(CONFIG_PMIC_MNTN) || defined(CONFIG_PMIC_MNTN_SPMI)
/* Open/Close smpl Function
 * SYS_CTRL1,0x0DC, np_smpl_open_en(bit 0), SMPL function enable bit;
 * enable:true, support smpl
 */
extern int pmic_mntn_config_smpl(bool enable);

/* config vsys_pwroff_abs_pd
 * 'SYS_CTRL0, 0x0DB,vsys_pwroff_abs_pd_mask(bit 0),
 * when vsys < 2.3v,wether auto poweroff bit
 * enable:true, 1do not auto poweroff
 */
extern int pmic_mntn_config_vsys_pwroff_abs_pd(bool enable);

extern int pmic_mntn_vsys_pwroff_abs_pd_state(void);
#else
static inline int pmic_mntn_config_smpl(bool enable)
{
	return 0;
}
static inline int pmic_mntn_config_vsys_pwroff_abs_pd(bool enable)
{
	return 0;
}
static inline int pmic_mntn_vsys_pwroff_abs_pd_state(void)
{
	return 0;
}
#endif
