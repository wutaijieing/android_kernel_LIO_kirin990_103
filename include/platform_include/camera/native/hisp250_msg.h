...
typedef enum {
	...
	COMMAND_QUERY_SENSOR_UPGRADE,
	COMMAND_SENSOR_UPGRADE,
	COMMAND_CONNECT_CAMERA,
	COMMAND_SENSOR_READSN,
	/* Response items. */
	...
	QUERY_SENSOR_UPGRADE_RESPONSE,
	SENSOR_UPGRADE_RESPONSE,
	CONNECT_CAMERA_RESPONCE,
	SENSOR_READ_SN_RESPONSE,
...
typedef struct _msg_req_sensor_read_sn_t {
	unsigned int cam_id;
} msg_req_sensor_read_sn_t;

typedef struct _msg_ack_sensor_read_sn_t {
	unsigned int cam_id;
	int status;
	char data[MAX_SN_LEN];
} msg_ack_sensor_read_sn_t;
...
typedef enum {
	...
	SUBCMD_SET_REDBLACKCORRECT_ENBALE = 256,
	SUBCMD_GET_UDS_INFO = 257,
	SUBCMD_SET_AF_ROI = 258,
	SUBCMD_SET_SENSOR_TYPE = 260,
	SUBCMD_MAX,
} extendset_info_e;
...
typedef struct _isp_msg_t {
	...
	union {
		...
		msg_req_sensor_read_sn_t req_sensor_read_sn;
		...
		msg_ack_sensor_read_sn_t ack_sensor_read_sn;
