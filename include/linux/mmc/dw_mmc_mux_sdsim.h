...
/*
 * iomux xml and pinctrl framework can't support such SD-SIM-IO-MUX case,
 * wo need config different five modes here manully in code
 */
...
/*
 * Description: while sd/sim plug in or out, gpio_cd detect pin is
 *		actived, we need call this sd_sim_detect_run function
 *		to make sure sd or sim which is inserted
 * dw_mci_host: MODULE_SD use dw_mci_host argu as input, while
 *		MODULE_SIM just use NULL
 * status: use STATUS_PLUG_IN or STATUS_PLUG_OUT by gpio_cd detect
 *		pin's value
 * current_module: sd or sim which module is calling this function
 * return value: return STATUS_PLUG_IN or STATUS_PLUG_OUT,just tell
 *		current_module sd or sim is inserted or not, and
 *		current_module can update gpio_cd detect pin value
