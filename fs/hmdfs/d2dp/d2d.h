...
/**
 * d2d_get_version - get the numeric value for D2DP version
 *
 * Numeric version can be used for upper layer when negotiating compatible
 * protocol version.
 */
 ...
 *
 * @D2D_RECV_FLAG_WAITALL: This flag requests that the recv function will block
 *                         until the full request is satisfied. Using this flag
 *                         also implies limitations in how many bytes can be
 *                         recv'ed in one call.
 */
enum d2d_recv_flags {
...
/* Define the maximum limit for the one `recv` call with WAITALL flag */
#define D2D_RECV_WAITALL_LIMIT ((size_t) 128UL << 10)
 ...
 * @security: crypto interface implementation
 * @remote_version: remote D2DP version
 *
 * This call will create a new instance of D2D Protocol. The @sock must be a UDP
 * socket in connected state. The @security implementation is required if
 * you want to protect the protocol, otherwise you can pass NULL here and the
 * protocol will not use any protection at all. Also the protocol will try to do
 * its best to achieve compatibility with protocol instance on the remote side,
 * this remote version is passed by @remote_version argument.
 *
 * Returns a pointer to the created protocol structure or NULL on error.
 *
 * NOTE: the @sock and @security (if not NULL) must be initialized before
 * calling d2d_protocol_create() and cannot be freed before d2d_protocol_close()
 */
