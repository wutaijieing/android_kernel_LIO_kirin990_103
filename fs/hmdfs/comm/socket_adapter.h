...
static inline bool is_loopback_device(struct hmdfs_peer *node)
{
	const char loopbackdevice[] = "networkStorage";
	if (strncmp(node->cid, loopbackdevice, HMDFS_CID_SIZE) == 0) {
		return true;
	}
	return false;
}
