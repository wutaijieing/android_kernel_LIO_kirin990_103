...
void hmdfs_try_get_p2p_session_async(struct hmdfs_peer *node)
{
	...
	if (node->pending_async_p2p_try < MAX_ASYNC_P2P_ATTEMPTS) {
		...
		notify_get_connection(node, 1 << NOTIFY_FLAG_P2P);
	} else {
...
int hmdfs_try_get_p2p_session_sync(struct hmdfs_peer *node)
{
	...
	if (node->status != NODE_STAT_SHAKING) {
		hmdfs_info_ratelimited("node status not as expected, device %llu, status %u",
...
static int do_sendmessage(struct hmdfs_peer *node, struct hmdfs_send_data *msg)
{
	...
	do {
		...
		if (!connect) {
			...
			ret = hmdfs_try_get_p2p_session_sync(node);
			if (!ret) {
				continue; /* p2p established */
			} else if (ret == -EFAULT) {
				notify_offline(node);
			}
			return -EAGAIN;
...
#define CONN_TOLERATED_TIMEOUTS 2
#define NODE_TOLERATED_TIMEOUTS 3

static void reget_conn_async(struct connection *conn)
{
	conn->status = CONNECT_STAT_STOP;
	connection_get(conn);
	if (!queue_work(conn->node->reget_conn_wq, &conn->reget_work))
		connection_put(conn);
}

static void record_request_timeout(struct hmdfs_peer *node)
{
	struct connection *conn_impl = NULL;

	if (++node->consecutive_timeouts >= NODE_TOLERATED_TIMEOUTS) {
		hmdfs_err("node %llu too many consecutive timeout requests, reget all sockets",
			  node->device_id);

		mutex_lock(&node->conn_impl_list_lock);
		list_for_each_entry(conn_impl, &node->conn_impl_list, list)
			reget_conn_async(conn_impl);
	...
	conn_impl = get_conn_impl(node, C_REQUEST);
	if (!conn_impl)
		return;

	hmdfs_err("node %llu fd %d request timeout", node->device_id, conn_impl->fd);
	if (++conn_impl->consecutive_timeouts >= CONN_TOLERATED_TIMEOUTS)
		reget_conn_async(conn_impl);
	connection_put(conn_impl);
}

static void clear_timeout_records(struct hmdfs_peer *node)
{
	struct connection *conn_impl = NULL;
	uint32_t node_timeouts = xchg(&node->consecutive_timeouts, 0);
	if (node_timeouts) {
		hmdfs_warning("node %llu clear the consecutive timeout counting from %u",
			  node->device_id, node_timeouts);
		mutex_lock(&node->conn_impl_list_lock);
		list_for_each_entry(conn_impl, &node->conn_impl_list, list) {
			hmdfs_warning("conn fd %d clear consecutive timeout counting from %u",
				  conn_impl->fd,
				  conn_impl->consecutive_timeouts);
			conn_impl->consecutive_timeouts = 0;
		}
		mutex_unlock(&node->conn_impl_list_lock);
	}
}
			      ...
			      struct hmdfs_send_command *sm)
{
	...
	if (time_left == -ERESTARTSYS || time_left == 0) {
		...
		hmdfs_client_resp_statis(con->sbi, sm->operations.command,
					 HMDFS_RESP_TIMEOUT, 0, 0);
		record_request_timeout(con);
		goto free;
	}
	clear_timeout_records(con);
...
{
	unsigned long start = jiffies;
	const struct cred *saved_cred = hmdfs_override_fsids(con->sbi, false);
