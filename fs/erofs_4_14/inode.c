...
static int read_inode(struct inode *inode, void *data)
{
	...
	if (unlikely(vi->datamode >= EROFS_INODE_LAYOUT_MAX)) {
		errln("unsupported data mapping %u of nid %llu",
		      vi->datamode, vi->nid);
		errln("advise %u, count %u, mode %u, nlink %u, size %u",
		      le16_to_cpu(v1->i_advise),
		      le16_to_cpu(v1->i_xattr_icount), le16_to_cpu(v1->i_mode),
		      le16_to_cpu(v1->i_nlink), le32_to_cpu(v1->i_size));
		DBG_BUGON(1);
		return -EIO;
	}
