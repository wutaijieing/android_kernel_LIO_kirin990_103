					...
					size_t count, loff_t *ppos)
{
	...
	ret = copy_from_user(scene, buffer, count);
	if (ret) {
		pr_err("smartgc %s error copy:%d\n", __func__, ret);
		return -EFAULT;
	}
...
static const struct proc_ops memstat_proc_fops = {
	.proc_open	= memstatscene_proc_open,
	.proc_read	= seq_read,
	.proc_write	= memstatscene_proc_write,
