...
static int tag_show(struct seq_file *m, void *v)
{
	struct inode *inode = m->private;
	struct task_struct *p = NULL;
...
static int tag_open(struct inode *inode, struct file *filp)
{
	return single_open(filp, tag_show, inode);
}

static ssize_t tag_write(struct file *file, const char __user *buf,
						size_t count, loff_t *offset)
{
	struct task_struct *p = NULL;
	unsigned int tag;
	...
	err = kstrtouint_from_user(buf, count, 0, &tag);
	if (err)
		return err;
...
#endif
#ifdef CONFIG_SCHED_CORE
	REG("tag", S_IRUSR|S_IWUSR, proc_tag_operations),
#endif
};
