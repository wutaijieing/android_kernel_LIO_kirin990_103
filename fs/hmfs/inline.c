...
process_inline:
	...
	if (f2fs_has_inline_data(inode)) {
		...
		ret = hmfs_truncate_blocks(inode, 0, false);
		if (ret)
			return ret;
					...
					struct page **res_page)
{
	...
	de = hmfs_find_target_dentry(&d, fname, NULL);
	unlock_page(ipage);
	if (IS_ERR(de)) {
...
static int f2fs_add_inline_entries(struct inode *dir, void *inline_dentry)
{
	...
	while (bit_pos < d.max) {
		...
		err = hmfs_add_regular_entry(dir, &fname, NULL, NULL, ino, fake_mode);
		if (err)
				...
				nid_t ino, umode_t mode)
{
	...
	if (inode) {
		...
		page = hmfs_init_inode_metadata(inode, dentry, dir, fname, ipage);
		if (IS_ERR(page)) {
			err = PTR_ERR(page);
			goto fail;
		}
