...
struct page *hmfs_get_sum_page(struct f2fs_sb_info *sbi, unsigned int segno)
{
	if (unlikely(f2fs_cp_error(sbi)))
		return ERR_PTR(-EIO);
	return hmfs_get_meta_page_retry(sbi, GET_SUM_BLOCK(sbi, segno));
}
...
bool hmfs_segment_has_free_slot(struct f2fs_sb_info *sbi, int segno)
{
	...
	pos = __hmfs_find_rev_next_zero_bit(target_map, sbi->blocks_per_seg, 0);
					...
					unsigned int start, unsigned int end)
{
	...
	/*
	 * DataMove cmd need lba continuous and reserves some lba in its
	 * cache. Before change gc curseg, we must clear all lba in cache.
	 * Or lba may not continuous and storage responses err. To avoid
	 * deadlock, gc_mutex need be hold on before curseg lock.
	 */
	if (IS_DMGCSEG(type)) {
		mutex_lock(&sbi->gc_mutex);
		/* wait for current gc complete */
		hmfs_wait_all_dm_complete(sbi);
		hmfs_datamove_force_flush(sbi, true);
		/* wait for force flush complete */
		hmfs_wait_all_dm_complete(sbi);
	}
...
static int __get_segment_type_6(struct f2fs_io_info *fio)
{
	if (fio->type == DATA) {
		...
		else if (file_is_cold(inode) || f2fs_need_compress_data(inode))
			return CURSEG_COLD_DATA;
...
drop_bio:
	if (io->bio) {
		io->bio->bi_status = BLK_STS_IOERR;
		bio_endio(io->bio);
	}
	return err;
}
...
static void hmfs_copy_page_for_reset(struct page *page)
{
	...
	// TODO: Wait mas block implementation
...
static struct page *get_current_sit_page(struct f2fs_sb_info *sbi,
					unsigned int segno)
{
	return hmfs_get_meta_page(sbi, current_sit_addr(sbi, segno));
}
