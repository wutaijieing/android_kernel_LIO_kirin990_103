				  ...
				  size_t desc_size, u64 merkle_tree_size)
{
	...
	err = hmfs_setxattr(inode, F2FS_XATTR_INDEX_VERITY,
...
cleanup:
	...
	err2 = hmfs_truncate(inode);
	if (err2) {
		hmfs_msg(sbi->sb, KERN_ERR, "Truncating verity metadata failed (errno=%d)",
			 err2);
				      ...
				      size_t buf_size)
{
	...
	res = hmfs_getxattr(inode, F2FS_XATTR_INDEX_VERITY,
			    F2FS_XATTR_NAME_VERITY, &dloc, sizeof(dloc), NULL);
	if (res < 0 && res != -ERANGE)
		return res;
	if (res != sizeof(dloc) || dloc.version != cpu_to_le32(F2FS_VERIFY_VER)) {
		hmfs_msg(F2FS_I_SB(inode)->sb, KERN_WARNING, "unknown verity xattr format");
		return -EINVAL;
	}
	...
	if (pos + size < pos || pos + size > inode->i_sb->s_maxbytes ||
		...
		hmfs_msg(F2FS_I_SB(inode)->sb, KERN_WARNING, "invalid verity xattr");
		return -EFSCORRUPTED;
	}
	if (buf_size) {
		if (size > buf_size)
...
const struct fsverity_operations hmfs_verityops = {
