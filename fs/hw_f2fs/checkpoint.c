...
retry:
	cycles++;
	if (cycles % 10000 == 0)
		f2fs_err(sbi, "%s:%d: too many cycles(%lu), inode: %lu dirty pages: %ld maybe an infinite loop!",
			 __func__, __LINE__, cycles, ino,
			 get_pages(sbi, is_dir ? F2FS_DIRTY_DENTS :
			 F2FS_DIRTY_DATA));
	if (unlikely(f2fs_cp_error(sbi))) {
...
void f2fs_wait_on_all_pages(struct f2fs_sb_info *sbi, int type)
{
	...
	for (;;) {
		...
		count++;
		//todo: adapte f2fs debug info in 5.10
